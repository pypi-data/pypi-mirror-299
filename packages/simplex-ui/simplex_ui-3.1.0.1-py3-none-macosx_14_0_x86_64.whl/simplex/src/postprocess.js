...
class PlotWindow {
    ...
    EditPlot(){
        let isanm = false
        if(this.m_isanim){
            isanm = this.m_fdata[0].length > 1 || this.m_fdata[1].length > 1;
        }
        ...
        if(plobj[PlotOptionsLabel.normalize[0]] == BySpecifiedLabel){
            PlotOptionsLabel.normalize[1] = [ForEachLabel, ByMaxLabel, BySpecifiedLabel];
        }
        else{
            PlotOptionsLabel.normalize[1] = [ForEachLabel, ByMaxLabel];
        }
    ...
    GetConfig(){
        ...
        if(this.m_isanim){
            config.normeach = this.m_ploptions[PlotOptionsLabel.normalize[0]] == ForEachLabel
                && this.m_fdata[0].length > 1 || this.m_fdata[1].length > 1;
        }
        else{
            ...
            if(this.m_dimension == 1){
                keys.forEach((el) => {
                    if(el.includes("yaxis.range")){
                        this.m_ploptions[PlotOptionsLabel.normalize[0]] = BySpecifiedLabel;
                    }
                });
                return;
            }
