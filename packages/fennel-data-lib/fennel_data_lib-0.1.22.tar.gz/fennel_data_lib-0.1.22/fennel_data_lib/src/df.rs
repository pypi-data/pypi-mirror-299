...
impl Dataframe {
    ...
    pub fn eval(
        ...
        let plexpr: PolarsExpr = cexpr.into_polar_expr(ctx).map_err(|err| {
            anyhow!(
        ...
        let values = from_arrow_array(&array, &outtype, false).with_context(|| {
            format!(
                "failed to convert polars array to fennel array for type '{:?}'",
                outtype
            )
        })?;
        Ok(Col::new(
