...
class KomidaAPI:
    ...
    def __init__(self, **kwargs):
        ...
        self.logger.debug(f"Kapi {__version__}.")

    def get(self, request: Request) -> dict:
        """"""
        options = "&".join("{}={}".format(*p) for p in request.as_parameters().items())
        with urllib.request.urlopen(f"https://app.growzer.be/MenuPlanner/GetMenuPlanner?{options}") as url:
            data = json.load(url)
    ...
    def parse_dish(self, dish: dict) -> Dish:
        return Dish(
            id=dish.get("Id"),
    ...
    def get_menu(self, menu_data: dict) -> Menu:
        menu_planner = menu_data.get("menuPlannerList")
