...
class TorchForwardTransform:
    ...
    def execute_all(self, xt: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if self._overlap_type == Overlap.OLA:
            ...
            ytn = self.W.view(1, -1) * torch.concatenate((ytn, torch.zeros((batches, frames, self.N - self.R),
                                                                               dtype=xt.dtype,
                                                                               device=self.device)),
                                                             dim=-1)
            if no_batch:
            ...
            tmp = torch.fft.rfft(ytn, dim=-1, n=self.N)
            ...
            energy_t = torch.mean(torch.square(ytn), dim=-1)
            tmp = torch.fft.rfft(ytn, dim=-1, n=self.N, norm='ortho')
            tmp = 1j * self.W[:self.R] * tmp[..., :self.R] + self.W[1:] * tmp[..., 1:]
