...
TRANSFORMERS:
    ...
    There are four different transformer based implementations in this file:
    ...
    recognition problems with transformers.  And the fourth,
    AutoencoderWithTransformer, is my starter code for future experiments with
    transformer-based variational autoencoding and vector quantization.
    ...
    4. AutoencoderWithTransformer:

    My demonstration of the AutoencoderWithTransformer class is also based on the
    same CIFAR-10 dataset as used for the visTransformer class.  This is meant to be
    my starter code for transformer based autoencoding. My plan is to develop this
    code further in future with transformer-based variational and vector quantization
    logic.  To experiment with this part of the Transformers module, your starting
    point should be the following script in the ExamplesTransformers directory of the
...
class visTransformer(nn.Module):             
    ...
    def __init__(self, dl_studio, patch_size, embedding_size, num_basic_encoders, num_atten_heads, encoder_out_size=None, decoder_out_size=None,
        ...
        self.num_basic_decoders = num_basic_encoders         ## used only in AutoencoderWithTransformer application of visTransformer
        self.num_atten_heads = num_atten_heads
        self.save_checkpoints = save_checkpoints
        self.max_seq_length = (self.num_patches_in_image + 1) if hasattr(dl_studio, 'class_labels') else self.num_patches_in_image
        self.encoder_out_size = encoder_out_size             ## used only in AutoencoderWithTransformer application of visTransformer
        self.decoder_out_size = decoder_out_size             ## used only in AutoencoderWithTransformer application of visTransformer 
    ...
    class BasicDecoderWithMasking(nn.Module):
        ...
        The main difference from the code in the same class in TransformerFG and the class definition
        that follows is regarding the output of the BasicDecoder.  Earlier, the goal was to make the
        maximum-likelihood prediction for the next word in the output sequence.  But now we want to 
        to reshape the output of the BasicDecoder into an image.

        Note that the BasicDecoder is NOT used in the transformer-based image recognition script 
        in the ExamplesTransformers directory of the distribution.  However, it is used in the
        AutoencoderWithTransformer class that is defined later in this file and, therefore, also by 
        the autoencoder based script in the ExamplesTransformers directory.

        ClassPath:  visTransformer  ->   BasicDecoderWithMasking
        """
        def __init__(self, dls, xformer, num_atten_heads, masking=True):
            super(visTransformer.BasicDecoderWithMasking, self).__init__()
            self.dls = dls
            self.masking = masking
            self.embedding_size = xformer.embedding_size                                             
            self.max_seq_length = xformer.max_seq_length                                                     
...
###%%%
#####################################################################################################################
###########################  Start Definition of Class AutoencoderWithTransformer  ##################################
#####################################################################################################################

class AutoencoderWithTransformer(nn.Module):             
    """
    For demonstrating the autoencoder architecture with a Transformer based implementation.  My plan is to use this 
    class as a starting point for experiments with variational encoding and vector quantization.

    ClassPath:  AutoencoderWithTransformer
    """
    def __init__(self, vis_trans, save_checkpoints=True, checkpoint_freq=10):
        super(AutoencoderWithTransformer, self).__init__()
        self.encoder = AutoencoderWithTransformer.EncoderForAutoenc( vis_trans )
        self.decoder = AutoencoderWithTransformer.DecoderForAutoenc( vis_trans )
        self.encH  = vis_trans.encoder_out_size[0]
        self.encW =  vis_trans.encoder_out_size[1]

    def forward(self, x):
        x, final_encoder_out =  self.encoder(x)
        x = rearrange(x, 'b d nh nw -> b (nh nw) d', nh=self.encH, nw=self.encW) 
        x =  self.decoder(x, final_encoder_out)
        return x    

    class EncoderForAutoenc(nn.Module):
        def __init__(self, vis_trans):
            super(AutoencoderWithTransformer.EncoderForAutoenc, self).__init__()
            self.autoenc_encoder  = vis_trans.master_encoder
            self.autoenc_encoder.requires_grad_()
            self.encH  = vis_trans.encoder_out_size[0]
            self.encW =  vis_trans.encoder_out_size[1]
            self.final_encoder_out = None

        def forward(self, x):
            x = self.autoenc_encoder(x)
            # Reshape into 2D grid    
            self.final_encoder_out = x
            x = rearrange(x, 'b (nh nw) d -> b d nh nw', nh=self.encH, nw=self.encW) 
            return x, self.final_encoder_out


    class DecoderForAutoenc(nn.Module):
        def __init__(self, vis_trans):
            super(AutoencoderWithTransformer.DecoderForAutoenc, self).__init__()
            self.autoenc_decoder = vis_trans.master_decoder
            self.autoenc_decoder.requires_grad_()
            self.encH  = vis_trans.encoder_out_size[0]
            self.encW =  vis_trans.encoder_out_size[1]
            self.outH  = vis_trans.decoder_out_size[0]
            self.outW =  vis_trans.decoder_out_size[1]
            in_ch  = vis_trans.embedding_size
            med_ch = in_ch // 2
            out_ch = 3                                     ## for the 3 channels of an output image
            self.upsampler1 = nn.ConvTranspose2d(in_ch, med_ch, 1, stride=2, dilation=2, output_padding=1, padding=0)
            self.upsampler2 = nn.ConvTranspose2d(med_ch, out_ch, 1, stride=2, dilation=2, output_padding=1, padding=0)
            self.upsampler3 = nn.ConvTranspose2d(out_ch, out_ch, 1, stride=2, dilation=2, output_padding=1, padding=0)
            self.bn1 = nn.BatchNorm2d(med_ch)                                                                                           
            self.bn2 = nn.BatchNorm2d(out_ch) 
            self.out_size = vis_trans.decoder_out_size

        def forward(self, x, final_encoder_out):
            x = self.autoenc_decoder(x, final_encoder_out)
            # Reshape into 2D grid    
            x = rearrange(x, 'b (nh nw) d -> b d nh nw', nh=self.encH, nw=self.encW) 
            x = self.upsampler1(x)
            x = self.bn1(x)
            ...
            if x.shape[2:] != self.out_size:
                sys.exit("AutoencoderWithTransformer output not according to specification")
            return x  


    def run_code_for_training_autoencoder_with_transformer(self, vis_transformer, autoencoder, display_train_loss=False, 
                                                                                                       checkpoint_dir='checkpoints'):

        if os.path.exists(checkpoint_dir):  
            """
            Clear out the previously stored checkpoints 
            """
            files = glob.glob(checkpoint_dir + "/*")
            for file in files: 
                if os.path.isfile(file): 
        ...
        dl_studio = vis_transformer.dl_studio
        autoencoder = self.to(dl_studio.device)
        ...
        autoencoder_optimizer = self.ScheduledOptim(optim.Adam(autoencoder.parameters(), betas=(beta1,beta2), eps=epsilon),  
                                                       lr_mul=2, d_model=vis_transformer.embedding_size, n_warmup_steps=num_warmup_steps) 
        patch_embedding_generator = vis_transformer.PatchEmbeddingGenerator( vis_transformer )
        patch_embedding_generator.to(dl_studio.device)
        ...
        debug = False
        print("")
        batch_size = dl_studio.batch_size
        print("\n\n batch_size: ", batch_size)
        print("\n\n number of batches in the dataset: ", len(vis_transformer.train_data_loader))

        for epoch in range(dl_studio.epochs):                                                              
            print("")
            running_loss = 0.0
            for i, data in enumerate(vis_transformer.train_data_loader):                                    
                input_images, _ = data                              ## With CIFAR-10, when batch_size is 48, the shape of input_images is [48, 3, 32, 32]
                input_images = input_images.to(dl_studio.device)
                patch_sequences = input_images.unfold(2, vis_transformer.patch_size[0], vis_transformer.patch_size[1]).unfold(3, 
                                                                                 vis_transformer.patch_size[0], vis_transformer.patch_size[1])
                patch_sequence_embeddings = patch_embedding_generator( patch_sequences )   
                patch_sequence_embeddings  = patch_sequence_embeddings.to(dl_studio.device)
                autoencoder.zero_grad()
                encoder_decoder_output = autoencoder( patch_sequence_embeddings )
                loss  =  criterion( encoder_decoder_output, input_images )
                loss.backward()                                                                                        
                autoencoder_optimizer.step_and_update_lr()
                running_loss += loss
                if i % 200 == 199:    
                    avg_loss = running_loss / float(200)
                    training_loss_tally.append(avg_loss.item())
                    ...
                    current_time = time.perf_counter()
                    time_elapsed = current_time-start_time
                    print("[epoch:%2d/%2d  i:%4d  elapsed_time: %4d secs]     loss: %.4f" % (epoch+1, dl_studio.epochs, i+1,time_elapsed,avg_loss)) 
                    accum_times.append(current_time-start_time)
            ##  At the beginning of the training session, the designated checkpoint_dir has already been flushed
            if vis_transformer.save_checkpoints and  (epoch + 1) % vis_transformer.checkpoint_freq == 0:
                vis_transformer.save_checkpoint_visTran( (autoencoder, patch_embedding_generator), checkpoint_dir, epoch+1)
                print("\nCheckpoint saved at the end of epoch %d" % (epoch+1))
        print("\nFinished Training\n")
        vis_transformer.save_visTran( (autoencoder, patch_embedding_generator) )
        if display_train_loss:
            plt.figure(figsize=(10,5))
            plt.title("Training Loss vs. Iterations")
            plt.plot(training_loss_tally)
    ...
    def run_code_for_evaluating_autoencoder_with_transformer(self, vis_transformer, autoencoder, visualization_dir = "visualization_dir"):
        ...
        dl_studio = vis_transformer.dl_studio
        patch_embedding_generator = vis_transformer.PatchEmbeddingGenerator( vis_transformer )
        patch_embedding_generator.load_state_dict(torch.load(dl_studio.path_saved_model + "_1"))
        patch_embedding_generator.to(dl_studio.device)

        autoencoder = self
        autoencoder.load_state_dict(torch.load(dl_studio.path_saved_model  + "_0"))
        autoencoder.to(dl_studio.device)
        debug = False
        with torch.no_grad():
            for i, data in enumerate(vis_transformer.test_data_loader):                                    
                print("\n\n\n=========Showing results for test batch %d===============" % i)
                test_images, _ = data     
                plt.figure(figsize=[15,15])
                plt.imshow(np.transpose(torchvision.utils.make_grid(test_images, normalize=False, padding=3, pad_value=255).cpu(), (1,2,0)))
                plt.title("Batch Input Images for iteration %d" % i)
                plt.savefig(visualization_dir + "/training_batch_%s" % str(i) + ".png")
                plt.show()
                test_images, _ = data                              ## When batch_size is 48, the shape of test_images is [48, 3, 32, 32]
                test_images = test_images.to(dl_studio.device)
                patch_sequences = test_images.unfold(2, vis_transformer.patch_size[0], vis_transformer.patch_size[1]).unfold(3, 
                                                                                 vis_transformer.patch_size[0], vis_transformer.patch_size[1])
                patch_sequence_embeddings = patch_embedding_generator( patch_sequences )   
                patch_sequence_embeddings  = patch_sequence_embeddings.to(dl_studio.device)
                encoder_decoder_output = autoencoder( patch_sequence_embeddings )
                ...
                plt.figure(figsize=(15,15))
                plt.imshow(np.transpose(torchvision.utils.make_grid(encoder_decoder_output.cpu(), normalize=False, padding=3, pad_value=255).cpu(), (1,2,0)))
                plt.title("Autoenc Output Images for iteration %d" % i)
...
################################  END Definition of Class AutoencoderWithTransformer END  ###########################
#####################################################################################################################
#####################################################################################################################
