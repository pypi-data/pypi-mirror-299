...
class PowerMeterClass(InstrumentClass):
    ...
    def SetUpRS_PowerMeter(self, AvgSamples):
        ...
        self.instr.write('CAL:ZERO:AUTO')
        time.sleep(2)
        ...
        BUFFER_SIZE_MAX = self.instr.query( 'BUFF:SIZE? MAX' )
        self.instr.write( 'BUFF:SIZE ' + BUFFER_SIZE_MAX )
        self.instr.write( 'BUFF:STAT ON' )

        # In this setting, trigger count needs to be the same as buffer size
        self.instr.write( 'TRIG:COUN ' + BUFFER_SIZE_MAX )
        self.instr.write( 'SENS:POW:AVG:APER 10e-6' )
    ...
    def MeasurePower(self, Freq):
        if self.HP:
            ...
            self.instr.write('SENS:FREQ ' + str(Freq))
            ...
            #     if ( self.instr.query( 'BUFF:COUN?') > 0 ):
            #         result = self.instr.query_binary_values( 'BUFF:DATA?' )
