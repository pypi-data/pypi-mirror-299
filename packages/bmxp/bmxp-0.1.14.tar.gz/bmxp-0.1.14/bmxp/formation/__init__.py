...
def report(
    ...
    with PdfPages(file_handle) as pdf:
        ...
        if "date_injected" in smdata.columns:
            plt.text(
                ...
                (
                    ...
                    f"{smdata['date_injected'].min().strftime('%B %d, %Y')}"
                ),
                ha="center",
                va="top",
                fontsize=18,
        ...
        colors = smdata["sample_type"].map(sample_colors)
    ...
    if not formation_params["keep_not_used"]:
        ...
        if sum(not_used) > 0:
            warnings.append(
                ...
                f" dataset: {', '.join(smdata.index[not_used])}"
            )
...
def filter_features_mask(data, smdata, fmdata, form_params):
    ...
    to_keep = to_keep & ~fmdata["Non_Quant"].fillna(False)
