...
class HubClient:
    ...
    def postgres_get_steps(
        ...
        def check_active_state(min_table_size, scope_section, working_clause, order_by):
            ...
            row = db.download_table(sql=f'select count(*), min(priority) as priority as amount from buelon_jobs_active where status != \'{buelon.core.step.StepStatus.working.value}\';')[0]
            amount = row['amount']
