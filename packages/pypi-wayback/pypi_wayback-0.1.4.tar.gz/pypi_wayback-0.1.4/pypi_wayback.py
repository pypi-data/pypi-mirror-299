...
def get_allowed_versions(package_name, cutoff_date):
...
def filter_html(package_name, allowed_versions):
    ...
    for anchor in soup.find_all('a'):
        matches = re.findall(r'(\d+\.\d+\.\d+)', anchor['href'])
        if not matches:
            continue
        version = matches[0]
...
def proxy_pypi_with_cutoff(package_name, date_string):
    ...
    latest_version = get_allowed_versions(package_name, cutoff_date)
    if not latest_version:
        return "No valid versions found before the cutoff date.", 404

    filtered_html = filter_html(package_name, latest_version)
