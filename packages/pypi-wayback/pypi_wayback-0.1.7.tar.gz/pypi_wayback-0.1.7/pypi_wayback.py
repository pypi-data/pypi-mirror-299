...
def get_latest_version(package_name, cutoff_date):
    ...
    latest_versions = []
    latest_version_date = datetime.min
    ...
    for version, uploads in releases.items():
        for upload in uploads:
            ...
            if cutoff_date >= release_date >= latest_version_date:
                if release_date > latest_version_date:
                    latest_versions = []
                    latest_version_date = release_date
...
def filter_html_to_latest_version(package_name, latest_version):
    ...
    result_html = ''
    latest_version_reached = False
    for anchor in soup.find_all('a'):
        if any(version in anchor['href'] for version in latest_version):
            latest_version_reached = True
        elif latest_version_reached:
            break
