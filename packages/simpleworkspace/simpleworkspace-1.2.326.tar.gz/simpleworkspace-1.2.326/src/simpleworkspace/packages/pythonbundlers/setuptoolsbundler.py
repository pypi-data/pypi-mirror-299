...
class SetupToolsBundler:
    ...
    def Command(self, args:list[str], title=None):
        ...
        if(title is None):
            title = f'{args}'
        LevelPrint(0, f"> Executing command {title}...")
        with _StopWatch() as sw1:
            process = subprocess.Popen(args, stdout=subprocess.PIPE, universal_newlines=True)
            for line in process.stdout:
                LevelPrint(1, line, end='')
            process.wait()
            if(process.returncode != 0): #something went bad
                raise RuntimeError(f"command failed...")
        LevelPrint(0, f' - Command finished in {sw1.GetElapsedSeconds(2)} seconds...')
    ...
    def Pipe_CleanUp(self):
        ...
        LevelPrint(0, "> Performing CleanUp...")
        for pathToRemove in [
            ...
            if(_os.path.isdir(pathToRemove)):
                ...
                LevelPrint(1, f'- Removed {pathToRemove}')
        return


    def Pipe_RunTests(self, testPath='tests/'):
        import unittest 
        LevelPrint(0, "> Running unittests...")
    ...
    def Pipe_IncrementPackageVersion(self):
        ...
        LevelPrint(1, f"> Incremented package version from {currentVersion} -> {newVersion}...")
    ...
    def Pipe_Finish(self):
        LevelPrint(1, f"> Installer finished! Elapsed: {self._stopwatch.GetElapsedSeconds(decimalPrecision=1)} seconds")
