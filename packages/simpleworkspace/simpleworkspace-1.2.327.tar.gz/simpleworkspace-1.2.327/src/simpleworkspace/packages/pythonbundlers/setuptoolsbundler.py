...
class SetupToolsBundler:
    ...
    def Command(self, args:list[str], title=None):
        ...
        LevelPrint(0, f' - Command finished in {sw1.GetElapsedSeconds(2)} seconds...', flush=True)
    ...
    def Pipe_Finish(self):
        LevelPrint(0, f"> Installer finished! Elapsed: {self._stopwatch.GetElapsedSeconds(decimalPrecision=1)} seconds", flush=True)
