...
class _StdoutProxy(_io.TextIOWrapper):
...
class _StderrProxy(_io.TextIOWrapper):
    def __init__(self, indent="  "):
        self._indent = indent
        super().__init__(_sys.stderr.buffer, encoding=_sys.stderr.encoding, line_buffering=_sys.stderr.line_buffering)

    def write(self, message):
        super().write(_strings.IndentText(f'[STDERR] {message}', 1, self._indent).rstrip(' '))
...
class SetupToolsBundler:
    def __init__(self):
        ...
        self._indentedStdout = _StdoutProxy()
        self._indentedStderr = _StderrProxy()
    ...
    def Command(self, args:list[str], title=None):
        ...
        with _StopWatch() as sw1:
            result = subprocess.run(args, stdout=self._indentedStdout, stderr=self._indentedStderr, text=True)
    ...
    def Pipe_RunTests(self, testPath='tests/'):
        ...
        test_runner = unittest.TextTestRunner(stream=self._indentedStdout, verbosity=2)
