...
# ---------------------------------------------------------------------------------------------------------------------------------------
	...
	def doInit(self):   
		try:	
			...
			ENV_FIREBASE_CAPI_ENABLED=CSetting.getInstance().doGet("ENV_FIREBASE_CAPI_ENABLED")
			if not IuText.StringEmpty(ENV_FIREBASE_CAPI_ENABLED):
				self.isCApiEnabled = False
				if str(ENV_FIREBASE_CAPI_ENABLED).lower() == "true":
					self.isCApiEnabled = True
	...
	def doAddApi(self):
		try:			
			if self.isCApiEnabled:
				...
				IuLog.doWarning(__name__, "API DISABLED")
