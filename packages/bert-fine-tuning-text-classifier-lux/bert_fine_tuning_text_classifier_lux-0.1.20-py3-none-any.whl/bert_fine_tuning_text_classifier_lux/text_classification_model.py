...
class TextClassificationModel:
    ...
    def predict(self, text_test, num_test, save_predictions_path):
        """
        ...
        """
        # Load pre-trained BERT model
        model = BertForSequenceClassification.from_pretrained(self.model_name, num_labels=len(self.class_mapping)).to(self.device)
        
        # Tokenize the test data
        test_encodings = self.tokenize_data(text_test)
        
        # Create the test dataset
        test_dataset = self.create_dataset(test_encodings, num_test, [0]*len(text_test))  # Placeholder labels
        ...
        df_predictions = pd.DataFrame({
            'Text': [' '.join(text) for text in text_test],  # Join multiple columns if the text data comes from multiple columns
