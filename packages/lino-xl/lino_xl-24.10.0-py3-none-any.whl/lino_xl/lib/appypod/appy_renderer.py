...
from odf.table import (Table, TableColumns, TableColumn, TableHeaderRows,
                       TableRows, TableRow, TableCell)

from lino.utils import isiterable
from lino.utils.restify import restify
from lino.utils.html2xhtml import html2xhtml, HAS_TIDYLIB
from lino.utils.html2odf import html2odf, toxml
from lino.utils.html import iselement, tostring
...
class AppyRenderer(OriginalAppyRenderer):
    ...
    def __init__(self, ar, template, context, result, **kw):
        ...
        # kw.update(deleteTempFolder=False)
    ...
    def insert_html(self, html, **kw):
        ...
        try:
            ...
            raise Exception("20150923 html2xhtml(%r) failed: %s" % (html, e))
    ...
    def finalize_func(self, fn, cls=False):
        ...
        self.insert_chunk(fn, 'content.xml', OAS,
                          ''.join([toxml(n) for n in self.my_automaticstyles]))
        self.insert_chunk(fn, 'styles.xml', OFFICE_STYLES,
                          ''.join([toxml(n) for n in self.my_styles]))
    ...
    def insert_table(self, *args, **kw):
        if True:
            ...
            try:
                ...
                dd.logger.warning("Exception during insert_table(%s):" %
                                  args[0])
    ...
    def insert_table_(self, ar, column_names=None, table_width=180):
        ...
        if False:
            ...
            if st is None:
                ...
                p.addElement(
                    ListLevelLabelAlignment(labelfollowedby="listtab",
        ...
        cell_style.addElement(
            TableCellProperties(paddingleft="1mm",
                                paddingright="1mm",
                                paddingtop="1mm",
                                paddingbottom="0.5mm",
                                border="0.002cm solid #000000"))

        header_row_style = add_style(name="Lino Header Row",
                                     family="table-row",
                                     parentstylename=cell_style)
        header_row_style.addElement(
            TableRowProperties(backgroundcolor="#eeeeee"))

        total_row_style = add_style(name="Lino Total Row",
                                    family="table-row",
                                    parentstylename=cell_style)
        ...
        def value2cell(ar, i, fld, val, style_name, tc):
            ...
            try:
                ...
                dd.logger.warning("20120614 addElement %s %s %r : %s", i, fld,
                                  val, e)
