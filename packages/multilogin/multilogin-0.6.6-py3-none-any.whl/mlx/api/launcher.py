...
class BrowserCoreApi(Api):
    ...
    def download(self, browser_type: str = "mimic", version: str = "128", wait=False):
        ...
        try:
            ...
            if e.response.status_code == 500 and "downloading of core already started" in e.response.text:
                if wait:
                    print("Core download in progress")
                else:
                    raise RuntimeError("Failed to download core: downloading of core already started")
            else:
                raise e
            ...
            while not condition:
                ...
                # requests.exceptions.HTTPError: 500 Server Error: Internal Server Error for url: https://launcher.mlx.yt:45001/api/v1/load_browser_core?browser_type=mimic&version=128: {"status":{"error_code":"INTERNAL_SERVER_ERROR","http_code":500,"message":"Failed to download core: downloading of core already started"}}
                    ...
                    if condition:
                        return data
                    if retries % 5 == 0:
                        print(f"Still waiting for download to complete; elapsed: {retries} seconds")
    ...
    def inspect(self):
        ...
        try:
            response = self.session.request("get", url, use_backoff=False)
            response.raise_for_status()
            return response.json()
        except requests.exceptions.ConnectionError:
            print("Failed to connect to the agent. Please ensure the agent is running.")
        except requests.exceptions.HTTPError as e:
            if e.response.status_code == 500 and "agent not started" in e.response.text:
