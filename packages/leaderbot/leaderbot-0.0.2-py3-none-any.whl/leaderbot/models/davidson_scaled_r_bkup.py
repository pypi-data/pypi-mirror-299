...
class DavidsonScaledR(BaseModel):
    ...
    .. code-block:: python
        ...
        >>> from leaderbot.models import DavidsonScaledR

        >>> # Create a model
        >>> data = load()
        >>> model = DavidsonScaledR(data)
    ...
    def __init__(
        ...
        self.basis = np.zeros((self.n_agents, self.n_tie_factors), dtype=float)
        if self.n_tie_factors > 0:
            ...
            for i in range(1, self.n_agents + 1):
                for j in range(1, self.n_tie_factors + 1):
                    self.basis[i-1, j-1] = np.sqrt(2.0 / self.n_agents) * \
