...
class Lnk(object):
    ...
    def _set_window_mode(self, value):
        if value not in list(_SHOW_COMMANDS.values()):
            raise ValueError("Not a valid window mode: %s. Choose any of winlnks.WINDOW_*" % value)
...
def main():
    ...
    f = asciitoart.asciitoart_format("Win LNKs",font="ansi_shadow")    
    print(f"{f}")
    ...
    parser_create.add_argument('--target', '-t', nargs='?', help='Target Path')
    parser_create.add_argument('--output', '-o', nargs='?', help='lnk filename to create')
    ...
    if args.help or not args.action:
        print('''
\t\tTool for Creating or Parsing .lnk files

usage: winlnks.py [p]arse / [c]reate ...

Examples:  
    python winlnks.py create -h
    python winlnks.py parse C:\\Temp\\calc.lnk
    python winlnks.py create -t C:\\windows\\system32\\calc.exe -o C:\\Temp\\calc.lnk
...
''')
    ...
    if args.action in ['create', 'c']:
        for_file(
            args.target, args.output, arguments=args.arguments,
