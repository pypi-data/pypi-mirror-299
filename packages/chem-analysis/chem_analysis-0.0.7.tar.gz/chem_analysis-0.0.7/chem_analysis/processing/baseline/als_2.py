...
def baseline(y, smoothness=1E6, asymmetry=1E-4, iterations=10, gradient=1E-4):
    """
    ...
    smoothness : float, optional
        Smoothing parameter (1E3 to 1E9), default is 1E6.
    asymmetry : float, optional
        Asymmetry parameter (1E-6 to 1E-1), default is 1E-4.
    iterations : int, optional
        Maximum number of baseline iterations, default is 10.
    gradient : float, optional
        Minimum change required for continued iterations, default is 1E-4.
    ...
    for i in range(n):
        ...
        y[np.abs(y[:, i]) > yavg[i] + ystd[i], i] = yavg[i]

        for j in range(iterations):
            try:
                lu = splu(W + diags(D))
                z = lu.solve(lu.solve(w[:, 0] * y[:, i], 'T'))
            except:
                break

            w[:, 1] = w[:, 0]
            w[:, 0] = asymmetry * (y[:, i] > z) + (1 - asymmetry) * (y[:, i] < z)

            if np.mean(np.abs(np.diff(w, axis=1))) <= gradient:
                break
...
# Example usage:
# y = np.array(...)  # your intensity values here
# b = baseline(y, asymmetry=1E-6, smoothness=1E5, iterations=50)
