...
def process_single(data_path):
    ...
    # fid
    fid.processor.add(ca.processing.edit.ReplaceSpans(value=0, x_spans=(1.3, 3.75), invert=True))
    fid.processor.add(
    ...
    # ms
    # ms.processor.add(ca.processing.edit.ReplaceSpans(value=0, x_spans=(None, 4), invert=True))
    ms.processor.add(
        ca.processing.baseline.SectionMinMax(sections=100, window=15, number_of_deviations=4, save_result=True)
    )
