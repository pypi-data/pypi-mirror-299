...
def parse_fid(file_path: str):
    ...
    with open(file_path, 'br') as file:
        ...
        data["file_info"] = fpascal(file, 347, 'UTF-16')
        data["sample_name"] = fpascal(file, 858, 'UTF-16')
        data["barcode"] = fpascal(file, 1369, 'UTF-16')
        data["operator"] = fpascal(file, 1880, 'UTF-16')
        data["date_time"] = fpascal(file, 2391, 'UTF-16')
        data["instmodel"] = fpascal(file, 2492, 'UTF-16')
        data["inlet"] = fpascal(file, 2533, 'UTF-16')
        data["method_name"] = fpascal(file, 2574, 'UTF-16')
        data["seqindex"] = fnumeric(file, 252, '>h')
        data["vial"] = fnumeric(file, 254, '>h')
        data["replicate"] = fnumeric(file, 256, '>h')
        data["start_time"] = fnumeric(file, 282, '>f') / 6E4  # min
        data["end_time"] = fnumeric(file, 286, '>f') / 6E4  # min
        data["channel_max"] = fnumeric(file, 290, '>f')
        data["channel_min"] = fnumeric(file, 294, '>f')
        data["dir_type"] = fnumeric(file, 258, '>h')
        data["dir_offset"] = (fnumeric(file, 260, '>i') - 1) * 512
        data["data_offset"] = (fnumeric(file, 264, '>i') - 1) * 512
        ...
        data["channel_detector"] = fnumeric(file, 4106, '>h')
        data["channel_units"] = fpascal(file, 4172, 'UTF-16')
        data['channel_desc'] = fpascal(file, 4213, 'UTF-16')

        if fnumeric(file, data["data_offset"], '>h') == 2048:
            data["data_offset"] = data["data_offset"] + 2048

        data['intensity'] = fdoublearray(file, data["data_offset"])
...
def main():
    file_path = r"C:\Users\nicep\Desktop\research_wis\data\10\11\10_11\DJW-10-11-2h-batch-PPh3.D\FID1A.ch"
