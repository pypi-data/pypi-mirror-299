...
def initialize_aws_session(aws_profile, region):
    ...
    except (botocore.exceptions.NoCredentialsError, botocore.exceptions.TokenRetrievalError,
            botocore.exceptions.PartialCredentialsError, botocore.exceptions.SSOTokenLoadError):
        ...
        try:
            ...
            if input_value.isdigit():
                ...
                if 1 <= cluster_index <= len(clusters):
                    ...
                    console.print(f"You have selected ==> {selected_cluster}", style="green")
                    return selected_cluster
                else:
                    console.print(f"Invalid cluster number: {cluster_index}. Please try again.", style="red")
            else:
                ...
                if input_value in clusters:
                    ...
                    console.print(f"Cluster '{input_value}' not found. Please try again.", style="red")
...
def generate_iam_token(session, db_host, db_port, db_user_name):
    ...
    if not db_host or not db_port or not db_user_name:
        console.print("Required database details are not present. Hence, database token is not generated.", style="red")
        return None
            ...
            if selected_cluster:
                ...
                if selected_service:
                    ...
                    if 'DB_HOST' in tags and 'DB_NAME' in tags and 'DB_PORT' in tags:
                        console.print(f"DB Host: {tags['DB_HOST']}, DB Port: {tags['DB_PORT']}, DB Name: {tags['DB_NAME']}", style="green")
                        ...
                        try:
                            ...
                            if pg_password:
                                ...
                                console.print("Token generation failed. No token to display.", style="red")

                        # Catch AWS SSO token-related errors and prompt the user to log in
                        except botocore.exceptions.SSOTokenLoadError:
                            click.echo(click.style(f"Error loading SSO Token. Please run the following command to log in via SSO:", fg='red'))
                            click.echo(click.style(f"aws sso login --profile {aws_profile}", fg='yellow'))
                        except Exception as e:
                            click.echo(click.style(f"An unexpected error occurred while generating IAM token: {e}", fg='red'))
                    else:
                        console.print("Required database details are not present. Hence, database token is not generated.", style="red")

    except botocore.exceptions.SSOTokenLoadError:
        click.echo(click.style(f"Error: AWS SSO authentication required. Please run this command:", fg='red'))
        click.echo(click.style(f"aws sso login --profile {aws_profile}", fg='yellow'))
    except botocore.exceptions.NoCredentialsError:
        click.echo(click.style(f"AWS credentials not found. Please configure your credentials using:", fg='red'))
        click.echo(click.style(f"aws configure --profile {aws_profile}", fg='yellow'))
    except botocore.exceptions.PartialCredentialsError:
        click.echo(click.style(f"Incomplete AWS credentials found. Please check your profile and try again.", fg='red'))
    except Exception as e:
