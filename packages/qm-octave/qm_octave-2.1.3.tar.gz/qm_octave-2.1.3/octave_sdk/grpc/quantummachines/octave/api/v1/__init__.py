...
class AdminClockUpdate(betterproto.Message):
    ...
    """
    There are there general modes:
     1. BUFFERED - The clock distributor will use the external clock and apply
     divider at most (no PLL)
     2. INTERNAL - Use only internal crystals to achieve the desired output
     clocks
     3. EXTERNAL - Use PLLs to lock the VCO on an external clock
    
     Both 1. and 3. requires the frequency of the external clock. In case 2.,
     the clock_frequency field is ignored.
    
     Modes 2. and 3. has a variant of using an external 1GHz VCXO, instead of
     the LMK internal 3GHz VCO.
    ...
    This is the clock frequency expected by the synthesizers (by default it is
     125MHz, but you may change it).
    
     It is important to understand the relation between the synthesizers_clock
     and the above congiuration. The clocks sent to the synthesizers must be an
     interger division of the "main" clock. The ratio can not exceed 31.
    
     In mode 1., the "main" clock is simply the external clock. For example, if
     you use an OPT's output (1GHz) and the desired synthesizer's clock is
     125MHz, it is a division by 8 (which is perfectly fine). You can use the
     500MHz NEL source as well, and the division will be 4.
    
     In modes 2. and 3., if NOT using the external 1GHz VCXO, we first lock an
     internal 3GHz VCO and this is the "main" clock. Again, if we assume 125MHz
     clock for the synthesizers, the divider is 24. This is absolutely fine as
     well.
    
     In modes 2. and 3., if USING the external 1GHz VCXO, the "main" clock is
     1GHz.
    
     Note that, for example, you can clock the synthesizers at 62.5MHz if using
     1GHz VXO, but not if you don't!
    """
