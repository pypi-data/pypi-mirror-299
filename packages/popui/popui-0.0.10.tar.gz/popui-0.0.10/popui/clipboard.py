...
def get(format: Literal['text', 'html'] = 'text') -> str | None:
    if format == 'html':
        cb = HtmlClipboard()
        if cb.HasHtmlFormat():
            return cb.GetFragment()
...
def put(data: str, format: Literal['text', 'html'] = 'text'):
    if format == 'html':
        cb = HtmlClipboard()
        cb.PutFragment(data)
    else:
...
class HtmlClipboard:
    ...
    MARKER_BLOCK_NS = \
        ...
        "StartFragment:(\d+)\s+" \
        "EndFragment:(\d+)\s+"
    MARKER_BLOCK_NS = re.compile(MARKER_BLOCK_NS)
    ...
    def DecodeClipboardSource(self, src):
        ...
        if matches:
            ...
            if matches:
                ...
                self.htmlClipboardVersion = matches.group(1)
                self.html = src[int(matches.group(2)):int(matches.group(3))]
                self.fragment = src[int(matches.group(4))
                                        :int(matches.group(5))]
                self.source = matches.group(6)
                self.selection = self.fragment
            else:
                matches = self.MARKER_BLOCK_NS.match(src)
                if matches:
                    self.prefix = matches.group(0)
                    self.htmlClipboardVersion = matches.group(1)
                    self.html = src[int(matches.group(2))
                                        :int(matches.group(3))]
                    self.fragment = src[int(
                        matches.group(4)):int(matches.group(5))]
                    self.source = None
                    self.selection = self.fragment
