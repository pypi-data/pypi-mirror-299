...
def main() -> None:
    ...
    insample_shape.insert(0, hparams["batch_size"])
    ...
    if batch_size == -1 and timesteps != -1:
        logger.info(f"Overriding input dimension 0 with dynamic size of 'batch_size'")
        dynamic_axes = {'input':  {0: 'batch_size'},  # variable length axes, batch_size is always dimension 0
                        'output': {0: 'batch_size'}}
    elif batch_size != -1 and timesteps == -1:
        logger.info(f"Overriding timestep dimension 1 with dynamic size of 'timesteps'")
        dynamic_axes = {'input':  {1: 'timesteps'},  # variable length axes, timesteps is always dimension 1
                        'output': {1: 'timesteps'}}
    elif batch_size == -1 and timesteps == -1:
        logger.info(
            f"Overriding batch_size and timestep dimensions 0,1 with dynamic sizes of 'batch_size' and 'timesteps'")
    ...
    logger.debug(f'Loading onnx model to check, add metadata, and optimize if requested.')
    ...
    if run_onnx_simplify:
        logger.info(f'Running onnx_simplify() with optimizations and constant folding ...')
        omodel, check = simplify(omodel)
        if check is not True:
            logger.warning(f'Onnx simplify check failed. Model may have problems.')
    ...
    if ovino_prec != "None":
        ...
        logger.info(f'Creating OpenVINO model ...')
        ...
        logger.info(f'Saving OpenVINO uncompiled model (.xml, .bin) ...')
        if not output_dir:
            ov.save_model(ov_model, ofname_vino,
                          compress_to_fp16=True)  # default fp16-compress is True, rarely needs fp32
        else:
