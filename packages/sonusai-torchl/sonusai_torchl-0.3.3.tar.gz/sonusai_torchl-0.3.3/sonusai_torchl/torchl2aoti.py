...
def main() -> None:
    ...
    if batch_size == -1 and timesteps != -1:
        logger.info(f"Overriding input batch dimension to be dynamic size. Note AOT Inductor has a max limit of 15)")
        ...
        logger.info(f"Overriding batch_size and timestep dimensions 0,1 with dynamic sizes.")
        example_shape = [batch_size] + hparams["input_shape"].copy()
