...
from pyspark.sql.functions import col, expr, greatest, when, datediff, lag, min as spark_min, expr, to_timestamp, count
...
def compare_cdc_timestamp_and_commit_timestamp(df, columns):
    df = df.withColumn("cdc_timestamp", to_timestamp("cdc_timestamp")) \
        .withColumn("commit_timestamp", to_timestamp("commit_timestamp"))
        
    partition_columns = columns + ["cdc_timestamp"]
    window_spec = Window.partitionBy(partition_columns).orderBy(col("commit_timestamp"))

    df_with_count = df.withColumn("record_count", count("*").over(window_spec))
    df_adjusted = df_with_count.withColumn(
        "cdc_timestamp",
        when((col("commit_timestamp") != col("cdc_timestamp")) & (col("record_count") > 1) & (col("commit_timestamp").isNotNull()), col("commit_timestamp"))
        .otherwise(col("cdc_timestamp"))
    )
    df_final = df_adjusted.drop("record_count")
    return df_final
