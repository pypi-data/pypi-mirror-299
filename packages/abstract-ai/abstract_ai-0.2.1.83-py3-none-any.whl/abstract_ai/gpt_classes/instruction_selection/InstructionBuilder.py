...
class InstructionManager:
    def __init__(self,instructions=None,instruction_bools={})->None:
        self.default_instructions = json.loads("""
{
    "api_response": {
    ...
    "database_query": {
        "instruction": "The given database schema, variables, values, and function are provided in the prompt data; please derive the most appropriate inputs for a database query from the context and assertions of the request. Always include a 'query' key containing the SQL query as a string. Example: {\"query\": \"SELECT * FROM table WHERE condition\"}. Ensure that the 'query' key is always present in the database_query object, and the query is valid SQL.",
        "example": {"query": "SELECT dnc_data FROM dncdata WHERE dnc_data ->> 'Data_Subscriber_First_Name' = 'Karen';"},
        "default": false
    },
    "notation": {
        "instruction": "Notation is a module functionality that allows the module, you, to preserve relevant information or context for subsequent prompts; allowing for communication between modules throughout the query iterations.",
