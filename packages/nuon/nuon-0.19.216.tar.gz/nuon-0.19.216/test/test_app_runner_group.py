# coding: utf-8

"""
    Nuon

    API for managing nuon apps, components, and installs.

    The version of the OpenAPI document: 0.19.216
    Contact: support@nuon.co
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest
import datetime

from nuon.models.app_runner_group import AppRunnerGroup

class TestAppRunnerGroup(unittest.TestCase):
    """AppRunnerGroup unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> AppRunnerGroup:
        """Test AppRunnerGroup
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `AppRunnerGroup`
        """
        model = AppRunnerGroup()
        if include_optional:
            return AppRunnerGroup(
                created_at = '',
                created_by = nuon.models.app/account.app.Account(
                    account_type = 'auth0', 
                    created_at = '', 
                    email = '', 
                    id = '', 
                    org_ids = [
                        ''
                        ], 
                    permissions = {
                        'key' : 'unknown'
                        }, 
                    roles = [
                        nuon.models.app/role.app.Role(
                            created_by = nuon.models.app/account.app.Account(
                                created_at = '', 
                                email = '', 
                                id = '', 
                                permissions = {
                                    'key' : 'unknown'
                                    }, 
                                subject = '', 
                                updated_at = '', ), 
                            created_at = '', 
                            created_by_id = '', 
                            id = '', 
                            policies = [
                                nuon.models.app/policy.app.Policy(
                                    created_at = '', 
                                    created_by = , 
                                    created_by_id = '', 
                                    id = '', 
                                    name = 'org_admin', 
                                    permissions = {
                                        'key' : ''
                                        }, 
                                    role_id = '', 
                                    updated_at = '', )
                                ], 
                            role_type = 'org_admin', 
                            updated_at = '', )
                        ], 
                    subject = '', 
                    updated_at = '', ),
                created_by_id = '',
                id = '',
                org_id = '',
                owner_id = '',
                owner_type = '',
                platform = 'aws-ecs',
                runners = [
                    nuon.models.app/runner.app.Runner(
                        created_at = '', 
                        created_by = nuon.models.app/account.app.Account(
                            account_type = 'auth0', 
                            created_at = '', 
                            email = '', 
                            id = '', 
                            org_ids = [
                                ''
                                ], 
                            permissions = {
                                'key' : 'unknown'
                                }, 
                            roles = [
                                nuon.models.app/role.app.Role(
                                    created_by = nuon.models.app/account.app.Account(
                                        created_at = '', 
                                        email = '', 
                                        id = '', 
                                        permissions = {
                                            'key' : 'unknown'
                                            }, 
                                        subject = '', 
                                        updated_at = '', ), 
                                    created_at = '', 
                                    created_by_id = '', 
                                    id = '', 
                                    policies = [
                                        nuon.models.app/policy.app.Policy(
                                            created_at = '', 
                                            created_by_id = '', 
                                            id = '', 
                                            name = 'org_admin', 
                                            permissions = {
                                                'key' : ''
                                                }, 
                                            role_id = '', 
                                            updated_at = '', )
                                        ], 
                                    role_type = 'org_admin', 
                                    updated_at = '', )
                                ], 
                            subject = '', 
                            updated_at = '', ), 
                        created_by_id = '', 
                        display_name = '', 
                        id = '', 
                        jobs = [
                            nuon.models.app/runner_job.app.RunnerJob(
                                available_timeout = 56, 
                                created_at = '', 
                                created_by_id = '', 
                                execution_timeout = 56, 
                                executions = [
                                    nuon.models.app/runner_job_execution.app.RunnerJobExecution(
                                        created_at = '', 
                                        created_by_id = '', 
                                        id = '', 
                                        org = nuon.models.app/org.app.Org(
                                            created_at = '', 
                                            created_by_id = '', 
                                            custom_cert = True, 
                                            health_checks = [
                                                nuon.models.app/org_health_check.app.OrgHealthCheck(
                                                    created_at = '', 
                                                    created_by_id = '', 
                                                    id = '', 
                                                    org_id = '', 
                                                    status = 'ok', 
                                                    status_description = '', 
                                                    updated_at = '', )
                                                ], 
                                            id = '', 
                                            latest_health_check = null, 
                                            notifications_config = nuon.models.app/notifications_config.app.NotificationsConfig(
                                                created_at = '', 
                                                created_by_id = '', 
                                                id = '', 
                                                org_id = '', 
                                                owner_id = '', 
                                                owner_type = '', 
                                                slack_webhook_url = '', 
                                                updated_at = '', ), 
                                            sandbox_mode = True, 
                                            status = '', 
                                            status_description = '', 
                                            updated_at = '', 
                                            vcs_connections = [
                                                nuon.models.app/vcs_connection.app.VCSConnection(
                                                    created_at = '', 
                                                    created_by_id = '', 
                                                    github_install_id = '', 
                                                    id = '', 
                                                    updated_at = '', 
                                                    vcs_connection_commit = [
                                                        nuon.models.app/vcs_connection_commit.app.VCSConnectionCommit(
                                                            author_email = '', 
                                                            author_name = '', 
                                                            component_config_connection_id = '', 
                                                            created_at = '', 
                                                            created_by_id = '', 
                                                            id = '', 
                                                            message = '', 
                                                            sha = '', 
                                                            updated_at = '', )
                                                        ], )
                                                ], ), 
                                        org_id = '', 
                                        result = nuon.models.app/runner_job_execution_result.app.RunnerJobExecutionResult(
                                            created_at = '', 
                                            created_by_id = '', 
                                            error_code = 56, 
                                            error_metadata = {
                                                'key' : ''
                                                }, 
                                            id = '', 
                                            org_id = '', 
                                            runner_job_execution_id = '', 
                                            success = True, 
                                            updated_at = '', ), 
                                        runner_job_id = '', 
                                        status = 'pending', 
                                        updated_at = '', )
                                    ], 
                                group = 'health-checks', 
                                id = '', 
                                max_executions = 56, 
                                operation = 'exec', 
                                org = nuon.models.app/org.app.Org(
                                    created_at = '', 
                                    created_by_id = '', 
                                    custom_cert = True, 
                                    id = '', 
                                    latest_health_check = null, 
                                    sandbox_mode = True, 
                                    status_description = '', 
                                    updated_at = '', ), 
                                org_id = '', 
                                overall_timeout = 56, 
                                owner_id = '', 
                                owner_type = '', 
                                queue_timeout = 56, 
                                runner_id = '', 
                                status = 'queued', 
                                status_description = '', 
                                type = 'health-check', 
                                updated_at = '', )
                            ], 
                        name = '', 
                        org = , 
                        org_id = '', 
                        runner_id = '', 
                        runner_job = nuon.models.app/runner_job.app.RunnerJob(
                            available_timeout = 56, 
                            created_at = '', 
                            created_by_id = '', 
                            execution_timeout = 56, 
                            id = '', 
                            max_executions = 56, 
                            org_id = '', 
                            overall_timeout = 56, 
                            owner_id = '', 
                            owner_type = '', 
                            queue_timeout = 56, 
                            runner_id = '', 
                            status_description = '', 
                            updated_at = '', ), 
                        status = '', 
                        status_description = '', 
                        updated_at = '', )
                    ],
                settings = nuon.models.app/runner_group_settings.app.RunnerGroupSettings(
                    container_image_tag = '', 
                    container_image_url = '', 
                    created_at = '', 
                    created_by = nuon.models.app/account.app.Account(
                        account_type = 'auth0', 
                        created_at = '', 
                        email = '', 
                        id = '', 
                        org_ids = [
                            ''
                            ], 
                        permissions = {
                            'key' : 'unknown'
                            }, 
                        roles = [
                            nuon.models.app/role.app.Role(
                                created_by = nuon.models.app/account.app.Account(
                                    created_at = '', 
                                    email = '', 
                                    id = '', 
                                    permissions = {
                                        'key' : 'unknown'
                                        }, 
                                    subject = '', 
                                    updated_at = '', ), 
                                created_at = '', 
                                created_by_id = '', 
                                id = '', 
                                policies = [
                                    nuon.models.app/policy.app.Policy(
                                        created_at = '', 
                                        created_by_id = '', 
                                        id = '', 
                                        name = 'org_admin', 
                                        permissions = {
                                            'key' : ''
                                            }, 
                                        role_id = '', 
                                        updated_at = '', )
                                    ], 
                                role_type = 'org_admin', 
                                updated_at = '', )
                            ], 
                        subject = '', 
                        updated_at = '', ), 
                    created_by_id = '', 
                    env = '', 
                    heart_beat_timeout = 56, 
                    id = '', 
                    org_id = '', 
                    otel_collector_config = '', 
                    runner_api_url = '', 
                    runner_group_id = '', 
                    settings_refresh_timeout = 56, 
                    updated_at = '', ),
                type = 'install',
                updated_at = ''
            )
        else:
            return AppRunnerGroup(
        )
        """

    def testAppRunnerGroup(self):
        """Test AppRunnerGroup"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
