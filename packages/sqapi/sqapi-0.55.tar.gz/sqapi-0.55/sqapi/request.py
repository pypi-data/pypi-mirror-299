...
class Request:
    ...
    def execute(self, build_url=True, with_api=None, verbosity=None, icon_ok="✅", icon_error="❌", _is_polling=False):
    ...
    def poll_task_status(self, result, poll_status_interval=1.0):
        if isinstance(poll_status_interval, (int, float)):
            task_data = result.execute().json()
            ...
            while True:
                ...
                if status.get('status') == "done" and status.get('result_available'):
                    result = self._sqapi.get(task_data.get('result_url'))
                    break
