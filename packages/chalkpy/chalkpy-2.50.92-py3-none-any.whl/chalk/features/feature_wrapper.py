...
class FeatureWrapper:
    ...
    def __getitem__(self, item: Any):
        ...
        def f():
            ...
            if dataframe_typ is not None:
                ...
                f_copy = FeatureWrapper(copy.copy(underlying))
                f_copy_underlying = f_copy._chalk_get_underlying()
                assert isinstance(f_copy_underlying, Feature)
                f_copy_underlying.typ = ParsedAnnotation(underlying=dataframe_typ[tuple_item])
                if (
                    hasattr(f_copy_underlying, "path")
                    and len(f_copy_underlying.path) > 0
                ):
                    f_copy_underlying.path[-1].child = unwrap_feature(f_copy)
    ...
    def __getattr__(self, item: str):
        ...
        def fn():
            ...
            assert underlying.features_cls is not None
            underlying.features_cls.__chalk_error_builder__.invalid_attribute(
                underlying.features_cls.namespace,
                item=item,
                candidates=[],
                back=1,
            )
