...
class ScheduledQuery:
    def __init__(
        ...
        if name in CRON_QUERY_REGISTRY:
            self.errors.append(f"Cron query with name {name} already exists. Cron names must be unique.")

        if len(output) == 0:
            self.errors.append("Cron query does not require any outputs. Chalk will not run it.")
        ...
        if not store_offline and not store_online:
            self.errors.append("Cron query does not store results. This means running it will have no effect.")
