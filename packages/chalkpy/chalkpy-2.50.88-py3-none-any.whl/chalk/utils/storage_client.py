...
class StorageClient(Protocol):
    ...
    def get_num_rows_parquet(self, filename: str) -> int:
        """
        Get the number of rows of a parquet file.
        Implementations should not read the full file into memory, but rather read the metadata only
        """
        ...
...
class GCSStorageClient(StorageClient):
    ...
    def get_num_rows_parquet(self, filename: str) -> int:
        if not filename.endswith(".parquet"):
            raise ValueError(f"filename must end with .parquet, got {filename}")
        with self._fs.open(self.get_uri(filename), "rb") as f:
...
class S3StorageClient(StorageClient):
    ...
    def upload_object(
        ...
        data = _IOWithLen(data)  # boto3 wants to call len() the io, manually wrapping it in a class that supports that
        if metadata is None:
            metadata = {}
        # Not using the actual filename in the tag to avoid tag blowup
        safe_distribution(
    ...
    def get_num_rows_parquet(self, filename: str) -> int:
        ...
        with self._fs.open(uri, mode="rb") as file_stream:
...
class LocalStorageClient(StorageClient):
    ...
    def get_num_rows_parquet(self, filename: str) -> int:
        if not filename.endswith(".parquet"):
            raise ValueError(f"filename must end with .parquet, got {filename}")
        parquet_file = pq.ParquetFile(self._normalize_path(filename))
        assert parquet_file.metadata is not None, "no parquet metadata found"
        return parquet_file.metadata.num_rows
