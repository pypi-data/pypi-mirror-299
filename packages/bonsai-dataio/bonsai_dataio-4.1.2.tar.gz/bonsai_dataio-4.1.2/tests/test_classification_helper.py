...
# Assuming you have a class containing the find_nearest_parent method
...
def test_filter_classifications(setup_classification_data):
    ...
    filtered_table, many_to_many = filter_classifications(classification)

    print(filtered_table)
    assert len(filtered_table) == 1
    assert len(many_to_many) == 4
    assert "many-to-many correspondence" in many_to_many["comment"].values
