import yaml, json, time, shlex, logging, subprocess
...
class Command:
    def run(self, cmd, host = None, exceptionOnFail = False):
        if isinstance(cmd, list) and host:
            raise Exception(f"When host type is specified, cmd must be a string, not a list.")
        if isinstance(cmd, list):
            cmd = shlex.join(cmd)
        if host:
            if host['type'] == 'deployment':
                name = f"deployment/{host['name']}"
            elif host['type'] == 'pod':
                name = host['name']
            else:
                raise Exception(f"Unsupport host type: {host['type']}")
            code, out, err = self.run(f"kubectl get {host['type']} {host['name']} --namespace {host['namespace']}")
            if code != 0:
                if "not found" in f"{out} ${err}":
                    logger.warn(f"{host['type']}/{host['name']} in namespace {host['namespace']} not found, warning.")
                    return 73737373, out, err
                return code, out, err
            cmd = shlex.quote(cmd)
            cmd = f"kubectl exec {name} --namespace {host['namespace']} -- bash -c {cmd}"
        logger.info(f"{cmd}: start...")
        ...
        if result.returncode != 0:
            ...
            logger.info(f"{cmd}: return {result.returncode}.")
        else:
    ...
    def runs(self, cmds, host = None, exceptionOnFail = False):
        for cmd in cmds:
            code, out, err = self.run(cmd, host, exceptionOnFail)
            if code != 0:
                return code
        return 0
