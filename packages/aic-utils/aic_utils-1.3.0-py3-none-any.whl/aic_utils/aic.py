...
class AIC():
    ...
    def push_source_code(AIC_prod, AIC_qa, prod_to_qa=False, qa_to_prod=False, pipelines=[], push_to_branch=False, replace_placeholders=False):
        ...
        # Helper function to replace placeholders in SQL queries
        def replace_sql_placeholders(pipeline_config, tables_dict):
            """Replace placeholders in the SQL query."""
            print(f"Checking for placeholders in pipeline: {pipeline_config['$id']}")  # Pipeline identifier
            replacements = []
            ...
            for stage in pipeline_config.get('stages', []):
                ...
                for task in stage.get('tasks', []):
                    ...
                    if task_type == 'SQL':
                        ...
                        sql_query = task.get('config', {}).get('query', '')
                        if sql_query:
                            placeholder_found = False
                            # Look for placeholders in the form ${PLACEHOLDER}
                            for placeholder, table_name in tables_dict.items():
                                placeholder_format = f"${{{placeholder}}}"
                                if placeholder_format in sql_query:
                                    placeholder_found = True
                                    sql_query = sql_query.replace(placeholder_format, table_name)
                                    replacements.append((placeholder_format, table_name))
                                    print(f"Placeholder '{placeholder_format}' found and replaced with '{table_name}'.")

                            # If any placeholders were replaced, update the task's SQL
                            if placeholder_found:
                                task['config']['query'] = sql_query
                            else:
                                print("No placeholders found in this SQL.")
                        else:
                            print(f"No SQL query found in task: {task_name}")
            return replacements
        ...
        def fetch_tables(aic_instance):
            ...
            for dataset in datasets:
                ...
                for table in tables:
                    table_name = f"{dataset['name']}.{table['name']}"
                    tables_dict[table['name']] = table_name  # Add to dictionary
            return tables_dict
        ...
        for prod_pipeline in AIC_prod.pipeline_configs:
            ...
            for qa_pipeline in AIC_qa.pipeline_configs:
                ...
                print(f"Checking QA pipeline: {qa_name}")
    ...
    def get_pipelines(self, workspace_id):
        """
        Retrieve all pipelines in the specified workspace.
