...
class GPGTool(Command):
	...
	def _gpgcheck(self, keys=[]):
		if self._gpgselfcheck:
			return
		cmd = '%s --always-trust --batch --armor'%self.binary
		pln = ' ^~ test ~^ '
		for i in self.recvs:
			cmd = '%s -r %s -e'%(cmd, i)
			enc = self.stdo(cmd, inputs=pln)
			if not enc:
				continue
			dec = self.stdo('%s -d'%cmd, inputs=enc)
			if pln == dec:
	...
	def sign(self, data, out):
		"""detached signing method method"""
		if self.dbg:
		...
		try:
			...
			pass
		#print(f'gpg --use-agent --always-trust --armor --batch --detach-sign -u {self.gpgkey} -o {out} {data}')
		e = self.call(f'gpg --use-agent --always-trust --armor --batch --detach-sign -u {self.gpgkey} -o {out} {data}')
		if path.isfile(out):
			chmod(out, 0o600)
			return True

	def verify(self, sign, data):
		ret = self.call(f'gpg --use-agent --always-trust --armor --batch --verify {sign} {data}', stdout=DEVNULL, stderr=DEVNULL)
		if ret == 0:
			return True
	...
	def encrypt(self, data, **kwargs):
		...
		out = '' if 'output' not in kwargs.keys() else kwargs['output']
		message = data
		...
		outf = '' if not out else '-o %s'%out
		try:
			remove(out)
		except FileNotFoundError:
			pass
		#print(f'{self._binary} --always-trust --armor --batch -e {recvs} {outf}', message)
		dosig = ''
		if self.sig and not self.dtg:
			dosig = ' --sign' if not self.dtg else ''
		o, e, r = self.oerc(f'gpg --use-agent --pinentry-mode=loopback --always-trust --armor --batch -e{dosig} {recvs} {outf}', inputs=message)
		crypt = o
		sign = crypt
	...
	def decrypt(self, data, **kwargs):
		...
		while self.__c < 4:
			...
			#print(f'{self._binary} --armor --always-trust -d {recvs} {data}')
			__plain = self.stdo(f'{self._binary} --armor --always-trust -d {recvs} {data}')
			#print(__plain)
			if __plain:
				if sigerr:
