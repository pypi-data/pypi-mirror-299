...
class PassCrypt(GPGTool):
	...
	def _writecrypt(self, __weaks):
		...
		kwargs = {
            ...
            'gpgkey': self.gpgkey,
            'recvs': self.recvs}
		__weaks = str(dump(__weaks, Dumper=Dumper))
		#print(__weaks)
		filerotate(self.crypt, 3)
		if self.sig and self.dtg:
			filerotate('%s.sig'%self.crypt, 3)
		cfi, sig = self.encrypt(__weaks, output=self.crypt)
		if not cfi:
			copy2(f'{self.crypt}.1', self.crypt)
			if self.sig and self.dtg:
				copy2(f'{self.crypt}.sig.1', f'{self.crypt}.sig')
