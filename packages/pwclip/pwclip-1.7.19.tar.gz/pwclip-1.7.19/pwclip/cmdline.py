...
from pyautogui import \
    write as winwrite, hotkey, keyDown as keydown, \
    keyUp as keyuṕ, press as keypress
...
def forkwaitclip(text, poclp, boclp, wait=3, out=None, enter=None, kbid=None):
	...
	if out:
		...
		if out == 'gui':
			...
			#	hotkey('alt', 'tab')
			#	winwrite(_text, interval=0.025)
			#else:
			cmd = fr'xvkbd -secure -no-keypad -delay 17 -text {sep}{text}{sep}'
			#print(cmd)
			cmmd.stdo(cmd)
		elif out == 'cli':
	...
	if enter:
		if osname == 'nt' or system() == 'Windows':
			...
			if kbid and str(kbid).isdigit():
				kbid = ' -i %s'%kbid
			elif kbid:
				kbid = ' -i %s'%xkbid(kbid)
			else:
				kbid = ''
			call('%s%s "key Return"'%(which('xte'), kbid), shell=True)
	copy(text, mode='pb')
	if fork() == 0:
		try:
			sleep(int(wait))
...
def optpars(cfgs, mode, name):
	...
	pars.add_argument(
        '--kbname',
        dest='keyboard', help='name of the keyboard to use (see xinput list)')
	pars.add_argument(
        '-A', '--all',
...
def confpars(mode):
	...
	cfg = path.join(path.expanduser('~'), '.config', f'{_me}.cfg')
	cfgs = {
        'crypt': path.join(path.expanduser('~'), '.passcrypt'),
        'plain': path.join(path.expanduser('~'), '.pwd.yaml'),
        'time': 3,
        'binary': which('gpg2') if osname == 'nt' else 'gpg.exe',
        'user': whoami(),
        }
	try:
...
def cli():
	...
	if args.yks or args.yks is None:
		...
		forkwaitclip(res, poclp, boclp, args.time, args.out, args.ent, args.keyboard)
		exit(0)
	__ents = {}
	err = None
	if args.add:
		...
		if __pc:
			...
			forkwaitclip(__pc[0], poclp, boclp, args.time, args.out, args.ent), args.keyboard
	elif args.chg:
		if args.pwd:
		...
		if __ent:
			...
			if __pc:
				...
				if not args.out and osname != 'nt':
					xnotify(notif, __me)
				forkwaitclip(
                    __pc[0], poclp, boclp, args.time, args.out, args.ent, args.keyboard)
				exit(0)
	elif args.scl:
		t = 0 if 'time' not in pkwargs.keys() else pkwargs['time']
