...
class DeepchecksLLMClient:
    ...
    def __init__(self,
        ...
        """
        self._api: API = None
        self._host = host or os.environ.get("DEEPCHECKS_LLM_HOST", "https://app.llm.deepchecks.com")
    ...
    def log_interaction(
        ...
        if custom_props:
            self._validate_custom_props(
                existing_custom_properties=self.get_custom_properties(app_name=app_name),
                interaction_custom_properties=custom_props,
            )
    ...
    def log_batch_interactions(
        ...
        """
        existing_custom_properties = self.get_custom_properties(
            app_name=app_name
        )
        ...
        for interaction in interactions:
            ...
            self._validate_custom_props(
                existing_custom_properties=existing_custom_properties,
                interaction_custom_properties=interaction.custom_props,
            )
    ...
    @staticmethod
    def _validate_custom_props(existing_custom_properties: t.List[CustomPropertyType], interaction_custom_properties: t.Union[dict, None]):
        if interaction_custom_properties is None:
            return

        existing_custom_properties = set(el.display_name.lower() for el in existing_custom_properties)
        for prop_name in interaction_custom_properties:
            if prop_name.lower().replace("cp_", "") not in existing_custom_properties:
                logger.info(f"Property does not exists for this application, {prop_name} will be created automatically.")
