...
def deal_with_input(state: State, file_content: str = None):
    ...
    while True:
        ...
        if fetched_line.startswith(COMMAND__initialiser) or fetched_line.startswith(COMMAND__with_parameters) or fetched_line.startswith(COMMAND__with_user):
            ...
            elif fetched_line.startswith(COMMAND__with_user):
                overriding_user = fetched_line.split(COMMAND__with_user)[1].strip()
            ...
            if state.reading_parameters:
                if fetched_line.strip().startswith("}"):
                    ...
                    if len(fetched_line.split("} ")) > 1 and (fetched_line.split("} ")[1].startswith(COMMAND__and_user) or fetched_line.split("} ")[1].startswith(COMMAND__with_user)):
                        delimiter = COMMAND__and_user if COMMAND__and_user in fetched_line else COMMAND__with_user
                        overriding_user = fetched_line.split(delimiter)[1].strip()
                        if "\"" in overriding_user or "'" in overriding_user:
                            print_error(state, "Please do not quote the user!")
                        state.get_current_connection().username = overriding_user
