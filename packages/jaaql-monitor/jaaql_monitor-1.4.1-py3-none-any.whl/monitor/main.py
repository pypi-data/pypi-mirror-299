...
def execute_command(state, command):
    ...
    try:
        ...
        with open("C:/Users/aaron.tasker/Desktop/output.txt", "a") as f:
            f.write(state.file_name + "_psql: " + str(State.time_delta_ms(start_time, end_time)) + "ms\r\n")
            f.write("")
        return result
    except Exception as e:
...
def initialise_from_args(args, file_name: str = None, file_content: str = None, do_exit: bool = True, override_url: str = None, do_prepare: bool = False):
    ...
    end_time = datetime.now()
    with open("C:/Users/aaron.tasker/Desktop/output.txt", "a") as f:
        f.write(state.file_name + ": " + str(State.time_delta_ms(start_time, end_time)) + "ms\r\n")
        f.write("")
