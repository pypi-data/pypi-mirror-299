...
def get_model_display_data(root_instance, user):
    ...
    """
    result = []
    new_objects = [(root_instance._meta.verbose_name.title(), root_instance, 'process')]
    processed_models, processed_objects = [], []

    # Ext models
    models_names = root_instance.add_models if hasattr(root_instance, 'add_models') else tuple()
    for name in models_names:
        if hasattr(root_instance, name):
            objs = getattr(root_instance, name)
            if objs and isinstance(objs, models.Model):
                new_objects.append((objs._meta.verbose_name.title(), objs, name))
            elif objs and isinstance(objs, QuerySet):
                for obj in objs:
                    new_objects.append((obj._meta.verbose_name.title(), obj, name))

    while new_objects:
        root_title, root, name = new_objects.pop(0)
        root_admin_url = None
        children = []
        ...
        # objects fields
        allowed_fields = models_names.get(name)
        for field in root._meta.fields:
            if isinstance(field, models.AutoField):
                continue
            elif field.auto_created:
                continue
            elif allowed_fields and field.name not in allowed_fields:
                continue
        ...
        if children:
            ...
            if user.has_perm("%s.%s" % (root._meta.app_label, change_perm)):
                ...
                try:
                    ...
                    pass
            if root_instance._meta.verbose_name.title() != root_title:
                result.append((str(root), root_title, children, root_admin_url))
