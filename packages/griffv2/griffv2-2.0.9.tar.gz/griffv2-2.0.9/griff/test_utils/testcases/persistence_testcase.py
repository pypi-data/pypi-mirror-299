...
from griff.infra.persistence.domain_persistence import DomainPersistence
from griff.test_utils.mixins.deserialize_persistence_test_mixin import (
    DeserializePersistenceTestMixin,
)
...
class PersistenceTestCase(
    DeserializePersistenceTestMixin, RuntimeTestMixin, TestCase, ABC
):
    @classmethod
    @abstractmethod
    ...
    async def run_crud(
        ...
        return {
            "inserted_data": self.deserialize(inserted_data),  # type: ignore
            "after_update": self.deserialize(after_update),
        }
