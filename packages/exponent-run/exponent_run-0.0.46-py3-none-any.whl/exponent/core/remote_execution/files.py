...
from exponent.core.remote_execution.git import GitIgnoreHandler, get_repo, git_file_walk
...
async def file_walk(
    ...
    """
    working_path = await AsyncPath(directory).resolve()

    results: list[str] = []

    ignore_handler = GitIgnoreHandler(
        working_directory=str(working_path), default_ignores=ignore_extra
    )

    for root, dirs, paths in walk(working_path):
        resolved_root = await AsyncPath(root).resolve()

        await ignore_handler.read_ignorefile(str(resolved_root))

        filtered_paths = ignore_handler.filter(paths, str(resolved_root))

        for sub_path in sorted(filtered_paths):
            current_path = resolved_root / sub_path
            relative_path = _get_safe_relative_path(current_path, working_path)
        ...
        dirs[:] = ignore_handler.filter(dirs, str(resolved_root))
