...
from jinja2 import Template # GITLAB CHANGES
from dataclasses import replace # GITLAB CHANGES
    ...
    def get_historical_features(
        ...
        def query_generator() -> Iterator[str]:
            ...
            for context in query_context:
                if context.is_template_table:
                    # Render the template query in the context
                    template = Template(context.table_subquery)
                    # Render the SQL query with the desired context
                    rendered_query = template.render(get_historical_features=True)
                    rendered_query = rendered_query.replace('USE_TEMPLATE_WORKFLOW', '')
                    # Create a new instance with the updated table_subquery
                    updated_context = replace(context, table_subquery=rendered_query)
                    updated_query_context.append(updated_context)
                else:
                    updated_query_context.append(context)
...
# GITLAB CHANGES: BELLOW QUERY TEMPLATE
...
{% if featureview.is_template_table == False %}
...
"{{ featureview.name }}__template_workflow_table" AS (
    SELECT
        "{{ featureview.timestamp_field }}" as "entity_timestamp",
        {{'"' ~ featureview.created_timestamp_column ~ '" as "created_timestamp",' if featureview.created_timestamp_column else '' }}
        {{featureview.entity_selections | join(', ')}}{% if featureview.entity_selections %},{% else %}{% endif %}
        {% for feature in featureview.features %}
            "{{ feature }}" as {% if full_feature_names %}"{{ featureview.name }}__{{featureview.field_mapping.get(feature, feature)}}"{% else %}"{{ featureview.field_mapping.get(feature, feature) }}"{% endif %}{% if loop.last %}{% else %}, {% endif %}
...
SELECT 
{% for field in entity_df_columns %}
    "entity_dataframe"."{{ field }}",
{% endfor %}
...
LEFT JOIN "{{ featureview.name }}__template_workflow_table"
    ON "{{ featureview.name }}__template_workflow_table"."entity_timestamp" = "entity_dataframe"."entity_timestamp"
{% for entity in featureview.entities %}
    AND "{{ featureview.name }}__template_workflow_table"."{{ entity }}" = "entity_dataframe"."{{ entity }}"
{% endfor %}
