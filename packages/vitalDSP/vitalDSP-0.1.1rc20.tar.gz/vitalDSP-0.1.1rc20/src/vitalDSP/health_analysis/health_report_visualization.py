...
class HealthReportVisualizer:
    ...
    def create_visualizations(self, feature_data, output_dir="visualizations"):
        ...
        for feature, values in feature_data.items():
            try:
                ...
                # Remove NaN or Inf values from the list of values
                values = [
                    value
                    for value in values
                ...
                visualization_paths[feature] = {
                    "heatmap": self._create_heatmap_plot(feature, values, output_dir),
                    "bell_plot": self._create_bell_shape_plot(
                        feature, values, output_dir
                    ),
                    "radar_plot": self._create_radar_plot(feature, values, output_dir),
                    "violin_plot": self._create_violin_plot(
                        feature, values, output_dir
                    ),
                    "line_with_rolling_stats": self._create_plot_line_with_rolling_stats(
                        feature, values, output_dir
                    ),
                    "lag_plot": self._create_plot_lag(feature, values, output_dir),
                    "plot_periodogram": self._create_plot_periodogram(
                        feature, values, output_dir
                    ),
                    "plot_spectrogram": self._create_plot_spectrogram(
                        feature, values, output_dir
                    ),
                    "plot_spectral_density": self._create_spectral_density_plot(
                        feature, values, output_dir
                    ),
                    "plot_box_swarm": self._create_box_swarm_plot(
                        feature, values, output_dir
                    ),
                }
            except Exception as e:
                print(f"Error creating visualizations for feature '{feature}': {e}")
                continue
    ...
    def _create_bell_shape_plot(self, feature, values, output_dir):
        ...
        # Calculate standard deviation based on normal range
        stddev_value = (normal_max - normal_min) / 4  # Assuming normal distribution

        # Define cap limits for outliers
        cap_limit = normal_max + 2.5 * stddev_value

        # Cap the values to fit the normal range and specified cap
        capped_values = np.clip(values, normal_min, cap_limit)
        ...
        x = np.linspace(
            normal_min - 2 * stddev_value, cap_limit + 2 * stddev_value, 200
        )
        ...
        plt.axvline(
            mean_value, color="#8e44ad", linestyle="--", label=f"Mean: {mean_value:.2f}"
        )

        # Handle outliers - mark them at the cap limit
        outliers = [value for value in values if value > cap_limit]
        if outliers:
            # Instead of plotting at their actual values, plot them at the cap limit
            plt.scatter(
                [cap_limit] * len(outliers),
                [0] * len(outliers),
                color="#e74c3c",
                s=100,
                zorder=5,
                edgecolor="black",
                label=f"Outliers capped at {cap_limit:.2f}",
            )
        ...
        plt.title(f"{feature} Bell Shape Plot with Histogram", fontsize=16)
    ...
    def _create_plot_periodogram(
        ...
        plt.semilogy(frequencies_cpm, Pxx, color="royalblue", linewidth=2.5, alpha=0.8)
    ...
    def _create_plot_seasonal_decomposition(
        ...
        decomposition = seasonal_decompose(df["data"], model="additive", period=freq)
    ...
    def _create_violin_plot(self, feature, values, output_dir):
        ...
        # Cap the values to fit within the plot range
        capped_values = np.clip(values, outlier_min, outlier_max)

        # Create the figure
        plt.figure(figsize=(10, 7))

        # Create the violin plot for the capped values
        sns.violinplot(
            data=capped_values,
            color="lightgreen",
            inner="quartile",
        ...
        for value, count in value_counts.items():
            ...
            plt.scatter(
                ...
                color=color,
                zorder=5,
                s=50 + count * 10,  # Adjust size based on count
                edgecolors="black",
            )
