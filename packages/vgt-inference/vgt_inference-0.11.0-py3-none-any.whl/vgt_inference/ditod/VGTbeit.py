...
class CrossBlock(nn.Module):
    def __init__(
        ...
        if init_values is not None:
            ...
            self.gamma_vis, self.gamma_grid, self.gamma_1, self.gamma_2 = (
                None,
                None,
                None,
                None,
    ...
    def cross_att(self, vis_input, grid_input):
        ...
        if self.gamma_vis is None:
            vis_att_output = vis_input + self.drop_path(
                self.vis_attn(self.norm_vis(vis_input), self.norm_grid(grid_input))
            )
    ...
    def forward(self, vis_input, grid_input):
        ...
        if self.gamma_1 is None:
            vis_output = vis_output + self.drop_path(
                self.vis_mlp(self.norm2_vis(vis_output))
            )
            grid_output = grid_output + self.drop_path(
                self.grid_mlp(self.norm2_grid(grid_output))
            )
        else:
            vis_output = vis_output + self.drop_path(
                self.gamma_1 * self.vis_mlp(self.norm2_vis(vis_output))
            )
            grid_output = grid_output + self.drop_path(
                self.gamma_2 * self.grid_mlp(self.norm2_grid(grid_output))
            )
...
class CrossSelfBlock(nn.Module):
    ...
    def self_att(self, vis_input, grid_input):
        ...
        if self.gamma_vis is None:
            vis_att_output = vis_input + self.drop_path(
                self.vis_attn(self.norm_vis(vis_input))
            )
            grid_att_output = grid_input + self.drop_path(
                self.grid_attn(self.norm_grid(grid_input))
            )
        else:
            vis_att_output = vis_input + self.drop_path(
                self.gamma_vis * self.vis_attn(self.norm_vis(vis_input))
            )
            grid_att_output = grid_input + self.drop_path(
                self.gamma_grid * self.grid_attn(self.norm_grid(grid_input))
            )
        return vis_att_output, grid_att_output
        ...
        for i, grid_blk in enumerate(self.grid_blocks):
            if self.use_checkpoint:
                grid_x = checkpoint.checkpoint(
                    grid_blk, grid_x, rel_pos_bias, grid_training_window_size
                )
            else:
                grid_x = grid_blk(
                    grid_x,
...
def VGT_dit_base_patch16(**kwargs):
    model = BEiT(
        patch_size=16,
        embed_dim=768,
        self_depth=12,
...
if __name__ == "__main__":
    ...
    input1 = torch.rand(2, 3, 512, 762).to(device)
    input2 = torch.rand(2, 3, 800, 1200).to(device)
    input3 = torch.rand(2, 3, 720, 1000).to(device)
