...
class QuboModel:
    ...
    def write_qubo(self, path: str) -> None:
        ...
        for key in self.objective.terms:
            ...
            if term.order() == 1:
                triplets.append(
                    (term.vars[0].id(), term.vars[0].id(), term.coefficient)
                )
            ...
            if term.order() == 2:
                triplets.append(
                    (term.vars[0].id(), term.vars[1].id(), 0.5 * term.coefficient)
                )
    ...
    def _read_qubo_file(cls, f: TextIO) -> QuboModel:
        ...
        while True:
            ...
            if (
                first_line == ""
                or cls.is_triplet(first_line)
                or cls.is_fixing(first_line)
            ):
                error_message = ("The .qubo file's format is wrong: It must start either "
                                 "with the sense or the offset of the problem. "
                                 + msg_ref_documentation)
                raise ModelError(error_message)
        ...
        if not cls.is_triplet(first_five_lines[1]) and not cls.is_triplet(
            first_five_lines[4]
        ):
        ...
        if cls.is_triplet(first_five_lines[1]):
            ...
            warnings.warn("This .qubo file format is deprecated. In the current file format, "\
                          "before the triplets, only the sense and the shift are given.", stacklevel=2)
            old_version = True
        ...
        if cls.is_fixing(line):
            error_message = ("The .qubo file's format is wrong: Fixings must be given after the triplets, "
        ...
        while cls.is_triplet(line) or cls.is_comment(line):
            ...
            if not cls.is_comment(line):
                ...
                if (ix_i < 0 or ix_j < 0):
                    error_message = ("The .qubo file's format is wrong: "
                                     "The indices of the triplet must be positive integers. "
                                     + msg_ref_documentation)
                    raise ModelError(error_message)
                if ix_i > ix_j:
        ...
        if cls.is_fixing(line):
            ...
            for line in f:
                if not cls.is_fixing(line) and not cls.is_comment(line):
                    error_message = ("The .qubo file's format is wrong: Fixings must be after the triplets, "
                                     "at the bottom of the file. " + msg_ref_documentation)
                    raise ModelError(error_message)
                if not cls.is_comment(line):
                    split = line.split(" ")
                    ...
                    if fixing not in (0, 1):
                        error_message = ("The fixings are wrong: A variable can be fixed only to 0 or 1. "
                                         + msg_ref_documentation)
                        raise ModelError(error_message)
                    vars_to_fix.append(var_id)
                    fixings.append(fixing)
        elif line != "":
            error_message = ("The format of the .qubo file is wrong: "
    ...
    def from_qiskit_qubo(cls, qp: QiskitQP) -> QuboModel:
        ...
        if (
            qp.get_num_linear_constraints() != 0
            or qp.get_num_quadratic_constraints() != 0
        ):
    ...
    @classmethod
    def from_dwave_bqm(
        cls, bqm: BinaryQuadraticModel, name: str = "fromdwaveqbm"
    ) -> QuboModel:
    ...
    @classmethod
    def from_pyqubo(
        cls, pqm: pqModel, constants: dict | None = None, name: str = "frompyqubo"
    ) -> QuboModel:
    ...
    def solve(
        self, hybrid_solver: HybridSolver, params: HybridSolverParameters | None = None
    ) -> dict[str, Any]:
