...
from .exchanges import CryptoExchanges
from .ohlc_loader import CryptoOhlcLoader
from .trades_loader import CryptoTradesLoader


class CryptoRawSeriesDataLoader:
    def __init__(self, instruments: List[str], day_from: datetime, day_to: datetime, exchanges: [CryptoExchanges] = None):
    ...
    def load_raw_series(self) -> None:
        for instrument in self.instruments:
            for target_day in self.selected_days:
                ohlc_series = CryptoOhlcLoader(target_day, instrument).get_ohlc_series()
                grouped_trades = CryptoTradesLoader(target_day, instrument, self.exchanges).get_grouped_trades()
