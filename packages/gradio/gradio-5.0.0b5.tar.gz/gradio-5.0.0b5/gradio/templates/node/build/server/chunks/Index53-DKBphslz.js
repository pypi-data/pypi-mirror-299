import { B as Block, S as Static, o as globals, q as colors } from './2-Ax4bcEC3.js';
...
import { g as get_next_color } from './color-C2IVRx8F.js';
import { B as BlockLabel } from './BlockLabel-CcEvnTTV.js';
import { E as Empty } from './Empty-CJWSov-X.js';
...
const StaticHighlightedtext = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return ` <div class="container svelte-1woixh4">${mode === "categories" ? `${show_legend ? `<div class="category-legend svelte-1woixh4" data-testid="highlighted-text:category-legend">${each(Object.entries(_color_map), ([category, color], i) => {
    return `  <div class="category-label svelte-1woixh4"${add_attribute("style", "background-color:" + color.secondary, 0)}>${escape(category)} </div>`;
  })}</div>` : ``} <div class="textfield svelte-1woixh4">${each(value, (v, i) => {
    return `${each(splitTextByNewline$1(v.token), (line, j) => {
      return `${line.trim() !== "" ? `   <span class="${[
        "textspan svelte-1woixh4",
        ...
        v.class_or_confidence === null || !_color_map[v.class_or_confidence] ? "no-label" : ""
								...
								<span class="label svelte-1woixh4"${add_styles({
...
const LabelInput = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `  ${!isScoresMode ? `<input class="label-input svelte-df6jzs" autofocus${add_attribute("id", `label-input-${indexOfLabel}`, 0)} type="text" placeholder="label"${add_attribute("value", category, 0)}${add_styles({
  ...
  })}>` : `<input class="label-input svelte-df6jzs" autofocus type="number" step="0.1"${add_styles(merge_ssr_styles(
    escape(
...
const InteractiveHighlightedtext = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    ...
    $$rendered = `<div class="container svelte-u7mykt">${mode === "categories" ? `${show_legend ? `<div class="class_or_confidence-legend svelte-u7mykt" data-testid="highlighted-text:class_or_confidence-legend">${_color_map ? `${each(Object.entries(_color_map), ([class_or_confidence, color], i) => {
      return `<div role="button" aria-roledescription="Categories of highlighted text. Hover to see text with this class_or_confidence highlighted." tabindex="0" class="class_or_confidence-label svelte-u7mykt"${add_attribute("style", "background-color:" + color.secondary, 0)}>${escape(class_or_confidence)} </div>`;
    })}` : ``}</div>` : ``} <div class="textfield svelte-u7mykt">${each(value, ({ token, class_or_confidence }, i) => {
      return `${each(splitTextByNewline(token), (line, j) => {
        return `${line.trim() !== "" ? `<span class="text-class_or_confidence-container svelte-u7mykt"><span role="button" tabindex="0" class="${[
          "textspan svelte-u7mykt",
        ...
        })}><span class="${["text svelte-u7mykt", class_or_confidence === null ? "no-label" : ""].join(" ").trim()}" role="button" tabindex="0">${escape(line)}</span> ${!show_legend && class_or_confidence !== null && labelToEdit !== i ? `<span${add_attribute("id", `label-tag-${i}`, 0)} class="label svelte-u7mykt" role="button" tabindex="0"${add_styles({
          ...
          {}
        )}` : ``}</span> ${class_or_confidence !== null ? `<span class="label-clear-button svelte-u7mykt" role="button" aria-roledescription="Remove label from text" tabindex="0" data-svelte-h="svelte-1fuy4vv">×
								</span>` : ``} </span>` : ``} ${j < splitTextByNewline(token).length - 1 ? `<br>` : ``}`;
      })}`;
    })}</div>` : `${show_legend ? `<div class="color-legend svelte-u7mykt" data-testid="highlighted-text:color-legend" data-svelte-h="svelte-mv3vmx"><span>-1</span> <span>0</span> <span>+1</span></div>` : ``} <div class="textfield svelte-u7mykt" data-testid="highlighted-text:textfield">${each(value, ({ token, class_or_confidence }, i) => {
      let score = typeof class_or_confidence === "string" ? parseInt(class_or_confidence) : class_or_confidence;
      return ` <span class="score-text-container svelte-u7mykt"><span class="${[
        "textspan score-text svelte-u7mykt",
        (class_or_confidence === null || active ? "no-cat" : "") + " " + (class_or_confidence !== null ? "hl" : "")
      ].join(" ").trim()}" role="button" tabindex="0"${add_attribute(
        "style",
        "background-color: rgba(" + (score && score < 0 ? "128, 90, 213," + -score : "239, 68, 60," + score) + ")",
        0
      )}><span class="text svelte-u7mykt">${escape(token)}</span> ${class_or_confidence && labelToEdit === i ? `${validate_component(LabelInput, "LabelInput").$$render(
        $$result,
        {
          labelToEdit,
          _color_map,
        ...
        {}
      )}` : ``}</span> ${class_or_confidence && activeElementIndex === i ? `<span class="label-clear-button svelte-u7mykt" role="button" aria-roledescription="Remove label from text" tabindex="0" data-svelte-h="svelte-hxhs1z">×
						</span>` : ``} </span>`;
    })}</div>`} </div>`;
  } while (!$$settled);
  return $$rendered;
...
//# sourceMappingURL=Index53-DKBphslz.js.map
