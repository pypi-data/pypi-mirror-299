...
const UploadText = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$result.css.add(css);
  [heading, paragraph] = placeholder ? inject(placeholder) : [false, false];
  return `<div class="wrap svelte-1xg7h5n"><span class="${["icon-wrap svelte-1xg7h5n", hovered ? "hovered" : ""].join(" ").trim()}">${type === "clipboard" ? `${validate_component(ImagePaste, "ImagePaste").$$render($$result, {}, {}, {})}` : `${validate_component(Upload, "UploadIcon").$$render($$result, {}, {}, {})}`}</span> ${heading || paragraph ? `${heading ? `<h2 class="svelte-1xg7h5n">${escape(heading)}</h2>` : ``} ${paragraph ? `<p class="svelte-1xg7h5n">${escape(paragraph)}</p>` : ``}` : `${escape(i18n(defs[type] || defs.file))} ${mode !== "short" ? `<span class="or svelte-1xg7h5n">- ${escape(i18n("common.or"))} -</span> ${escape(message || i18n("upload_text.click_to_upload"))}` : ``}`} </div>`;
});

export { UploadText as U };
//# sourceMappingURL=UploadText-ro6A0c4A.js.map
