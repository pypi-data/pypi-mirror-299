...
const Tabs = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${has_tabs ? `<div class="${[
    "tabs " + escape(elem_classes.join(" "), true) + " svelte-1e5gm09",
    !visible ? "hide" : ""
  ].join(" ").trim()}"${add_attribute("id", elem_id, 0)}><div class="tab-wrapper svelte-1e5gm09"><div class="tab-container svelte-1e5gm09" role="tablist"${add_attribute("this", tab_nav_el, 0)}>${each(tabs, (t, i) => {
    return `<button role="tab"${add_attribute("aria-selected", t.id === $selected_tab, 0)}${add_attribute("aria-controls", t.elem_id, 0)} ${!t.interactive ? "disabled" : ""}${add_attribute("aria-disabled", !t.interactive, 0)}${add_attribute("id", t.elem_id ? t.elem_id + "-button" : null, 0)}${add_attribute("data-tab-id", t.id, 0)} class="${["svelte-1e5gm09", t.id === $selected_tab ? "selected" : ""].join(" ").trim()}">${escape(t.name)} </button>`;
  })}</div> <span class="${["overflow-menu svelte-1e5gm09", "hide"].join(" ").trim()}"${add_attribute("this", overflow_menu, 0)}><button class="${[
    "svelte-1e5gm09",
    ""
  ].join(" ").trim()}">${validate_component(OverflowIcon, "OverflowIcon").$$render($$result, {}, {}, {})}</button> <div class="${["overflow-dropdown svelte-1e5gm09", !overflow_menu_open ? "hide" : ""].join(" ").trim()}"${add_attribute("this", overflow_nav, 0)}></div></span></div></div>` : ``} <div>${slots.default ? slots.default({}) : ``} </div>`;
});

export { TABS as T, Tabs as a };
//# sourceMappingURL=Tabs-BXoZlMhg.js.map
