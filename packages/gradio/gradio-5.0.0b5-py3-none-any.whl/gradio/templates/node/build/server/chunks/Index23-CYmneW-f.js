...
const Index = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${validate_component(Block, "Block").$$render(
    ...
    {
      default: () => {
        return `<div class="label svelte-x1loj2"><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" aria-hidden="true" role="img" width="1em" height="1em" preserveAspectRatio="xMidYMid meet" viewBox="0 0 32 32" class="svelte-x1loj2"><path fill="currentColor" d="M10 6h18v2H10zm0 18h18v2H10zm0-9h18v2H10zm-6 0h2v2H4zm0-9h2v2H4zm0 18h2v2H4z"></path></svg> ${escape(label)}</div> ${gallery ? `<div class="gallery svelte-x1loj2">${each(selected_samples, (sample_row, i) => {
          return `${sample_row[0] ? `<button class="gallery-item svelte-x1loj2">${sample_labels ? `${validate_component(Example, "BaseExample").$$render(
            $$result,
            {
              value: sample_row[0],
              selected: current_hover === i,
        ...
        })}</div>` : `<div class="table-wrap svelte-x1loj2"><table tabindex="0" role="grid" class="svelte-x1loj2"><thead><tr class="tr-head svelte-x1loj2">${each(headers, (header) => {
          return `<th class="svelte-x1loj2">${escape(header)} </th>`;
        })}</tr></thead> <tbody>${each(component_meta, (sample_row, i) => {
          return `<tr class="tr-body svelte-x1loj2">${each(sample_row, ({ value: value2, component }, j) => {
            let component_name = components[j];
            return ` ${component_name !== void 0 && component_map.get(component_name) !== void 0 ? `<td style="${"max-width: " + escape(component_name === "textbox" ? "35ch" : "auto", true)}" class="${escape(null_to_empty(component_name), true) + " svelte-x1loj2"}">${validate_component(component || missing_component, "svelte:component").$$render($$result, Object.assign({}, component_props[j], { value: value2 }, { samples_dir }, { type: "table" }, { selected: current_hover === i }, { index: i }, { root }), {}, {})} </td>` : ``}`;
          })} </tr>`;
        })}</tbody></table></div>`} ${paginate ? `<div class="paginate svelte-x1loj2">Pages:
			${each(visible_pages, (visible_page) => {
          return `${visible_page === -1 ? `<div data-svelte-h="svelte-12rhcfw">...</div>` : `<button class="${["svelte-x1loj2", page === visible_page ? "current-page" : ""].join(" ").trim()}">${escape(visible_page + 1)} </button>`}`;
        })}</div>` : ``}`;
      }
    }
  )}`;
...
//# sourceMappingURL=Index23-CYmneW-f.js.map
