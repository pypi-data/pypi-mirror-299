...
import { B as Block, S as Static, h as BlockTitle, l as Clear, x as Send, y as Square } from './2-Ax4bcEC3.js';
...
export { default as BaseExample } from './Example18-CtxeL7Eu.js';
...
const MultimodalTextbox = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    ...
    $$rendered = `<div class="${["full-container svelte-1pfxcef", dragging ? "dragging" : ""].join(" ").trim()}" role="group" aria-label="Multimedia input field"${add_attribute("this", full_container, 0)}> <label${add_classes((container ? "container" : "").trim())}>${validate_component(BlockTitle, "BlockTitle").$$render($$result, { show_label, info }, {}, {
      default: () => {
        return `${escape(label)}`;
      }
    })} ${value.files.length > 0 || uploading ? `<div class="thumbnails scroll-hide svelte-1pfxcef" aria-label="Uploaded files" data-testid="container_el" style="${"display: " + escape(value.files.length > 0 || uploading ? "flex" : "none", true) + ";"}">${each(value.files, (file, index) => {
      return `<span role="listitem" aria-label="File thumbnail"><button class="thumbnail-item thumbnail-small svelte-1pfxcef"><button class="${["delete-button svelte-1pfxcef", disabled ? "disabled" : ""].join(" ").trim()}">${validate_component(Clear, "Clear").$$render($$result, {}, {}, {})}</button> ${file.mime_type && file.mime_type.includes("image") ? `${validate_component(Image$1, "Image").$$render(
        $$result,
        {
          src: file.url,
          title: null,
    ...
    })} ${uploading ? `<div class="loader svelte-1pfxcef" role="status" aria-label="Uploading"></div>` : ``}</div>` : ``} <div class="input-container svelte-1pfxcef">${validate_component(Upload, "Upload").$$render(
      $$result,
      {
        file_count,
        filetype: file_types,
      ...
      {}
    )} <button data-testid="upload-button" class="upload-button svelte-1pfxcef">${validate_component(Paperclip, "Paperclip").$$render($$result, {}, {}, {})}</button> <textarea data-testid="textbox" class="${["scroll-hide svelte-1pfxcef", !show_label ? "no-label" : ""].join(" ").trim()}"${add_attribute("dir", rtl ? "rtl" : "ltr", 0)}${add_attribute("placeholder", placeholder, 0)}${add_attribute("rows", lines, 0)} ${disabled ? "disabled" : ""} ${autofocus ? "autofocus" : ""}${add_attribute("style", text_align ? "text-align: " + text_align : "", 0)}${add_attribute("this", el, 0)}>${escape(value.text || "")}</textarea> ${submit_btn ? `<button class="${["submit-button svelte-1pfxcef", submit_btn !== true ? "padded-button" : ""].join(" ").trim()}">${submit_btn === true ? `${validate_component(Send, "Send").$$render($$result, {}, {}, {})}` : `${escape(submit_btn)}`}</button>` : ``} ${stop_btn ? `<button class="${["stop-button svelte-1pfxcef", stop_btn !== true ? "padded-button" : ""].join(" ").trim()}">${stop_btn === true ? `${validate_component(Square, "Square").$$render($$result, { fill: "none", stroke_width: 2.5 }, {}, {})}` : `${escape(stop_btn)}`}</button>` : ``}</div></label> </div>`;
  } while (!$$settled);
  return $$rendered;
});
...
//# sourceMappingURL=Index46-kgC-08m1.js.map
