...
def encode_no_number(cell: FsCell) -> IEncodable:
    ...
    class ObjectExpr269(IEncodable):
    ...
    return ObjectExpr269()


def encode_rows(cell: FsCell) -> IEncodable:
    def _arrow271(__unit: None=None, cell: Any=cell) -> IEncodable:
        value_1: int = cell.ColumnNumber or 0
        class ObjectExpr270(IEncodable):
            def Encode(self, helpers: IEncoderHelpers_1[Any]) -> Any:
                return helpers.encode_signed_integral_number(value_1)

        return ObjectExpr270()

    values: IEnumerable_1[tuple[str, IEncodable]] = to_enumerable([("column", _arrow271()), ("value", encode(cell.Value))])
    class ObjectExpr272(IEncodable):
...
def decode_rows(row_number: int | None=None) -> Decoder_1[FsCell]:
    def _arrow275(builder: IGetters, row_number: Any=row_number) -> FsCell:
        def _arrow273(__unit: None=None) -> tuple[Any, DataType] | None:
            object_arg: IOptionalGetter = builder.Optional
            return object_arg.Field("value", decode)

        pattern_input: tuple[Any, DataType] = default_arg(_arrow273(), ("", DataType(4)))
        def _arrow274(__unit: None=None) -> int | None:
            object_arg_1: IOptionalGetter = builder.Optional
            return object_arg_1.Field("column", int_1)

        c: int = default_arg(_arrow274(), 0) or 0
        return FsCell(pattern_input[0], pattern_input[1], FsAddress__ctor_Z37302880(default_arg(row_number, 0), c))

    return object(_arrow275)


def encode_cols(cell: FsCell) -> IEncodable:
    def _arrow279(__unit: None=None, cell: Any=cell) -> IEncodable:
        value_1: int = cell.RowNumber or 0
        class ObjectExpr278(IEncodable):
            def Encode(self, helpers: IEncoderHelpers_1[Any]) -> Any:
                return helpers.encode_signed_integral_number(value_1)

        return ObjectExpr278()

    values: IEnumerable_1[tuple[str, IEncodable]] = to_enumerable([("row", _arrow279()), ("value", encode(cell.Value))])
...
def decode_cols(col_number: int | None=None) -> Decoder_1[FsCell]:
    def _arrow285(builder: IGetters, col_number: Any=col_number) -> FsCell:
        def _arrow282(__unit: None=None) -> tuple[Any, DataType] | None:
            object_arg: IOptionalGetter = builder.Optional
            return object_arg.Field("value", decode)

        pattern_input: tuple[Any, DataType] = default_arg(_arrow282(), ("", DataType(4)))
        def _arrow284(__unit: None=None) -> int | None:
            object_arg_1: IOptionalGetter = builder.Optional
            return object_arg_1.Field("row", int_1)

        return FsCell(pattern_input[0], pattern_input[1], FsAddress__ctor_Z37302880(default_arg(_arrow284(), 0), default_arg(col_number, 0)))

    return object(_arrow285)
