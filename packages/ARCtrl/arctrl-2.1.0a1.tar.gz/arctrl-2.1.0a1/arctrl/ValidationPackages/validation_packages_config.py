...
class ValidationPackagesConfig:
    ...
    def __str__(self, __unit: None=None) -> str:
        this: ValidationPackagesConfig = self
        def _arrow2888(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow2887(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow2886(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow2885(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow2884(__unit: None=None) -> IEnumerable_1[str]:
                            def _arrow2883(__unit: None=None) -> IEnumerable_1[str]:
                                return singleton("}")

                            return append(singleton("]"), delay(_arrow2883))

                        return append(singleton(join((";" + "\n") + "", map(to_string, this.ValidationPackages))), delay(_arrow2884))

                    return append(singleton(" ValidationPackages = ["), delay(_arrow2885))

                return append(singleton((" ARCSpecification = " + value(this.ARCSpecification)) + "") if (this.ARCSpecification is not None) else empty(), delay(_arrow2886))

            return append(singleton("{"), delay(_arrow2887))

        return join("\n", to_list(delay(_arrow2888)))
    ...
    def StructurallyEquals(self, other: ValidationPackagesConfig) -> bool:
        ...
        if equals(this.ARCSpecification, other.ARCSpecification):
            ...
            def _arrow2891(__unit: None=None) -> IEnumerable_1[bool]:
                def _arrow2890(i: int) -> bool:
                    return equals(item(i, a), item(i, b))

                return map(_arrow2890, range_big_int(0, 1, length(a) - 1))

            return fold(folder, True, to_list(delay(_arrow2891))) if (length(a) == length(b)) else False
...
ValidationPackagesConfig_reflection = _expr2892
