...
def to_fs_columns(dc: Array[DataContext]) -> FSharpList[FSharpList[FsCell]]:
    ...
    class ObjectExpr1189:
        @property
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow1188(x: str, y: str) -> bool:
                return x == y
    ...
    comment_keys: FSharpList[str] = to_list(distinct(collect(mapping_1, dc), ObjectExpr1189()))
    ...
    def _arrow1200(__unit: None=None, dc: Any=dc) -> IEnumerable_1[FSharpList[FsCell]]:
        def _arrow1199(__unit: None=None) -> IEnumerable_1[FSharpList[FsCell]]:
            def _arrow1198(dc_4: DataContext) -> FSharpList[FsCell]:
                dc_3: DataContext = dc_4
                def _arrow1197(__unit: None=None) -> IEnumerable_1[FsCell]:
                    def _arrow1190(__unit: None=None) -> FSharpList[FsCell]:
                    ...
                    def _arrow1196(__unit: None=None) -> IEnumerable_1[FsCell]:
                        def _arrow1195(__unit: None=None) -> IEnumerable_1[FsCell]:
                            def _arrow1194(__unit: None=None) -> IEnumerable_1[FsCell]:
                                def _arrow1193(__unit: None=None) -> IEnumerable_1[FsCell]:
                                    def _arrow1192(__unit: None=None) -> IEnumerable_1[FsCell]:
                                        def _arrow1191(__unit: None=None) -> IEnumerable_1[FsCell]:
                                        ...
                                        return append(create_text(DataContext__get_GeneratedBy(dc_3)), delay(_arrow1191))

                                    return append(create_text(DataContext__get_Description(dc_3)), delay(_arrow1192))

                                return append(create_term(DataContext__get_ObjectType(dc_3)), delay(_arrow1193))

                            return append(create_term(DataContext__get_Unit(dc_3)), delay(_arrow1194))

                        return append(create_term(DataContext__get_Explication(dc_3)), delay(_arrow1195))

                    return append(_arrow1190(), delay(_arrow1196))

                return to_list(delay(_arrow1197))

            return map_1(_arrow1198, dc)

        return append(singleton_1(headers), delay(_arrow1199))

    return transpose(to_list(delay(_arrow1200)))
