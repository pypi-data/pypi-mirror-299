...
def encoder(dc: DataContext) -> IEncodable:
    ...
    def _arrow2395(oa: OntologyAnnotation, dc: Any=dc) -> IEncodable:
        return OntologyAnnotation_encoder(oa)

    def _arrow2399(oa_1: OntologyAnnotation, dc: Any=dc) -> IEncodable:
        return OntologyAnnotation_encoder(oa_1)

    def _arrow2402(oa_2: OntologyAnnotation, dc: Any=dc) -> IEncodable:
        return OntologyAnnotation_encoder(oa_2)

    def _arrow2410(value: str, dc: Any=dc) -> IEncodable:
    ...
    def _arrow2416(value_2: str, dc: Any=dc) -> IEncodable:
    ...
    def _arrow2420(value_4: str, dc: Any=dc) -> IEncodable:
    ...
    values: FSharpList[tuple[str, IEncodable]] = choose(chooser, of_array([("data", encoder_1(dc)), try_include("explication", _arrow2395, DataContext__get_Explication(dc)), try_include("unit", _arrow2399, DataContext__get_Unit(dc)), try_include("objectType", _arrow2402, DataContext__get_ObjectType(dc)), try_include("description", _arrow2410, DataContext__get_Description(dc)), try_include("generatedBy", _arrow2416, DataContext__get_GeneratedBy(dc)), try_include("label", _arrow2420, DataContext__get_Label(dc))]))
...
def _arrow2452(get: IGetters) -> DataContext:
    data: Data
    object_arg: IRequiredGetter = get.Required
    data = object_arg.Field("data", decoder_1)
    def _arrow2437(__unit: None=None) -> OntologyAnnotation | None:
    ...
    def _arrow2443(__unit: None=None) -> OntologyAnnotation | None:
    ...
    return DataContext__ctor_Z780A8A2A(data.ID, data.Name, data.DataType, data.Format, data.SelectorFormat, _arrow2437(), _arrow2441(), _arrow2443(), _arrow2445(), _arrow2449(), _arrow2451(), data.Comments)


decoder: Decoder_1[DataContext] = object(_arrow2452)
