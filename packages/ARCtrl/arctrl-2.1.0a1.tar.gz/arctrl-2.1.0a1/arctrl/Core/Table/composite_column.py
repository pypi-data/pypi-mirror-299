...
class CompositeColumn(Record):
    ...
    def TryGetColumnUnits(self, __unit: None=None) -> Array[OntologyAnnotation] | None:
        this: CompositeColumn = self
        def _arrow610(__unit: None=None) -> IEnumerable_1[OntologyAnnotation]:
            def _arrow609(cell: CompositeCell) -> IEnumerable_1[OntologyAnnotation]:
                return singleton(cell.AsUnitized[1]) if cell.is_unitized else empty()

            return collect(_arrow609, this.Cells)

        arr: Array[OntologyAnnotation] = to_array(delay(_arrow610))
...
CompositeColumn_reflection = _expr611
