...
Init_reflection = _expr2961

def _expr2962() -> TypeInfo:
...
def Clone_createCloneContract_5000466F(remote_url: str, merge: bool | None=None, branch: str | None=None, token: tuple[str, str] | None=None, nolfs: bool | None=None) -> Contract:
    ...
    def _arrow2968(__unit: None=None, remote_url: Any=remote_url, merge: Any=merge, branch: Any=branch, token: Any=token, nolfs: Any=nolfs) -> IEnumerable_1[str]:
        def _arrow2967(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow2966(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow2965(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow2964(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow2963(__unit: None=None) -> IEnumerable_1[str]:
                            return singleton(".") if merge_1 else empty()

                        return append(singleton(remote_url_1), delay(_arrow2963))

                    return append(singleton(value(branch)) if (branch is not None) else empty(), delay(_arrow2964))

                return append(singleton(Clone_get_branchFlag()) if (branch is not None) else empty(), delay(_arrow2965))

            return append(singleton(Clone_get_noLFSConfig()) if nolfs_1 else empty(), delay(_arrow2966))

        return append(singleton(Clone_get_clone()), delay(_arrow2967))

    return create_git_contract(to_array(delay(_arrow2968)))
