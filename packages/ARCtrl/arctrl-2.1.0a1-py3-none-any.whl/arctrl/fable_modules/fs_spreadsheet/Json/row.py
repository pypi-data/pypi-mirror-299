...
def encode(row: FsRow) -> IEncodable:
    def _arrow277(__unit: None=None, row: Any=row) -> IEncodable:
        value: int = row.Index or 0
        class ObjectExpr276(IEncodable):
            def Encode(self, helpers: IEncoderHelpers_1[Any]) -> Any:
                return helpers.encode_signed_integral_number(value)

        return ObjectExpr276()

    def mapping(cell: FsCell, row: Any=row) -> IEncodable:
        return encode_rows(cell)

    values: IEnumerable_1[tuple[str, IEncodable]] = to_enumerable([("number", _arrow277()), ("cells", seq(map(mapping, row.Cells)))])
...
def encode_no_numbers(row: IEnumerable_1[FsCell]) -> IEncodable:
    ...
    class ObjectExpr283(IEncodable):
    ...
    return ObjectExpr283()


def _arrow287(builder: IGetters) -> tuple[int | None, IEnumerable_1[FsCell]]:
    n: int | None
    object_arg: IOptionalGetter = builder.Optional
    n = object_arg.Field("number", int_1)
    def _arrow286(__unit: None=None) -> IEnumerable_1[FsCell] | None:
    ...
    return (n, default_arg(_arrow286(), empty()))


decode: Decoder_1[tuple[int | None, IEnumerable_1[FsCell]]] = object(_arrow287)
