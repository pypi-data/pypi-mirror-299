...
class LeadGenerationCompanyFilters(BaseModel):
    ...
    def to_mongo_filters(
        ...
        if self.regions:
            regions = [r.value if r.value != RegionBe.not_found else None for r in self.regions]
            filters["address.region"] = {"$in": regions}
        if self.provinces:
            provinces = [p.value if p.value != ProvinceBe.not_found else None for p in self.provinces]
