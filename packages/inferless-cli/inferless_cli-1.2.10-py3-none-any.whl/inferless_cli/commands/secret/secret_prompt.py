...
def list_secrets():
    try:
        ...
        for secret in secrets:
            ...
            created_at = "-" if not secret["created_at"] else dateutil.parser.isoparse(secret["created_at"]).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
            updated_at = "-" if not secret["last_used_in_model_import"] else dateutil.parser.isoparse(
