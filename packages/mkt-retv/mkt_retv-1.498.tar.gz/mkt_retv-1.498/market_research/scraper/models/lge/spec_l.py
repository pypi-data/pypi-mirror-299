...
class ModelScraper_l(Scraper, DataVisualizer):
        ...
        def extract_sepcs(url_dict):
            ...
            for key, url in tqdm(url_dict.items()):
                try:
                    ...
                    raise ValueError(f"\nFailed to get info from {url}")
                    pass
            return dict_models
        
        def transform_format(dict_models, json_file_name: str="'l_scrape_model_data.json'") -> pd.DataFrame:
    ...
    def _extract_models_from_series(self, url: str, prefix="https://www.lg.com/") -> dict:
        def extract_model_url(elements)->set:
            url_models_set= set()
            for element in elements:
                try:
                    element_url = prefix + element['href']
                    url_models_set.update(element_url.strip())
                except Exception as e:
    ...
    def _extract_model_details(self, url: str) -> dict:
        ...
        def extract_info_from_model(model: str):
