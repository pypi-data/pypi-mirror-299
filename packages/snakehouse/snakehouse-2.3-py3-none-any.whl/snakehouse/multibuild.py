...
from .satella import split, find_files
...
class Multibuild:
    ...
    def __init__(self, extension_name: str, files: tp.Iterator[str],
        ...
        if files:
            ...
            self.files = [file for file in files if not file.endswith('__bootstrap__.pyx') and not file.endswith('.pyc')]
        ...
        self.pyx_files = list(set(self.pyx_files))
        self.c_files = list(set(self.c_files))
    ...
    def generate_header_files(self):
        for filename in self.pyx_files:
            ...
            data = HFILE_MAKO.format(initpy_name=coded_module_name)
            with open(h_file, 'w', encoding='utf-8') as f_out:
                f_out.write(data)
    ...
    def write_bootstrap_file(self):
        with open(os.path.join(self.bootstrap_directory, '__bootstrap__.pyx'), 'w', encoding='utf-8') as f_out:
            f_out.write(self.generate_bootstrap())

    def alter_init(self):
        if os.path.exists(os.path.join(self.bootstrap_directory, '__init__.py')):
            with open(os.path.join(self.bootstrap_directory, '__init__.py'), 'r', encoding='utf-8') as f_in:
                data = f_in.read()
        else:
            data = ''
        ...
        with open(os.path.join(self.bootstrap_directory, '__init__.py'), 'w', encoding='utf-8') as f_out:
            f_out.write(data)
    ...
    def for_cythonize(self, debug: bool = False, *args, **kwargs):
        if self.dont_snakehouse:
            ...
            self.files = [file for file in self.files if file.endswith('.pyx')]
            self.files.extend(self.c_files)
            self.files.append(os.path.join(self.bootstrap_directory, '__bootstrap__.pyx'))
