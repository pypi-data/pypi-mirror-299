...
class Configurate:
    ...
    def __init__(self):
        ...
        if not self.is_venv:
            ...
            dirpath = os.path.join(self.is_venv,'ffmpeg')
    ...
    def configure(self):
        """Define variáveis de ambiente com base no sistema operacional e arquitetura."""
        if self.FFMPEG_URL is None or self.FFMPEG_BINARY is None:
            if os.name == 'nt':
                # Configuração para Windows
                self.FFMPEG_URL = ('https://raw.githubusercontent.com/PauloCesar-dev404/binarios/main/windows/win'
                                   '-ffmpeg-7.0.2-full-amd64-intel64.zip')
                self.FFMPEG_BINARY = 'ffmpeg.exe'

            elif os.name == 'posix':
                # Configuração para Unix-like (Linux/macOS)
                arch = platform.machine()  # Detecta a arquitetura

                if arch in ['x86_64', 'amd64']:
                    # Arquitetura de 64 bits
                    self.FFMPEG_URL = (
                        'https://raw.githubusercontent.com/PauloCesar-dev404/binarios/main/linux/ffmpeg-7.0.2-amd64.zip')
                    self.FFMPEG_BINARY = 'ffmpeg'
                elif arch in ['i686', 'x86']:
                    # Arquitetura de 32 bits
                    self.FFMPEG_URL = (
                        'https://raw.githubusercontent.com/PauloCesar-dev404/binarios/main/linux/ffmpeg-7.0.2-i686.zip')
                    self.FFMPEG_BINARY = 'ffmpeg'
                elif arch in ['arm64', 'aarch64']:
                    # Arquitetura ARM de 64 bits
                    self.FFMPEG_URL = (
                        'https://raw.githubusercontent.com/PauloCesar-dev404/binarios/main/linux/ffmpeg-7.0.2-arm64.zip')
                    self.FFMPEG_BINARY = 'ffmpeg'
                elif arch in ['arm', 'armhf', 'armel']:
                    # Arquitetura ARM de 32 bits
                    self.FFMPEG_URL = (
                        'https://raw.githubusercontent.com/PauloCesar-dev404/binarios/main/linux/ffmpeg-7.0.2-armel.zip')
                    self.FFMPEG_BINARY = 'ffmpeg'
                else:
                    raise M3u8FileError(f"Arquitetura '{arch}' não suportada.")
