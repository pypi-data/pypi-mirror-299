...
class ModelhubEmbedding(Embedding):
    ...
    AUTH_URL = "https://auth.sprint.autonomize.dev/realms/autonomize/protocol/openid-connect/token"
    MODEL_URL = (
    ...
    def __init__(
        ...
        if client_id is None or client_secret is None:
            ...
            logger.warning(
                ...
                "You can use `ModelhubEmbedding.is_token_expired(your_token)` to check if it has expired or not."
            )
    ...
    def is_token_expired(token: str) -> bool:
        ...
        Raises:
            ValueError: When the token provided is not base64
        """
        ...
        if len(b64_components) != 3:
            raise ValueError("Ill-formatted token provided. Please recheck your token.")
    ...
    def _create_embedding(
        ...
        **kwargs: Any,
        ...
        if not self._token or ModelhubEmbedding.is_token_expired(self._token):
    ...
    async def acreate_embedding(
        ...
        **kwargs: Any,
        ...
        if not self._token or ModelhubEmbedding.is_token_expired(self._token):
            self._token = await self._aget_token()
