...
class SmartbulkConnector(Connector):
    ...
    def listdir_server(
        ...
        """
        List all files and directory in the SmarBulk Server
        """
        base_dir = self.get_base_dir
        base_dir_ls = self.listdir(
            path=base_dir,
            ignore_hidden=True,
            get_details=False,
        )
        if storage not in base_dir_ls:
            return []

        current_dir = os.path.join(base_dir, storage)
        ...
        for element in res:
            if fullpath:
                element = os.path.join(file_path, element)
    ...
    def validate_recipes(self, recipe_path: str):
        ...
        for dataset_name, group in grouped:
            # Get unique species in the dataset (should be only one)
            species = group['species'].unique()
            if len(species) > 1:
                raise ValueError(f"Species mismatch in dataset {dataset_name}")
            species = species[0]
            
            # Get metadata and matrix paths
            metadata_paths = group.loc[group['file_type'] == 'metadata', 'file_path'].tolist()
            matrix_paths = group.loc[group['file_type'] == 'matrix', 'file_path'].tolist()
            
            # Validate at least one metadata and one matrix
    ...
    def create_project(self,
        ...
        """
        Create a new project and submit the first dataset.
        ...
        dataset_name : str, optional
            Name of the dataset for the project (default is 'Dataset TBD').
        ...
        if species not in ['human', 'mouse']:
            print(f"Error: {species} not human or mouse")
            return 
        ...
        return self.openapi.create_project(
            group_id, species, project_name, matrix_paths, metadata_paths, dataset_name, project_id
        )
    ...
    def validate_project_id(self, project_id: str, group_id: str):
        ...
        if group_id not in list_group_id:
            print(f"Group ID: {group_id} not found in your workspace")
            return False
        
        # Check project_id
        list_project_id_in_group = self.get_all_projects_info_in_group(group_id)
        list_project_id_in_group = [
            info['project_id'] for info in list_project_id_in_group
        ]
        if project_id not in list_project_id_in_group:
            print(f"Project ID: {project_id} not found in group {group_id}")
            return False
    ...
    def add_project(
        ...
        validate_id = self.validate_project_id(project_id=project_id, group_id=group_id)
        if not validate_id:
            return 
    ...
    def create_project_from_recipes(
        ...
        This recipes file is a csv file that includes: 
            ...
            file_path: server path to the file in one dataset
            file_type: can be either matrix or metadata, identify the file_path type
        ...
        for data in recipes:
            ...
            try:
                submit_resp = self.create_project(
                    group_id=group_id,
                ...
                project_status = self.check_project_status(submit_reresult=submit_resp)
    ...
    def add_project_from_recipes(
        ...
        return self.create_project_from_recipes(
            group_id=group_id,
    ...
    def check_project_status(self, submit_reresult: str, log_file: str = ''):
        """
        Waits for the log file to contain 'Done' or 'Error' within a specified timeout.

        :param log_file: The path to the log file.
        :return: True if 'Done' or 'Error' is found in the log, False if timeout is reached.
        """
        if log_file == '': 
            submit_reresult = json.loads(submit_reresult)
            log_file = submit_reresult['msg']['message']['log_file']
            project_id = submit_reresult['msg']['message']['project_id']
        else:
            project_id = ''
