...
class SmartbulkConnector(Connector):
    ...
    def create_project(self,
        ...
        """
        if group_id == 'personal':
            group_id = next((group['group_id'] for group in self.get_user_groups() if group['group_name'] == 'Personal workspace'), None)
    ...
    def create_project_from_recipes(
            ...
            project_name: str,
            trace_log: str = '',
            retry_overwrite: bool = False,
        ):

        if not os.path.isfile(recipes_path):
            raise Exception(f"Invalid file: {recipes_path}")
        ...
        if trace_log == '':
            ...
            trace_log_path = os.path.join(project_id, 'trace_log.json')
            trace_log_list_json = {
            ...
            project_id = os.path.basename(os.path.dirname(trace_log))
    ...
    def upload_folder(
        ...
        for src_child in src_path.iterdir():
            if src_child.is_dir():
                ...
                uploaded_log['file_path'].append(dst_child)
                uploaded_log['server_path'].append(resp['path'])
