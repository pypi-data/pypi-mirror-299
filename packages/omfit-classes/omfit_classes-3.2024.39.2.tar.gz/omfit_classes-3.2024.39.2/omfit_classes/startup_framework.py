                ...
                if len(fnames) > 1:
                    ...
                    for fname in fnames:
                        if os.path.exists(fname):
                            shutil.copy(fname, directory + os.sep + os.path.split(filename)[1] + os.sep + os.path.split(fname)[1])
                        else:
                            printw(fname + ' not found!')
                else:
                    shutil.copytree(filename, directory + os.sep + os.path.split(filename)[1])
            else:
                shutil.copy(filename, directory + os.sep + os.path.split(filename)[1])
...
def server_picker(root, MainSettings):
    ...
    elif is_localhost('polaris'):
        server_picker_cache[device] = 'polaris'
    elif is_localhost(['pppl_sunfire', 'nx', 'portal', 'sunfire', 'transp-srv-stag'], looseCheck=True):
