...
from skillcorner_tactical_on_demand.method import METHOD_DOCSTRING, METHODS_BINDING

logger = logging.getLogger(__name__)

BASE_URL = 'https://tactical.skillcorner.com'
BASE_CLIENT_NAME = 'skillcorner_tactical_on_demand'
...
class SkillcornerTacticalOnDemandClient(FitRequest):
    """Client for the Tactical On Demand API."""
    ...
    def upload_match_video(
        self, request_id: int, path: str, is_update: bool = False
    ) -> dict:
        ...
        ```py
            with open('video.mp4', 'rb') as video_file:
                response = client.upload_match_video(1, video_file)
            # {'status': 201}
        ```
        ...
        try:
            ...
            presigned_urls, upload_id = self._request_presigned_urls(
                request_id, video_parts, is_update
            )
            parts = self._multipart_upload(presigned_urls, path)
            return self.complete_multipart_upload(
                request_id, parts, upload_id, is_update
            )
        except Exception as e:
            return e

    def _request_presigned_urls(
        self, request_id: int, parts: int, is_update: bool = False
    ) -> tuple[list[str], str]:
        if is_update:
            res = self.session.put(
                self.base_url + f'/api/request/{request_id}/video/',
                json={'parts': parts},
    ...
    def upload_match_sheet(self, request_id: int, path: str, is_update: bool = False):
        """Post a match sheet to the server.

        Args:
            request_id (int): The ID of the request.
            match_sheet (dict): The match sheet data.
        ...
        Examples:
        ```py
            response = client.post_match_sheet(1, match_sheet)
    ...
    def upload_period_limits(self, request_id: int, path: str, is_update: bool = False):
        ...
        Examples:
        ```py
            response = client.upload_period_limits(1, 'period_limits.csv')
    ...
    def upload_home_team_side(
        self, request_id: int, path: int, is_update: bool = False
    ):
        ...
        Examples:
        ```py
            response = client.upload_home_team_side(1, 'home_team_side.csv')
