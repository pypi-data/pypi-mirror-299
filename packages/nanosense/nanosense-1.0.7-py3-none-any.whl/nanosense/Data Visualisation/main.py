...
class MainApp (QMainWindow ):
    ...
    def validate_data (self ):
        ...
        if self .data .shape [1 ]!=9 :
            QMessageBox .warning (self ,"Error",f"Unexpected number of columns. Expected 9, got {self.data.shape[1]}.")
            return False 
        return True 
    ...
    def read_file (self ,file ):
        try :
            with np .load (file )as data :
                ...
                print (f"Loaded {X.shape[0]} valid rows from {file}")
                return X 
        except Exception as e :
    ...
    def send_to_fitting (self ,new_params ):
        ...
        x_data =np .log (delta_t )if use_log else delta_t 
    ...
    def update_plot_with_params (self ,new_params ,refit =False ):
        ...
        try :
            ...
            x_label ='log(Δt (ms))'if use_log and self .dt_plot_button .isChecked ()else 'Δt (ms)'if self .dt_plot_button .isChecked ()else self .get_standard_label ()
            self .plot_21 .set_xlabel (x_label ,fontsize =14 )
