...
class Checker:
    ...
    def check_p12(self, p12_data: bytes, password: str = ""):
        try:
            ...
            if cert is None:
                return {"certificate_status": "Invalid certificate: No private certificate found in the P12 file."}
            
            cert_info = self.get_certificate_info(cert)
            p12_cert, full_chain_cert = self.create_cert_files(cert, cert_chain)
        except ValueError:
            return {"certificate_status": "Invalid certificate: Incorrect password or unable to process the P12 file."}
