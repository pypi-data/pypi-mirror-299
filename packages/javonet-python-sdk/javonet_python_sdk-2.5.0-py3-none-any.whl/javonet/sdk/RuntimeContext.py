...
class RuntimeContext(AbstractTypeContext):
    ...
    def __init__(self, runtime_name: RuntimeName, connection_data: IConnectionData):
        ...
        asyncio.run(SdkMessageHelper().send_message_to_app_insights("SdkMessage", str(runtime_name) + " initialized"))
    ...
    def execute(self):
        ...
        if response_command.command_type == CommandType.Exception:
            ...
            asyncio.run(SdkMessageHelper().send_message_to_app_insights("SdkException", str(exception)))
            raise exception
