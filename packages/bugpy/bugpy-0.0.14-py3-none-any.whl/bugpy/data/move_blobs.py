...
def _copy_one_file(input_file, from_bucket, to_bucket, extension, prefix='', verbose=False, s3_client=None):
    ...
    if ext[-1] != extension and extension is not None:
        ...
        to_file = '.'.join(to_file.split('.')[:-1]+[extension])

        name, in_img = stream_one_image(from_file, from_bucket)
        out_img = BytesIO()
        in_img.save(out_img, format=extension)
        out_img.seek(0)
        s3_client.upload_fileobj(out_img, to_bucket, to_file)
        return
...
def delete_files(filelist, bucket, s3=None):
    ...
    failures, successes = multithread(filelist, func, description=f"Deleting images from {bucket}")
