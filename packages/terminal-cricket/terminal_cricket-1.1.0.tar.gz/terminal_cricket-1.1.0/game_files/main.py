...
def perform_user_batting():
    ...
    global runs_user, runs_cpu
    print(Fore.LIGHTYELLOW_EX + "=============================================================\nNow your batting" + Style.RESET_ALL)
    for _ in range(6):
        ...
        if user_input == computer_input:
            ...
            print(Fore.MAGENTA + f"Your final score: {runs_user}" + Style.RESET_ALL)
            break
        else:
            if runs_user is None:
                runs_user = 0  # Initialize runs_user when batting starts
...
def perform_user_bowling():
    ...
    global runs_user, runs_cpu
    print(Fore.LIGHTYELLOW_EX + "=============================================================\nNow your bowling" + Style.RESET_ALL)
    for _ in range(6):
        user_input = get_user_choice()
        computer_input = generate_computer_choice()
        ...
        if user_input == computer_input:
            ...
            print(Fore.MAGENTA + f"CPU's current score: {runs_cpu}" + Style.RESET_ALL)
            break
        else:
        ...
        # Handle error if `runs_user` is None when comparing scores
        try:
            if runs_cpu > runs_user:
                print(Fore.BLUE + '--CPU WON--' + Style.RESET_ALL)
                sys.exit()  # Close the program if CPU overtakes the user
        except TypeError:
            # runs_user is None, so we cannot compare scores yet
            pass

def determine_winner():
    """Compare scores and announce the winner if it reaches the final comparison."""
    print(f"Final Score - You: {runs_user}, CPU: {runs_cpu}")
    if runs_cpu > runs_user:
        print(Fore.BLUE + '--CPU WON--' + Style.RESET_ALL)
        sys.exit()  # Close the program after the CPU wins
    elif runs_cpu < runs_user:
        print(Fore.BLUE + '--YOU WON--' + Style.RESET_ALL)
        sys.exit()  # Close the program after the user wins
    else:
        print(Fore.BLUE + '--IT\'S A DRAW--' + Style.RESET_ALL)
        sys.exit()  # Close the program after a draw
...
runs_user = None  # Initialize to None to signify that user hasn't batted yet
runs_cpu = 0
