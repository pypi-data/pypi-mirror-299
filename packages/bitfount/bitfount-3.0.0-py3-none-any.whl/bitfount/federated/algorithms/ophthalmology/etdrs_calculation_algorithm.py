...
class _WorkerSide(BaseWorkerAlgorithm):
    ...
    def run(
        ...
        if filenames and is_file_iterable_source(self.datasource):
            ...
            test_data_dfs = get_dataframe_iterator_from_datasource(self.datasource)
        ...
        # Check that we have the expected number of results for the number of files
        if filenames:
            assert len(filenames) == len(bscans)  # nosec [assert_used]
            assert len(filenames) == len(binarised_predictions)  # nosec [assert_used]
            assert len(filenames) == len(enface_output)  # nosec [assert_used]
            # GA Model returns a 0-dim array for `slos` if slo is set to false
            if slos.ndim != 0:
                assert len(filenames) == len(slos)  # nosec [assert_used]
...
class ETDRSAlgorithm(BaseAlgorithmFactory):
    """Algorithm for computing ETDRS subfields.
