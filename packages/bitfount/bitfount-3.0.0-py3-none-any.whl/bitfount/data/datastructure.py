...
class DataStructure(BaseDataStructure, _BaseSerializableObjectMixIn):
    ...
    def _validate_schema_features(self) -> None:
        ...
        # We only call this method if schema_types_override is not None.
        assert self.schema_types_override is not None  # nosec[assert_used]
    ...
    def _override_schema(
        ...
        if isinstance(self.query, str):
            ...
            feature_overrides = cast(
                SchemaOverrideMapping,
                self.schema_types_override[data_identifier],  # type: ignore[index] # Reason: An error will be caught by the _validate_schema_features. # noqa: E501
            )
        else:
            raise ValueError(
