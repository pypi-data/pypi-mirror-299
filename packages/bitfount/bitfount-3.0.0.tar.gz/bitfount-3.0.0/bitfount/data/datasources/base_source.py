...
# loaded.
FAST_LOAD_NUMBER_OF_FILES_WITHOUT_NEW_FIELDS: int = 10
...
BaseSourceInitSignature = Callable[
    [
        Arg(T, "self"),
        VarArg(Any),
        DefaultNamedArg(Optional[DatasetSplitter], "data_splitter"),
        DefaultNamedArg(Optional[int], "seed"),
        DefaultNamedArg(Optional[dict[str, DataPathModifiers]], "modifiers"),
        DefaultNamedArg(Union[str, Sequence[str], None], "ignore_cols"),
        KwArg(Any),
    ],
    None,
]
...
class FileSystemIterableSource(IterableSource, MultiProcessingMixIn, ABC):
    ...
    def _scan_files_for_extension(
        self, files: list[Path], file_extension: list[str], strict: bool = False
    ) -> list[str]:
        ...
        Args:
            ...
            file_extension: The list of file extensions to consider against.
            strict: If True, the file extension must explicitly match against
                `file_extension`. Otherwise, allow inferring the file extension
                 from the file type.
        ...
        # Paths added to matched files should be fully resolved with `Path.resolve()`
        ...
        for file in files:
            ...
            meets_extension_criteria = self._filter_file_by_extension(
                file, file_extension, file_type, strict
            )

            if meets_extension_criteria:
                matched_files.append(file)
            # If guessing the extension failed the result is ".None"
            elif file_type == ".None":
                self.skipped_files.add(str(file))
            # Otherwise the exclusion reason must be that the file_type wasn't allowed
            else:
                ignored_file_types.add(file_type)
        ...
        return [str(x) for x in matched_files]
    ...
    def _filter_files_by_date(self, file_names: list[str]) -> list[str]:
        ...
        for file in file_names:
            meets_date_criteria = self._filter_file_by_dates(file)
            if meets_date_criteria:
    ...
    def _filter_files_by_size(self, file_names: list[str]) -> list[str]:
        ...
        filtered_files = []
        for file in file_names:
            meets_size_criteria = self._filter_file_by_size(file)
