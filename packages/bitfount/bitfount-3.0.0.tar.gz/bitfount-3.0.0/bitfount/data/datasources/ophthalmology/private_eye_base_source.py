...
env = Env()
LOG_PRIVATE_EYE_FIXES: bool = env.bool("LOG_PRIVATE_EYE_FIXES", default=False)
...
class _PrivateEyeSource(_OphthalmologySource, ABC):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if isinstance(private_eye_parser, Mapping) and self.file_extension is not None:
            file_extensions: set[str] = set([i.lower() for i in self.file_extension])
    ...
    @staticmethod
    def _resize_oct_images(
        image_sequences: _ImageSequence, oct_images: list[Path]
    ) -> None:
        """Resize the OCT images.

        Args:
            image_sequences: The metadata about the image sequences.
            oct_images: A list of paths to the OCT images.
        """
        # TODO: [BIT-3487] Avoid opening and saving the image again because it is very
        # slow. This should either be removed or moved to be a batch transformation.

        # Establish the num, height and width of the images in the series
        size = (
            len(image_sequences[1]["contents"]) - 1,
            image_sequences[1]["size"]["height"],
            image_sequences[1]["size"]["width"],
        )

        # Calculate the needed information for resizing the images
        dims: set[tuple[str, tuple[Any, ...]]] = set(
            [
                (
                    image_sequence["modality"],
                    tuple(image_sequence["resolutions_mm"].values()),
                )
                for image_sequence in image_sequences
            ]
        )
        resized_pixel_num = (
            int((dict(dims)["OCT"][0] / PIXEL_DIVISION_CONSTANT) * (size[2] / 2)) * 2
        )
        axial_resize_factor = resized_pixel_num / size[2]

        # Resize the OCT images based on the information from the metadata
        for image in oct_images:
            img = Image.open(image.resolve())
            # TODO: [BIT-3487] Should be reassigning `img` here
            img.resize((int(axial_resize_factor * size[2]), int(size[1])))
            img.save(image.resolve())
    ...
    def _process_ophthalmology_file(
        ...
        # TODO: [BIT-3487] This has been commented out because it turned out that
        # the implementation was buggy such that we were not actually even saving
        # the resized image because we weren't reassigning the `img` variable so it
        # is best to comment it out for now.
        # self._resize_oct_images(image_sequences, oct_images)
