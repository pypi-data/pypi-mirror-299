...
class HeidelbergSource(_PrivateEyeSource):
    ...
    Args:
        ...
        file_extension: The file extension of the Heidelberg files.
            Defaults to '.sdb'.
        parsers: The private eye parsers to use for the different file extensions.
            Only needs to be supplied if file_extension is non-default. Can either
            be a single parser to use for all file extensions or a mapping of file
            extensions to parser type. Defaults to appropriate parser(s) for the
    ...
    def __init__(
        ...
        ] = HEIDELBERG_FILE_EXTENSION,  # kept for backward compatibility
        parsers: Optional[
        ...
        **kwargs: Any,
        ...
        if isinstance(file_extension, list):
            ...
            file_extension = HEIDELBERG_FILE_EXTENSION
        elif file_extension != HEIDELBERG_FILE_EXTENSION:
            logger.warning(
                "HeidelbergSource only supports '.sdb' files. Ignoring "
                f"file_extension={file_extension}."
            )
    ...
    @cached_property
    def _dob_filtered_file_names(self) -> list[str]:
        ...
        return self._filter_files_by_dob(file_names)

    @cached_property
    def _num_bscan_filtered_file_names(self) -> list[str]:
        """Returns a list of file names in the directory that match the num bscan args.

        These file paths will be absolute paths, regardless of their origin. This filter
        is applied after the DOB filter.
        """
        return self._filter_files_by_num_bscans(self._dob_filtered_file_names)
    ...
    def file_names(self) -> list[str]:
        ...
        file_names = [
            i
            for i in self._num_bscan_filtered_file_names
            if i not in self.skipped_files
        ]
    ...
    def clear_file_names_cache(self) -> None:
        ...
        # Clear the DOB file names cache
        try:
            del self._dob_filtered_file_names
        except AttributeError:
            pass

        # Clear the num bscan file names cache
        try:
            del self._num_bscan_filtered_file_names
        except AttributeError:
            pass
    ...
    def stale(self) -> bool:
        ...
        the number of rows with the same filename in the dataframe.
        """
        # Check that there are as many rows with unique filenames as
        # those in the set of files.
        if self._data_is_loaded and len(
            self.data["_original_filename"].unique()
        ) == len(self.file_names):
            return False
        return True
