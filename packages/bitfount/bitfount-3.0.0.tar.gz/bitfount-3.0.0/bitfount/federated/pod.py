...
from bitfount.data.types import _ForceStypeValue
...
class Pod:
    ...
    def _load_basesource_schema_if_necessary(
        ...
        if ds.schema:
            ...
            try:
                ...
                # Check if force stypes have the appropriate
                # type in the schema, if not we need to update
                # the schema
                if data_config.force_stypes is not None:
                    for table in schema.tables:
                        if table.name == ds.name:
                            stype: _ForceStypeValue
                            for stype, cols in data_config.force_stypes[
                                table.name
                            ].items():
                                if stype == "image_prefix":
                                    # If the stype is image_prefix, it is safest
                                    # to re-generate the schema, as we cannot
                                    # check for the features on the go. This is
                                    # not an option when editing the schema in
                                    # the app, so it should not affect the
                                    # connections of datasets in the app.
                                    update_schema = True
                                elif stype not in table.features.keys():
                                    update_schema = True
                                else:
                                    for col in cols:
                                        if col not in table.features[stype]:
                                            update_schema = True
        ...
        if ds.datasource.multi_table and schema is not None:
            ...
            if any(table not in schema_tables for table in ds.datasource.table_names):  # type: ignore[attr-defined] # reason: see below: # noqa: E501
    ...
    def _setup_schema(
        ...
        try:
            schema.freeze()
        except BitfountSchemaError as e:
            logger.warning(f"Ignoring schema exception: {e}")
        return schema
