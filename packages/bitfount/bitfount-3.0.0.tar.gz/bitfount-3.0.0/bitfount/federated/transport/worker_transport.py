...
class _WorkerMailbox(_BaseMailbox):
    ...
    async def send_evaluation_results(
        ...
        message_metadata: Optional[_MessageMetadata] = (
            None if notification is None else _MessageMetadata(None, notification)
        )
    ...
    async def send_task_abort_message(
        ...
        message_metadata = None if reason is None else _MessageMetadata(reason, None)
        await self._send_aes_encrypted_message(
