...
class _Worker:
    ...
    async def run(self) -> None:
        ...
        if datastructure is not None:
            datastructure = self._update_datastructure(datastructure)
    ...
    def _update_datastructure(self, datastructure: DataStructure) -> DataStructure:
        """Update datastructure with the appropriate pod identifiers."""
    ...
    def _load_new_records_only_for_task(self, project_db_con: Connection) -> None:
        ...
        new_records = [
            i for i in self.datasource.file_names if i not in run_records_filenames
        ]
