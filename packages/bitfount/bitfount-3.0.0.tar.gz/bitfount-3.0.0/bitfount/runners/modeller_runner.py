...
from bitfount.config import BITFOUNT_DEFAULT_BATCHED_EXECUTION, BITFOUNT_OUTPUT_DIR
...
def setup_modeller_from_config(
    ...
    if config.batched_execution is None:
...
def setup_modeller(
    ...
    for pod_id in pod_identifiers[1:]:
        ...
        if aux_schema != schema_0:
            raise ValueError(
                "Pod schemas must match in order to be able to train on them."
            )

    schema = combine_pod_schemas(
        pod_identifiers, hub=bitfount_hub, project_id=project_id
    )
    ...
    if data_config:
        if (
            not data_config.select.include
            and not data_config.select.exclude
            and not data_config.table_config.query
        ):
    ...
    for algo in task_algorithm:
        ...
        try:
            ...
            if not algo_name_in_config.startswith("bitfount."):
                algo_name_in_config = f"bitfount.{algo_name_in_config}"

            algorithm_cls = _ALGORITHMS[AlgorithmType(algo_name_in_config).name]
        ...
        # algorithm arguments config and must be added separately
            ...
            # account for no datastructure provided
            datastructure = None if not data_config else data_structure
            additional_algo_kwargs.update(schema=schema, datastructure=datastructure)
    ...
    try:
        ...
        if not protocol_name_in_config.startswith("bitfount."):
            protocol_name_in_config = f"bitfount.{protocol_name_in_config}"

        protocol = _PROTOCOLS[ProtocolType(protocol_name_in_config).name](
