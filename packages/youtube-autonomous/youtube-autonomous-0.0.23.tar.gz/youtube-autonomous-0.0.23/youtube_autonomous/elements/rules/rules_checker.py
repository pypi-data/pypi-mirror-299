...
class RulesChecker:
    ...
    @classmethod
    def check_need_rules(cls, element: dict, rules_class: 'ElementRules'):
        """
        Receives the 'element' dictionary that must contain the fields that
        are mandatory by the rules configuration. This method will raise an
        Exception if any rule is broken.
        ...
        ElementParameterValidator.validate_mandatory_parameter('rules_class', rules_class)
        ElementParameterValidator.validate_rules(rules_class)
        
        if rules_class.need_narration and not cls.is_need_narration_rule_satisfied(element):
        ...
        if rules_class.need_specific_duration and not cls.is_need_specific_duration_rule_satisfied(element):
        ...
        if rules_class.need_text and not cls.is_need_text_rule_satisfied(element):
            raise Exception('An specific "text" field is needed and does not exist.')
        
        if rules_class.need_filename_or_url and not cls.is_need_filename_or_url_rule_satisfied(element):
            raise Exception('The "filename" field or the "url" field is needed (at least one of them) and none is provided.')
        
        if rules_class.need_keywords and not cls.is_need_keywords_rule_satisfied(element):
