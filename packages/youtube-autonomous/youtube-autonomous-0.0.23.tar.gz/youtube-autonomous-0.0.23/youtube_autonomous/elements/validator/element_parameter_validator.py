...
class ElementParameterValidator(ParameterValidator):
    ...
    @classmethod
    def validate_segment_or_enhancement(cls, segment: dict):
        """
        Validates if the provided 'segment' dict has the structure
        and required fields and that stuff of a segment.
        """
        RulesChecker.check_need_rules(segment, ElementRules.get_subclass_by_type(segment['type']))
    ...
    def validate_edition_manual_term(cls, edition_manual_term: dict):
        ...
        # TODO: Apply Enums
        ElementParameterValidator.validate_edition_manual_term_mode(dict.get('mode', None))
        ElementParameterValidator.validate_edition_manual_term_context(dict.get('context', None))

        enhancements = dict.get('enhancements', None)
        if enhancements is None:
            raise Exception('No "enhancements" field found in the provided "dict".')
