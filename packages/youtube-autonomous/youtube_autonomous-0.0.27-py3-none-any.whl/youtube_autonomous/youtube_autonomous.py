    ...
    def validate_data(self, json_data: dict):
        ...
        for index, segment in enumerate(segments):
            ...
            for index, enhancement in enumerate(segment.get(SegmentField.ENHANCEMENTS.value, [])):
                ...
                mode = ElementValidator.validate_enhancement_mode_field(enhancement.get(EnhancementField.MODE.value, None))

                # We can only accept 'FILE_DURATION' for some specific types
                ElementValidator.validate_enhancement_duration_field_for_type(duration, type)

                # We can only accept the mode if its type accepts it
                ElementValidator.validate_enhancement_mode_field_for_type(mode, type)
