...
class EditionManual:
    ...
    def init_from_file(filename: str):
        ...
        return EditionManual(terms_dictionary)

    def __init__(self, terms: Union[list[dict], list[EditionManualTerm]]):
        # TODO: We need an ID or something maybe
        # TODO: Convert any term to ManualEditionTerm
        for term in terms:
            if not isinstance(term, EditionManualTerm):
                term = ElementParameterValidator.validate_edition_manual_term(term)
    ...
    def apply(self, segment: 'Segment'):
        ...
        for term in self.terms:
            enhancements_found.append(term.search(segment.transcription))
