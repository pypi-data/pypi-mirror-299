...
class Enhancement(Element):
    ...
    @segment.setter
    def segment(self, segment: Segment):
    ...
    def origin(self, origin: EnhancementOrigin):
        ...
        if self._do_update_database:
            self.database_handler.update_project_segment_enhancement_field(self.project_id, self.segment.index, self.index, 'origin', origin)
    ...
    def status(self, status: str):
        ...
        if self._do_update_database:
            self.database_handler.update_project_segment_enhancement_field(self.project_id, self.segment.index, self.index, 'status', status)
    ...
    def audio_filename(self, audio_filename: Union[str, None]):
        ...
        if self._do_update_database:
            self.database_handler.update_project_segment_enhancement_field(self.project_id, self.segment.index, self.index, ElementBuildingField.AUDIO_FILENAME.value, audio_filename)
    ...
    def video_filename(self, video_filename: Union[str, None]):
        ...
        if self._do_update_database:
            self.database_handler.update_project_segment_enhancement_field(self.project_id, self.segment.index, self.index, ElementBuildingField.VIDEO_FILENAME.value, video_filename)
    ...
    def full_filename(self, full_filename: Union[str, None]):
        ...
        if self._do_update_database:
            self.database_handler.update_project_segment_enhancement_field(self.project_id, self.segment.index, self.index, ElementBuildingField.FULL_FILENAME.value, full_filename)
    ...
    def transcription(self, transcription: Union[dict, None]):
        ...
        if self._do_update_database:
            self.database_handler.update_project_segment_enhancement_field(self.project_id, self.segment.index, self.index, ElementBuildingField.TRANSCRIPTION.value, transcription)
    ...
    def shortcodes(self, shortcodes: Union[list[Shortcode], None]):
        ...
        if self._do_update_database:
            self.database_handler.update_project_segment_enhancement_field(self.project_id, self.segment.index, self.index, ElementBuildingField.SHORTCODES.value, shortcodes)
    ...
    def calculated_duration(self, calculated_duration: Union[float, int, None]):
        ...
        if self._do_update_database:
            self.database_handler.update_project_segment_enhancement_field(self.project_id, self.segment.index, self.index, ElementBuildingField.CALCULATED_DURATION.value, calculated_duration)

    def __init__(self, project_id: Union[str, ObjectId], segment: Segment, index: int, data: dict):
