...
class YoutubeAutonomous:
    ...
    def validate_data(self, json_data: dict):
        ...
        for index, segment in enumerate(segments):
            ...
            if duration == StringDuration.FILE_DURATION and type not in [SegmentType.MEME.value, SegmentType.SOUND.value]:
                raise Exception(f'The "FILE_DURATION" string duration can be set only in {SegmentType.MEME.value} and {SegmentType.SOUND.value} types.')
            ...
            for index, enhancement in enumerate(segment.get(SegmentField.ENHANCEMENTS.value, [])):
                ...
                # positive numeric value
                ElementValidator.validate_enhancement_duration_field(enhancement.get(EnhancementField.DURATION.value, None))

                # We can only accept 'FILE_DURATION' for some specific types
                if duration == StringDuration.FILE_DURATION and type not in [EnhancementType.MEME.value, EnhancementType.SOUND.value]:
                    raise Exception(f'The "FILE_DURATION" string duration can be set only in {EnhancementType.MEME.value} and {EnhancementType.SOUND.value} types.')
