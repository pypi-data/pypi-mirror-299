...
class PlacementFetcher:
  ...
  def get_placements_for_account(
    ...
    Args:
      ...
      preview_pagination_index_zero_based: Preview pagination index (zero
        based).
      preview_pagination_size: Preview pagination size.
    ...
    if not (
      placements := self._get_placement_performance_data(account, task_obj)
    ):
      return None
    if preview_pagination_index_zero_based is not None:
      resource_name_index = placements.column_names.index('resource_name')
      start = preview_pagination_size * preview_pagination_index_zero_based
      end = preview_pagination_size * (preview_pagination_index_zero_based + 1)
      placements = gaarf.report.GaarfReport(
        results=sorted(
          placements.results, key=lambda x: x[resource_name_index]
        )[start:end],
  ...
  def _get_placement_performance_data(
    ...
    Returns:
      Report with fetched placement data.
    """
    placements = self._get_placement_data(task_obj=task_obj, account=account)
