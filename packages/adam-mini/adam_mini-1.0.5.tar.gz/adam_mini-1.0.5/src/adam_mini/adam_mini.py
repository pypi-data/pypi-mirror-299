...
class Adam_mini(torch.optim.Optimizer):
    def __init__(
        ...
        '''
        This is the official implementation of Adam-mini (version 1.0.5).
        ...
        self.wqk_names = {"k_proj.weight", "q_proj.weight", "wq.weight", "wk.weight","v_proj.weight","wv.weight"} # it is okay to remove "v_proj.weight" and "wv.weight", but removing these will bring extra all_reduce operations
        # MLPs
        self.mlp_names = {"feed_forward", "o_proj.weight",
                          "wo.weight"}
        ...
        if verbose:
            print(
                f'Adam-mini found {count_embd} embedding layers, {count_output} output layers, {count_wqk} Querys, Keys, and Values.')

        if count_embd == 0 and verbose:
            # warning
            print(
                "=====>>> Warning by Adam-mini: No embedding layer found. If you are training Transformers, please check the name of your embedding layer and manually add them to 'self.embd_names' of Adam-mini. You can do this by adding an additional line of code: optimizer.embd_names.add('the keyword in the name of your embedding layer, e.g., embed'). ")
        if count_output == 0 and verbose:
            # warning
            print(
                "=====>>> Warning by Adam-mini: No output layer found. If you are training Transformers (without weight-tying), please check the name of your output layer and manually add them to 'self.output_names' of Adam-mini. You can do this by adding an additional line of code: optimizer.output_names.add('the keyword in the name of your output layer, e.g., output').  Please ignore this warning if you are using weight-tying.")
        if count_wqk == 0 and verbose:
            # warning
            print(
                "=====>>>  Warning by Adam-mini: No Query, Key or Value found. If you are training Transformers, please check the name of your Query, Key, and Value in attention blocks and manually add them to 'self.wqk_names' of Adam-mini. You can do this by adding several additional lines of code: optimizer.wqk_names.add('the keyword in the name of your Query, e.g., wq.weight' ); optimizer.wqk_names.add('the name of your Key, e.g., wk.weight'); optimizer.wqk_names.add('the name of your Key,  e.g., wv.weight'). ")
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                state = self.state[p]
                if any(embd_name in name for embd_name in self.embd_names) or any(output_name in name for output_name in self.output_names):  # this is for embedding and output layer
                    if p.grad is None:
                        continue
                    if len(state) == 0:
                ...
                elif any(wqk_name in name for wqk_name in self.wqk_names):  # this is for query, key, and values.
                    if p.grad is None:
                        continue
                    ...
                    if len(state) == 0:
                        ...
                        state["vmean"] = torch.zeros_like(state["m"][0:state["head_per_gpu"], 0:1], memory_format=torch.preserve_format)
                ...
                elif any(mlp_name in name for mlp_name in self.mlp_names): # This is for MLPs and attn.proj
                    if p.grad is None:
                        continue
                ...
                else:  # other blocks. By default settings of Transformers, this is for LayerNorms. For non-Transformer models, this is for all blocks.
                    if len(state) == 0:
