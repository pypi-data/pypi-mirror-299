...
class NextFrameGeneratorDataset(Dataset):
    def __init__(self, model: NextFrameGenerator, autovae: AutoVAE, folder_path: str, chunk_size: int = 16, frame_frequency: int = 5, overwrite_cache: bool = False, overwrite_frames: bool = False):
        """
...
class ConvLSTMCell(nn.Module):
    def __init__(self, input_dim, hidden_dim, kernel_size):
        ...
        # Layer norm to stabilize LSTM gates
        # Match tensor dimensions of conv output
        self.layer_norm = nn.LayerNorm([4 * hidden_dim, 64, 64])
    ...
    def forward(self, x: torch.Tensor, hidden_state: tuple[torch.Tensor, torch.Tensor]):
        ...
        # Apply layer normalization after convolution
        ...
        for epoch in range(iterations):
            ...
            for dataset_iter, inputs in enumerate(dataset):
                ...
                for i in range(len(inputs) - 1):
                    input_tensor, _ = inputs[i]
                    output_tensor, _ = inputs[i + 1]
                ...
                avg_loss = accumulated_loss / (len(inputs) - 1)
                ...
                progressbar.set_description(
                    f"Epoch: [{epoch+1}/{iterations}] Step: [{dataset_iter +
                                                              1}/{len(dataset)}] Loss: [{total_loss / (loss_count + 1):.4f}]"
                )
