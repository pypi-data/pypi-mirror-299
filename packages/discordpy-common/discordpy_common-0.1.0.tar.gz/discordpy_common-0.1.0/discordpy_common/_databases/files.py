...
def create_guild_pattern(guild_id: str) -> None :
    ...
    for file in _guild_files :
        ...
        file = file.format(guild_id=guild_id, date=dt.strftime('%Y-%m-%d'))
        if not os.path.exists(file) :
            with open(file, 'w', encoding='UTF-8') as f :
                f.write('')

    # Write a message in logs
    with open(f'.src/guilds/{guild_id}/logs/{dt.strftime('%Y-%m-%d')}.txt', 'a+', encoding='UTF-8') as f :
        f.write(f'{dt.strftime("[%Y-%m-%d] [%H:%M:%S]")} Guild ({guild_id}) created.\n')
