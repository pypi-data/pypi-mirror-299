...
def _mongo_auth(
    ...
    user_or_role = auth_collection.find_one({
        '_id': _id,
        'type': obj_type,
        'auth': {
            '$in': [auth.value]
...
# auth_only decorator
def auth_only(func: Callable[commands.Cog, ds.Interaction, tuple[Any, ...], dict[str, Any]], auth: AuthType | None = None) -> Callable or None :
    """
    ...
    async def wrapper(cog: commands.Cog, interaction: ds.Interaction, *args, **kwargs) -> Callable or None :
        ...
        if not auth_modes[cog.bot.typeDB](
                auth=auth,
