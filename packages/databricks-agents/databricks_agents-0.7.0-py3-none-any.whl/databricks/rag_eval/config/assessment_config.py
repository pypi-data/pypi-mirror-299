...
class AssessmentConfig:
    ...
    require_grading_notes: bool = field(default=False)
    """Whether the assessment requires grading notes to be present in the dataset to eval."""

    require_ground_truth_answer_or_grading_notes: bool = field(default=False)
    """Whether the assessment requires ground truth answer or grading notes to be present in the dataset to eval."""
...
def _get_example_configs_from_examples_df(
    ...
    if assessment_conf.require_answer:
        cols_to_select.append(schemas.RESPONSE_COL)
    if (
        assessment_conf.require_retrieval_context
        or assessment_conf.require_retrieval_context_array
    ):
        cols_to_select.append(schemas.RETRIEVED_CONTEXT_COL)

    ground_truth_cols_to_include = []
    if assessment_conf.require_ground_truth_answer_or_grading_notes:
        # Include the ground truth answer and grading notes column, if each is present in the
        # original dataframe
        ground_truth_cols_to_include = list(
            set(examples_df.columns)
            & {schemas.EXPECTED_RESPONSE_COL, schemas.GRADING_NOTES_COL}
        )
        if not len(ground_truth_cols_to_include):
            # If neither are present in the dataframe, return an empty set of examples
            return []
        cols_to_select += ground_truth_cols_to_include
    elif assessment_conf.require_ground_truth_answer:
        ground_truth_cols_to_include = [schemas.EXPECTED_RESPONSE_COL]
        cols_to_select.append(schemas.EXPECTED_RESPONSE_COL)
    elif assessment_conf.require_grading_notes:
        ground_truth_cols_to_include = [schemas.GRADING_NOTES_COL]
        cols_to_select.append(schemas.GRADING_NOTES_COL)

    relevant_df = examples_df[cols_to_select]
    # Exclude any rows where all ground truth response columns are None
    if len(ground_truth_cols_to_include) != 0:
        relevant_df = relevant_df.dropna(how="all", subset=ground_truth_cols_to_include)

    # Exclude rows where any column is None except for rationale and ground truth response columns
    relevant_df = relevant_df.dropna(
        subset=relevant_df.columns.difference(
            [rationale_col, schemas.EXPECTED_RESPONSE_COL, schemas.GRADING_NOTES_COL]
        )
    )
...
def create_builtin_assessment_configs(
    ...
    for assessment_name in assessment_list:
        builtin_assessment_conf = (
            builtin_judge.get_builtin_assessment_config_with_name_with_instruction(
