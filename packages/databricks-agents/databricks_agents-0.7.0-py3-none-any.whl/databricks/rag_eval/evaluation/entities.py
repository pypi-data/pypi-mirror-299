...
class EvalItem:
    ...
    managed_evals_eval_id: Optional[str]
    """Unique identifier for the managed-evals eval item."""

    managed_evals_dataset_id: Optional[str]
    ...
    def from_pd_series(cls, series: pd.Series):
        ...
        grading_notes = input_output_utils.output_to_string(
            series.get(schemas.GRADING_NOTES_COL)
        )
...
class EvalResult:
    ...
    def get_inputs_dict(self) -> Dict[str, Any]:
        """Get the inputs as a dictionary. Keys are defined in schemas."""
        inputs = {
            ...
            schemas.GRADING_NOTES_COL: self.eval_item.grading_notes,
            schemas.TRACE_COL: serialization_utils.serialize_trace(
                self.eval_item.trace
            ),
        }
        if self.eval_item.model_error_message:
            inputs[schemas.MODEL_ERROR_MESSAGE_COL] = self.eval_item.model_error_message
        if self.eval_item.managed_evals_eval_id is not None:
            inputs[schemas.MANAGED_EVALS_EVAL_ID_COL] = (
                self.eval_item.managed_evals_eval_id
            )
        if self.eval_item.managed_evals_dataset_id is not None:
            inputs[schemas.MANAGED_EVALS_DATASET_ID_COL] = (
                self.eval_item.managed_evals_dataset_id
            )
        return inputs
    ...
    def get_assessment_results_dict(self) -> Dict[str, schemas.ASSESSMENT_RESULT_TYPE]:
        ...
        for assessment in self.assessment_results:
            ...
            try:
                builtin_assessment_config = builtin_judge.get_builtin_assessment_config_with_service_assessment_name(
