...
class RegistryRestApi(MicroComponent):
    def __init__(self, name: str, parent=None, host="0.0.0.0", port=8000, start_server=True):
        ...
        def get_instance_attributes(instance_name: str):
            ...
            return {"attributes": self._get_instance_attributes(instance, filter_types=["properties", "attributes"])}
        ...
        def get_instance_properties(instance_name: str):
            ...
            if not instance:
                raise HTTPException(status_code=404, detail="Instance not found")
            return {"properties": self._get_instance_attributes(instance, filter_types=["properties"])}

        @self.app.get(self.prefix + "/instance/{instance_name}/attribute/{attribute_name}")
        ...
        @self.app.post(self.prefix + "/instance/{instance_name}/attribute/{attribute_name}")
                ...
                if hasattr(instance, attr):
                    ...
                    if current_value != new_value:  # Only update if values differ
                        try:
                            ...
                            error_messages.append(f"Failed to update '{attr}': {str(e)}")
                else:
                    error_messages.append(f"Attribute '{attr}' not found on instance '{instance_name}'")
        ...
        for attr_name in dir(component):
            ...
            if "all" not in filter_types:
                if "properties" not in filter_types and is_property:
                    continue
                if "attributes" not in filter_types and not is_property and not is_method:
                    continue
            ...
            if is_method:
                ...
                parameters = {param_name: str(param) for param_name, param in signature.parameters.items()}
                ...
                for item in attr_value:
                    if isinstance(item, MicroComponent):
                        list_info.append({"type": "MicroComponent", "component_name": item.name})
                    else:
                        list_info.append({"type": type(item).__name__, "value": safe_stringify(item)})
