...
ABOUT_tok2me = "tok2me is a CLI to interact with large language models in a Chat-style interface, enabling the assistant to execute commands and code on the local machine, letting them assist in all kinds of development and terminal-based work."
...
config_path = os.path.expanduser("~/.config/tok2me/config.toml")
...
def get_workspace_prompt(workspace: str) -> str:
    project_config_paths = [
        p
        for p in (
            Path(workspace) / "tok2me.toml",
            Path(workspace) / ".github" / "tok2me.toml",
        )
        if p.exists()
    ]
