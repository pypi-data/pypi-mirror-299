...
class Wrapper(L.LightningModule):
    ...
    def on_test_epoch_end(self) -> None:
        ...
        indices = torch.cat(idx, dim=-1).flatten()

        self.res_dict["idx"] = [int(i) for i in torch.cat(idx, dim=-1).flatten()[indices]]
        self.res_dict["gt"] = [int(g) for g in torch.cat(gt, dim=-1).flatten()[indices]]
        self.res_dict["pred"] = [int(p) for p in torch.cat(pred, dim=-1).flatten()[indices]]
        self.res_dict["loss"] = [float(l) for l in torch.cat(loss, dim=-1).flatten()[indices]]
        self.res_dict["conf"] = [float(c) for c in torch.cat(conf, dim=-1).flatten()[indices]]
        self.res_dict["gt_conf"] = [float(gc) for gc in torch.cat(gt_conf, dim=-1).flatten()[indices]]
