...
extensions = [
    Extension(
        ...
        ["vnstockdata/config/config.pyx"]  # Đường dẫn đến file config.c
    ),
    Extension(
        "vnstockdata.config.ensure_logged_in",  # Tên module cho file ensure_logged_in.c
        ["vnstockdata/config/ensure_logged_in.pyx"]  # Đường dẫn đến file ensure_logged_in.c
    ),
    Extension(
        "vnstockdata.process_client.login",  # Tên module cho file login.c
        ["vnstockdata/process_client/login.pyx"]  # Đường dẫn đến file login.c
    ),
    Extension(
        "vnstockdata.process_stock.ohlcv",  # Tên module cho file trong thư mục stock/price
        ["vnstockdata/process_stock/ohlcv.pyx"]  # Đường dẫn đến file price.c
    )
]
