...
class PortfolioSyncManager():
    ...
    def get_strategy_position(self, strategy_name, at):
        ...
        market = get_market_info_by_name(strategy_position['market'])
    ...
    def get_position(self, at='close', market_name=None):
        ...
        """
        position = self.data['position']
        ret = sum([Position.from_list(
            self.get_strategy_position(pname, at)) 
            for pname in position.keys()], Position({}))

        if market_name is not None:
            ret = sum([Position.from_list(
                self.get_strategy_position(pname, at)) 
                for pname in position.keys() 
                if self.data['position'][pname]['market'] == market_name
                ], Position({}))
