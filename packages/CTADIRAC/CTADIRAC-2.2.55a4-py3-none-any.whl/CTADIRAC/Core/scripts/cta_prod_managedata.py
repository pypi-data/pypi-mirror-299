...
def main():
    ...
    if update_ts:
        ...
        for out_lfn in out_lfn_list:
            ...
            # Temporary fix to take into account different fromat for simtelarray file names
            out_run_number_5d = "run%05d" % int(out_run_number)
            out_run_number_6d = "run%06d" % int(out_run_number)
            for in_lfn in in_lfn_list:
                in_file_name = os.path.basename(in_lfn)
                # Temporary fix to take into account different fromat for simtelarray file names
                if (
                    out_run_number_5d in in_file_name
