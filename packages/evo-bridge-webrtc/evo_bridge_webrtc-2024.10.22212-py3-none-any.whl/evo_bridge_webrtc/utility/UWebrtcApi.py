...
# ---------------------------------------------------------------------------------------------------------------------------------------
    def __init__(self):   
        if UWebrtcApi.__instance != None:
            raise Exception("ERROR:SINGLETON")
        else:
            super().__init__()
            UWebrtcApi.__instance = self
...
# ---------------------------------------------------------------------------------------------------------------------------------------
    ...
    Returns:
        _type_: UWebrtcApi instance
    """
    @staticmethod
    def getInstance():
        if UWebrtcApi.__instance is None:
            uObject = UWebrtcApi()  
            uObject.doInit()  
        return UWebrtcApi.__instance
# ---------------------------------------------------------------------------------------------------------------------------------------
    """doInit
...
#>   
            ...
            raise
# ---------------------------------------------------------------------------------------------------------------------------------------
    async def doOnGetSdpAnswer(self, eRtc:ERtc) -> ERtc :
        try:
            if eRtc is None:
                raise Exception("ERROR_REQUIRED|eRtc|")
                ...
                async def on_message(message):
                    try:
                        ...
                        if str(channel.label).startswith("action_"):
                                await self.doAction(eRtcPeer=eRtcPeer, dataChannel=channel, data=message)
...
# ----------------------------------------------------------------------------------------------------------------------------------------      
    async def getIceServer(self):
        try:
            ...
            raise
# ----------------------------------------------------------------------------------------------------------------------------------------     
    async def doAction(self, eRtcPeer:ERtcPeer, dataChannel:RTCDataChannel, data:bytes):
        try:
            
            async for dataResponse in CApiFlow.getInstance().doEAction(data):      
                #TODO:chunk
                dataChannel.send(dataResponse)
            '''
