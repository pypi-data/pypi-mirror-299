...
class Workflow:
    ...
    def correct_sequence(self, input_list):
        ...
        for item in input_list:
            inp_occurrences = item.count('INP')
            for _ in range(inp_occurrences):
                item = item.replace('INP', f'INP{inp_count}', inp_count + 1)
                item = item.replace(f'INP{inp_count}', 'INP', inp_count)
                inp_count += 1
    ...
    def call(self, data):
        ...
        for i, function_call in enumerate(self.call_sequence):
            for _ in range(function_call.count("INP")):
                match = re.search(r"INP(\d+)", function_call)
