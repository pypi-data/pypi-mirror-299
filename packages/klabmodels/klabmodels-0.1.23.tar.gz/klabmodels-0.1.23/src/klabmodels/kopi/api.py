...
def create_tenant(name: str, apps:List[Application], max_users=100):
   ...
   """
   api_key = uuid.uuid4()
   tenant = Tenant(tenant=name, api_key=api_key, apps=apps, max_users=max_users)
   tenant.save()
   return tenant


def create_user(username, hashed_password, email, tenant, full_name=None):
    # Add a new user
    user = User(username=username, email=email, hashed_password=hashed_password, tenant=tenant, full_name=full_name, disabled=False)
    user.save()
    return user
...
def register_user(username, password, email, full_name=None):
   """
   Add a new user for an application
   """
   ...
   if read_user_by_name(username):
      ...
      user = create_user(username, pwd_hash, email, full_name=full_name)   
      return user
...
def add_scopes_to_user(user_pk: str, scopes: List[str]):
   """
   ...
   if user:
      ...
      update_user(user_pk, scopes=user.scopes)
      return user
   else:
      logging.error(f"User {user_pk} not found.")
