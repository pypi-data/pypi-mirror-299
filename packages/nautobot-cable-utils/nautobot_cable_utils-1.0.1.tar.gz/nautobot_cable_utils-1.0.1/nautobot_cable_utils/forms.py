...
from nautobot.apps.forms import (
    BootstrapMixin, ColorSelect, CSVModelForm, CSVModelChoiceField,
    DynamicModelChoiceField, StaticSelect2, add_blank_choice, parse_alphanumeric_range, DynamicModelMultipleChoiceField, NautobotFilterForm, NautobotBulkEditForm
)
...
class ConnectCableToDeviceForm(BootstrapMixin, forms.ModelForm):
    ...
    class Meta:
        model = Cable
        fields = [
            'termination_a_location', 'termination_a_rack', 'termination_a_device', 'termination_a_id',
            'termination_b_location', 'termination_b_rack', 'termination_b_device', 'termination_b_id'
        ]
...
class ConnectPowerfeedToPowerPortForm(ConnectCableToDeviceForm):
    ...
    class Meta:
        model = Cable
        fields = [
            'termination_a_location', 'termination_a_powerpanel', 'termination_a_id',
            'termination_b_location', 'termination_b_rack', 'termination_b_device', 'termination_b_id'
        ]
...
class ConnectCircuitTerminationToInterfaceForm(ConnectCableToDeviceForm):
    ...
    class Meta:
        model = Cable
        fields = [
            'termination_a_provider', 'termination_a_location', 'termination_a_circuit', 'termination_a_id',
            'termination_b_location', 'termination_b_rack', 'termination_b_device', 'termination_b_id'
        ]
...
class ConnectCircuitTerminationForm(BootstrapMixin, forms.ModelForm):
    ...
    class Meta:
        ...
        fields = [
            'termination_a_provider', 'termination_a_location', 'termination_a_circuit', 'termination_a_id',
            'termination_b_provider', 'termination_b_location', 'termination_b_circuit', 'termination_b_id', 'type',
...
class CableInventoryItemBulkEditForm(NautobotBulkEditForm):
    pk = forms.ModelMultipleChoiceField(queryset=CableInventoryItem.objects.all(), widget=forms.MultipleHiddenInput)
...
class CommissionForm(BootstrapMixin, forms.Form):
    ...
    def clean(self):
        super().clean()
        try:
            if CableInventoryItem.objects.get(name=self.cleaned_data['inventory_item']).cable is not None:
                raise forms.ValidationError({"inventory_item": "the assigned cable inventory_item is already in use"})
        except ObjectDoesNotExist:
            raise forms.ValidationError({"inventory_item": "the assigned cable inventory_item does not exist"})

    def save(self):

        inventory_item = CableInventoryItem.objects.get(name=self.cleaned_data['inventory_item'])
