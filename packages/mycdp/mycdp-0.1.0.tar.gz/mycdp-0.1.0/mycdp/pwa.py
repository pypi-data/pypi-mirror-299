...
class FileHandlerAccept:
    ...
    https://crsrc.org/c/chrome/browser/web_applications/proto/
...
def get_os_app_state(
    ...
    :param manifest_id: The id from the webapp's manifest file,
    commonly it's the url of the site installing the webapp.
    See https://web.dev/learn/pwa/web-app-manifest.
...
def install(
    ...
    or bundle overriding the one derived from the manifestId.
    """
    params: T_JSON_DICT = dict()
    params["manifestId"] = manifest_id
...
def launch(
    ...
    instead of the default start url if it is provided.
    Returns a page Target.TargetID,
...
def change_app_user_settings(
    ...
    or extended scope if the manifest has scope extensions and the flags
    ``DesktopPWAsLinkCapturingWithScopeExtensions``
    and ``WebAppEnableScopeExtensions``` are enabled.
    Note, the API does not support resetting the linkCapturing
    to the initial value, uninstalling and installing the web app again
    will reset it.
