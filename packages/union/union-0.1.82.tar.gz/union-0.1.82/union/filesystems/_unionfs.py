...
class AsyncUnionFS(AsyncFileSystem):
    ...
    def __init__(self, logger: logging.Logger = _DEFAULT_LOGGER, *args, **kwargs):
        cfg = _UNION_CONFIG.config_obj
        remote_connection = _create_channel(config=cfg)
        self._client = ObjectStoreServiceStub(remote_connection.channel)
        self._is_remote = remote_connection.is_remote
        loop = remote_connection.channel._loop
    ...
    async def _get_file(self, rpath: str, lpath: str, callback=_DEFAULT_CALLBACK, **kwargs):
        ...
        if self._is_remote:
            ...
            # Download the file
            with fs.open(presign_response.signed_url, "rb") as f:
                content = f.read()
