# coding: utf-8

"""
    DataForSEO API documentation

    DataForSEO API is the starting point on your journey towards building powerful SEO software. With DataForSEO you can get all the data you need to build an efficient application while also saving your time and budget. DataForSEO API is using the REST technology for interchanging data between your application and our service. The data exchange is made through the widely used HTTP protocol, which allows applying our API to almost all programming languages.

    The version of the OpenAPI document: 1.0.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from dataforseo_client.models.on_page_content_parsing_live_task_info import OnPageContentParsingLiveTaskInfo

class TestOnPageContentParsingLiveTaskInfo(unittest.TestCase):
    """OnPageContentParsingLiveTaskInfo unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> OnPageContentParsingLiveTaskInfo:
        """Test OnPageContentParsingLiveTaskInfo
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `OnPageContentParsingLiveTaskInfo`
        """
        model = OnPageContentParsingLiveTaskInfo()
        if include_optional:
            return OnPageContentParsingLiveTaskInfo(
                id = '',
                status_code = 56,
                status_message = '',
                time = '',
                cost = 1.337,
                result_count = 56,
                path = [
                    ''
                    ],
                data = dataforseo_client.models.data.data(),
                result = [
                    dataforseo_client.models.on_page_content_parsing_live_result_info.OnPageContentParsingLiveResultInfo(
                        crawl_progress = '', 
                        crawl_status = dataforseo_client.models.crawl_status_info.CrawlStatusInfo(
                            max_crawl_pages = 56, 
                            pages_in_queue = 56, 
                            pages_crawled = 56, ), 
                        items_count = 56, 
                        items = [
                            dataforseo_client.models.on_page_content_parsing_item.OnPageContentParsingItem(
                                type = '', 
                                fetch_time = '', 
                                status_code = 56, 
                                page_content = dataforseo_client.models.page_content_info.PageContentInfo(
                                    header = dataforseo_client.models.page_section_content_info.PageSectionContentInfo(
                                        primary_content = [
                                            dataforseo_client.models.content_item_info.ContentItemInfo(
                                                text = '', 
                                                url = '', 
                                                urls = [
                                                    dataforseo_client.models.content_url_info.ContentUrlInfo(
                                                        url = '', 
                                                        anchor_text = '', )
                                                    ], )
                                            ], 
                                        secondary_content = [
                                            dataforseo_client.models.content_item_info.ContentItemInfo(
                                                text = '', 
                                                url = '', )
                                            ], 
                                        table_content = [
                                            dataforseo_client.models.table_content.TableContent(
                                                body = [
                                                    dataforseo_client.models.table_content_item_info.TableContentItemInfo(
                                                        row_cells = [
                                                            dataforseo_client.models.row_cell_info.RowCellInfo(
                                                                text = '', 
                                                                is_header = True, )
                                                            ], )
                                                    ], 
                                                footer = [
                                                    dataforseo_client.models.table_content_item_info.TableContentItemInfo()
                                                    ], )
                                            ], ), 
                                    footer = dataforseo_client.models.page_section_content_info.PageSectionContentInfo(), 
                                    main_topic = [
                                        dataforseo_client.models.topic_info.TopicInfo(
                                            h_title = '', 
                                            main_title = '', 
                                            author = '', 
                                            language = '', 
                                            level = 56, )
                                        ], 
                                    secondary_topic = [
                                        dataforseo_client.models.topic_info.TopicInfo(
                                            h_title = '', 
                                            main_title = '', 
                                            author = '', 
                                            language = '', 
                                            level = 56, )
                                        ], ), )
                            ], )
                    ]
            )
        else:
            return OnPageContentParsingLiveTaskInfo(
        )
        """

    def testOnPageContentParsingLiveTaskInfo(self):
        """Test OnPageContentParsingLiveTaskInfo"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
