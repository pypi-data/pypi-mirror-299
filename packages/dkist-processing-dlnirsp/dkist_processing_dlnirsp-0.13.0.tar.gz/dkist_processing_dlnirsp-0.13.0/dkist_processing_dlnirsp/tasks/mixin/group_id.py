...
class GroupIdMixin:
    ...
    def group_id_get_idx(
        ...
        """
        group_id_array = (
            self.parameters.group_id_array if not rectified else self._rectified_group_id_array
        )
    ...
    def _rectified_group_id_array(self) -> np.ndarray:
        ...
        for slitbeam, group_ids in self.group_id_slitbeam_group_dict.items():
            slitbeam_array = (
                np.zeros((self.parameters.group_id_array.shape[0], reference_wave_axis.size))
                * np.nan
            )
    ...
    def _unrectified_array_shape(self) -> tuple[int, int]:
        ...
        return self.parameters.group_id_array.shape
    ...
    def group_id_illuminated_idx(self) -> np.ndarray:
        ...
        return ~np.isnan(self.parameters.group_id_array)
    ...
    def group_id_num_groups(self) -> int:
        ...
        return int(np.nanmax(self.parameters.group_id_array) + 1)
