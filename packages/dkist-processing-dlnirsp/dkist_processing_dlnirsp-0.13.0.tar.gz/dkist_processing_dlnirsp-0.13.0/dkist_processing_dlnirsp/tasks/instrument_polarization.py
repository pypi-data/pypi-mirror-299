...
class InstrumentPolarizationCalibration(
    ...
    def run(self) -> None:
        ...
        with self.apm_processing_step("Reample demodulation matrices"):
            ...
            final_demod = self.reshape_demodulation_matrices(binned_demod_dict, full_array_shape)
    ...
    def _bin_global_data(self, data: np.ndarray, beam: Literal[1, 2]) -> np.ndarray:
        ...
        idx = self.parameters.group_id_array % 2 == beam - 1
    ...
    def reshape_demodulation_matrices(
        self, binned_demod_dict: dict[int, np.ndarray], final_shape: tuple[int, ...]
    ) -> np.ndarray:
        """Populate a full frame from a set of demodulation matrices that are organized by binned IFU group."""
        if binned_demod_dict[1].shape != binned_demod_dict[2].shape:
            raise ValueError(
                f"Demodulation matrices have different shapes for each beam ({binned_demod_dict[1].shape}, "
                f"{binned_demod_dict[2].shape})"
            )

        # The non-demodulation matrix part of the larger array
        beam1_data_shape = binned_demod_dict[1].shape[:-2]
        beam2_data_shape = binned_demod_dict[2].shape[:-2]
        demod_shape = binned_demod_dict[1].shape[-2:]  # The shape of a single demodulation matrix
        full_demod_matrices = np.zeros(final_shape + demod_shape)

        logger.info(
            f"Demodulation FOV sampling shape for each beam: ({beam1_data_shape}, {beam2_data_shape})"
        )
        ...
        for group in range(self.group_id_num_groups):
            if group % 2 == 0:
                group_demod = binned_demod_dict[1][group // 2]
            else:
                group_demod = binned_demod_dict[2][group // 2]
    ...
    def record_polcal_quality_metrics(self, polcal_fitter_dict: dict[int, PolcalFitter]):
        ...
        for beam, fitter in polcal_fitter_dict.items():
            self.quality_store_polcal_results(
                ...
                label=f"Beam {beam}",
                bin_nums=[self.group_id_num_groups // 2],
