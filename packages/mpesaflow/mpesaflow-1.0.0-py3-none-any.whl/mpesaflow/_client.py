...
ENVIRONMENTS: Dict[str, str] = {
    "production": "https://dev.mpesaflow.com/v1",
    "environment_1": "https://prod.mpesaflow.com/v1",
}
...
class Mpesaflow(SyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        This automatically infers the `api_key` argument from the `MPESAFLOW_API_KEY` environment variable if it is not provided.
        """
        if api_key is None:
            api_key = os.environ.get("MPESAFLOW_API_KEY")
        if api_key is None:
            raise MpesaflowError(
                "The api_key client option must be set either by passing api_key to the client or by setting the MPESAFLOW_API_KEY environment variable"
            )
