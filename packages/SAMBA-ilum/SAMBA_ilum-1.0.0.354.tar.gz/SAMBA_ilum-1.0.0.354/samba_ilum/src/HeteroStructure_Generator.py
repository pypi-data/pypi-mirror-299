    ...
    if (cont == 1 and n_Lattice == 2):
       ...
       for i in range(len(v1Ax)):
           for j in range(len(v2Ax)):
               ...
               #--------------------------------------------------
               if (mismatch_area <= crit_area):                          # ????????????????????????????????? Atualizar critério ???????????????????????????????
                  #--------------------------------------------
                  ...
                  #-----------------------------------------------------------------------------
                  if (mismatch_ang <= crit_angle_perc and mismatch_ang_diff <= crit_angle_diff):      # ????????????????????????????????? Atualizar critério ???????????????????????????????
                     #--------------------------------------------------------------------------
                     ...
                     #-----------------------------------------
                     if (mismatch_A <= crit_mismatch):                                                # ????????????????????????????????? Atualizar critério ???????????????????????????????
       ...
       if (n_structures != 0):
          ...
          for i in range(n_structures):
              ...
              if (mismatch_type == 0):
                 ...
                 #------------------------------------
                 var_A1 = ((mod_A4 -modA1)/modA1)*100
                 var_B1 = ((mod_B4 -modB1)/modB1)*100
                 var_A2 = ((mod_A4 -modA2)/modA2)*100
                 var_B2 = ((mod_B4 -modB2)/modB2)*100
                 if (n_Lattice == 3):
                    var_A3 = ((mod_A4 -modA3)/modA3)*100
                    var_B3 = ((mod_B4 -modB3)/modB3)*100
                 #--------------------------------------
                 ...
                 #===============================================================
                 # Cálculo da variação percentual da área das Lattice ===========
                 #===============================================================
                 area_A_deform = np.abs(np.linalg.det(F_A))
                 var_area_A = (area_A_deform - 1)*100
                 var_area_A = round(var_area_A, 4)
                 #---------------------------------------------------------------
                 area_B_deform = np.abs(np.linalg.det(F_B))
                 var_area_B = (area_B_deform - 1)*100
                 var_area_B = round(var_area_B, 4)
                 #---------------------------------------------------------------
                 if (n_Lattice == 3):
                    area_C_deform = np.abs(np.linalg.det(F_C))
                    var_area_C = (area_C_deform - 1)*100
                    var_area_C = round(var_area_C, 4)
                 #===============================================================
                 ...
                 #------------------------------------------------------------
                 if (n_Lattice == 2):
                    poscar_new.write(f' | mismatch_area = {var_area_A}_{var_area_B}_{var_A1}_{var_B1}_{var_A2}_{var_B2} | mismatch_angle_12 = {mismatch_ang12[i]}')
                    poscar_new.write(f' | rotation_12 = {angle1}')
                 if (n_Lattice == 3):
                    poscar_new.write(f' | mismatch_area = {var_area_A}_{var_area_B}_{var_area_C}_{var_A1}_{var_B1}_{var_A2}_{var_B2}_{var_A3}_{var_B3} | mismatch_angle_12_13 = {mismatch_ang12[i]}_{mismatch_ang13[i]}')
                 ...
                 if (n_Lattice == 3): poscar_new.write(f' | MDeform_3 = {float(F_C[0][0])}_{float(F_C[0][1])}_{float(F_C[1][0])}_{float(F_C[1][1])}')
                 #----------------------------------------------
              ...
              if (mismatch_type == 1):
                 ...
                 """
                 Lattice_A_inv = np.linalg.inv(Lattice_A)
                 F_A = np.dot(Lattice_A, Lattice_A_inv)                      # ???????????????????????????????????????????????????? Corrigir para mismatch_type == 1
                 MStrain_A = 0.5 * (np.dot(F_A.T, F_A) - I)
                 for aa in range(2):
                     for bb in range(2):
                 ...
                 #--------------------------------------------------------------
                 Lattice_B_inv = np.linalg.inv(Lattice_B)
                 F_B = np.dot(Lattice_A, Lattice_B_inv)                      # ???????????????????????????????????????????????????? Corrigir para mismatch_type == 1
                 MStrain_B = 0.5 * (np.dot(F_B.T, F_B) - I)
                 for aa in range(2):
                     for bb in range(2):
                 ...
                 if (n_Lattice == 3): 
                    Lattice_C_inv = np.linalg.inv(Lattice_C)
                    F_C = np.dot(Lattice_A, Lattice_C_inv)                      # ???????????????????????????????????????????????????? Corrigir para mismatch_type == 1
                    MStrain_C = 0.5 * (np.dot(F_C.T, F_C) - I)
                    for aa in range(2):
                        for bb in range(2):
                            MStrain_C [aa][bb] = round(MStrain_C [aa][bb], 4)   # ???????????????????????????????????????????????????? Corrigir para mismatch_type == 1
                            if (MStrain_C[aa][bb] == -0.0): MStrain_C[aa][bb] = 0.0
                 """
...
for s in range(len(files0)):
    ...
    for i in range(len(files)):
        ...
        if os.path.exists(diret2 + files0[s] + '/' + files[i]):
           ...
           if (n_Lattice == 3):
              temp_angle = VTemp[12].replace('_',' ').split()
...
for i in range(len(files)):
    ...
    VTemp = poscar.readline().split()
    if (n_Lattice == 2):
       mismatch_area = str(VTemp[7])
       angle = str(VTemp[15])
    if (n_Lattice == 3):
       mismatch_area = str(VTemp[8])
       angle = str(VTemp[16])
    #-------------------------
