...
if os.path.isfile('output/POSCAR'):
   ...
   if (VTemp[0] == 'SAMBA'):
      ...
      #--------------------
      if (n_materials > 1):
         rotate_materials = [];  mismatch_materials = [];  matrix_transf = [];  matrix_strain = []
         passo = n_materials +1
         for i in range(n_materials -1):
             passo += 4
             temp1 = str(VTemp[passo]).replace('_', ' ').split()
             mismatch_materials.append([float(temp1[0]), float(temp1[1]), float(temp1[2])])
         #---------------------------------------------------------------------------------
         for i in range(n_materials -1):
             passo += 4
         ...
         if (n_materials > 1):
            for i in range(n_materials):
                passo += 4
                temp1 = str(VTemp[passo]).replace('_', ' ').split()
                matrix_transf.append([[int(temp1[0]), int(temp1[1])], [int(temp1[2]), int(temp1[3])]])
                #--------- 
                passo += 4
                temp1 = str(VTemp[passo]).replace('_', ' ').split()
                matrix_strain.append([[float(temp1[0]), float(temp1[1])], [float(temp1[2]), float(temp1[3])]])
         #----------------------------------------------------------------------------------------------------
         passo += 1
         for i in range(n_materials):
...
total_thickness = max(temp_z) -min(temp_z)
poscar.close()
#--------------------
if (n_materials > 1):
   poscar = open('output/POSCAR', "r")
   ...
   for i in range(n_materials):
       ...
       for j in range(int(nions_materials[i])):
           VTemp = poscar.readline().split()
           temp_z.append(float(VTemp[2]))
       thickness.append( max(temp_z) -min(temp_z) )
       #-------------------------------------------
       if (i > 0): z_separation.append( min(temp_z) -temp_max )
       temp_max = max(temp_z)
       #---------------------
   poscar.close()   
#----------------
...
for n in range(2):
    ...
    if (crit == 1):
       ...
       mismatch = '--';  transformation_matrix = '--';  strain_matrix = '--';  rotate_angle = '--';
       ...
       dados0 = {
                ...
                "transformation_matrix": matrix_transf  if n_materials > 1 else None,
                "strain_matrix": matrix_strain  if n_materials > 1 else None,
