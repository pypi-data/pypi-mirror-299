...
#---------------------------------------------------------------------------------------
vLattice = [dir_poscar + '/' + Lattice1, dir_poscar + '/' + Lattice2];  n = n_Lattice -2
...
for k in range(n_Lattice):
    ...
    for i in range(len(vx)):
        for j in range(len(vx)):
            ...
            n_ions = Area*(dens_ions[0] + dens_ions[1])
            #------------------------------------------

            # if ( n_ions >= ions_crit_i and n_ions <= ions_crit_f ): 
            if ( n_ions >= (ions_crit_i*fator_n) and n_ions <= (ions_crit_f*fator_p) ):        # ?????????????????? Verificar este critério ??????????????????
               try:
...
print(f' ')
print(f'=================================================================')
print(f'Passo {n}: Analisando os casamentos de rede =======================')
print(f'=================================================================')
    ...
    if (cont == 1):
       ...
       for i in range(len(A1x)):
           for j in range(len(A2x)):
               ...
               #--------------------------------------------
               A1 = np.array([A1x[i], A1y[i]]);  B1 = np.array([B1x[i], B1y[i]])
               mod_A1 = np.linalg.norm(A1);  mod_B1 = np.linalg.norm(B1)
               area_cell1 = np.linalg.norm(np.cross(A1, B1));  angle_1 = np.arccos(np.dot(A1,B1)/(mod_A1*mod_B1))
               #----------------------------------------------------------------------------------------------------------------
               A2 = np.array([A2x[j], A2y[j]]);  B2 = np.array([B2x[j], B2y[j]]);  area_cell2 = np.linalg.norm(np.cross(A2, B2))
               mod_A2 = np.linalg.norm(A2);  mod_B2 = np.linalg.norm(B2)
               area_cell2 = np.linalg.norm(np.cross(A2, B2));  angle_2 = np.arccos(np.dot(A2,B2)/(mod_A2*mod_B2))
               #-------------------------------------------------------------------------------------------------
               mod_medio_A = (mod_A1 + mod_A2)/2
               mod_medio_B = (mod_B1 + mod_B2)/2
               angle_medio = (angle_1 + angle_2)/2
               #---------------------------------------------
               var_angle_1 = ((angle_2 -angle_1)/angle_1)*100
               var_angle_2 = ((angle_1 -angle_2)/angle_2)*100
               #---------------------------------------------

               if (abs(var_angle_1) <= crit_angle_perc and abs(var_angle_2) <= crit_angle_diff):
                  if (mismatch_type == 0):
                     A4 = (A1/mod_A1)*mod_medio_A
                     A5 = (A2/mod_A2)*mod_medio_A
                     #---------------------------------------------------------
                     # Calculando a rotação para ajustar os vetores B4 e B5 ---
                     #---------------------------------------------------------
                     ...
                     #----------------------------------------------
                     B4 = matriz_rotacao @ ((A1/mod_A1)*mod_medio_B)
                     B5 = matriz_rotacao @ ((A2/mod_A2)*mod_medio_B)
                     #----------------------------------------------
                     area_cell4  = np.linalg.norm(np.cross(A4, B4))
                     mod_A4 = np.linalg.norm(A4)
                     mod_B4 = np.linalg.norm(B4)
                     #-----------------------------------------------------------------------------------------------------
                     var_area_cell1 = ((area_cell4 -area_cell1)/area_cell1)*100;  var_area_cell1 = round(var_area_cell1, 4)
                     var_area_cell2 = ((area_cell4 -area_cell2)/area_cell2)*100;  var_area_cell2 = round(var_area_cell2, 4)
                     #-----------------------------------------------------------------------------------------------------
                  if (mismatch_type == 1):
                     #--------------------
                     A4 = A1; B4 = B1
                     #--------------------------
                     cos_angle = np.cos(angle_1)
                     sin_angle = np.sin(angle_1)
                     matriz_rotacao = np.array([ [cos_angle, -sin_angle], [sin_angle, cos_angle] ])
                     A5 = (A2/mod_A2)*mod_A1;  B5 = matriz_rotacao @ ((A2/mod_A2)*mod_B1)
                     #------------------------------------------------------------------------
                     var_area_cell1 = 0.0
                  ...
                  if (abs(var_area_cell1) <= crit_area and abs(var_area_cell2) <= crit_area):
                     if (mismatch_type == 0):
                        #-----------------------------------------------------------------
                        var_A1 = ((mod_A4 -mod_A1)/mod_A1)*100;  var_A1 = round(var_A1, 4)
                        var_B1 = ((mod_B4 -mod_B1)/mod_B1)*100;  var_B1 = round(var_B1, 4)
                        #-----------------------------------------------------------------
                     if (mismatch_type == 1):
                        #-------------------------
                        var_A1 = 0.0; var_B1 = 0.0
                        #-------------------------

                     if (abs(var_A1) <= crit_mod_vector and abs(var_B1) <= crit_mod_vector):
                        if (mismatch_type == 0):
                           #-----------------------------------------------------------------
                           var_A2 = ((mod_A4 -mod_A2)/mod_A2)*100;  var_A2 = round(var_A2, 4)
                           var_B2 = ((mod_B4 -mod_B2)/mod_B2)*100;  var_B2 = round(var_B2, 4)
                           #-----------------------------------------------------------------
                        if (mismatch_type == 1):
                           #--------------------
                           var_A2 = ((mod_A1 -mod_A2)/mod_A2)*100;  var_A2 = round(var_A2, 4)
                           var_B2 = ((mod_B1 -mod_B2)/mod_B2)*100;  var_B2 = round(var_B2, 4)
                           #-----------------------------------------------------------------
                        ...
                        if (abs(var_A2) <= crit_mod_vector and abs(var_B2) <= crit_mod_vector):
                           ...
                           angle0 = round(angle0, 4)
                           if (abs(angle0) == 0.0):  angle0 = 0.0
                           #-------------------------------------
                           rot_angle = str(angle0)
                           #----------------------

                           #--------------------------------------------------------------------
                           # Obtendo as matrizes de Deformação F e Strain S: -------------------
                           #--------------------------------------------------------------------
                           Lattice_A = [ [A1[0], A1[1]], [B1[0], B1[1]] ]
                           Lattice_B = [ [A2[0], A2[1]], [B2[0], B2[1]] ]
                           Lattice_C = [ [A4[0], A4[1]], [B4[0], B4[1]] ]
                           Lattice_D = [ [A5[0], A5[1]], [B5[0], B5[1]] ]
                           #---------------------------------------------
                           I = np.eye(2)
                           #---------------------------------------
                           Lattice_A_inv = np.linalg.inv(Lattice_A)
                           F_A = np.dot(Lattice_C, Lattice_A_inv)
                           S_A = 0.5 * (np.dot(F_A.T, F_A) - I)
                           for aa in range(2):
                               for bb in range(2):
                                   #----------------------------------
                                   F_A[aa][bb] = round(F_A[aa][bb], 9)
                                   S_A[aa][bb] = round(S_A[aa][bb], 9)
                                   #------------------------------------------
                                   if (F_A[aa][bb] == -0.0): F_A[aa][bb] = 0.0
                                   if (S_A[aa][bb] == -0.0): S_A[aa][bb] = 0.0
                           #--------------------------------------------------
                           Lattice_B_inv = np.linalg.inv(Lattice_B)
                           F_B = np.dot(Lattice_D, Lattice_B_inv)
                           S_B = 0.5 * (np.dot(F_B.T, F_B) - I)
                           for aa in range(2):
                               for bb in range(2):
                                   #----------------------------------
                                   F_B[aa][bb] = round(F_B[aa][bb], 9)
                                   S_B[aa][bb] = round(S_B[aa][bb], 9)
                                   #------------------------------------------
                                   if (F_B[aa][bb] == -0.0): F_B[aa][bb] = 0.0
                                   if (S_B[aa][bb] == -0.0): S_B[aa][bb] = 0.0
                                   #------------------------------------------

                           #====================================================================================
                           Structures.write(f'{A1[0]:>14,.9f} {A1[1]:>14,.9f} {B1[0]:>14,.9f} {B1[1]:>14,.9f} ')
                           Structures.write(f'{A2[0]:>14,.9f} {A2[1]:>14,.9f} {B2[0]:>14,.9f} {B2[1]:>14,.9f} ')
                           Structures.write(f'{A4[0]:>14,.9f} {A4[1]:>14,.9f} {B4[0]:>14,.9f} {B4[1]:>14,.9f} ')
                           Structures.write(f'{var_area_cell1}_{var_area_cell2}_{var_A1}_{var_B1}_{var_A2}_{var_B2} ')
                           Structures.write(f'{var_angle_1}_{var_angle_2} {rot_angle} ')
                           Structures.write(f'{MT1_00[i]}_{MT1_01[i]}_{MT1_10[i]}_{MT1_11[i]} ')
                           Structures.write(f'{MT2_00[j]}_{MT2_01[j]}_{MT2_10[j]}_{MT2_11[j]} ')
                           Structures.write(f'{F_A[0][0]}_{F_A[0][1]}_{F_A[1][0]}_{F_A[1][1]} ')
                           Structures.write(f'{F_B[0][0]}_{F_B[0][1]}_{F_B[1][0]}_{F_B[1][1]} ')
                           Structures.write(f'{S_A[0][0]}_{S_A[0][1]}_{S_A[1][0]}_{S_A[1][1]} ')
                           Structures.write(f'{S_B[0][0]}_{S_B[0][1]}_{S_B[1][0]}_{S_B[1][1]} \n')
                           #======================================================================
...
if (n_test == 1):
   for s in range(len(dir_matchings)):
       ...
       #--------------------------------------------------------------
       A4x = tfile[:,8].astype(float);  A4y = tfile[:,9].astype(float)
       B4x = tfile[:,10].astype(float); B4y = tfile[:,11].astype(float)
       #------------------------------------------------------------------------------
       var_area_cell = tfile[:,12];  var_angle = tfile[:,13];  rot_angle = tfile[:,14]
       #------------------------------------------------------------------------------
       MT1 = tfile[:,15];  MT2 = tfile[:,16]
       MF1 = tfile[:,17];  MF2 = tfile[:,18]
       MS1 = tfile[:,19];  MS2 = tfile[:,20]
       #------------------------------------
       ...
       if (n_structures != 0):
          ...
          for i in range(n_structures):
              ...
              #-----------------------------------------------------------------
              poscar_new.write(f' | mismatch_area_vectors = {var_area_cell[i]} | mismatch_angle = {var_angle[i]} | rotation_angle = {rot_angle[i]}')
              #----------------------------------------------------------------
              poscar_new.write(f' | MSCell_1 = {MT1[i]} | MSCell_2 = {MT2[i]}')
              poscar_new.write(f' | MDeform_1 = {MF1[i]} | MDeform_2 = {MF2[i]}')
              poscar_new.write(f' | MStrain_1 = {MS1[i]} | MStrain_2 = {MS2[i]}')
              #------------------------------------------------------------------
              poscar_new.write(f' | {ID_Heteroestrutura} \n')
              #----------------------------------------------
              poscar_new.write(f'1.0 \n')
              poscar_new.write(f'{A4x[i]} {A4y[i]} 0.0 \n')
              poscar_new.write(f'{B4x[i]} {B4y[i]} 0.0 \n')
...
for i in range(len(files)):
    ...
    #------------------------------------ 
    VTemp = poscar.readline().split()
    mismatch_area = str(VTemp[7]).replace('_', ' ').split()
    mismatch_area = str(mismatch_area[0]) + '_' + str(mismatch_area[1])
    angle = str(VTemp[15])
    #-------------------------
    ...
    new_name = diret2 + n_ion + '_' + str(mismatch_area) + '_' + angle + '_' + str(VTemp1[-1]) + '+' + number + '.vasp'
