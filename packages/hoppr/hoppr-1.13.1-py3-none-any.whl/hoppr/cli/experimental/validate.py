...
class HopprValidateSummary(HopprBasePanel):  # pylint: disable=too-few-public-methods
    ...
    def _component_issues_table(self, issue_list: IssueList):
        ...
        table = Table("Check", "Count", box=box.MINIMAL, title="Components", title_justify="left", title_style="bold")
    ...
    def _license_issues_table(self, issue_list: IssueList):
        ...
        table = Table("Check", "Count", box=box.MINIMAL, title="Licenses", title_justify="left", title_style="bold")
    ...
    def _metadata_issues_table(self, issue_list: IssueList):
        ...
        table = Table("Check", "Count", box=box.MINIMAL, title="Metadata", title_justify="left", title_style="bold")
...
def _init_globals(log_file: Path, verbose: bool):
    global all_issues, layout, logger, summary_panel  # pylint: disable=global-statement
...
def _get_result_status(issues: IssueList) -> str:
    ...
    for issue in issues:
        if issue.severity == IssueSeverity.MINOR:
            ...
            global _EXIT_CODE  # pylint: disable=global-statement
            _EXIT_CODE = 1
...
def _validate_experimental(  # pylint: disable=too-many-arguments, too-many-locals, unused-argument
