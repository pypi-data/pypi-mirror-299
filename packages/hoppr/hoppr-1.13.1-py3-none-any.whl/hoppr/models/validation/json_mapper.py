...
@dataclass
class JSONElement:  # pylint: disable=too-many-instance-attributes
...
class JSONLexer:  # pylint: disable=too-few-public-methods
    ...
    TOKEN_REGEX_NAME_PAIRS: Final[list[tuple[re.Pattern, JSONElementName | None]]] = [
        ...
        (re.compile(r'(?:"(?:\\(?:["\\\/bfnrt]|u[a-fA-F0-9]{4})|[^"\\\0-\x1F\x7F]+)*")'), JSONElementName.STRING),
        (re.compile(r"((?:-?(?:0|[1-9][0-9]*)(?:\.[0-9]+)?(?:[eE][+-]?[0-9]+)?))"), JSONElementName.NUMBER),
        (re.compile(r"([^\S\r\n]+)", flags=re.M), JSONElementName.HORIZONTAL_WHITESPACE),
...
class JSONParser:  # pylint: disable=too-few-public-methods
    ...
    def _reduce(self, production_name: JSONElementName, handle_start_name: JSONElementName | None = None):
    ...
    def _handle_token(self, token: JSONElement) -> JSONElement | None:
        ...
        match (self._parse_stack, token):
            ...
            case [*_, JSONElement(name=JSONElementName.NAME_SEPARATOR | JSONElementName.BEGIN_ARRAY)] | [
                *_,
                JSONElement(name=JSONElementName.VALUE),
                JSONElement(name=JSONElementName.VALUE_SEPARATOR),
            ], token if token.name in self.ALLOWED_AT_ROOT:
                self._shift(token)
            ...
            # ----- Property Names -----
            case [*_, JSONElement(name=JSONElementName.BEGIN_OBJECT)] | [
                *_,
                JSONElement(name=JSONElementName.VALUE),
                JSONElement(name=JSONElementName.VALUE_SEPARATOR),
            ], JSONElement(name=JSONElementName.STRING):
            ...
            # ----- Value Separators -----
            case [*_, JSONElement(), JSONElement(name=JSONElementName.VALUE | JSONElementName.MEMBER)], JSONElement(
                name=JSONElementName.VALUE_SEPARATOR
            ):
                self._shift(token)

            # ----- Name Separators -----
            case [*_, JSONElement(name=JSONElementName.STRING)], JSONElement(name=JSONElementName.NAME_SEPARATOR):
                self._shift(token)

            # ----- Object End -----
            case [*_, JSONElement(name=JSONElementName.BEGIN_OBJECT | JSONElementName.MEMBER)], JSONElement(
                name=JSONElementName.END_OBJECT
            ):
                self._shift(token)
            ...
            # ----- Array End -----
            case [*_, JSONElement(name=JSONElementName.BEGIN_ARRAY)] | [
                *_,
            ...
            # ----- Accept -----
            case [JSONElement(name=JSONElementName.VALUE)], JSONElement(name=JSONElementName.END_OF_FILE):
...
class JSONLocationMapper:
    ...
    def __init__(self, location_map: defaultdict[str, list[IssueLocationPositionRange]] | None = None):
