...
class HopprSigner:
    ...
    def __init__(
        self, sign: bool, functionary_key_path: Path | None = None, functionary_key_password: str | None = None
    ) -> None:
        self.sign = sign
        ...
        if self.sign and functionary_key_path:
            ...
            if "BEGIN OPENSSH PRIVATE KEY" in key_text:
                self.private_key = serialization.load_ssh_private_key(
                    key_text.encode(), password=password_bytes, backend=default_backend()
                )
            else:
    ...
    def sign_blob(self, blob_file: Path):
        ...
        if self.sign:
            ...
            if signature:
                signature_file = Path(f"{str(blob_file)}.sig")
