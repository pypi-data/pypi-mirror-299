...
class CollectNpmPlugin(SerialCollectorPlugin):
    ...
    def _download_package(
        ...
        response = hoppr.net.download_file(
            url=str(download_url), dest=str(dest_file), creds=creds, proxies=self._get_proxies_for(str(download_url))
        )
        try:
    ...
    def _get_package_checksum(self, metadata_url: RepositoryUrl, pkg_version: str, auth: HTTPBasicAuth | None) -> Hash:
        ...
        raw_checksum = jmespath.search(
            expression=f'versions."{pkg_version}".dist.integrity', data=npm_metadata_response.json()
        ).lstrip("sha512-")
    ...
    def _validate_hashes(
        self, comp: Component, metadata_url: RepositoryUrl, pkg_version: str, auth: HTTPBasicAuth | None
    ) -> Result:
        try:
