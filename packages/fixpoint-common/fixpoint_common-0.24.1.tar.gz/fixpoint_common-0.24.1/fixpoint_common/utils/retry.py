...
def async_retry(
    ...
    Args:
        ...
        exception_handler: Optional function to handle exceptions, returning
            True if they should be skipped, and False otherwise. Arguments are
            the exception and the current retry count.
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if exception_handler:
                        should_skip = await exception_handler(e, attempt)
                        if should_skip:
                            continue
        ...
        def wrapper(*args: Params.args, **kwargs: Params.kwargs) -> Ret:
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if exception_handler:
                        should_skip = exception_handler(e, attempt)
                        if should_skip:
                            continue
