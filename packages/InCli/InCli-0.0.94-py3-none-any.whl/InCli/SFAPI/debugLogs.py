...
def save_to_store(logId,body):
    filepath = f"{restClient.logFolder()}{logId}.log"
...
def worker_deleteRecords(delete_queue):
    ...
    while True:
        ...
        try :
            ...
            restClient.glog().debug(f"deleted record {logId}")
...
def do_parse_tail(pc):
    ...
    recordTime = logRecords[0].get(timefield) if logRecords else '2000-12-12T17:19:35Z'
               ...
               # print(ids_not_processed)
                if pc['noScreen']==False:
                    ...
                    for r in logRecords_not_processed:
                        if r['Operation'] not in ['<empty>','VFRemoting']: 
                            records1.append(r)
                        elif r['Operation'] == 'VFRemoting' and r['LogLength']>1000: 
                            records1.append(r)
...
def parse_apexlogs_tail(pc,logIds=None,raiseKeyBoardInterrupt=False,printProgress=False,threads=10,raise_no_log=True):
    ...
    # if threads >0: 
    #     start_download_worker_threads(pc,threads)
       # threads = 1
        # if pc['downloadBody_queue'] == None:
        #     pc['downloadBody_queue'] = Queue(maxsize=0)
        #     for x in range(0,threads):
        #         threading.Thread(target=worker_read_logBody_from_org,args=(pc['downloadBody_queue'],), daemon=True).start()
        # for logid in logIds:
        #     pc['downloadBody_queue'].put(logid)
  ...
  #  items = filepaths if filepaths != None else logIds
    ...
    for num,item in enumerate(items):
        ...
        try:
            ...
            if ('delete_queue' in pc and pc['delete_queue']!= None):
