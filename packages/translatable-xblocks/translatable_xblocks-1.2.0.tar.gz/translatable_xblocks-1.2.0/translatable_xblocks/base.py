...
from translatable_xblocks.utils import language_selection_to_django_locale
...
class TranslatableXBlock(XBlock):
    ...
    def should_translate(self):
        ...
        3) The provided language codes are allowed by our configuration.
        4) The language codes differ.
        """
        ...
        if (
            self.source_lang not in settings.AI_TRANSLATIONS_LANGUAGE_CONFIG
            or self.translate_lang not in settings.AI_TRANSLATIONS_LANGUAGE_CONFIG
        ):
            logger.warning(
                f"Selected language pair {self.source_lang} to {self.translate_lang} not available for translation"
            )
            return False
    ...
    def student_view(self, context, **kwargs):
        ...
        if self.should_translate:
            ...
            self.runtime.service(self, "i18n").translator.activate(
                language_selection_to_django_locale(self.translate_lang)
            )
