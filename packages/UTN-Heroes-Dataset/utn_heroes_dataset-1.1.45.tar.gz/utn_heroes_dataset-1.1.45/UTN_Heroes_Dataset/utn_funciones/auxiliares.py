...
from terminaltexteffects.effects import (
    effect_matrix, effect_beams, effect_rain, effect_expand, effect_binarypath
)
import random as rd
...
def mostrar_texto_con_efecto(texto: str) -> None:
    ...
    selects an effect from a list ('Matrix', 'Beam', 'Rain', 'Expand', 'BinaryPath
    :type texto: str
    """
    effect_selected = rd.choice([
        'Matrix', 'Beam', 'Rain', 'Expand', 'BinaryPath'])
    match effect_selected:
        case 'Matrix':
            effect = effect_matrix.Matrix(texto)
        case 'Beam':
            effect = effect_beams.Beams(texto)
        case 'Rain':
            effect = effect_rain.Rain(texto)
        case 'Expand':
            effect = effect_expand.Expand(texto)
        case 'BinaryPath':
            effect = effect_binarypath.BinaryPath(texto)
