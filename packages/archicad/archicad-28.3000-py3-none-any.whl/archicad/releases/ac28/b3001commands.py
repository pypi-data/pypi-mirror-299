...
from .b3001types import ClassificationItemAvailabilityOrError, LineAttributeOrError, Document3DNavigatorItemOrError, ElementComponentsOrError, SurfaceAttributeOrError, ElementsOrError, AttributeIndexAndGuidOrError, PropertyDefinitionOrError, BuiltInContainerNavigatorItemOrError, ClassificationItemArrayItem, PenTableAttributeOrError, AttributeFolderRenameParameters, PropertyIdOrError, ClassificationSystemId, ProfileAttributeOrError, PropertyIdsOfElementOrError, RGBColor, StoryNavigatorItemOrError, AttributeFolderId, PropertyValuesOrError, NavigatorTreeId, AttributeFolderCreationParameters, AttributeIdOrError, ClassificationSystemIdArrayItem, AddOnCommandParameters, CompositeAttributeOrError, BoundingBox3DOrError, PropertyDefinitionAvailabilityOrError, AttributeFolderOrError, LayoutParameters, AddOnCommandResponse, SectionNavigatorItemOrError, ExecutionResult, ZoneCategoryAttributeOrError, ClassificationSystemOrError, FillAttributeOrError, ClassificationSystem, AttributeIdWrapperItem, WorksheetNavigatorItemOrError, PropertyIdArrayItem, ClassificationItemOrError, NavigatorItemId, ElevationNavigatorItemOrError, NavigatorItemIdAndTypeOrError, PropertyGroupIdArrayItem, AddOnCommandId, AttributeFolderIdWrapperItem, LayerCombinationAttributeOrError, AttributeFolderStructure, BuildingMaterialAttributeOrError, PropertyUserId, LayerAttributeOrError, InteriorElevationNavigatorItemOrError, ImageOrError, ElementPropertyValue, ClassificationItemId, BoundingBox2DOrError, ClassificationItemIdArrayItem, DetailNavigatorItemOrError, ElementClassificationOrError, ElementComponentIdArrayItem, TypeOfElementOrError, PropertyGroupOrError, NavigatorTree, ElementIdArrayItem, NavigatorItemIdWrapper, Subset, ElementClassification, FolderParameters
...
class Commands:
    ...
    def CreateAttributeFolders(self, attributeFolders: List[AttributeFolderCreationParameters]) -> List[ExecutionResult]:
        ...
        Args:
            attributeFolders (:obj:`list` of :obj:`AttributeFolderCreationParameters`): EMPTY STRING
        ...
        class CreateAttributeFolders_parameters(_ACBaseType):
            ...
            def __init__(self, attributeFolders: List[AttributeFolderCreationParameters]):
                self.attributeFolders: List[AttributeFolderCreationParameters] = attributeFolders

        CreateAttributeFolders_parameters.get_classinfo().add_field('attributeFolders', List[AttributeFolderCreationParameters])
    ...
    def DeleteAttributeFolders(self, attributeFolderIds: List[AttributeFolderIdWrapperItem]) -> List[ExecutionResult]:
        ...
        class DeleteAttributeFolders_parameters(_ACBaseType):
            __slots__ = ("attributeFolderIds", )
            def __init__(self, attributeFolderIds: List[AttributeFolderIdWrapperItem]):
                self.attributeFolderIds: List[AttributeFolderIdWrapperItem] = attributeFolderIds

        DeleteAttributeFolders_parameters.get_classinfo().add_field('attributeFolderIds', List[AttributeFolderIdWrapperItem])

        result = post_command(self.__req, json.dumps({"command": "API.DeleteAttributeFolders", "parameters": DeleteAttributeFolders_parameters(attributeFolderIds).to_dict()}))
        if not result["succeeded"]:
            raise UnsucceededCommandCall(result)
        executionResultsListBuilder = _ListBuilder(ExecutionResult)
        return executionResultsListBuilder(result["result"]["executionResults"])
    ...
    def GetAttributeFolders(self, attributeFolderIds: List[AttributeFolderIdWrapperItem]) -> List[AttributeFolderOrError]:
        ...
        Returns:
            :obj:`list` of :obj:`AttributeFolderOrError`: A list of attribute folders.

        """
        class GetAttributeFolders_parameters(_ACBaseType):
            __slots__ = ("attributeFolderIds", )
            def __init__(self, attributeFolderIds: List[AttributeFolderIdWrapperItem]):
                self.attributeFolderIds: List[AttributeFolderIdWrapperItem] = attributeFolderIds

        GetAttributeFolders_parameters.get_classinfo().add_field('attributeFolderIds', List[AttributeFolderIdWrapperItem])

        result = post_command(self.__req, json.dumps({"command": "API.GetAttributeFolders", "parameters": GetAttributeFolders_parameters(attributeFolderIds).to_dict()}))
        if not result["succeeded"]:
            raise UnsucceededCommandCall(result)
        attributeFoldersListBuilder = _ListBuilder(AttributeFolderOrError)
        return attributeFoldersListBuilder(result["result"]["attributeFolders"])

    def GetAttributeFolderStructure(self, attributeType: str, path: Optional[List[str]] = None) -> AttributeFolderStructure:
        ...
        Returns:
            :obj:`AttributeFolderStructure`: The details of an attribute folder structure.

        """
        class GetAttributeFolderStructure_parameters(_ACBaseType):
            __slots__ = ("attributeType", "path", )
            def __init__(self, attributeType: str, path: Optional[List[str]] = None):
                self.attributeType: str = attributeType
                self.path: Optional[List[str]] = path

        GetAttributeFolderStructure_parameters.get_classinfo().add_field('attributeType', str, value_set(['BuildingMaterial', 'Composite', 'Fill', 'Layer', 'LayerCombination', 'Line', 'PenTable', 'Profile', 'Surface', 'ZoneCategory']))
        GetAttributeFolderStructure_parameters.get_classinfo().add_field('path', Optional[List[str]], listitem_validator(min_length(1)))

        result = post_command(self.__req, json.dumps({"command": "API.GetAttributeFolderStructure", "parameters": GetAttributeFolderStructure_parameters(attributeType, path).to_dict()}))
        if not result["succeeded"]:
            raise UnsucceededCommandCall(result)
        return AttributeFolderStructure(**result["result"]["attributeFolder"])
    ...
    def GetAttributesIndices(self, attributeIds: List[AttributeIdWrapperItem]) -> List[AttributeIndexAndGuidOrError]:
        ...
        Returns:
            :obj:`list` of :obj:`AttributeIndexAndGuidOrError`: A list of complete attribute identifiers.

        """
        class GetAttributesIndices_parameters(_ACBaseType):
            __slots__ = ("attributeIds", )
            def __init__(self, attributeIds: List[AttributeIdWrapperItem]):
                self.attributeIds: List[AttributeIdWrapperItem] = attributeIds

        GetAttributesIndices_parameters.get_classinfo().add_field('attributeIds', List[AttributeIdWrapperItem])

        result = post_command(self.__req, json.dumps({"command": "API.GetAttributesIndices", "parameters": GetAttributesIndices_parameters(attributeIds).to_dict()}))
        if not result["succeeded"]:
            raise UnsucceededCommandCall(result)
        attributeIndicesAndGuidsListBuilder = _ListBuilder(AttributeIndexAndGuidOrError)
        return attributeIndicesAndGuidsListBuilder(result["result"]["attributeIndicesAndGuids"])
    ...
    def GetSelectedElements(self, onlyEditable: Optional[bool] = None, onlySupportedTypes: Optional[bool] = None) -> List[ElementIdArrayItem]:
        ...
        Args:
            ...
            onlySupportedTypes (:obj:`bool`, optional): Optional parameter. When it is set to true, only elements with types that are supported by any other JSON API command will be returned.
        ...
        class GetSelectedElements_parameters(_ACBaseType):
            __slots__ = ("onlyEditable", "onlySupportedTypes", )
            def __init__(self, onlyEditable: Optional[bool] = None, onlySupportedTypes: Optional[bool] = None):
                self.onlyEditable: Optional[bool] = onlyEditable
                self.onlySupportedTypes: Optional[bool] = onlySupportedTypes

        GetSelectedElements_parameters.get_classinfo().add_field('onlyEditable', Optional[bool])
        GetSelectedElements_parameters.get_classinfo().add_field('onlySupportedTypes', Optional[bool])

        result = post_command(self.__req, json.dumps({"command": "API.GetSelectedElements", "parameters": GetSelectedElements_parameters(onlyEditable, onlySupportedTypes).to_dict()}))
        if not result["succeeded"]:
            raise UnsucceededCommandCall(result)
        elementsListBuilder = _ListBuilder(ElementIdArrayItem)
        return elementsListBuilder(result["result"]["elements"])
    ...
    def MoveAttributesAndFolders(self, attributeFolderIds: List[AttributeFolderIdWrapperItem], attributeIds: List[AttributeIdWrapperItem], targetFolderId: AttributeFolderId):
        """Moves attributes and attribute folders.

        Args:
            attributeFolderIds (:obj:`list` of :obj:`AttributeFolderIdWrapperItem`): A list of attribute folder identifiers.
            attributeIds (:obj:`list` of :obj:`AttributeIdWrapperItem`): A list of attribute identifiers.
            targetFolderId (:obj:`AttributeFolderId`): The identifier of an attribute folder.

        """
        class MoveAttributesAndFolders_parameters(_ACBaseType):
            __slots__ = ("attributeFolderIds", "attributeIds", "targetFolderId", )
            def __init__(self, attributeFolderIds: List[AttributeFolderIdWrapperItem], attributeIds: List[AttributeIdWrapperItem], targetFolderId: AttributeFolderId):
                self.attributeFolderIds: List[AttributeFolderIdWrapperItem] = attributeFolderIds
                self.attributeIds: List[AttributeIdWrapperItem] = attributeIds
                self.targetFolderId: AttributeFolderId = targetFolderId

        MoveAttributesAndFolders_parameters.get_classinfo().add_field('attributeFolderIds', List[AttributeFolderIdWrapperItem])
        MoveAttributesAndFolders_parameters.get_classinfo().add_field('attributeIds', List[AttributeIdWrapperItem])
        MoveAttributesAndFolders_parameters.get_classinfo().add_field('targetFolderId', AttributeFolderId)

        result = post_command(self.__req, json.dumps({"command": "API.MoveAttributesAndFolders", "parameters": MoveAttributesAndFolders_parameters(attributeFolderIds, attributeIds, targetFolderId).to_dict()}))
        if not result["succeeded"]:
            raise UnsucceededCommandCall(result)
    ...
    def RenameAttributeFolders(self, attributeFolderParametersList: List[AttributeFolderRenameParameters]) -> List[ExecutionResult]:
        """Rename attribute folder.

        Args:
            attributeFolderParametersList (:obj:`list` of :obj:`AttributeFolderRenameParameters`): EMPTY STRING

        Returns:
            :obj:`list` of :obj:`ExecutionResult`: A list of execution results.

        """
        class RenameAttributeFolders_parameters(_ACBaseType):
            __slots__ = ("attributeFolderParametersList", )
            def __init__(self, attributeFolderParametersList: List[AttributeFolderRenameParameters]):
                self.attributeFolderParametersList: List[AttributeFolderRenameParameters] = attributeFolderParametersList

        RenameAttributeFolders_parameters.get_classinfo().add_field('attributeFolderParametersList', List[AttributeFolderRenameParameters])

        result = post_command(self.__req, json.dumps({"command": "API.RenameAttributeFolders", "parameters": RenameAttributeFolders_parameters(attributeFolderParametersList).to_dict()}))
        if not result["succeeded"]:
            raise UnsucceededCommandCall(result)
        executionResultsListBuilder = _ListBuilder(ExecutionResult)
        return executionResultsListBuilder(result["result"]["executionResults"])
