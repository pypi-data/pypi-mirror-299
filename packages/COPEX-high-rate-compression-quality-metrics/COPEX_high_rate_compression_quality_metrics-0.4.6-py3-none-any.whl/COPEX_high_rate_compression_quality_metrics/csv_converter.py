...
def csv_to_png_plot(csv_input_path: str, png_output_path: str, metric: str, verbose=False,show_output = False):
    """
    ...
    if metric not in utils.metrics_dictionary:
        raise ValueError(
            f"Métrique '{metric}' non reconnue. Choisissez parmi {list(utils.metrics_dictionary.keys())}.")
    ...
    for algorithm_name, data in algorithm_data.items():
        color = utils.algorithm_colors.get(algorithm_name, 'blue')  # Utiliser la couleur définie ou bleu par défaut
    ...
    plt.xlim(utils.data_range_for_compression["min"], utils.data_range_for_compression["max"])
    plt.ylim(utils.metrics_dictionary[metric]["min"], utils.metrics_dictionary[metric]["max"])
...
def csv_to_png_plot_for_all_metrics(csv_input_path: str, output_folder: str, verbose=False, show_output=False):
    ...
    :param verbose: Booléen pour activer les messages d'information supplémentaires.
    :param show_output: Booléen pour afficher les graphiques à l'écran.
    """
    ...
    with open(csv_input_path, 'r') as csv_file:
        ...
        for row in reader:
            ...
            for i, metric in enumerate(metrics):
                algorithm_data[algorithm_name][metric].append(float(row[i+2]))  # Les valeurs des métriques sont dans les colonnes 3 à 6
...
def csv_mono_metric_to_png_plot_from_decompressed_folder_name(root_directory:str, dataset_name:str, test_case_number, verbose =True):
    decompressed_folder_path = utils.get_decompressed_folder_path(root_directory, dataset_name, test_case_number,
                                                                  verbose=verbose)
    summary_csv_folder_path = os.path.join(decompressed_folder_path,utils.summary_output_folder_name,utils.summary_output_csv_folder_name)
    summary_png_folder_path = os.path.join(decompressed_folder_path,utils.summary_output_folder_name,utils.summary_output_png_folder_name)
    ...
    for csv_file_name in csv_files:
        ...
        csv_to_png_plot(input_csv_path, output_png_path, wanted_metric, verbose=True)
def csv_multi_metric_to_png_plot_from_decompressed_folder_name(root_directory:str, dataset_name:str, test_case_number, verbose =True):
    decompressed_folder_path = utils.get_decompressed_folder_path(root_directory, dataset_name, test_case_number,
                                                                  verbose=verbose)
    summary_csv_folder_path = os.path.join(decompressed_folder_path,utils.summary_output_folder_name,utils.summary_output_csv_folder_name)
    summary_png_folder_path = os.path.join(decompressed_folder_path,utils.summary_output_folder_name,utils.summary_output_png_folder_name)
...
"""wanted_band = "B08"
wanted_metric = "SSIM"
input_csv_name = wanted_band+"_"+wanted_metric+".csv"
output_png_name = wanted_band+"_"+wanted_metric+".png"

# Exemple d'utilisation :
csv_to_png_plot(input_csv_name, output_png_name, wanted_metric, verbose=True)"""

"""root_directory = os.path.join("..","test","data")
dataset_name = "S2_L1C"
test_case_number = 1
csv_multi_metric_to_png_plot_from_decompressed_folder_name(root_directory,dataset_name,test_case_number)"""
