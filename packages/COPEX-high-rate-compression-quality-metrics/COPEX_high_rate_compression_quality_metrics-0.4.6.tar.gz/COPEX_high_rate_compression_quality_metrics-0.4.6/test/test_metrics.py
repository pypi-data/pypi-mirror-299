...
result_path = utils.get_algorithm_results_full_path(root_directory=root_directory, dataset_name=dataset_name,
                                                    test_case_number=test_case_number,
                                                    nnvvppp_algoname=nnvvppp_algoname)
original_path = utils.get_original_full_path(root_directory=root_directory, dataset_name=dataset_name,
                                             test_case_number=test_case_number)
print("\nresult_path = [", result_path, "}")
print("original_path = [", original_path, "}")
result_products = utils.get_product_path_list_from_path(path=result_path)
original_products = utils.get_product_path_list_from_path(path=original_path)
...
for i in range(0, 2):
    ...
    result = metrics.calculate_psnr(image1, image2)
    print("PSNR = ", result)
    end_time = time.time()  # Enregistre le temps de fin
    execution_time = end_time - start_time  # Calcule le temps d'exécution
    print(f"Temps d'exécution pour le PSNR ({i}): {execution_time:.4f} secondes")"""
...
for i in range(0, 2):
    ...
    image2 = io.imread(result_products[i])
    start_time = time.time()  # Enregistre le temps de début
    result = metrics.calculate_rmse(image1, image2)
    print("RMSE = ", result)
    end_time = time.time()  # Enregistre le temps de fin
    execution_time = end_time - start_time  # Calcule le temps d'exécution
    print(f"Temps d'exécution pour le RMSE ({i}) : {execution_time:.4f} secondes")"""
...
for i in range(0, 1):
    ...
    print(f"Type de données : {image1.dtype}")

    # Vérifier les dimensions
    print(f"Dimensions : {image1.shape}")

    # Vérifier les valeurs minimales et maximales
    print(f"Valeur minimale : {np.min(image1)}")
    print(f"Valeur maximale : {np.max(image1)}")
    #print(image1)
    start_time = time.time()  # Enregistre le temps de début
    result = metrics.calculate_ssim_multiband_with_blocks_of_even_size(image1, image2)
    print("SSIM = ", result)
    end_time = time.time()  # Enregistre le temps de fin
    execution_time = end_time - start_time  # Calcule le temps d'exécution
    print(f"Temps d'exécution pour le SSIM ({i}) : {execution_time:.4f} secondes")
