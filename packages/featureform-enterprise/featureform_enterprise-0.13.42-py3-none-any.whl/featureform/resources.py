...
class PrimaryData:
    ...
    def kwargs(self) -> Dict[str, Any]:
        ...
        if isinstance(self.location, SQLTable):
            ...
            primary_data_kwargs["kafka"] = pb.Kafka(
                topic=self.location.topic
            )
        else:
...
class Transformation:
    @classmethod
    def from_proto(cls, source_transformation):
        if source_transformation.DFTransformation.query != bytes():
            return DFTransformation.from_proto(source_transformation)
        elif source_transformation.SQLTransformation.query != "":
            return SQLTransformation.from_proto(source_transformation)
        else:
...
class SQLTransformation(Transformation):
    ...
    snowflake_dynamic_table_config: Optional[SnowflakeDynamicTableConfig] = None
    ...
    def kwargs(self) -> Dict[str, pb.Transformation]:
        ...
        transformation = pb.Transformation(
            SQLTransformation=pb.SQLTransformation(
                ...
                streaming_sources=self.streaming_inputs,
                snowflake_dynamic_table_config=(
                    self.snowflake_dynamic_table_config.to_proto()
                    if self.snowflake_dynamic_table_config
                    else None
                ),
            ),
    ...
    @staticmethod
    def from_proto(source_transformation):
        return SQLTransformation(source_transformation.SQLTransformation.query)
...
class DFTransformation(Transformation):
    ...
    def kwargs(self):
        ...
        transformation = pb.Transformation(
            DFTransformation=pb.DFTransformation(
                ...
                is_incremental=self.is_incremental,
                incremental_sources=name_variants,
            ),
            spark_flags=self.spark_flags.to_proto() if self.spark_flags else None,
            **partition_kwargs,
        )
    ...
    @staticmethod
    def from_proto(source_transformation):
        transformation = source_transformation.DFTransformation
