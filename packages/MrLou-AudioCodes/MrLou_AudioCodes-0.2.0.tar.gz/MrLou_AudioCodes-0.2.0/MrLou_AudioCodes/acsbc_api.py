...
def upload_incremental_cli_script(fqdn, auth, cli_name, cli_file_path):
    url = f'api/v1/files/cliScript/incremental'
    payload = {}
    files = [
        ('', (cli_name, open(cli_file_path, 'rb'), 'application/octet-stream'))
...
def change_user_config(fqdn, auth, username, password, status, password_age):
    url = f'api/v1/files/cliScript/incremental'
    cli_name = 'change_user_cliscript'
    cli_content = f"""configure system
user {username}
  password {password}
  status {status}
...
exit"""
    ...
    files = [
        ('', (cli_name, cli_content, 'application/octet-stream'))

    ]
    request = PUTRequest(fqdn, auth)
    response = request.send(url, payload={}, files=files)
    if response:
        print(f"Upload Response: {response.text}")
    return response
