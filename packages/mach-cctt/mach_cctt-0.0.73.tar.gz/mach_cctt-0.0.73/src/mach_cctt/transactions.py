...
async def send_transaction(
    w3: AsyncWeb3, account: LocalAccount, params: TxParams, log: Logger = logger
) -> None:
...
async def approve(
    ...
    approve_function = token_contract.functions.approve(spender, max(amount, config.solidity_uint_max))
