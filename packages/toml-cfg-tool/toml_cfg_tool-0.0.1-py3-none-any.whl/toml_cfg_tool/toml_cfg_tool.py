...
from toml_cfg_tool.src.creation import create_setup_cfg_template, create_pyproject_toml_template, create_workflow_files
...
def main():
    ...
    if not updates and not args.create_templates:
        print("No updates specified and --create-templates not set. Use --help for more information.")
        sys.exit(0)
    ...
    for file in files:
        ...
        if os.path.exists(file) and updates:
            ...
            if file.endswith(".cfg"):
                update_setup_cfg(file, updates, dry_run=args.dry_run, backup=args.backup)
            
            elif file.endswith(".toml"):
                update_pyproject_toml(file, updates, dry_run=args.dry_run, backup=args.backup)
