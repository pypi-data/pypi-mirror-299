        ...
        for attempt in range(max_attempts):
            ...
            if len(task.public_key) % 2 == 0:
                try:
                    ...
                    public_key = task.public_key[2:] if task.public_key.startswith('0x') else task.public_key
    ...
    async def fetch(
        ...
        if min_outputs is None:
            if isinstance(task_id, list):
                min_outputs = len(task_id)
            else:
                min_outputs = 1

        if min_outputs > len(task_id) and isinstance(task_id, list):
            logger.warning(f"min_outputs is greater than the number of task_id. Setting min_outputs to {len(task_id)}")
            min_outputs = len(task_id)
