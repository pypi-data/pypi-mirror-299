...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--infile', metavar='infile', type=str, default='model.ecore', help='the file to be converted', dest='infile')
    parser.add_argument('-o', '--outfile', metavar='outfile', type=str, default='model.eoq', help='the converted file', dest='outfile')
    #user model settings
    parser.add_argument('-m','--metafile', metavar='metafile', type=str, default=None, help='the .ecore file (only necessary if a user model is loaded. Otherwise infile will be the metafile.)', dest='metafile')
    parser.add_argument('-n','--m1modelname', metavar='m1modelname', type=str, default='m1model', help='M1 model name is loaded (only applicable if a user model is loaded.)', dest='m1modelname')
    parser.add_argument('-x','--checksyntax', metavar='checksyntax', type=int, default=0, help='validate input files before conversion', dest='checksyntax')
    parser.add_argument('-f','--format', type=str, default='TXT', help='EOQ file format: TXT, JS, PY or JSO')
    ...
    config.fileSerializer = args.format
