...
- **Class `Singleton`**: A metaclass for creating singleton classes. It ensures that only one instance of a class exists.

- **Class `NothingClass`**: A class that represents a singleton "nothing" value, which evaluates to `False`.

- **Type Checking Functions**:
  - `is_callable(obj)`: Checks if an object is callable.
  - `is_collection(obj)`: Checks if an object is a collection (and a sequence) but not a string or bytes.
  - `is_iterable(obj)`: Checks if an object is iterable.

- **Utility Functions**:
  - `unique(iterable, key=None, include=None, exclude=None)`: Yields unique elements from an iterable, optionally using a key function and inclusion/exclusion sets.
  - `trim_module_path(full)`: Trims the module path to exclude standard library directories and returns a tuple indicating if it's standard library code and the trimmed path.
  - `is_internal(x)`: Determines if an object is part of the standard library or built-in modules.
  - `class_of(obj)`: Returns the class of an object.
  - `issubstance(obj, types)`: Checks if an object matches a given type or types, handling complex typing constructs like `Union` and `Generic`.
  - `iter_inheritance(obj, include=None, exclude=None, exclude_self=True, exclude_stdlib=True, reverse=False)`: Iterates over the inheritance hierarchy (MRO) of an object with options to include or exclude certain types or modules.
  - `get_mro(obj, glue=None, **kw)`: Returns the method resolution order (MRO) of an object as a tuple or joined string.
  - `get_entity(obj, name, **kw)`: Retrieves an attribute from an object or its superclasses, returning the attribute and its owner class.
  - `get_owner(obj, name, **kw)`: Retrieves the owner class of an attribute.
  - `get_attr(obj, name, default=None, **kw)`: Retrieves an attribute from an object or its superclasses, with a default value if not found.
  ...
  - `Who(obj, full=True, addr=False)`: Returns the fully qualified name of an object, with options to include the module and memory address.
  - `Is(obj, **kw)`: Returns a string representation of an object, including its type and value.
  - `objectname(obj, full=True)`: Returns the name of an object, handling functions, methods, classes, and modules.
  - `sourcefile(obj, **kw)`: Returns the source file path where an object is defined.
  - `stackoffset(order=None, shift=0)`: Calculates the stack frame offset, ignoring frames from specified files or modules.
  - `stackfilter(x)`: Filters out stack frames based on predefined patterns, used to clean up stack traces.
  - `stacktrace(count=None, join=True)`: Retrieves the current stack trace as a string or list of strings.
  - `about(something)`: Provides detailed information about an object, including its type, source file, and MRO.
  - `its_imported_module_name(name)`: Checks if a given module name corresponds to an imported module.
  - `get_module_from_path(path)`: Attempts to find the module name corresponding to a given file path.
...
The module relies on standard libraries such as `inspect`, `functools`, `typing`, `pathlib`, and `sys`, as well as some utilities from the `kalib` package. It is designed to support advanced introspection tasks, making it useful for debugging, serialization, type checking, and dynamic code analysis.
"""
...
def get_mro(obj, /, glue=None, **kw):
    mro = map(Who, iter_inheritance(obj, **kw))
    return glue.join(mro) if glue else tuple(mro)
