...
- **Exceptions**:
  - `ConfigurationError`: Base exception class for configuration-related errors.
  - `ConfigurationEvaluationError`: Raised when there is an error evaluating a configuration.
  - `ConfigurationSchemeError`: Raised when there is an issue with the configuration schema.
  - `ConfigurationSchemeMissingError`: Raised when a required configuration scheme is missing.
  - `ConfigurationTypeError`: Raised when a configuration has an invalid type.

- **Data Classes**:
  - `FieldDescriptions`: A data class that holds metadata about fields, including name, type, default value,
    factory function, type name, default class, and whether it is required.
  - `dataclass`: A custom data class decorator that extends the standard `dataclass` with additional
    functionality, such as:
    - Custom initialization to handle extra parameters and maintain memoization.
    - Policy handling for including or excluding extra items not defined in the schema.
    - Methods for loading data from various sources (e.g., files, JSON strings, dictionaries).
    - Validation of field types and default values against the defined schema.
    - Serialization methods (`as_dict`, `as_json`, `as_sql`) for converting instances to different formats.
    - Overloaded operators (`&`, `|`, `^`, `-`, `+`) for combining and comparing data class instances.
...
- **Data Class Variants**:
  ...
  - `DataClassFlexible`: A variant that allows extra fields to be excluded based on a policy.

- **Automatic Class Generation**:
  - `BaseAutoClass`: A base class facilitating automatic class generation with field definitions.
    - Manages automatic field detection and inheritance for subclasses.
  - `autoclass`: A function that generates classes with automatic configuration loading and validation based
    on provided field names.
    ...
    - Includes methods like `from_dict`, `from_tuple`, `from_module`, and `from_namespace` for constructing
      instances from various sources.

- **Utility Functions**:
  - `capitalize_snakes(something)`: Capitalizes words in a snake_case string.
  - `make_getters(field)`: Generates getter methods for a given field to be used in an auto-generated class.
  - `simple(classname, *names, **kw)`: Creates a simple data class with specified field names and optional
    base classes (MRO).
  - `from_dict(*args, **kw)`: Creates a data class instance from a dictionary, handling various argument
    patterns and optional MRO.
  - `from_tuple(*fields, **kw)`: Returns a function that constructs a data class instance from a tuple of
    values corresponding to the specified fields.
  - `from_module(module, /, order=None, name=None)`: Creates a data class from the exported variables of a
    module, optionally specifying the order of fields and a custom class name.
  - `from_namespace(namespace, /, callback=capitalize_snakes, name=None)`: Creates a data class from a
    namespace object (e.g., from `argparse`), optionally transforming field names with a callback function.

**Aliases and Class Attributes**:
- `autoclass.Base`: Alias for `BaseAutoClass`.
- `autoclass.Config`: An auto-generated class for handling configuration fields.
- `dataclass.auto`: Alias for `autoclass`.
- `dataclass.dict`: Alias for `from_dict`.
- `dataclass.module`: Alias for `from_module`.
- `dataclass.space`: Alias for `from_namespace`.
- `dataclass.tuple`: Alias for `from_tuple`.
- `dataclass.Simple`: Alias for `simple`.
- `dataclass.Mutable`: Alias for `DataClassMutable`.
- `dataclass.Flexible`: Alias for `DataClassFlexible`.
- `dataclass.Exception`: Alias for `ConfigurationError`.

**Usage and Purpose**:
- The module is designed to provide a flexible and powerful way to work with configurations and data classes,
  especially in scenarios where configurations need to be loaded, validated, and converted between different
  formats.
- It allows for automatic loading and validation of configurations from various sources, enforcing type
  correctness and required fields as defined in the schema.
- The custom `dataclass` class extends functionality by integrating with a logging system (`Logging.Mixin`),
  enabling informative messages and error reporting.
- Overloaded operators and serialization methods facilitate easy manipulation and comparison of data class
  instances, supporting operations like merging configurations or extracting differences.
- The auto class generation utilities enable dynamic creation of classes based on configuration schemas,
  which is useful for applications that need to handle different configurations at runtime or generate classes
...
- It integrates with standard Python modules like `dataclasses`, `enum`, `typing`, and `datetime` to provide
  enhanced functionality while maintaining compatibility with standard data class features.

**Notes**:
- Care should be taken to handle exceptions properly, especially when loading configurations, to ensure that
  errors are reported and managed appropriately.
- The module is designed to be extensible and adaptable to various use cases involving data classes and
  configurations, making it a valuable tool for developers working with complex data structures and dynamic
  class generation.
"""
...
class dataclass(Logging.Mixin):  # noqa: N801
    ...
    def _process_extra_items(cls, data):
        ...
        if data:
            ...
            if policy & cls.Policy.Exclude:
                ...
                msg = f'{Who(cls)}[{policy=}]: extra {data=} passed'
                raise ConfigurationEvaluationError(msg)
    ...
    def load(cls, data, *args, **kw):
        ...
        try:
            if validate and not self.is_valid:
                data = json.repr(data)
                self.log.warning(f'{Who(cls)}: invalid {data=}')

        except ConfigurationError:
            data = json.repr(data)
            self.log.error(f'{Who(cls)}: invalid data={data}')  # noqa: TRY400
            raise
    ...
    def __sub__(self, other):
        ...
        return from_dict(
            {k: v for k, v in self.export.items() if k not in other}, name=Who(other, full=False))
...
autoclass.Base          = BaseAutoClass
autoclass.Config        = autoclass('config')

autoclass.auto   = autoclass
autoclass.dict   = from_dict
autoclass.module = from_module
autoclass.space  = from_namespace
autoclass.tuple  = from_tuple

autoclass.Simple        = simple
autoclass.FromDict      = from_dict
autoclass.FromModule    = from_module
autoclass.FromNamespace = from_namespace
autoclass.Exception     = ConfigurationError
...
dataclass.FromDict      = from_dict
dataclass.FromModule    = from_module
dataclass.FromNamespace = from_namespace

dataclass.Mutable       = DataClassMutable
dataclass.Flexible      = DataClassFlexible
