...
class WebullTA:
    ...
    async def get_candle_streak(self, ticker, headers=None):
        """Returns the streak of green or red candles and its length along with the trend for multiple intervals, with timespans mapped to readable format"""
        
        async def calculate_streak(ticker, interval, data):
            """Helper function to calculate the streak for a given dataset"""
            # Verify that data is not empty
            if data is None or len(data) == 0:
                print(f"Not enough data for {ticker} on interval {interval}")
                return {f"streak_{conversion[interval]}": 0, "trend": "no trend"}

            # Reverse the dataframe to have the oldest candle first for easy scanning
            ...
            # Start from the most recent candle and scan backward
            for i in range(1, len(data)):
                ...
                if current_close > current_open:  # Green candle
                    if streak_type is None:
                        streak_type = 'up'  # Trend is up for green candles
                    elif streak_type == 'down':  # Break if we encounter a red candle after a green streak
                        break
                    streak_length += 1
                elif current_close < current_open:  # Red candle
                    if streak_type is None:
                        streak_type = 'down'  # Trend is down for red candles
                    elif streak_type == 'up':  # Break if we encounter a green candle after a red streak
                        break
                    streak_length += 1
                else:
                    break  # No change in the candle (neutral), stop counting

            # If no streak was detected (no green or red candle), return no streak
            if streak_type is None:
                return {f"streak_{conversion[interval]}": 0, "trend": "no trend"}

            # Return the current streak type (up or down) and its duration
            return {f"streak_{conversion[interval]}": streak_length, "trend": streak_type}
        ...
        try:
            ...
            data_list = await asyncio.gather(*[trading.async_get_td9(ticker=ticker, interval=interval, headers=headers, count=200) for interval in intervals])

            # Process each interval's data
            result = {}
            for interval, data in zip(intervals, data_list):
                streak_data = await calculate_streak(ticker, interval, data)
                result.update(streak_data)  # Add streak_{interval} and trend to the result
