...
class CasetextSDK:
    ...
    async def bulk_search(self, query, type:str='case', state:str='tx'):
        url = f"https://casetext.com/api/search-api/search?count=50&start=0&types=case&aggregateJxTypes=all&countTypes=all&sort=relevance&q={query}&jxs={state}sct,{state}app"
        print(url)
        async with httpx.AsyncClient() as client:
