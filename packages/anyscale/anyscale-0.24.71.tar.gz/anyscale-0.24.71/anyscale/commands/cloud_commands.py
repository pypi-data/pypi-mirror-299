...
@click.option(
    ...
    help="On the Kubernetes compute stack, a comma-separated list of namespace(s) that can be used for running workloads. The first namespace in the list will be used as the default.",
    required=False,
    type=str,
)
@click.option(
    "--kubernetes-ingress-external-address",
    help="On the Kubernetes compute stack, the address of the ingress used for serving traffic for Anyscale services.",
    required=False,
    type=str,
)
@click.option(
...
@click.option(
    ...
    help="On the Kubernetes compute stack, the IAM identity for the Kubernetes Manager deployment. IN AWS EKS, this is the ARN of the IAM role associated with the Kubernetes Manager deployment. For GCP GKE, this is the service account email associated with the Kubernetes Manager deployment.",
    required=False,
    type=str,
)
@click.option(
...
def register_cloud(  # noqa: PLR0913, PLR0912
    ...
    kubernetes_namespaces: Optional[str],
    kubernetes_ingress_external_address: Optional[str],
    kubernetes_zones: Optional[str],
    kubernetes_dataplane_identity: Optional[str],
    functional_verify: Optional[str],
    ...
    if provider == "aws":
        ...
        for resource in [
            ...
            (s3_bucket_id, "--s3-bucket-id", (ComputeStack.VM, ComputeStack.K8S)),
            (kubernetes_namespaces, "--kubernetes-namespaces", (ComputeStack.K8S)),
            (
                kubernetes_ingress_external_address,
                "--kubernetes-ingress-external-address",
                (ComputeStack.K8S),
            ),
            (kubernetes_zones, "--kubernetes-zones", (ComputeStack.K8S)),
            (
                kubernetes_dataplane_identity,
                "--kubernetes-dataplane-identity",
                (ComputeStack.K8S),
            ),
        ]:
        ...
        CloudController().register_aws_cloud(
            ...
            functional_verify=functional_verify,
            kubernetes_namespaces=kubernetes_namespaces.split(",")
        ...
        for resource in [
            (project_id, "--project-id", (ComputeStack.VM, ComputeStack.K8S)),
            (vpc_name, "--vpc-name", (ComputeStack.VM)),
            (subnet_names, "--subnet-names", (ComputeStack.VM)),
            (filestore_instance_id, "--filestore-instance-id", (ComputeStack.VM)),
            ...
            (provider_name, "--provider-name", (ComputeStack.VM, ComputeStack.K8S)),
        ...
        # For the 'generic' provider type, for the time being, most fields are optional; only 'name', 'provider', 'compute-stack', and 'kubernetes-namespaces' are required.
        if not name:
        ...
        if not kubernetes_namespaces:
            raise click.ClickException(
                "Please provide at least one namespace to `--kubernetes-namespaces`."
            )
