    ...
    with patch(
        ...
        if not correct_memorystore_name:
            ...
            mock_register_cloud.assert_called_once_with(
                ...
                kubernetes_ingress_external_address=None,
                kubernetes_zones=[],
                kubernetes_dataplane_identity=None,
                functional_verify=None,
...
@pytest.mark.parametrize("kubernetes_namespaces", [None, "namespace1"])
@pytest.mark.parametrize("auto_add_user", [True, False])
def test_cloud_register_generic(kubernetes_namespaces, auto_add_user):
    runner = CliRunner()
    mock_cloud_controller = Mock()
    mock_register_generic_cloud = Mock()
    mock_cloud_controller().register_generic_cloud = mock_register_generic_cloud

    provider = "generic"
    region = "us-west1"
    name = "unit-test-generic-cloud"
    ...
    if kubernetes_namespaces:
        args.extend(["--kubernetes-namespaces", kubernetes_namespaces])

    if auto_add_user:
        args.append("--enable-auto-add-user")
    ...
    with patch(
        ...
        if not kubernetes_namespaces:
            assert "Please provide at least one namespace" in result.output
            mock_register_generic_cloud.assert_not_called()
        else:
            mock_register_generic_cloud.assert_called_once_with(
                name=name,
                auto_add_user=auto_add_user,
                region=region,
                cloud_storage_bucket_name=None,
                cloud_storage_bucket_endpoint=None,
                nfs_mount_targets=[],
                nfs_mount_path=None,
