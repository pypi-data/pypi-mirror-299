...
async def check_utilization(gateway_host: str):
    if gateway_host == 'localhost':
        ...
        command = f"ssh {gateway_host} 'top -bn1 | grep \"Cpu(s)\" && nvidia-smi --query-gpu=utilization.gpu,memory.total,memory.used --format=csv,noheader,nounits && free -m | grep Mem'"
    ...
    try:
        ...
        if process.returncode != 0:
            logging.error(f"Error executing check_utilization command: {stderr.decode()}")
            return None
        ...
        logging.error(f"Error checking utilization: {str(e)}")
        return None
...
async def get_gateway_specs(gateway_host: str):
    ...
    try:
        ...
        if process.returncode != 0:
            logging.error(
                f"Error executing command for gateway specs: {stderr.decode()}")
            return None
...
async def load_test(app_id: str, token: str, gateway_id: str, gateway_host: str, pipeline_ids: List[str], stream_ids: List[str],
    ...
    try:
        for pipeline_id in pipeline_ids:
            ...
            while not threshold_exceeded:
                ...
                for _ in range(deployment_buffer):
                    if terminate_event.is_set():
                        break
                    utilization = await check_utilization(gateway_host)
                    if utilization:
        ...
        if cleanup_task is None:
            await cleanup_and_save(api_client, gateway_id, [], results, deployment_prefix, gateway_name, cleanup)
        else:
            await cleanup_task
