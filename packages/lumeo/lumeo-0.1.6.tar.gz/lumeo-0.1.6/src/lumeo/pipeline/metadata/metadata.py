...
def object_has_attributes(obj, attribute_labels) -> bool:
    ...
    Returns:
        bool: True if the object has the specified attributes, False otherwise.
    """
    if 'attributes' in obj:
        # wildcard matching, obj should have at least one attribute
        if '*' in attribute_labels and len(obj['attributes']) > 0:
            return True

        attribute_labels_set = set(attribute_labels)
        obj_attributes_set = {attribute['label'].lower() for attribute in obj['attributes'] if 'label' in attribute}
        
        if attribute_labels_set.issubset(obj_attributes_set):
            return True
    return False
...
def object_matches_type(obj, object_type) -> bool:
    ...
    """
    if 'label' in obj:
        obj_label_match = (object_type[0] == obj['label'].lower())

        if len(object_type) > 1:
            obj_attrib_match = object_has_attributes(obj, object_type[1:])
        else:
            obj_attrib_match = True
        return obj_label_match and obj_attrib_match
    return False
