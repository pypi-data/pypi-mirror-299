...
import arcgis.map._dataclasses._webmap_spec as ws
    ...
    def _on_extent_change(self, change):
        ...
        if self._webmap.initial_state:
            if self._webmap.initial_state.viewpoint.target_geometry != change["new"]:
                new_extent = ws.Extent(**change["new"])
                self._webmap.initial_state.viewpoint.target_geometry = new_extent
        else:
            self._webmap.initial_state = ws.InitialState(
                viewpoint=ws.Viewpoint(target_geometry=ws.Extent(**change["new"]))
            )
    ...
    **Parameter**        **Description**
    ...
    def _update_operational_layers(self, change):
        ...
        if self._raster_to_media_layer:
            try:
                media_layer = ws.MediaLayerMedia(**self._raster_to_media_layer)
    ...
    def _basemap_changed(self, change):
        ...
        if "baseMapLayers" in change["new"]:
            self._webmap.base_map = ws.BaseMap(**change["new"])
    ...
    def _setup_webmap_properties(self, item):
        ...
        if item:
            ...
            if "spatialReference" not in data:
                data["spatialReference"] = ws.SpatialReference(
            ...
            spatial_reference = ws.SpatialReference(latestWkid=3857, wkid=102100)
            ...
            if self._gis.properties["defaultBasemap"]:
                ...
                basemap = ws.BaseMap(**self._gis.properties["defaultBasemap"])
            else:
    ...
    def zoom_to_layer(
        ...
        **Parameter**           **Description**
        ...
        ==================     ====================================================================
        """
        self._helper.zoom_to_layer(item)
    ...
    def sync_navigation(self, map: Map | list[Map]) -> None:
        ...
        **Parameter**           **Description**
        ...
        """
        self._helper.sync_navigation(map)
    ...
    def unsync_navigation(self, map: Optional[Map | list[Map]] = None) -> None:
        ...
        **Parameter**           **Description**
        ...
        ==================     ===================================================================
        """
        self._helper.unsync_navigation(map)
    ...
    def export_to_html(
        ...
        **Parameter**           **Description**
        ...
        ==================     ====================================================================
        """
        return self._helper.export_to_html(path_to_file, title)
    ...
    def enabled(self, value):
        if isinstance(value, bool):
            if value != self._source._show_bookmarks:
                ...
                # if value is True:
                #     # update so the latest bookmarks are shown in widget
                #     self.map._webmap_dict = self.map._webmap.dict()
...
class Bookmark:
    ...
    def __init__(self, bookmark: ws.Bookmark, bm_mngr: Bookmarks) -> None:
        ...
        **Parameter**                **Definition**
        ...
        if extent is not None:
            ...
            if self._bookmark.viewpoint is None:
                ...
                self._bookmark.viewpoint = ws.Viewpoint(targetGeometry=new_extent)
            else:
        ...
        if rotation is not None:
            ...
            if self._bookmark.viewpoint is None:
                # If viewpoint is None, create one
                self._bookmark.viewpoint = ws.Viewpoint(
        ...
        if time_extent is not None:
            ...
            new_time_extent = ws.TimeExtent(**time_extent)
            # Set new time extent
...
class BasemapManager:
    ...
    def basemap(self, basemap: str | _gis_mod.Item | dict):
        ...
        # Set in definition, check if scene or map
        self._pydantic_class.base_map = ws.BaseMap(**basemap_dict)
    ...
    def move_from_basemap(self, index: int) -> bool:
        ...
        **Parameter**                **Definition**
        ...
        .. code-block:: python
            ...
            wm.update()
        """
        self._helper.move_from_basemap(index)
        # Refresh widget
    ...
    def title(self, title: str):
        self._helper.basemap_title(title)
    ...
    def remove_basemap_layer(self, index: int) -> bool:
        ...
        **Parameter**                **Definition**
        ...
        """
        self._helper.remove_basemap_layer(index)
        self._source._update_source()
        return True
...
class MapContent:
    ...
    def __init__(self, webmap: Map) -> None:
        ...
        # list to be added/removed to when performing operations
        self.layers = self._helper._layers

        # Initialize tables list
        # list to be added/removed to when performing operations
        self.tables = self._helper._tables
    ...
    def add(
        ...
        **Parameter**           **Description**
        ...
        # Section: Item or Layer Item
            ...
            if isinstance(layer, ws.GroupLayer):
                ...
                self._source._update_source()
                return
            elif isinstance(layer, ws.Table):
        ...
        if isinstance(layer, ws.Table):
            ...
            self._source._webmap.operational_layers.insert(index, layer)
            # Refresh the widget to have the latest webmap dict if update made
            # Not outside of if/else because we don't want to update if not needed (JS side udpated)
            self._source._update_source()
    ...
    def move_to_basemap(self, index: int) -> bool:
        ...
        **Parameter**                **Definition**
        ...
        .. code-block:: python
            ...
            wm.update()
        """
        self._helper.move_to_basemap(index)
        # Refresh the widget
        # We don't refresh only the `_basemap` property since changes were also made to operational layers
        self._source._update_source()
        return True
    ...
    def popup(self, index: int, is_table: bool = False) -> popups.PopupManager:
        ...
        **Parameter**           **Description**
        ...
        """
        # Check if group layer
        if not is_table and self._source._webmap.operational_layers[
            index
        ].layer_type.lower() in [
            "group",
            "group layer",
