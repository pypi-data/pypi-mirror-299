...
import arcgis.map._dataclasses._webscene_spec as ws
    ...
    **Parameter**        **Description**
    ...
    def _setup_webscene_properties(self, item):
        ...
        if item:
            ...
            self._webscene = ws.Webscene(**data)
        else:
            # New Scene
            self.item = None
            ...
            initial_state = ws.InitialState(
                environment=environment, viewpoint=ws.Viewpoint(camera=camera)
            )
            # New Webscene from pydantic dataclass from webscene spec generation
            self._webscene = ws.Webscene(
                operationalLayers=[],
                baseMap=basemap,
                ground=ground,
                viewingMode=ws.ViewingMode.global_,
    ...
    def _basemap_changed(self, change):
        ...
        if "baseMapLayers" in change["new"]:
            self._webscene.base_map = ws.BaseMap(**change["new"])
    ...
    def viewing_mode(self, mode):
        ...
        if mode != self.viewing_mode:
            if mode == "local":
                self._webscene.viewing_mode = ws.ViewingMode.local
            else:
                self._webscene.viewing_mode = ws.ViewingMode.global_
            self._update_source()
    ...
    def sync_navigation(self, scene: Scene | list[Scene]) -> None:
        ...
        **Parameter**           **Description**
        ...
        """
        self._helper.sync_navigation(scene)
    ...
    def unsync_navigation(self, scene: Optional[Scene | list[Scene]] = None) -> None:
        ...
        **Parameter**           **Description**
        ...
        ==================     ===================================================================
        """
        self._helper.unsync_navigation(scene)
    ...
    def popup(self, index: int) -> popups.PopupManager:
        ...
        **Parameter**           **Description**
        ...
        return popups.PopupManager(
            layer=self._source._webscene.operational_layers[index], source=self, spec=ws
        )
