...
class GroupLayer:
    ...
    def __init__(
        ...
        self._layer: wm_spec.Layer = layer
        ...
        if isinstance(map, arcgismapping.Map):
            self._spec: wm_spec = wm_spec
            self._is_map: bool = True
            self._definition: wm_spec.Webmap = map._webmap
        elif isinstance(map, arcgismapping.Scene):
            self._spec: ws_spec = ws_spec
            self._is_map: bool = False
            self._definition: ws_spec.Webscene = map._webscene
        else:
    ...
    def popup(self, index: int) -> popups.PopupManager:
        ...
        **Parameter**           **Description**
        ...
        ==================      =====================================================================
        """
        is_table = isinstance(self._layers.layers[index], self._spec.Table)
        # Pass in the layer instance from the group layer
        return popups.PopupManager(
            self._layer.layers[index], source=self, spec=self._spec, is_table=is_table
        )
    ...
    def renderer(self, index: int) -> renderers.RendererManager:
        ...
        **Parameter**           **Description**
        ...
        return renderers.RendererManager(
            layer=self._layer.layers[index], source=self, spec=self._spec
        )
    ...
    def update_layer(
        ...
        **Parameter**           **Description**
        ...
        if form is not None:
            form_info = wm_spec.FormInfo(**form.dict())
            # Update the form
