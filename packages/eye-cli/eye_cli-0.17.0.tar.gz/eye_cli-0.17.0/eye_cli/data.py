...
def get_local_captures():
    ...
    return [d for d in Path(bucket).iterdir() if d.is_dir]
...
@app.command()
def capture(
    name: Annotated[Optional[str], typer.Argument()] = None, preview: bool = True
):
    ...
    to_path = (
        f"gs://gecko-chase-photogrammetry-dev/testing/capture/{name}"
        if testing
        else f"gs://gecko-chase-photogrammetry-dev/capture/{name}"
    )
