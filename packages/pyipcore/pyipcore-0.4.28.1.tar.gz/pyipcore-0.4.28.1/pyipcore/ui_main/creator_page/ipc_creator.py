from pyipcore.ui_main.fileitem import *
...
class QIpcPreviewExporter(QWidget):
    ...
    def initUI(self):
        ...
        label.setFixedHeight(QFixWidthLabel.GetHeight(1, font=LBL_FONT))
        ...
        self.export_btn = QPushButton("导出到...")
        self.export_btn.setFont(BTN_FONT)
        self.export_btn.clicked.connect(self.exportPushed)
...
class QIpcMainFilesCollector(QWidget):
    ...
    *使用红色边框修饰必要文件
    ...
    def add_file_slot(self, label_text, formats, required=False):
        ...
        label.setFixedHeight(QFixWidthLabel.GetHeight(2, font=LBL_FONT_MINI))
        label.setFont(LBL_FONT_MINI)
        file_slot = QFileSlot(self, formats=formats)
...
class QIpcSubFilesCollector(QWidget):
    ...
    def initUI(self):
        ...
        self.lbl0.setFixedHeight(QFixWidthLabel.GetHeight(1, font=LBL_FONT))
        ...
        self.lbl1.setFixedHeight(QFixWidthLabel.GetHeight(1, font=LBL_FONT))
...
class QIpCoreCreator(QWidget):
    ...
    """
    compileIpcEvent = pyqtSignal(
        str, str, str, str, str, str,       # name, author, brand, model, board, group
        str, str, str, str,                     # fmain, ficon, freadme, fmanual
        list, list                              #  subfiles, otherpaths
    )
    def __init__(self, parent=None):
        super().__init__(parent)
        self.initUI()
    ...
    def initUI(self):
        ...
        self.info_collector.setFixedWidth(300)
        layout.addWidget(self.info_collector, 0, 0)

        # 创建QIpcMainFilesCollector对象
        self.main_files_collector = QIpcMainFilesCollector()
        self.main_files_collector.setFixedWidth(150)
        ...
        self.preview_exporter.exportPushed.connect(self.export)
    ...
    def export(self):
        # 检查必填项name和fmain
        if not self.info_collector.name:
            PopError("缺失:", "IP核名称不能为空")
            return
        if not self.main_files_collector.fmain:
            PopError("缺失:", "主代码文件不能为空")
            return
        self.compileIpcEvent.emit(
            self.name, self.author, self.brand, self.model, self.board, self.group,
            self.fmain, self.ficon, self.freadme, self.fmanual,
            self.subfiles, self.otherpaths
        )
    ...
    @property
    def subfiles(self):
        return self.sub_files_collector.subfiles
