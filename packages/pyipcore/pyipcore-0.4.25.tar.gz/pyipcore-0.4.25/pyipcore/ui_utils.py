...
from PyQt5.QtWidgets import QScrollArea, QGridLayout, QHBoxLayout, QFormLayout, QLineEdit, QFileDialog
...
DODGERBLUE = QColor(32, 144, 245, 225)
N_DODGERBLUE = QColor(255 - 32, 255 - 144, 255 - 245, 225)
DARK_N_DODGERBLUE = QColor(int(5/8*(255 - 32)), int(5/8*(255 - 144)), int(5/8*(255 - 245)), 225)
DARK_DODGERBLUE = QColor(20, 90, 155, 225)
JLCYELLOW = QColor(255, 255, 0, 50)
LT_JLCYELLOW = QColor(255, 255, 0, 25)
SGGREEN = QColor(30, 124, 90, 175)
OLVORANGE = QColor(238, 121, 66, 175)
LT_BLACK = QColor(0, 0, 0, 50)
LT_RED = QColor(225, 25, 25, 50)
...
def set_ipcdir_icon(fdir_path):
    ...
    if not is_executable_in_path('FolderIco'):
        raise ThirdToolFolderIcoNotFound("Please retry after install 'Teorex FolderIco' (url refer:https://www.folderico.com/download.html) and add it to path.")
    if " " in fdir_path:
        raise FolderIcoPathContainSpace("<SPACE> in path may cause FolderIco execute failed.")
    # os.system(f'FolderIco --folder "{fdir_path}" --icon"{FP_ICON_IPCDIR}"')

    # ['FolderIco', f'--folder {fdir_path}', f'--icon {FP_ICON_IPCDIR}']
    result = os.system(f'FolderIco --folder {fdir_path} --icon {FP_ICON_IPCDIR}')
    if result != 0:
