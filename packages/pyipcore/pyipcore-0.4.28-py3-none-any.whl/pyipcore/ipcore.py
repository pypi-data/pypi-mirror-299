...
class IpCore:
    ...
    def Compile(cls,
        ...
        subfiles = [os.path.abspath(f) for f in subfiles]
        ...
        fnames_subfiles = [os.path.basename(f) for f in subfiles]
        ...
        # create files_bytes
        _total_length = 0
        bsmain = files.f3pack(fmain)  # 考虑到subs和others的文件可能很多，所以这里统一用f3pack打包一下
        _length = len(bsmain)
        _total_length += _length
        rec['main'].append(f"f3pack target: {fmain} -> bytes<len={_length}>")
        bsicon = files.f3pack(ficon) if ficon else None
        if ficon:
            _length = len(bsicon)
            _total_length += _length
            rec['main'].append(f"f3pack target: {ficon} -> bytes<len={_length}>")
        else:
            rec['main'].append("No icon file. Skip.")
        bsreadme = files.f3pack(freadme) if freadme else None
        if freadme:
            _length = len(bsreadme)
            _total_length += _length
            rec['main'].append(f"f3pack target: {freadme} -> bytes<len={_length}>")
        else:
            rec['main'].append("No readme file. Skip.")
        bsmanual = files.f3pack(fmanual) if fmanual else None
        if fmanual:
            _length = len(bsmanual)
            _total_length += _length
            rec['main'].append(f"f3pack target: {fmanual} -> bytes<len={_length}>")
        else:
            rec['main'].append("No manual file. Skip.")
        bssubfiles = [files.f3pack(f) for f in subfiles]
        _length = sum([len(b) for b in bssubfiles])
        _total_length += _length
        rec['main'].append(f"f3pack subfiles: {len(bssubfiles)} files. Total bytes: {_length} bytes")
        bsotherpaths = [files.f3pack(f) for f in otherpaths]
        _length = sum([len(b) for b in bsotherpaths])
        _total_length += _length
        rec['main'].append(f"f3pack otherpaths: {len(bsotherpaths)} files. Total bytes: {_length} bytes")
    ...
    def decode_file(self, ipc_fname: str, *, to_str=False) -> str | bytes:
        ...
        bs: bytes = files.f3loads(self._ipc['files'][ipc_fname])
        if to_str:
    ...
    def __get_f(self, skey, *, unzip=True, to_str=False):
        ...
        """
        _zip_bs = files.f3loads(self.f[self.key, skey])
    ...
    def buildsubs(self, target_dir, overwrite=False):
        ...
        for i, _f3bs in enumerate(_f3bs_subs):
            zipf = files.f3unpack(_f3bs)  # zipfile
            _paths = []  # 解压后的文件路径
    ...
    def buildothers(self, target_dir, overwrite=False):
        ...
        for i, _f3bs in enumerate(_f3bs_others):
            try:
                files.f3unpack(_f3bs, target_dir, overwrite=overwrite, error=True)
...
if __name__ == '__main__':
    ...
    #     f.write(data)
    # print(files.f3loads(open('test.inst', 'rb').read()))
