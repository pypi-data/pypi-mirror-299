...
def volkanoban_classifier(X, y, test_size=0.3, random_state=42):
    ...
    # Return relevant information including X
...
def lime_analysis(model, X_train, X_test, y_test, feature_names):
    """
    Uses LIME to explain model predictions locally.
    
    Parameters:
    - model: Trained stacking model
    - X_train: Training features as a pandas DataFrame or NumPy array
    - X_test: Test features as a pandas DataFrame or NumPy array
    - y_test: True labels for the test set
    - feature_names: List of feature names from the original dataset
    """
    # Create a LIME explainer for tabular data
    explainer = LimeTabularExplainer(
        training_data=X_train,  # Use training data as is
        mode="classification",
        feature_names=feature_names,
        class_names=[str(i) for i in np.unique(y_test)],  # Class names based on y_test
        discretize_continuous=True
    )

    # Fixed instance to explain (first instance in this case)
    i = 0  # You can change this index to explain any other instance

    # Explain the prediction for the specified instance
    exp = explainer.explain_instance(
        X_test[i],  # Directly use the NumPy array for the instance
