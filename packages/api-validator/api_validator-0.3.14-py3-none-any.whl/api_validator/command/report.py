...
from api_validator.tools.newman_csv_utils import NewmanCsvData
...
@click.option("--app-name", "-a", "app_name", help="App name in the config.", required=True)
def report(data_directory: str, config_file: str, output_file: str, csv_file: str, app_name: str):
    run_single_report(data_directory, config_file, output_file, csv_file, app_name)
...
def run_single_report(data_directory: str, config_file: str, output_file: str, combined_csv_file: str, app_name: str):
    ...
    newman_report = NewmanReport(app_manager, app_details=app_details, report_name=f"{app_name} Traffic Validation Report")
