...
def send_reading_summary(api_inputs: ApiInputs, df: pd.DataFrame):
    ...
    df['TimestampLocal'] = pd.to_datetime(df['TimestampLocal']).dt.date # change to day date
    df = df.drop_duplicates(subset=['ObjectPropertyId', 'TimestampLocal']) # remove duplicates
    ...
    if len(groups_with_one_member) > 0:
        ...
        logger.info(f"Reading Summary request sent for CustomSensor Type for {len(custom_sensor_summary_payload['customsensor'])} sensor/s.")
    ...
    if len(groups_with_multiple_members) > 0:
        ...
        for row in groups_with_multiple_members.iterrows():
            ...
            sensor_summary_payload = {"optype": "sensor", "apiprojectid": "", "data": [], "localfrom": "", "localto": ""}
            ...
            response = requests_retry_session(method_whitelist=['POST']).post(
                url=url, json=custom_sensor_summary_payload, headers=headers)
            response_status = '{} {}'.format(response.status_code, response.reason)
