...
from lib.utils import CORPUS_SUFFIX
    ...
    def launch(self):
        ...
        while i < len(self.options.docker_command):
            ...
            # get config if present to validate it against schema
                ...
                if "sub_configs" in config:
                    lps = list(config["sub_configs"].keys())
                if src_lang == 'xx' or tgt_lang == 'yy':
                    if "default_lp" in config:
                        lp = config["default_lp"]
                        if lp in lps:
                            lps.remove(lp)
                    elif lps:
                        lp = lps.pop()
                        ...
                        raise RuntimeError(
                            'Cannot guess the lp, if this is a multilingual model please add "default_lp" in the config'
                        )
                ...
                if tok == '-t':
                    ...
                    for filename in input_files:
                        ...
                        elif not filename.endswith(CORPUS_SUFFIX):
                ...
                new_params.extend(["-lps"] + [f'{src_lang}_{tgt_lang}']*len(input_files))
        ...
        if self.options.no_test_trans:
            ...
            trans_files, ref_files = self.get_testfiles_and_reffiles(src_lang, tgt_lang)
            self.options.totranslate = trans_files
            self.options.toscore = ref_files
            self.options.lps = [f'{src_lang}_{tgt_lang}']*len(trans_files)
            for lp in lps:
                extra_src_lang = lp[:2]
                extra_tgt_lang = lp[-2:]
                extra_trans_files, extra_ref_files = self.get_testfiles_and_reffiles(extra_src_lang, extra_tgt_lang)
                self.options.totranslate.extend(extra_trans_files)
                self.options.toscore.extend(extra_ref_files)
                self.options.lps.extend([f'{extra_src_lang}_{extra_tgt_lang}']*len(extra_trans_files))


            launcher.LOGGER.info('found %d test references' % len(self.options.toscore))
        return res

    def get_testfiles_and_reffiles(self, src_lang, tgt_lang):
        if src_lang < tgt_lang:
            test_dir = src_lang + "_" + tgt_lang
        else:
            test_dir = tgt_lang + "_" + src_lang
        trans_files = utils._get_testfiles(self.options.url, self.auth,
                                                        self.options.service, test_dir,
                                                        "<MODEL>", src_lang, tgt_lang)
        ref_files = utils._get_reffiles(self.options.url, self.auth,
                                                   self.options.service, src_lang,
                                                   tgt_lang, trans_files)
        return trans_files, ref_files
