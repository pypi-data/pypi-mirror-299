...
def update_report_with_uniqueness_check_result(
    ...
    print(f"Found {num_duplication_errors_total} rows with duplicate values")
...
def update_report_with_invalid_row_removal_result(
    ...
    print(f"Dropped {num_rows_removed} invalid rows")
    with open(report_path, "r") as f:
        report = json.load(f)
    report["report"]["dropInvalidRows"] = {
        "numInvalidRowsDropped": num_rows_removed
    }
