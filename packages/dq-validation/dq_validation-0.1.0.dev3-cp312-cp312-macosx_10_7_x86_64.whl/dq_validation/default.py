...
def run(
    ...
    if did_pre_validation_pass and not should_check_uniqueness and not should_write_parquet:
    ...
    print(f"Reading of file took {(after - before).total_seconds() / 60} min")
    string_columns = [
        jx for jx, col in enumerate(config.root.config.columns)
        if col.formatType == FormatType.STRING
    ]
    print(f"Filling in NA values in {len(string_columns)} string columns")
    ...
    # # In case the initial row-level validation failed (e.g. because some rows
    # # had malformed values), do not proceed to check for duplicates and simply return
    # # early.
    # did_validation_fail = get_report_outcome(report_path) == ValidationOutcome.FAILED
    # if did_validation_fail and not drop_invalid_rows:
    #     sys.exit(0)
