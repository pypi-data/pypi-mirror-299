        ...
        for voter in self.active_voters.values():
            for voter_membership in voter.values():
                if voter_membership.id not in keep_memebrship_ids:
                    ...
                    logger.debug(
                        f"membership {voter_membership.id} of person with id {voter_membership.person.id} has to be end, role: {voter_membership.role}"
                    )
                    memberships_to_end.append(voter_membership)
                    if (
                        self.count_active_voter_membership(voter_membership.person.id)
                        > 1
                    ):
