...
async def search(
    ...
    query = select(SearchIndex).where(
        # to_tsvector
        func.to_tsvector("english", SearchIndex.title + " " + SearchIndex.content_type).op(
            "@@"
        )(func.plainto_tsquery("english", search_query))
    )
    # query = select(SearchIndex).where(
    #     func.to_tsvector("english",
    #         func.concat(SearchIndex.title, " ", func.cast(SearchIndex.content_id, sqlalchemy.String))
