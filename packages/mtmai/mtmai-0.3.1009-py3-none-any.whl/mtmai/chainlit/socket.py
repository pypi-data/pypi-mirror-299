...
from mtmai.auth import get_current_user, require_login
...
from mtmai.chainlit.context import init_ws_context
...
from mtmai.chainlit.message import ErrorMessage, Message
from mtmai.chainlit.server import sio
...
from mtmai.crud.curd_site import list_sites
from mtmai.models.listview import ListItemResponse, ListviewItem
...
async def connection_successful(sid):
    ...
    logger.info(f"connection_successful: {sid}")
...
#  新功能开始 #############################################################################################
@sio.on("init_ui")
async def init_ui(sid, ui_state: Dict[str, Any]):
    """前端用户首次打开界面时"""
    context = init_ws_context(sid)
    user_id = context.session.user.id
    logger.info("init_ui sid: %s, uiSate: %s", sid, ui_state)
    module_name = ui_state.get("module")
    if module_name == "site":
        logger.info("用户当前打开的模块 %s", module_name)
        async with get_async_session() as session:
            count, sites = await list_sites(
                session=session, q="", user_id=user_id, skip=0, limit=100
            )
        ...
        response = ListItemResponse(count=count, items=list_items)
        await context.emitter.send_list_item(response)
