...
def ij(tag='',mode='',c=cb.ij,end='\n'):
    global ij_global
    if str(tag) not in ij_global.keys():
        ...
        if 'n' in mode:ij_global[tag].round = 1
...
def ji(tag='',mode='',c=cb.ji,end='\n'):
    global ji_global
    if str(tag) not in ji_global.keys():
        if mode=='full':mode='iltaprv'
        ...
        if 'n' in mode:ji_global[tag].round = 1
...
def jk(tag='',mode='',c=cb.jk,end='\n'):
    global jk_global
    if str(tag) not in jk_global.keys():
        if mode=='full':mode='iltaprv'
        ...
        if 'n' in mode:jk_global[tag].round = 1
