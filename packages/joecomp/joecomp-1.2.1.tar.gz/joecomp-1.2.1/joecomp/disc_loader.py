...
class Disc_class:
    ...
    def calculate_2D(self, time=None, ngrain=NGRAIN, nchem=NCHEM, nz=NZ, summed=True):
        ...
        grain_cc   = (2/np.pi) * self.sigma_g[self.it, :] * self.stokes_number_small[self.it, :] / self.rho_solid
        pebbles_cc = (2/np.pi) * self.sigma_g[self.it, :] * self.stokes_number_pebbles[self.it,   :] / self.rho_solid
        drift_cc   = (2/np.pi) * self.sigma_g[self.it, :] * self.stokes_number_drift[self.it, :] / self.rho_solid
        frag_cc    = (2/np.pi) * self.sigma_g[self.it, :] * self.stokes_number_frag[self.it,  :] / self.rho_solid

        len_r = len(self.r)
        grain_sizes         = np.zeros((len_r, ngrain))        # grain sizes at all radii for chemcomp
        size_distr_arr      = np.zeros((len_r, ngrain))        # number distribution at all radii
        surface_density_arr = np.zeros((len_r, ngrain))        # surface density at all radii for each grain family
        molecule_sigma_arr  = np.zeros((len_r, ngrain, nchem)) # surface density for all the molecular species
        ...
        # Global maximum
        max_grain_size = np.max(pebbles_cc)
        # s_min_cc = np.min(pebbles_cc)
        # s_min_cc = np.min(grain_cc)                                                          # Smallest grain size in DALI
        s_min_cc = 5e-7
        print(f'Global maximum: {max_grain_size:.4e}')
        print(f'Global minimum: {s_min_cc:.4e}')
        sizes = np.logspace(np.log10(s_min_cc), np.log10(max_grain_size), ngrain)   # Size distribution to find dlog(s)
        ...
        for i in range( len_r ):
            ...
            local_maximum = np.maximum(pebbles_cc[i], s_min_cc) # max grain size from twopoppy
            print(f'\nLoop #{i}')
            print(f'Global maximum: {max_grain_size:.4e}')
            print(f'Global minimum: {s_min_cc:.4e}')
            ...
            geometric_grains = self.geo_array_to_maximum(s_min_cc, ratio, local_maximum, ngrain)
            grain_sizes[i, :len(geometric_grains)] = geometric_grains

            # power for the size distribution
            q = -3.5
            
            # Calculating normalisation factors
            n0      = self.calc_n0(q, sigma_for_n0, dlogs, grain_sizes[i,:])
            n0_ice  = self.calc_n0(q, sigma_ice_for_n0, dlogs, grain_sizes[i,:])
            
            # Size distributions
            size_distr = n0 * grain_sizes[i,:] **(q)                        # Dust grains
            size_distr_ice = n0_ice[:,None] * grain_sizes[None,i,:] ** q    # Ice grains; vectorized

            # Calculating surface density due to each of the grain sizes
            sigma_size_distr = size_distr * (4/3) * np.pi * self.rho_solid * grain_sizes[i,:]**4 * dlogs
            sigma_ice_distr   = size_distr_ice * (4/3) * np.pi * self.rho_solid * grain_sizes[i,:]**4 * dlogs

            #### Sanity checks ####
            #   Loop for ice grain sizes
            temp = np.zeros((19, ngrain))
            for f, n in enumerate(n0_ice):
                temp[f,:] =  n * grain_sizes[i,:] ** q
            ...
            for x, sizes in enumerate(size_distr_ice):
                what_the_sigma[:,x] = sizes * (4/3) * np.pi * self.rho_solid * grain_sizes[i,:]**4 * dlogs
            ...
            size_distr_arr[i, :] = size_distr               # number of particles per cm^2 for a given size
            surface_density_arr[i, :] = sigma_size_distr    # dust surface density for a given size
            molecule_sigma_arr[i, :, :] = sigma_ice_distr.T    # ice surface densities for a given size

        # Other skibidi garbage to produce the 2D plot
        omegaK = np.sqrt( (self.G * self.M_star) / (self.r*self.AU)**3 )
        ...
        for i in range(ngrain):
            temp[:,i] = (np.pi / 2) * self.rho_solid * grain_sizes[:,i] / self.sigma_g[self.it,:] 

        stokes_distr = ( (np.pi / 2) * self.rho_solid * grain_sizes[:,:].T / self.sigma_g[self.it,:] ).T
        ...
        for i, z in enumerate(self.z.T):
            ...
            for j in range(ngrain):
                ...
                sigma = surface_density_arr[:,j] # what the sigma
                ...
                for k in range(nchem):
                    sigma_ice = molecule_sigma_arr[:, j, k-1]
