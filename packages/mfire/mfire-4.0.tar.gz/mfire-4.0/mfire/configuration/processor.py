...
class Processor(BaseModel):
    ...
    * a data configuration (self.data_config) : config which describes all
    ...
    drafting_datetime: Union[Datetime, str] = Datetime.now()
    ...
    def init_drafting_datetime(cls, v: Union[Datetime, str]) -> Datetime:
        """
    ...
    def init_rules(cls, v: Union[str, Rules], info: ValidationInfo):
        return (
    ...
    def _handle_dble_and_oper(data_config: dict, resource_handler: dict):
        ...
        Args:
            data_config (dict): The data configuration dictionary containing sources
                information.
            resource_handler (dict): The resource handler dictionary.
        """
        experiment = Settings().experiment
        if experiment not in ["OPER", "DBLE"]:
            return
            ...
            for (file_id, param), resource_handler in data.items():
                ...
                if full_root_param in DOWNSCALABLE_PARAMETERS:
                    ...
                    if len(sources_dico) > 1 and (
                        ...
                        data_config["downscaled"][downscale_id][
                            "param"
                        ] = full_root_param
    ...
    def configurations(self) -> List[dict]:
        ...
        Raises:
            ConfigurationError: If the configuration file format is invalid (not JSON or
                list/dict within).
        """
        raw_configurations = []
    ...
    def _process_config(
        ...
        Args:
            production_config (dict): The production configuration dictionary to
                process.
            rules (Rules): Rules of production
            global_hash (str): Global hash of production

        Returns:
            tuple[dict, dict, dict, int, int]: A tuple containing:
                - processed_production_config (dict): The processed production
                    configuration.
                - data_config (dict): The extracted data configuration dictionary.
                - mask_config (dict): The extracted mask configuration dictionary.
                - compo_ok (int): The number of successfully processed components.
                - compo_nok (int): The number of components that encountered errors
                    during processing.
        """
        LOGGER.info(
            f"Starting configuration processing: {production_config['production_id']}"
        )
        ...
        processed_production_config, compo_ok, compo_nok = production.process()

        # Log processing completion with elapsed time and error information
        elapsed_time = time() - start_time
        LOGGER.info(
            f"Configuration processing completed for: "
            f"{production_config['production_id']}."
    ...
    def _append_config(self, result: Tuple[Dict, Dict, Dict, int, int]):
        ...
        Args:
            result (tuple): A tuple containing processed data for a single
                configuration:
                - result[0] (dict): Product configuration dictionary
                - result[1] (dict): Data configuration dictionary
                - result[2] (dict): Mask configuration dictionary
                - result[3] (int): Number of successful component processing
                - result[4] (int): Number of failed component processing
        """
        prod_id = result[0]["id"]
    ...
    def process(self, nproc: int = cpu_count()):
        ...
        Args:
            nproc (int, optional): The number of processes to use for parallel
                execution. Defaults to the number of available CPU cores.

        Returns:
            tuple[dict, dict, dict]: A tuple containing the processed mask, data, and
        ...
        if self._nbr_nok > 0:
            LOGGER.error(
                f"{self._nbr_ok} components processed successfully, "
