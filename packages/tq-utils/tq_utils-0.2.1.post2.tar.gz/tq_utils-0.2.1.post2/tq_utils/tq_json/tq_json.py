...
class TqJSONEncoder(json.JSONEncoder):
    ...
    You can set dictify function through set_class_dictify_function, so that TqJSONEncoder can transform the correct type of class to the dict.
    Also, you don't have to self-make Encoder again.
    See set_class_dictify_function function for more details.
    """
    # 存储设置的类对应的字典化方法，只需要得到可序列化对象即可，不用得到 json 字符串。
    class_dictify_functions: dict[Union[type, Tuple[type, ...]], Callable[[Any], dict]] = dict()

    @classmethod
    def set_class_dictify_function(cls, class_type: Union[type, List[type]],
                                   dictify_function: Callable[[Any], dict[str, Any]]):
        """
        Set class_dictify_function.
        In condition that the object is instance of class type, the dump methods in the tq_json package will call class_dictify_function to create dict through object.
        ...
        """
        if isinstance(class_type, list) and len(class_type) > 0:
            for ct in class_type:
                cls.class_dictify_functions[ct] = dictify_function
        else:
            cls.class_dictify_functions[class_type] = dictify_function

    def default(self, obj):
        for key, value in self.class_dictify_functions.items():
...
class TqJSONDecoder(json.JSONDecoder):
    ...
    You can set objectify function through set_class_objectify_function, so that TqJSONDecoder can transform correct dict to the specified class.
...
def dump_json(path, json_data, json_encoder=TqJSONEncoder):
    """
...
def dump_json_safe(path, json_data, json_encoder=TqJSONEncoder):
    """
...
def dumps_json(json_data, json_encoder=TqJSONEncoder):
    """
    将对象转换得到 json 文本
    :param json_data: 转换成 json 文本的对象
    :param json_encoder: 自定义 JSONEncoder
