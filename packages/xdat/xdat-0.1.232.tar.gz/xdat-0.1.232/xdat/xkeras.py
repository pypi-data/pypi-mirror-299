...
class CachedDataGenerator(Sequence):
    ...
    def __init__(self, data, batch_size=32, mode=None, n_jobs=1, dtype=np.float16, cache_folder=None):
        if mode == 'train':
            mode = 'fit'
    ...
    def calc_XYw(self, df, with_tqdm=None):
        ...
        #
        # gather the samples
        samples_raw = xparallel.x_on_iter(iterrows(df), self._get_sample_wrapper, with_tqdm=with_tqdm, tqdm_total=len(df), n_jobs=self.n_jobs, batch_size=32, tqdm_desc=f'calc XYw - {self.mode}')
