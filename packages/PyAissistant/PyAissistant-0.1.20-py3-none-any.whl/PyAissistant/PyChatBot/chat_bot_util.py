...
import os
import uuid
from .Chat import Message as ChatBotMessage
...
def parse_message(msg_obj:ChatBotMessage):
    try:
        return msg_obj.message_dict
        # role = msg_obj.role
        # if role == TOOL_ROLE:
        #     return {'content': msg_obj.content, 'role': role, 'tool_call_id': msg_obj.tool_id}
        # else:
        #     return get_message(msg_obj.content, role)
    except:
        # logging.error(f'Error parsing message {msg_obj}')
        return msg_obj
