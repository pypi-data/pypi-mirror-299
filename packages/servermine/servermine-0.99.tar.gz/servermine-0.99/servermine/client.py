    ...
    for file in files:
        if file.endswith(".py"):
            try:
                file = file[:-3]
                imported_files.setdefault(file, importlib.import_module(file))
                # users = importlib.import_module('plugins.users')
                # mtd = getattr(users, 'auth')
                # mtd()
                # users.auth()
            except ImportError as err:
                print('Error:', err)
...
async def update_redis_hash():
    try:
        ...
        redis_lib.get_redis_connection().close()
        await redis_lib.get_redis_connection().wait_closed()
