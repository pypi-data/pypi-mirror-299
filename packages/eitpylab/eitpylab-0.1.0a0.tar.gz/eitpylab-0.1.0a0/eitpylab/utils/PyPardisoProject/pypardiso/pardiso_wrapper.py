...
class PyPardisoSolver:
    ...
    def _check_A(self, A):
        ...
        if sp.isspmatrix_csr(A):
            ...
            msg = "PyPardiso requires matrix A to be in CSR or CSC format, but matrix A is: {}".format(
                type(A)
            )
            raise TypeError(msg)
        ...
        if A.dtype != np.float64:
            raise TypeError(
                "PyPardiso currently only supports float64, but matrix A has dtype: {}".format(
                    A.dtype
                )
            )
    ...
    def _check_b(self, A, b):
        if sp.isspmatrix(b):
            warnings.warn(
                "PyPardiso requires the right-hand side b to be a dense array for maximum efficiency",
    ...
    def _call_pardiso(self, A, b):
        ...
        self._mkl_pardiso(
            self.pt.ctypes.data_as(ctypes.POINTER(self._pt_type[0])),  # pt
            ctypes.byref(ctypes.c_int32(1)),  # maxfct
            ctypes.byref(ctypes.c_int32(1)),  # mnum
            ctypes.byref(
                ctypes.c_int32(self.mtype)
    ...
    def set_iparm(self, i, value):
        ...
        if i not in {
            ...
            warnings.warn(
                "{} is no input iparm. See the Pardiso documentation.".format(value),
                PyPardisoWarning,
            )
    ...
    def set_num_threads(self, num_threads):
        ...
        mkl.domain_set_num_threads(num_threads, domain="pardiso")
