...
def test_startup(redis_url):
    with memory_tracker_ctx(
        ...
        assert "Redis max. memory: 23MB" in lines[0]
        assert "Mem. usage to trigger cleanup: 72%" in lines[1]
        assert "Protected history length: 321 seconds" in lines[2]
...
def test_tracking_scan(redis_url, data_store, dummy_id):
    with memory_tracker_ctx(
        ...
        # run the scan
        encoder = NumericStreamEncoder(dtype=int)
        stream1 = scan.create_stream("abcd", encoder)
        stream2 = scan.create_stream("efgh", encoder)
        scan.prepare()
        scan.start()
        stream1.send(12345)
        stream2.send(54321)
...
def test_delete_inactive_scan(redis_url, data_store, dummy_id):
    with memory_tracker_ctx(
        ...
        encoder = NumericStreamEncoder(dtype=int)
        stream = active_scan.create_stream("mystream", encoder)
    ...
    with memory_tracker_ctx(
        ...
        with four_scans() as scans:
            encoder = NumericStreamEncoder(
                dtype=float, shape=(32, 32)
            )  # 8kB datapoints
            streams = [
                scan.create_stream(f"stream_{i}", encoder)
                for scan in scans
                for i in range(4)
            ]
            for scan in scans:
...
def test_protected_history(redis_url, data_store, dummy_id):
    with memory_tracker_ctx(
        ...
        try:
            ...
            streams = [scan.create_stream(f"stream_{i}", encoder) for i in range(4)]
            scan.prepare()
            scan.start()
            for i in range(1024):
...
def test_out_of_memory_handling(redis_url, data_store, dummy_id):
    with memory_tracker_ctx(
        ...
        while True:
            try:
                scan = data_store.create_scan(dummy_id)
                stream = scan.create_stream("mystream", encoder)
                scan.prepare()
                scan.start()
                for _ in range(10):
                    stream.send(
