...
# Enabling dataclass slots when python version allows
if sys.version_info >= (3, 10):
    dataclass_opt = {"slots": True}
else:
    dataclass_opt = {}


@dataclass(**dataclass_opt)
class StreamEntry:
    id: int
    length: int
    batch: EncodedBatch
...
class EventStream:
    ...
    def wait_seal(self, timeout=0):
        if self._seal is not None:
            return True
        if timeout is not None:
            timeout = int(timeout * 1000)
        raw = self._data_store._redis.xread(
            {self.key: f"{_MAX_STREAM_ID}-{_MAX_STREAM_ID-1}"}, block=int(timeout)
        )
        try:
