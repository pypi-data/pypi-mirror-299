...
class KISSimpleOverseasStock:
    ...
    def get_historical_daily_data(
        ...
        Args:
            ticker (str): 조회할 자산의 티커(symbol).
            first_date (datetime.date): 조회를 시작할 첫 번째 날짜 (YYYY-MM-DD).
            last_date (datetime.date): 조회를 마칠 마지막 날짜 (YYYY-MM-DD).
    ...
    def get_today_minute_data(self, ticker: str) -> pd.DataFrame:
        ...
        r = self.stock_api.inquire_time_itemchartprice(
            excd=exchange_to_code[ticker_info["exchange"].loc[ticker]],
            symb=ticker,
        )
    ...
    def get_price(self, ticker: str) -> pd.DataFrame:
        ...
        r = self.stock_api.get_price(
            exchange_to_code[ticker_info["exchange"].loc[ticker]],
            ticker,
        )
    ...
    def get_pending_orders(self, to_frame: bool = False) -> List[OverseasStockOrder] | pd.DataFrame:
        ...
        for d in output:
            order = OverseasStockOrder(
                ...
                order_time=dtm.datetime.combine(d["ord_dt"], d["ord_tmd"]),
                filled_price=d["ft_ccld_unpr3"],
                current_price=None,
                is_pending=True,
                org_order_no=d["orgn_odno"],
                order_type=None,
                req_type=self._code_to_order_request_type(d["rvse_cncl_dvsn_cd"]),
            )
    ...
    def get_today_order_history(self, to_frame: bool = False) -> List[OverseasStockOrder] | pd.DataFrame:
        """
        오늘의 주문 내역을 조회하여 리스트 또는 데이터프레임 형식으로 반환합니다.
        ...
        r = self.stock_api.inquire_ccnl(
            self.account_no,
            self.account_product_code,
            dtm.date.today() - dtm.timedelta(days=1),
            dtm.date.today(),
            "NASD",
            "%",
            "00",
        ...
        for d in output:
            order = OverseasStockOrder(
                ...
                filled_quantity=d["ft_ccld_qty"],
                pending_quantity=d["nccs_qty"],
                order_time=dtm.datetime.combine(d["dmst_ord_dt"], d["ord_tmd"]),
                filled_price=d["ft_ccld_unpr3"],
    ...
    def get_scheduled_orders(
        ...
        Args:
            start_date (datetime.date, optional): 조회할 시작 일자. 기본값은 당일.
            end_date (datetime.date, optional): 조회할 종료 일자. 기본값은 당일.
        ...
        Returns:
            List[dict]: 예약 주문 목록.
                ...
                - reservation_date (str): 예약 주문 접수 일자 (YYYYMMDD).
    ...
    def get_orderbook(self, ticker: str) -> Dict:
        ...
        result = {
            ...
            "bid_price": o2["pbid1"],
            "bid_volume": o2["vbid1"],
            "time": dtm.datetime.strptime(o1["dymd"] + o1["dhms"], "%Y%m%d%H%M%S"),
