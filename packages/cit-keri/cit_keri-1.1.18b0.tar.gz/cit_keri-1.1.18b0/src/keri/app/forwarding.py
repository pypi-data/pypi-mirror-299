...
class Poster(doing.DoDoer):
    ...
    def forwardToWitness(self, hab, ends, recp, serder, atc, topic):
        ...
        try:
            if len(mailbox) == 0:
                raise kering.ConfigurationError("No urls for mailbox")
            witer = agenting.messengerFrom(hab=hab, pre=mbx, urls=mailbox)
        except Exception as e:
            print(f"Error creating messenger for {mbx} with urls={mailbox}.  Err={e}")
            raise e
        if witer is None:
            print("Error creating messenger for", mbx, "with urls=", mailbox)
        msg.extend(ims)
        msg.extend(atc)
