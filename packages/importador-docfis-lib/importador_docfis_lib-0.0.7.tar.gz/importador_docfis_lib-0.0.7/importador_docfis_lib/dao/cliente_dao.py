...
class ClienteDAO:
    ...
    def obterSQLInsertPessoa() -> str:
        ...
        return f"""insert into ns.pessoas (id, pessoa, tp_identificacao, nome, nomefantasia, 
                ...
                suframa, fichaativado, tributoativado, tomadorfolhaativado, formatributacaofunrural) values ({','.join(['%s']*31)})"""
    ...
    def obterSQLInsertConjuntos() -> str:
        ...
        return f"insert into ns.conjuntosclientes \
                (conjuntocliente, registro, conjunto) values ({','.join(['%s']*3)})"
    ...
    def obterSQLInsertEndereco() -> str:
        """Retorna o comando SQL para insert na tabela ns.pessoas"""
        return f"""insert into ns.enderecos (tipologradouro, logradouro, numero, complemento, cep, bairro, tipoendereco, uf, ibge, cidade, endereco, id_pessoa) values ({','.join(['%s']*12)})"""
