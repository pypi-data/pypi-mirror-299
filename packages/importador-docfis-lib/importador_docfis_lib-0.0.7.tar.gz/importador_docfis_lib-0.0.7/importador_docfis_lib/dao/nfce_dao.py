...
from importador_docfis_lib.dao.pagamento_dao import PagamentoDAO
...
from importador_docfis_lib.dao.item_nota_dao import ItemNotaDAO
from importador_docfis_lib.dao.lanfis_dao import LanfisDAO
from importador_docfis_lib.dao.lf_itens_dao import Lf_ItensDAO
...
class NFCeDAO:
    ...
    def atualizar_nfce(
        self, notas: list[NFCE], listaEstabelecimentos: list[Estabelecimento], icms_desonerado_desconto: bool = False
    ):
        """
        ...
        try:
            ...
            self.gravarDocfisUpdate(notas, icms_desonerado_desconto)
    ...
    def incluir_nfce(
        ...
        icms_desonerado_desconto: bool = False
    ):
        """
        Grava as notas no banco de dados, incluindo ns.df_docfis, ns.df_itens, \
        scritta.lanfis e scritta.lf_itens, ns.df_pagamentos e fila de \
        ...
        try:
            ...
            self.desativarRastro()
            self.desativarContabilizacao()
            self.gravarDocfisInsert(notas, icms_desonerado_desconto)
            self.gravarDfLinhas(notas)
            self.gravarDfItens(notas)
            self.gravarLanfis(notas)
            self.gravarLfItens(notas)
    ...
    def gravarDocfisInsert(self, notas: list[NFCE], icms_desonerado_desconto=False):
        """Grava na tabela ns.df_docfis"""

        try:
            parametros = [NFCeDAO.obterListaValoresNFCe(nfe, icms_desonerado_desconto) for nfe in notas]
            if len(parametros) > 0:
    ...
    def obterSQLInutilizacaoNotas() -> str:
        SQL = f""" INSERT INTO NS.DF_DOCFIS(id, id_ano, sinal, tipo, modelo, serie, subserie, numero, periodo, lancamento, 
    ...
    def obterSQLInutilizacaoLancamentos() -> str:
        SQL = f""" WITH nsdoc(id_ano, tipo, datalanc, modelo, serie, subserie, numero, cfop, emissao, atenumero, 
    ...
    def obterSQLInsertAjuste() -> str:
        ...
        return f"INSERT INTO scritta.lanaju( \
    ...
    def obterSQLInsertDocFis() -> str:
        ...
        return f"""INSERT INTO NS.DF_DOCFIS ({parametros_docfis}) VALUES ({','.join(['%s'] * len(parametros_docfis.split(',')))})"""
    ...
    def obterSQLMovimentoEstoque():
        return f""" WITH valor(data, origem, sinal, numero, historico, quantidade, valor, valorqtd, precomedio, tipomovimento, 
