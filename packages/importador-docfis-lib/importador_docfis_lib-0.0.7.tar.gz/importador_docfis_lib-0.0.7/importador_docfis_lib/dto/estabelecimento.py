...
class Estabelecimento:
    ...
    def retornar_produtos_novos(
        ...
        for item in nfce.lista_itens:
            if item.CFOP == "" and nfce.cfop != "":
                item.CFOP = nfce.cfop

            if item.CFOP in self.cfops:
                item.id_cfop = self.cfops[item.CFOP].id
                if item.item_gorjeta is True:
                    item.CFOP = nfce.gorjetaCFOP(item.CFOP)
            else:
                raise Exception(f"CFOP {item.CFOP} não existente no estabelecimento")
            item.idLocalDeEstoque = self.id_localdeestoque
            item.idFiguraTributaria = self.id_figuratributaria
            ...
            if item.cProd in produtos_novos:
                ...
                produto_novo.estabelecimento = self.id

                produto_novo.id = uuid.uuid4()
                produto_novo.item = item.cProd
                produto_novo.especificacao = item.xProd
                produto_novo.cadastro = self.cadastro
                if item.uCom in self.unidades:
                    item.id_unidade = self.unidades[item.uCom].id
                elif "UN" in self.unidades:
                    item.id_unidade = self.unidades["UN"].id
                else:
                    for unidade in self.unidades.values():
                        item.id_unidade = unidade.id
                        break
                if item.uTrib in self.unidades:
                    item.id_unidade_trib = self.unidades[item.uCom].id
                elif "UN" in self.unidades:
                    item.id_unidade_trib = self.unidades["UN"].id
                else:
                    for unidade in self.unidades.values():
                ...
                produto_novo.conjunto = self.conjunto_produtos
