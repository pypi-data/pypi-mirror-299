...
def load_datacube(adr,comp=[0],force_freq=None):
    ...
    ifreq=int(df.attrs['S_RATE'])

    if force_freq is not None:
        if (ifreq/force_freq).is_integer():
            if ifreq/force_freq > 1 :
                print('Sample signal to '+str(force_freq)+' Hz')
                for i in comp:
                    df['comp_'+str(i)]=df['comp_'+str(i)][::int(ifreq/force_freq)]
                    df['comp_'+str(i)].attrs['S_RATE']=str(force_freq)

                df.attrs['S_RATE']=str(force_freq)      
        else:
            raise ValueError("force_freq is not possible !")
