...
def importar_notas_outras_empresas(data_emissao, numero_nota):
...
async def wait_transmit():
    ...
    timeout = 30
    timeout_try = 0
    while(aguardando_nota or timeout_try >= timeout):
        screenshot_path = take_screenshot()
        ...
        if target:
            ...
            screenshot_path = take_screenshot()
            target_nota_lancada = find_target_position(screenshot_path, "100", 1, 0, 1)
...
def verify_import_nf():
    try:
        nota_incluida = pyautogui.locateOnScreen(ASSETS_PATH + "\\entrada_notas\\no_data_display.png", confidence=0.85)
        if nota_incluida:
            # pyautogui.click(1095, 686)
            return False
    except:
        return True