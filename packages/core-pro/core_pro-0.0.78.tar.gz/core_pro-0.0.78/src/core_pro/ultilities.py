...
def upload_ds(file_path: Path, api_endpoint: str, ingestion_token: str, sleep_time: int = 10, max_retries: int = 3):
    ...
    logger.add(
        ...
        format='<level>{level}</level> | <cyan>{function}</cyan> | <level>{message}</level>',
    )
    ...
    for attempt in range(max_retries):
        ...
        with tqdm(total=total_size, unit='B', unit_scale=True, unit_divisor=1024, desc=desc, leave=True) as pbar:
            try:
                ...
                logger.bind(special=True).success(f'[DataHub] ConnectionError')
        ...
        if response.status_code == 200:
            logger.bind(special=True).success(f'[DataHub] - {code}: {message}, {file_path.name}')
            return response
        else:
            sleep_with_progress(60 * sleep_time, desc='Waiting DataHub')

    if attempt == max_retries - 1:
        logger.info('[DataHub] Max retries reached. Unable to upload.')
