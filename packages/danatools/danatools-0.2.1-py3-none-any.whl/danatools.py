...
def sin2_to_deg(sin_sqr):
    """
    Calculate the angle in degrees from its sine square
    """
    sin = np.sqrt(sin_sqr)
...
def deg_to_sin2(angle_deg):
    """
    Calculate the sine square of an angle in degrees
    """
    angle_rad = np.deg2rad(angle_deg)
    return np.sin(angle_rad)**2
