...
class LDataFile:
    ...
    def _download(self):
        ...
        async def get_signed_url() -> str:
            ...
            async with self.fs.nucleus_sess.post(
                ...
                json={"path": path, "egress_event_data": {"purpose": "ldata-fuse"}},
            ) as res:
        ...
        @trace_app_function
        async def get_file_size(url) -> int:
            try:
                            ...
                            async with self.fs.generic_sess.get(url, headers=headers) as res:
                                ...
                                with app_tracer.start_as_current_span("write"):
                                    async for chunk in res.content.iter_any():
                                        ...
                                        with self.io_lock:
                                            local_segments = self.local_writes.range_intersection(
                                                offset, offset + len(chunk)
                                            ).segments

                                            if local_segments == [(offset, offset + len(chunk))]:
                                                continue
                                            ...
                                            if len(local_segments) > 0:
                                                ...
                                                for start, end in local_segments:
                                                    ...
                                                    chunk[start:end] = disk_data[start:end]
    ...
    def open(self):
        ...
        span.set_attributes(
            dict_to_attrs(
                {"path": str(self.path), "flags": human_readable_open_flags(self.flags), "flags.raw": self.flags, "mode": human_readable_mode(self.mode), "mode.raw": self.mode},
                "args",
            )
        )
        ...
        if self.download_started:
            if self.download_failed and self.download_attempts < self.max_retry_attempts:
                self.download_started = False
                self.download_failed = False
    ...
    def write(self, data: bytes, offset: int):
        ...
        with self.io_lock:
            ...
            res = os.pwrite(self.workfile_fd, data, offset)
...
class LDataFS(Fuse):
    ...
    def ensure_not_pending(self, path: PurePosixPath):
    ...
    def getattr(self, path: str):
        ...
        span.set_attributes(dict_to_attrs({"path": path}, "args"))
    ...
    def readdir(self, path: str, offset: int):
        ...
        span = get_current_span()
        span.set_attributes(dict_to_attrs({"path": path, "offset": offset}, "args"))
    ...
    @syscall_impl
    def write(self, path: str, data: bytes, offset: int, f: LDataFile) -> int:
    ...
    def truncate(self, path, size):
        span = get_current_span()
        span.set_attributes(dict_to_attrs({"path": path, "size": size}, "args"))
        ...
        if path in self.opened_files:
            ...
            try:
                ...
                with self.ensure_refresh(
                    ...
                    subscription_timeout=30,
                    timeout=10,
                    refresh_filter=lambda: self.ensure_not_pending(path),
                ):
                    self.req_thread.call(LDataFile.upload(self, path, 0, None), 5)
            finally:
                self.block_open_events.pop(str(path)).set()
    ...
    def chown(self, path: str, uid: int, gid: int) -> None:
        span = get_current_span()
        span.set_attributes(
            dict_to_attrs({"path": path, "uid": uid, "gid": gid}, "args")
        )
        # unsupported, noop
        return None
    ...
    def utimens(self, path: str, a_time: float, m_time: float) -> None:
        ...
        span.set_attributes(
            dict_to_attrs(
                {"path": path, "a_time": str(a_time), "m_time": str(m_time)}, "args"
            )
        )
        # unsupported, noop
        return None
    ...
    def rename(self, src: str, dst: str):
        span = get_current_span()
        span.set_attributes(dict_to_attrs({"src": src, "dst": dst}, "args"))
