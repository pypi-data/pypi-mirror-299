...
def load_query_records(file: str):
    ...
    if file_ext in fasta_ext:
        records = load_fasta_file(file, None, None, 17, None, 0, 0, 1, True, 1)
    elif file_ext in fastq_ext:
        records = load_fastq_file(file, None, None, 17, None, 0, 0, 1, True, 1)
    else:
        sys.exit(message)
...
def find_shared_kmers(queryKmers: dict, records: list, ksize: int):
    """see if the query kmers can be found in the subject kmers"""

    kmer_results = []

    for record in records:
        subj_kmers = build_kmers(record.seq, ksize, record.seq_length)
...
def Kmerize(args):
    ...
    if ftype == "fastq":
        hits = load_fastq_file(subject_file, limit, query_records,
                               ksize, output_fh, record_cnt, hits, 
                               seq_cnt, False, thread_num)
    
    elif ftype == "fasta":
        #kmer_pipeline nested in loading
        hits = load_fasta_file(subject_file, limit, query_records,
                               ksize, output_fh, record_cnt, hits,
                               seq_cnt, False, thread_num)
