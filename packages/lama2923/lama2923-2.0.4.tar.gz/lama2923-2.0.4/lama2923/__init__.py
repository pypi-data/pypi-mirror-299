...
class Hotkey:
    def __init__(self, *key):
        ...
        if not self.keyboardm and not self.pynputm:
            raise ImportError('No module named keyboard or pynput')
    ...
    def __call__(self, func):
        ...
        if self.keyboardm:
            import keyboard
            keyboard.add_hotkey(self.key_combination, call)
            ...
            def on_press(key):
                if isinstance(key, pynput_keyboard.Key) and key == pynput_keyboard.Key.f1:
                    call()
