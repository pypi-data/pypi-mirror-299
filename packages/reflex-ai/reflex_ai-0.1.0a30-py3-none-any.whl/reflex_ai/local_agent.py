...
class LocalAgent:
    ...
    async def process(self, request: InternRequest):
        ...
        intern_response = InternResponse(**response)
        yield intern_response.request_id

        # Process and yield the tool use messages.
        tool_calls = intern_response.tool_calls
        while True:
            tool_results: list[ToolResult] = []
            ...
            tool_response_request = ToolRequestResponse(
                request_id=intern_response.request_id,
            ...
            # Otherwise, update the tool calls and continue.
            tool_calls = [ToolCall(**tc) for tc in response]
    ...
    async def _handle_tool_result(
        ...
        if not tool_result.is_error and tool_call.name in [
            ...
            new_source = path_utils.convert_from_scratch_code(paths.base_paths[0].name, new_source)
            ...
            modified_filename = modified_filename.replace(str(paths.tmp_root_path), str(paths.base_paths[0].parent))
            ...
            try:
                ...
                main_directory_validation_file = request.parent_component["filename"].replace(str(paths.tmp_root_path), str(paths.base_paths[0].parent))
                ...
                write_to = modified_filename.replace(str(paths.base_paths[0].parent), str(paths.tmp_root_path))
                with open(write_to, "w") as file:
                    new_source = path_utils.convert_to_scratch_code(paths.base_paths[0].name, new_source)
