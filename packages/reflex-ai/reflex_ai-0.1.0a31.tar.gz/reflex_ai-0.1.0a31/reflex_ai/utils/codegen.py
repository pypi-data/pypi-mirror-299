...
def to_unified_diff(ndiff: str) -> str:
    ...
    previous_source = path_utils.format_code(path_utils.convert_from_scratch_code(app_name, "".join(difflib.restore(ndiff, 1))))
    new_source = path_utils.format_code(path_utils.convert_from_scratch_code(app_name, "".join(difflib.restore(ndiff, 2))))
    return "\n".join(
...
def add_python_element(
    ...
    if isinstance(new_element, ast.FunctionDef) or isinstance(new_element, ast.ClassDef):
        ...
        raise ValueError("The new element you want to create is not a class, function, or variable.")
