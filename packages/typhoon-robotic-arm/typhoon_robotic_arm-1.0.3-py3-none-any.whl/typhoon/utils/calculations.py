...
def angles_from_coords(x: int, y: int, z: int, tool_offset_x: int = 0, tool_offset_y= 0, tool_offset_z: int = 0) -> tuple[float, float, float]:
    """
    ...
    z -= config.HEIGHT_FROM_GROUND + tool_offset_z
    r = math.sqrt(x**2 + y**2) - tool_offset_y - 45
    q1 = math.atan2(y, x)
