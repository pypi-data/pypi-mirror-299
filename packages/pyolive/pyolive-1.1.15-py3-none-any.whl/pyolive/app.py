...
class Athena:
    ...
    def _callback_subscribe(self, ch, method, properties, body):
        ...
        self.app_logger = AppLog(_s).create_logger()
        ...
        for task in self.tasks:
            if app_name == task['app']:
                ...
                if filenames:
                    ...
                    if not _fset:
                        return
                w = Worker(task['resource'], self.channel, self.agent_logger, self.app_logger, ctx)
                w.start()

    def run(self):
        self.agent_logger = AgentLog(self.alias).create_logger()
