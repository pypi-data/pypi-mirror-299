...
def update_allowed_hosts(content):
    return content.replace("ALLOWED_HOSTS = []", "ALLOWED_HOSTS = ['*']")
...
def create_project_structure(project_name, app_names, use_tailwind, secret_key, install_drf):
    try:
        total_steps = 16 if install_drf else 15  # Tambahkan 1 langkah baru jika install_drf
        with tqdm(total=total_steps, desc=click.style("Creating project", fg="green", bold=True), unit="step", bar_format="{l_bar}{bar}| {n_fmt}/{total_fmt}") as pbar:
            ...
            if install_drf:
                install_django_rest_framework()  # Install DRF
                update_settings_for_drf(project_name)  # Update settings for DRF
                pbar.update(1)
...
def create_serializers_and_views(app_names):
    for app_name in app_names:
        serializers_path = f"{app_name}/serializers.py"
