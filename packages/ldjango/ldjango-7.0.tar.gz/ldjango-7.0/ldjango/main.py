...
def update_installed_apps(content, app_names):
    ...
    if installed_apps_pattern in content:
        insert_index = content.find(installed_apps_pattern) + len(installed_apps_pattern)
        new_apps = '\n    ' + ',\n    '.join([f"'apps.{app_name}'" for app_name in app_names])
        return content[:insert_index] + new_apps + content[insert_index:]
    return content
    ...
    try:
        ...
        with tqdm(total=total_steps, desc=click.style("Creating project", fg="green", bold=True), unit="step", bar_format="{l_bar}{bar}| {n_fmt}/{total_fmt}") as pbar:
            ...
            try:
                ...
                click.echo(click.style(f"\nWarning: Could not update settings.py. Error: {str(e)}", fg="yellow"))
            ...
            try:
                ...
                click.echo(click.style(f"\nWarning: Could not update settings for environment variables. Error: {str(e)}", fg="yellow"))
            pbar.update(1)
...
'''
    ...
    if 'import environ' not in settings_content:
