...
import subprocess
import shlex
from .utils import interpolate_points, extract_values_from_tif
...
def _compile_meshchecker(isParallel=False):
        ...
        g++ -O3 -fopenmp -o meshchecker src/cpp/*.cpp src/cpp/writer/*.cpp src/cpp/interfaces/*.cpp src/cpp/helpers/*.cpp -std=c++20
        ...
        if isParallel:
            compiler_opt = '-O3 -fopenmp'
        else:
            compiler_opt = ''
        try:
            subprocess.call(shlex.split('g++ '+compiler_opt+' -o '+relpath+'/'+_MESHCHECKER_NAME+ ' '+src_files+' -std=c++20 -static'),
...
class WatlabModel():
    ...
    def _launch_meschecker(self, display=True,isParallel=False):
    ...
    def set_picture_times(self,n_pic=0,dt_picture = 0, pic_array = None):
        ...
        self.__is_picture = 1
        if dt_picture != 0:
            self.__picture_times = np.arange(0, self.ending_time, dt_picture)
        elif n_pic > 0:
            self.__picture_times = np.linspace(0, self.ending_time, n_pic)
...
class WatlabExport():
    ...
    def _export_blind_data(self, data_key, file_path):
        """
...
class Plotter():
    ...
    def plot_profile_along_line(self, picture_file_path: str, variable_name: str, x_coordinate=None, y_coordinate=None, new_fig=False, n_points=50, label="") -> None:
        """ 
    ...
    def plot(self, picture_file_path, variable_name: str, opacity=1, colorbar_values=None):
        ...
        def __on_clicked(event):
            ...
            if len(coords_x)==2:
                ...
                self.plot_profile_along_line(picture_file_path, variable_name, coords_x, coords_y,new_fig=True, n_points = 50, label=variable_name)
        ...
        if colorbar_values is None:
            levels = np.linspace(min(data), max(data), 100)
        else: 
    ...
    def plot_on_map(self, pic_path, variable_name:str, opacity=0.5, colorbar_values=None, csr=31370):
        """
