...
class skin(config):
    ...
    def __init__(self, path:str) -> None:
        ...
        self._gitobj = git(self.get_dirname())
        self.set('local', 'path', self._gitobj.local_path)
    ...
    def fix_xdg_dirs(self) -> list:
        ...
        """Checks all user dirs, based on the xdg group.
        ...
        for key in self.options("xdg"):
            ...
            if self.get("xdg", key) == "None":
                ...
                if not p.is_ok():
                #     results.append(f"xdg:{key} set to None")
                # else:
                    results.append(f"xdg:{key} does not exist!")
                continue

            val = os.path.join(self.get("local", "path"), self.get("xdg", key))
            if not os.path.exists(val):
                results.append(f"xdg:{key} points to non-existent file/folder!")
            else:
                p.run(f"xdg-user-dirs-update --set {key} {val}")
                if p.is_ok():
                    results.append(f"xdg:{key} set to {val}")
                else:
                    results.append(f"xdg:{key} does not exist!")
        return results
