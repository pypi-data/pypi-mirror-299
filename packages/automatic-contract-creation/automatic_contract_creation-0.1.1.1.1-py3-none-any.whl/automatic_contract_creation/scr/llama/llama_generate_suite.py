...
class LlamaGenerateSuite(LlamaModel):
    ...
    def get_description(self):
        columns_desc = OMDClient(base_url='https://metadata.wb.ru',
                      ...
                      ).get_table(f'HIVE.{self.helper.creds["catalog"]}.{self.helpe.rcreds["schema"]}.{self.helper.table}').columns
        ...
        if suite_dict:
            for col, checks in suite_dict.items():
                for check, val in checks.items():
                    if os.getenv('connection_name')=='trino':
                        business_desc = check_names.filter(pl.col('column_name') == col).select(pl.col('description')) if check_names else None
