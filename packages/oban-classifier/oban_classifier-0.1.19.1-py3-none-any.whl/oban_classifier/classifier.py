...
def post_classification_analysis(X, y_true, y_proba, threshold=0.5):
    ...
    # Filter probabilities by class labels
    class_0_correct_proba = class_0_proba[(y_true == 0) & correct_predictions]
    class_1_correct_proba = class_1_proba[(y_true == 1) & correct_predictions]
    class_0_incorrect_proba = class_0_proba[(y_true == 0) & incorrect_predictions]
    class_1_incorrect_proba = class_1_proba[(y_true == 1) & incorrect_predictions]

    print("\nCorrect Predictions Probability Analysis:")
    if len(class_0_correct_proba) > 0:
        print(f"Class 0 Correct Predictions: {np.mean(class_0_correct_proba):.4f}")
    else:
        print("No correct predictions for Class 0.")
    if len(class_1_correct_proba) > 0:
        print(f"Class 1 Correct Predictions: {np.mean(class_1_correct_proba):.4f}")
    else:
        print("No correct predictions for Class 1.")

    print("\nIncorrect Predictions Probability Analysis:")
    if len(class_0_incorrect_proba) > 0:
        print(f"Class 0 Incorrect Predictions: {np.mean(class_0_incorrect_proba):.4f}")
    else:
        print("No incorrect predictions for Class 0.")
    if len(class_1_incorrect_proba) > 0:
        print(f"Class 1 Incorrect Predictions: {np.mean(class_1_incorrect_proba):.4f}")
    else:
        print("No incorrect predictions for Class 1.")
