...
def plot_feature_importance(model, X_test):
    """
    Plots feature importance using SHAP values for the trained model.
    """
    # Initialize SHAP explainer for PyTorch models
    explainer = shap.DeepExplainer(model.module_, torch.tensor(X_test.astype(np.float32)))
    shap_values = explainer.shap_values(torch.tensor(X_test.astype(np.float32)))
...
# Example usage:
# Train the model
netv, X_test, y_test = oban_classifier(X, y, num_units=256, max_epochs=100, lr=0.001)

# Plot feature importance
plot_feature_importance(netv, X_test)
