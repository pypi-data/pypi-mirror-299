...
class ObanModule(nn.Module):
    def __init__(self, input_dim, num_units=128, num_classes=None, nonlin=nn.ReLU(), dropout_rate=0.5):
        ...
        self.output = nn.Linear(num_units, num_classes)  # num_classes dynamic now
...
def oban_classifier(X, y, num_units=128, num_classes=None, nonlin=nn.ReLU(), dropout_rate=0.5, 
    ...
    """
    # Number of classes is dynamically determined
...
def evaluate_performance(y_true, y_pred, num_classes):
    ...
    # Performance Table
    console.print(f"[bold black]Accuracy: {accuracy:.4f}[/bold black]")
    
    table = Table(title="Confusion Matrix", title_style="bold black")
    for i in range(cm.shape[1]):
        table.add_column(f"Class {i}")
    for row in cm:
        table.add_row(*[str(item) for item in row])
    console.print(table)

    # Precision, Recall, F1 Score Table
    metrics_table = Table(title="Performance Metrics", title_style="bold black")
    ...
    for i, (prec, rec, f1) in enumerate(zip(precision_per_class, recall_per_class, f1_per_class)):
        metrics_table.add_row(
            f"[bold black]Class {i}[/bold black]", 
            f"[bold black]{prec:.4f}[/bold black]", 
            f"[bold black]{rec:.4f}[/bold black]", 
            f"[bold black]{f1:.4f}[/bold black]"
        )
