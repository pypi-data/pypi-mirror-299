...
def test_update_package_info(init_package_info):
    ...
    # case of setup.py
    get_source.update_package_info(package, os.path.join(base_path, "mock-1"), sys.executable)
    assert package.name == "dummy_package"
    assert package.version == "3.1.0"

    # case of metadata
    package = init_package_info
    get_source.update_package_info(package, os.path.join(base_path, "mock-2"), sys.executable)
    assert package.name == "mock_package"
    assert package.version == "2.2.0"

    # case of dist-info folder
    package = init_package_info
    get_source.update_package_info(package, os.path.join(base_path, "mock-3"), sys.executable)
    assert package.name == "mock_package"
...
def test_get_source_git_clone(init_package_info):
    ...
    get_source.get_source(package, 0, sys.executable)
...
def test_get_source_dist_file_zip(init_package_info):
    ...
    get_source.get_source(package, 1, sys.executable)
...
def test_get_source_dist_file_whl(init_package_info):
    ...
    get_source.get_source(package, 2, sys.executable)
...
def test_get_source_dist_file_tar(init_package_info):
    ...
    get_source.get_source(package, 3, sys.executable)
...
def test_get_source_pip_whl(init_package_info):
    ...
    get_source.get_source(package, 4, sys.executable)
...
def test_get_source_pip_zip(init_package_info):
    ...
    package.extra_index_url = None
    package.prefer_source_distribution = True
    get_source.get_source(package, 5, sys.executable)
...
def test_get_source_zip_file_at_position_0(init_package_info):
    ...
    package.extra_index_url = None
    package.prefer_source_distribution = True
    get_source.get_source(package, 6, sys.executable)
