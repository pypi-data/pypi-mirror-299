...
class TestSESUtils(unittest.TestCase):
    ...
    def test_send_email_to_support(self, mock_send_email, mock_get_parameter):
        mock_get_parameter.return_value = 'support@example.com'
        
        send_email_to_support('from@example.com', 'Test Subject', 'Test Content')
        
        mock_get_parameter.assert_called_once_with('cmp-referentiel', 'to_support_email')
        mock_send_email.assert_called_once_with('from@example.com', 'support@example.com', 'Test Subject', 'Test Content', [])
