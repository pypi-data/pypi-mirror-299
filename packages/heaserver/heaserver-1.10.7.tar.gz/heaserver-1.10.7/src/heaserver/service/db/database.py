...
class Database(abc.ABC):
    ...
    async def get_credentials_from_volume(self, request: web.Request, volume_id: str) -> Credentials | None:
        ...
        return await get_credentials_from_volume(request, volume_id, Credentials)  # type:ignore[func-returns-value]
...
async def _get_credentials(app: web.Application, volume: Volume, cred_type: type[CredentialTypeVar],
    ...
    service, or the type of credential associated with the volume does not match cred_type.
    """
...
class DatabaseContextManager(Generic[DatabaseClientTypeVar, CredentialTypeVar], AbstractAsyncContextManager[DatabaseClientTypeVar], ABC):
    ...
    def __init__(self, request: Request, volume_id: str | None = None, credentials: CredentialTypeVar | None = None) -> None:
        ...
        self.__credentials = deepcopy(credentials)
