...
class DeliberationIntensity:
    ...
    def argumentativeness(self, df):
        ...
        for group in tqdm(unique_groups, desc="Processing groups", disable=not self.verbose):
            group_df = df[df[self.columns['group']] == group].copy()
            ...
            if total_speech_length == 1:
                argumentativeness[self.columns['argument']]
            elif total_speech_length == 2:
                argumentativeness[self.columns['argument']] *= 2
            else:
