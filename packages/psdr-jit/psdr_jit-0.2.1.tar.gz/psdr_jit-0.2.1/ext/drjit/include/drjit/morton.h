/*
    drjit/morton.h -- Morton/Z-order curve encoding and decoding routines
...
template <typename Value> constexpr Value morton_magic(size_t dim, size_t level) {
    ...
    size_t block_size = size_t(1) << (level - 1);
    size_t count = 0;
...
DRJIT_INLINE Value scatter_bits(Value x) { return x; }
...
DRJIT_INLINE Value scatter_bits(Value x) {
...
DRJIT_INLINE Value gather_bits(Value x) { return x; }

/// Bit gather function. \c Dimension defines the final distance between two input bits
template <size_t Dimension, typename Value,
          size_t Level = clog2i(sizeof(Value) * 8),
          enable_if_t<Level != 0 && (!(has_avx2 && has_x86_64) || !std::is_integral_v<Value>)> = 0>
DRJIT_INLINE Value gather_bits(Value x) {
    using Scalar = scalar_t<Value>;
...
#if defined(DRJIT_X86_AVX2) && defined(DRJIT_X86_64)
template <size_t Dimension, typename Value,
...
template <typename Array, size_t Index, enable_if_t<Index == 0> = 0>
DRJIT_INLINE value_t<Array> morton_encode(Array a) {
    return detail::scatter_bits<Array::Size>(a.entry(0));
}
...
DRJIT_INLINE Array morton_decode(value_t<Array> value) {
