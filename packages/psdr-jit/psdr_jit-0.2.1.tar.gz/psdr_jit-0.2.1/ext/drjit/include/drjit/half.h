/*
    drjit/half.h -- minimal half precision number type
...
NAMESPACE_BEGIN(std)
template<> struct is_floating_point<drjit::half> : true_type { };
template<> struct is_arithmetic<drjit::half> : true_type { };
template<> struct is_signed<drjit::half> : true_type { };
NAMESPACE_END(std)
...
struct half {
    ...
    #define DRJIT_IF_SCALAR template <typename Value, enable_if_t<std::is_arithmetic_v<Value>> = 0>

    DRJIT_IF_SCALAR half(Value val) : value(float32_to_float16(float(val))) { }
    ...
    DRJIT_IF_SCALAR friend half operator+(Value val, half h) { return half(val) + h; }
    DRJIT_IF_SCALAR friend half operator-(Value val, half h) { return half(val) - h; }
    DRJIT_IF_SCALAR friend half operator*(Value val, half h) { return half(val) * h; }
    DRJIT_IF_SCALAR friend half operator/(Value val, half h) { return half(val) / h; }
    ...
    DRJIT_IF_SCALAR operator Value() const { return Value(float16_to_float32(value)); }
...
template<> struct numeric_limits<drjit::half> {
    ...
    static drjit::half min() noexcept { return drjit::half::from_binary(0x0400); }
    static drjit::half lowest() noexcept { return drjit::half::from_binary(0xFBFF); }
    static drjit::half max() noexcept { return drjit::half::from_binary(0x7BFF); }
    static drjit::half epsilon() noexcept { return drjit::half::from_binary(0x1400); }
    static drjit::half round_error() noexcept { return drjit::half::from_binary(0x3C00); }
    static drjit::half infinity() noexcept { return drjit::half::from_binary(0x7C00); }
    static drjit::half quiet_NaN() noexcept { return drjit::half::from_binary(0x7FFF); }
    static drjit::half signaling_NaN() noexcept { return drjit::half::from_binary(0x7DFF); }
    static drjit::half denorm_min() noexcept { return drjit::half::from_binary(0x0001); }
};
