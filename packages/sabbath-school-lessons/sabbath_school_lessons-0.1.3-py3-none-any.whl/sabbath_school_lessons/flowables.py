...
class FirstPageRender(Flowable):
    def __init__(self, title, subtitle, copyright_info, width, height, title_color=colors.black, copyright_color=colors.gray,lesson_book=None):
        Flowable.__init__(self)
        self.title = title
    ...
    def draw(self):
        ...
        canvas.drawCentredString(self.width / 2.0, self.height - 1 * inch, "Sabbath School Lesson")
        ...
        if self.lesson_book:
            ...
            canvas.setFillColor(colors.gray)
            canvas.drawCentredString(self.width / 2.0, self.height - 2.0 * inch, self.lesson_book)
        
        canvas.setFont("Helvetica-Bold", 18)
        canvas.setFillColor(colors.black)
        canvas.drawCentredString(self.width / 2.0, self.height - 3.5 * inch, self.subtitle)

        # Draw the copyright info at the bottom center
        canvas.setFont("Helvetica", 10)
        canvas.setFillColor(self.copyright_color)
        canvas.drawCentredString(self.width / 2.0, 0.5 * inch, self.copyright_info)
