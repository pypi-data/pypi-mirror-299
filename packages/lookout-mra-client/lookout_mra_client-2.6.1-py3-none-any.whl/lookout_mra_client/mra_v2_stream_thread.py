...
class MRAv2StreamThread(threading.Thread):
    ...
    def run(self) -> None:
        ...
        """
        self.logger.info(
            f"{self.name} - Fetching {self.stream.event_type} events starting at: {self.stream.last_event_id}"
        )
        for event in self.stream.listenForEvents():
            ...
            if event.event == "events":
                ...
                list(
                    map(lambda event: self.event_forwarder.write(event, self.ent_name), mra_events)
                )
