...
class Reporting(NodeState):
            ...
            if changes:
                ...
                try:
                    ...
                    try:
                        ...
                        logger.exception(e)
                    self._cancel_tasks()
                    await Disconnected(self._context).establish_session()
                    return
    ...
    async def _recieve_host_messages(self):
        while True:
            ...
            if not message.payload.to_dict()["online"]:
                logger.info(f"Detected host is connected, stopped data sending")
                self._cancel_tasks()
    ...
    async def _rebirth(self):
        await self._context.scanner.stop()
        birth = await self._context.scanner.start()
        logger.debug(f"Found {len(birth)} root metrics for birth certificate")
        await self._context.carrier.send_birth(birth)
        logger.info(f"Sent birth certificate to MQTT server")
...
class Connected(NodeState):
    ...
    async def _recieve_host_messages(self):
        while True:
            ...
            if message.payload.to_dict()["online"]:
                logger.info("The host application is REPORTING!!")
                self._cancel_tasks()
                Reporting(self._context)
                return
