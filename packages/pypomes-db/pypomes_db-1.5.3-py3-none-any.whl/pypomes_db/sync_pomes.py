...
def db_sync_data(errors: list[str] | None,
    ...
    :param sync_columns: the columns to be synchronized
    ...
    if curr_source_conn and curr_target_conn:
        ...
        _log(logger=logger,
             engine=source_engine,
             stmt=(f"Started synchronizing data, "
        ...
        try:
            ...
            while source_rows or target_rows:
                ...
                if log_count % log_step == 0:
                    _log(logger=logger,
                         engine=source_engine,
                         stmt=(f"Synchronizing {log_count} tuples, "
                                  f"from {source_engine}.{source_table} "
                                  f"to {target_engine}.{target_table}"))
            # close the cursors
            source_cursor.close()
            ...
            if not op_errors and rows_for_delete:
        ...
        _log(logger=logger,
             ...
             err_msg=err_msg,
             errors=op_errors,
             stmt=(f"Synchronized {log_count} tuples, "
                      f"from {source_engine}.{source_table} "
                      f"to {target_engine}.{target_table}"))
