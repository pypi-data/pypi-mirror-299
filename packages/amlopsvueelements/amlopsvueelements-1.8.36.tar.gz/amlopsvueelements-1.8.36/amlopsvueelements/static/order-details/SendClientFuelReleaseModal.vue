          ...
          <OrderForm add-default-classes>
            <template #header>
              <div class="header w-full flex justify-between">
                <div class="text-[1.25rem] font-medium text-grey-1000">Send Client Fuel Release</div>
            ...
            <template #content>
                <div class="form-body-wrapper">
                  ...
                  <TextareaField class="w-full" v-model="body" :is-validation-dirty="v$?.form?.$dirty"
                    :errors="v$?.form?.jobs?.job_title?.$errors" label-text="Body Text"
                    placeholder="Please enter body text" />
                  <TextareaField class="w-full" v-model="clientNote" :is-validation-dirty="v$?.form?.$dirty"
                    :errors="v$?.form?.jobs?.job_title?.$errors" label-text="Client Note"
                  ...
                  <div class="flex items-center justify-start pb-3">
                    ...
                    <p class="text-base whitespace-nowrap font-semibold text-main">AML Fuel Release for 84-0001</p>
                  </div>
                </div>
            </template>
          </OrderForm>
