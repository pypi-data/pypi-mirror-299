      ...
      <template #content class="grid-cols-1" v-if="isFirstStep">
        ...
        <div class="content-wrap flex flex-col">
          ...
          <SelectField class="w-11/12" v-model="formModel.currency" label-text="Currency"
            placeholder="Please select currency" :disabled="!meta && !isAdmin" @search="onSearchCurrencies"
...
} = useFetch<any[], (search: string, pageNumber: number) => void>(async (search: string, pageNumber: number) => {
  const { currencies: res, meta } = await OrderReferences.fetchCurrencies(search, pageNumber)
  currencies.value = [...currencies.value, ...res];
  if (meta.pagination.page !== meta.pagination.pages) {
    fetchCurrencies(search, ++meta.pagination.page);
  }
})
...
const onSearchCurrencies = (event: string) =>
  debounce(async () => {
    if (!event) return
    await fetchCurrencies(event, 1)
  })
