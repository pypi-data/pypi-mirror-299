<template>
  <div class="w-full flex justify-center">
    <div class="w-full flex flex-col h-screen justify-between">
      ...
      <div class="pb-[1.5rem] pl-[1.5rem] pr-[1.5rem] w-full max-w-screen-lg">
        <div class="w-full flex flex-row-reverse items-center justify-between">
          ...
          <Button v-if="!orderStore.isFirstStep" :class="[$style['ops-page-wrapper__btn']]" :loading="isLoading"
            :disabled="!orderFormStore.validateSecondStep()" @click="onValidate">
...
watch(() => orderForm.value.type, (newVal: any) => {
...
.ops {
  &-page-wrapper {
    ...
    &__btn {
      @apply flex shrink-0 focus:shadow-none text-white bg-grey-900 mb-0 mt-2 p-2 px-4 border-transparent rounded-xl #{!important};
    ...
    &__go-back {
      @apply flex shrink-0 focus:shadow-none text-grey-900 bg-grey-75 mb-0 mt-2 p-2 px-4 border-transparent #{!important};
    }
