...
import type { IClient, ICode, ICurrency, IGroundHandler, IOperator, IOrderStatus, IOrderType } from '@/types/order/order.types'
...
class OrderReferenceService extends Api {
  ...
  async fetchCurrencies(search: string, pageNumber?: number) {
    try {
      const { data: { results, meta } } = await this.get<IPaginatedResponse<ICurrency[]>>('api/v1/currencies/',
        {
          params: {
            search,
            'page[size]': 98,
      ...
      let currencies = results.map(currency => {
        return {
          ...currency, display_name: `${currency.name} (${currency.code})`
        }
      })
