...
class TriggerControllerUpac96(TriggerController):
    ...
    def trigger_values(self, trigger_values: int | list[int] | int) -> None:
        trigger_values = self._convert_trigger_values(trigger_values)
        self.write_triggers(trigger_values=trigger_values)
        self.board.trigger_values = [v for v in trigger_values.values()]
    ...
    def _convert_trigger_values(
        ...
        if isinstance(trigger_values, int):
            ...
            trigger_val = {c: v for c, v in trigger_values.items()}
    ...
    def _set_trigger_thresholds(
        ...
        if trigger_values is None:
            trigger_val = self._convert_trigger_values(self.board.trigger_values)
        else:
            self._validate_trigger_dict_or_raise(trigger_values)
            prev_channels = {c: v for c, v in enumerate(self.board.trigger_values)}
    ...
    def set_coincidence_trigger_enabled(self, chips: dict[int, bool]):
        ...
        self.board.params.get("trigger", {})["coincidence_enabled"] = chips
    ...
    def _validate_trigger_dict_or_raise(self, trigger_dict: dict[int, int]):
        ...
        for channel, threshold in trigger_dict.items():
            ...
            if threshold > self._max_threshold or threshold < self._min_threshold:
                raise ValueError(
                    f"Threshold {threshold} is out of bounds ({self._min_threshold}, {self._max_threshold})"
                )
