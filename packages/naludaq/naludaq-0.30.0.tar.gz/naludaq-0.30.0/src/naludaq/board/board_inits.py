...
from naludaq.controllers import get_dac_controller, get_trigger_controller
from naludaq.controllers.si5341_controller import Si5341Controller
...
class BoardInits:
    ...
    def init_ASoC(self) -> bool:
        ...
        Si5341Controller(self.board).program(
            filename=self.board.params["default_si5341"]
        )
        time.sleep(0.25)
        ...
        self.analogStartup()
        get_dac_controller(self.board).set_dacs(self.board.params["ext_dac"])

        sendI2cCommand(self.board, "30", ["05"])  # tempSensor to temp reg
        sendI2cCommand(self.board, "D0", ["00111011"])  # set up current sense ADC
        get_trigger_controller(self.board).trigger_values = 1350
    ...
    def init_ASoCv3(self) -> bool:
        ...
        for chan, dac_val in ext_dac_val["channels"].items():
            get_dac_controller(self.board).set_single_dac(
                chan, dac_val
    ...
    def init_AARDVARCv2(self) -> bool:
        ...
        ext_dac_val = self.board.params.get("ext_dac", {})
        for chan, dac_val in ext_dac_val["channels"].items():
            get_dac_controller(self.board).set_single_dac(chan, dac_val)
