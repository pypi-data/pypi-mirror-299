...
import threading
import time
from typing import IO, TYPE_CHECKING, Any, BinaryIO, Callable, Dict, List, Mapping, Optional, Sequence, Union, cast
...
class S3Object:
    def __init__(self, bucket: str, key: str) -> None:
        ...
        self._method: str = ""
        self._errfileobj: Optional[IO[bytes]] = None
...
def _curl_handle_for_s3(
    s3: "S3Client", method: str, s3object: S3Object, extra_params: Optional[Mapping[str, Union[str, int]]] = None
) -> pycurl.Curl:
    if extra_params is None:
        extra_params = {}
    s3object._method = method
    params: Dict[str, Union[str, int]] = {"Bucket": s3object.bucket, "Key": s3object.key, **extra_params}
...
def set_s3_timeout(
    c: pycurl.Curl,
    s3object: S3Object,
    timeout_seconds_per_kilobyte: Optional[float],
...
def get_objects(
    ...
    timeout_seconds_per_kilobyte: Optional[float] = None,
...
def get_object(
    ...
    get_objects(s3, [s3object], timeout_seconds_per_kilobyte, timeout_min_seconds)
    if s3object.error is not None:
        raise s3object.error
...
def upload_parts(s3: "S3Client", s3object: S3Object, parts: Sequence[UploadPart]) -> Dict[int, str]:
    ...
    def curl_handle_func(_s3object: S3Object) -> pycurl.Curl:
        ...
        params: Dict[str, Union[int, str]] = {"UploadId": part.upload_id, "PartNumber": part.number}
