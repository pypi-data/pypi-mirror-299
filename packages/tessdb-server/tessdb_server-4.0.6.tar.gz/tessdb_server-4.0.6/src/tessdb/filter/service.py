...
class FilterService(Service):
    ...
    def startService(self):
        log.info(
            "Starting Filtering Service with depth = {depth}", depth=self.depth)
        if not self.enabled:
            log.warn(
    ...
    def isSequenceMonotonic(self, aList):
        ...
        second_diff = [abs(first_diff[i+1] - first_diff[i])
                       for i in xrange(len(first_diff)-1)]
    ...
    def doFilter(self, new_sample):
        ...
        if len(fifo) < self.depth//2:   # Avoid loosing the past half window when initializing the filter
            ...
            log.debug("{log_tag}: seqList = {s}. magList = {m}",
                      s=seqList, m=magList, log_tag=new_sample['name'])
