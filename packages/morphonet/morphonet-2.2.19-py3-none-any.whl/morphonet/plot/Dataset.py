...
from morphonet.tools import mkdir, printv, _add_line_in_file, _read_last_line_in_file, \
    ...
    start_init_raw, get_image_at, set_image_at, compute_scikit_region, rm, resize_voxel_size,axis_ratio, \
...
class Dataset():
    ...
    def __init__(self, parent, begin=0, end=0, raw=None, segment=None, raw_dict=None, seg_dict=None, log=True,
        ...
        self.additional_regionprops_name=['volume_real','volume_bbox','volume_filled','axis_major_length','axis_minor_length','equivalent_diameter_area','euler_number','extent','intensity_max','intensity_mean','intensity_min','axis_ratio'] #Additional region asked by the user
    ...
    def _set_last(self, t):
        ...
        if t not in self.seg_datas:
            ...
            #on first time loaded: check how many we will be able to load in predicted available memory
            if self.max_times_loaded == -1 and data_size != 0:
                self.max_times_loaded = int(floor(self.allowed_memory / data_size))
                if self.max_times_loaded <= 0:
                    self.max_times_loaded = 1
            if len(self.lasT) > self.max_times_loaded and self.max_times_loaded != -1:
    ...
    def ask_regionprop(self,property,channel=0): #TODO now we only compute properties for channel 0 (Change firt in unity the cell-channel system)
        '''
    ...
    def compute_voxel_regionprops_at(self, t, channel,voxel=True,filename=None):
        ...
        if not voxel:
            data = resize_voxel_size(data, self.get_voxel_size(t, channel), order=0)
        ...
        if need_raw:
            ...
            if not voxel:
                raw_data = resize_voxel_size(raw_data, self.get_voxel_size(t, channel))

        region = compute_scikit_region(data, raw_data, t, channel, background=self.background)
