...
class Prope(MorphoPlugin):
    ...
    The user selects or applies a label to the objects to propagate at the current time (called source objects).
    The plugin can be executed Forward (source objects are propagated to the next time point) or Backward (source
    objects propagated to the previous time point). The  barycenter of the source objects are used as seeds into empty
    areas (e.q. the segmentation images with background value at 0) at the next or previous time point, and then a
    watershed is computed on the intensity images. The watershed will be applied only if the seeds are not intersecting
    another label of the image. If a seed ends up in another label, then it is deleted.
