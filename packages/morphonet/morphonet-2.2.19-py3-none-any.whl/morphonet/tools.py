...
def resize_voxel_size(im,voxel_size,order=None):
    if im is None : return None
    if voxel_size[2] == voxel_size[0]: return im
    from skimage.transform import resize
    if voxel_size[0]!=1: voxel_size=[1,1,voxel_size[2]/voxel_size[0]]
    img_shape=[im.shape[0],im.shape[1],floor(im.shape[2]*voxel_size[2])]
    if order is not None:
        return resize(im, img_shape, preserve_range=True, order=order).astype(im.dtype)
    return resize(im, img_shape, preserve_range=True).astype(im.dtype)



def compute_scikit_region(data,raw_data,t,channel,background=0):
    ...
    if raw_data is not None:
        ...
        if raw_data.shape != data.shape:
            ...
            region = regionprops(data, intensity_image=raw_data)
    else:
                ...
                if not self.channel in self.dataset.regions[self.t]: #Not already Computed ?
                    ...
                    if raw_data is not None:
                        ...
                        if raw_data.shape!=data.shape:
                            ...
                            raw_data = resize_voxel_size(raw_data, self.dataset.get_voxel_size(self.t,self.channel))
                            data = resize_voxel_size(data, self.dataset.get_voxel_size(self.t,self.channel), order=0)
...
class calcul_regionprops_thread_from_data(Thread):
    ...
    def run(self):  # START Compute regions
            ...
            self.sr = save_properties_thread_from_data(self.t, self.filename,self.regionprops_name,rprops)
            self.sr.start()
...
class save_properties_thread(Thread):
    ...
    def run(self):  # START Save properties
        ...
        if self.t in self.dataset.regions and self.channel in self.dataset.regions[self.t]: #Clear the Regions prop
            del self.dataset.regions[self.t][self.channel]
