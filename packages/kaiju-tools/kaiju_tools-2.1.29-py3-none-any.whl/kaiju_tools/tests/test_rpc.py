...
class TestRPCServer:
    ...
    @pytest.mark.profile
    @pytest.mark.parametrize('body', [{'id': 0, 'method': 'do.echo', 'params': {'test': True}}], ids=['default'])
    async def test_profile_performance(self, rpc, body):
        requests, parallel, n = 20000, 8, 10
        counter = 0
        ...
        class _Service(AbstractRPCCompatible):
            ...
            def routes(self) -> dict:
                return {'echo': self.do_echo}

            async def do_echo(self, data):
                return data
