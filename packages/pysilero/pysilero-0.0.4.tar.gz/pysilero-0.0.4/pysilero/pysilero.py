...
class VADIterator(SileroVAD):
    def __init__(self, version: str = "v5", threshold: float = 0.5):
        """
        ...
        for frame_start, frame_end, frame in self.add_chunk(chunk, last):
            ...
            if speech_prob >= self.threshold:
                ...
                if not self.triggered:
                    ...
                    speech_dict = {"start": speech_start}
                    yield speech_dict, self.queue.get_frame(True)
                ...
                if frame_end - self.temp_end >= self.min_silence_samples:
                    ...
                    self.temp_end = 0
                    self.triggered = False
                    speech_dict = {"end": speech_end, "segment": self.segment}
                    yield speech_dict, self.queue.get_frame()
                    self.segment += 1
