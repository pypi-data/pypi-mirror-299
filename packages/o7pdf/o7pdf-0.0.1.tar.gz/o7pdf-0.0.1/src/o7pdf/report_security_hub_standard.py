...
class ReportSecurityHubStandard(Template):
    ...
    def checks_historic_charts(self, data: ReportCompiledData):
        ...
        chart.axis["left"].min = 0
        chart.axis["left"].max = (int(chart_max / 1000) + 1) * 1000
        chart.param.spacing = 0.60
        chart.param.x_label_step = 4
        chart.generate()
    ...
    def accounts_section(self, data: ReportCompiledData):
        ...
        columns = [
            pdt.ColumnParam(
                name="Id", width=20, data="AwsAccountId", align="L", merge=False
            ),
            pdt.ColumnParam(
                name="Account Name",
                width=30,
        ...
        pdt.PandasTable(
            df=data.df_accounts, columns=columns, pdf=self, font_size=6
        ).generate()
    ...
    def controls_section(self, data: ReportCompiledData):
        ...
        columns = [
            ...
            pdt.ColumnParam(
                name="Failed Check", width=20, data="Failed_Check", align="C"
            ),
        ]

        pdt.PandasTable(
            df=data.df_controls, columns=columns, pdf=self, font_size=6
        ).generate()
    ...
    def top_findings_section(self, data: ReportCompiledData):
        ...
        columns = [
            ...
            pdt.ColumnParam(
                name="Note", width=30, data="WorkflowNote", align="L", text_trim=True
            ),
            pdt.ColumnParam(
                name="Res Type", width=20, data="ResType", align="L", text_trim=True
            ),
            pdt.ColumnParam(
                name="Res Name", width=30, data="ResName", align="L", text_trim=True
            ),
            pdt.ColumnParam(
                name="Title",
                width=80,
        ...
        pdt.PandasTable(
            df=data.df_findings_top, columns=columns, pdf=self, font_size=6
        ).generate()
    ...
    def suppressed_section(self, data: ReportCompiledData):
        ...
        gb_controls = data.df_suppressed.sort_values(
            by=["ControlId", "AccountName"]
        ).groupby("ControlId")

        for control_id, df in gb_controls:
            ...
            remediation = (
                eval(remediation) if isinstance(remediation, str) else remediation
            )
            control_url = (
            ...
            columns = [
                pdt.ColumnParam(
                    name="Account Name", width=30, data="AccountName", align="L"
                ),
    ...
    # *************************************************
    def compile_data(
        self, dfs: dict[pd.DataFrame], standard_arn: str
    ) -> ReportCompiledData:
        ...
        df_controls_gb["Pass"] = (
            df_controls_gb["CheckPass"] == df_controls_gb["CheckCount"]
        )
        ...
        df_controls_gb["SeverityColor"] = df_controls_gb["SeverityRating"].map(
            severity_mapping
        )
        ...
        print(
            f"Total Controls: {ctrl_total} - Total Pass: {ctrl_total_pass} - Score: {ctrl_score}"
        )
        ...
        # list the top XX findings
        df_top_findings = (
            df_findings.sort_values(
                by=["SeverityN", "WorkflowStatus"], ascending=[False, True]
            )
            .head(40)
            .copy()
        )
        df_top_findings["SeverityColor"] = df_top_findings["SeverityL"].map(
            severity_mapping
        )
        ...
        df_findings_summary["SeverityColor"] = df_findings_summary[
            "SeverityRating"
        ].map(severity_mapping)
        df_findings_summary["TextColor"] = df_findings_summary["SeverityColor"].apply(
            lambda x: self.SECTION_TITLE_FG
        )
        df_findings_summary["SeverityOrder"] = df_findings_summary[
            "SeverityRating"
        ].apply(lambda x: ["CRITICAL", "HIGH", "MEDIUM", "LOW"].index(x))
        ...
        # --------------------------------------
        if "history" in dfs:
            gb_history = dfs["history"].groupby("date")
            df_history = gb_history.agg(
                {"critical": "sum", "high": "sum", "medium": "sum", "low": "sum"}
            )

            print(f"Found {len(df_history.index)} historical data")
    ...
    def generate(self, dfs: dict[pd.DataFrame], standard_arn: str):
        ...
        # print(f'L Marging: {self.l_margin} - R Margin: {self.r_margin} - T Margin: {self.t_margin} - B Margin: {self.b_margin}')
