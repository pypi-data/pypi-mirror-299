...
def raise_for_status(response: Response) -> Response:
    ...
    if response.headers.get("deprecation", False) == "true":  # TODO: NIM-16022
        version = response.url.path.split("/")[2]
        logger.warning(
            f"API endpoint '{response.url.path}' is deprecated. If your IP Fabric version equals '{version}' then the "
            f"Endpoint will be removed in the next major release."
        )
...
def validate_ip_network(ip: Union[str, VALID_IP], max_size: Optional[int] = None) -> IPv4Network:
    if isinstance(ip, str):
        ...
        ip = IPv4Network(ip, strict=False)
    if max_size and ip.prefixlen < max_size:
