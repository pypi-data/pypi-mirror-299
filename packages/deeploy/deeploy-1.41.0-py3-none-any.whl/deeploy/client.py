...
class Client(object):
    ...
    def validate_personal_key(func):
        def _validate(self, *args, **kwargs):
            if not (self.__config.access_key and self.__config.secret_key):
                raise Exception(
                    "access_key and secret_key not found, please define them when initiating the client."
                )
    ...
    def create_external_deployment(
        ...
        if ("repository_id" in options) and ("branch_name" in options):
            options = self.__check_local_git_config(local_repository_path, options)
        elif ("repository_id" not in options) and ("branch_name" not in options) is None:
            pass
        else:
            raise Exception("Both repository ID and branch name not provided. Either set them both null or provide both.")
    ...
    def update_external_deployment(
        ...
        if ("repository_id" in options and options["repository_id"] is not None) or ("branch_name" in options and options["branch_name"] is not None):
            options = self.__check_local_git_config(local_repository_path, options)
        elif ("repository_id" not in options or ("repository_id" in options and options["repository_id"] is None)) and ("branch_name" not in options or ("branch_name" in options and options["branch_name"] is None)):
            pass
        else:
            raise Exception("Both repository ID and branch name not provided. Either set them both null or provide both.")
