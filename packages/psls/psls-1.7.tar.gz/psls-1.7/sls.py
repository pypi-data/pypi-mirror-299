...
def gen_up(starID, numax,dt,T, mag , delta_nu =  -1. , mass = -1. , seed = None, pn_ref = 7.7, wn_ref= 0.,
    ...
    # mean power spectrum [ppm2/Hz]
    mpsd =   (UPm * (b2m**2) * ( (oscillation == True)*1.)  + GS  + GSf  + AS + ASf ) * 1e6 *(np.sinc(0.5*f/nyq))**2   +  2.*W**2   #
...
def GenLC(Data,seed=None,time_shift=0.,verbose=False,time_contraction=1.):
    '''
...
def SimSystematicErrorV2(Sampling,N,DataAllCamera,camera,time_shift=0.,Verbose=False,
    ...
    Nquarter = int(math.ceil(duration/90.))
    if(Verbose):
    ...
    for q in range(Nquarter):
        ...
        lc_q,lcrn_q,rawlc,rawlcrn,rawlcvar_q,t_q,Tupd_q,flag_q,LCmean,RawLCmean = GenLC(DataAllCamera[j],seed=seeds[q],time_shift=time_shift,verbose=Verbose,time_contraction=TimeContractionFactor)
        n = len(lc_q)//nc
        if(AddRandomNoise):
            lc_q = lcrn_q
        ...
        lc_q = rebin1d(lc_q,n)
        rawlcvar_q =  rebin1d(rawlcvar_q,n)
        flag_q = rebin1d(flag_q,n)
...
def SimSystematicError(Sampling,N,Data,camera,seed=None,time_shift=0.,version=2,Verbose=False):

    if(version==2):
        return SimSystematicErrorV2(Sampling,N,Data,camera,time_shift=time_shift,
