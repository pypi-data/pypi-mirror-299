...
def extract_pairwise_connections(connections):
    ...
    def chunk_data_pairs(data, chunk_size):
        """Helper function to chunk data into smaller parts."""
    ...
    def process_sublist_pairs(connections):
        ...
        pairs_within_sublist = set(map(tuple, pairs_within_sublist))

        pairs_within_sublist = [list(pair) for pair in pairs_within_sublist]

        return pairs_within_sublist

    pairwise_connections = []
    num_cpus = mp.cpu_count()  # Get the number of CPUs available

    # Define the chunk size
    chunk_size = max(1, len(connections) // num_cpus)

    # Chunk the data
    connection_chunks = list(chunk_data_pairs(connections, chunk_size))
...
def break_and_label_skeleton(skeleton, branch_removal = 0, directory = None):
    ...
    verts = label_vertices(skeleton, branch_removal = branch_removal)
...
def label_branches(array, branch_removal = None, down_factor = None, directory = None):
    """
    ...
    :param branch_removal: (Optional, Val = None; int) - An optional into to specify what size of pixels to remove branches. Use this if the skeleton is branchy and you want to remove the branches from the larger filaments. Note this requires extra processing.
    ...
    other_array = break_and_label_skeleton(other_array, branch_removal)
...
def label_vertices(array, peaks = 1, branch_removal = 0, down_factor = 0, directory = None):
    """
    ...
    :param array: (Mandatory, string or ndarray) - If string, a path to a tif file to label. Note that the ndarray alternative is for internal use mainly and will not save its output.
    :param branch_removal: (Optional, Val = 0; int) - An optional into to specify what size of pixels to remove branches. Use this if the skeleton is branchy and you want to remove the branches from the larger filaments. Note this requires extra processing.
    :param directory: (Optional - Val = None; string) - A filepath to save outputs.
...
class Network_3D:
    ...
    def calculate_node_centroids(self, down_factor = None):
    ...
    def calculate_edges(self, binary_edges, diledge = None, inners = True, hash_inner_edges = True, search = None, remove_edgetrunk = 0, GPU = True, fast_dil = False):
        """
    ...
    def calculate_all(self, nodes, edges, xy_scale = 1, z_scale = 1, down_factor = None, search = None, diledge = None, inners = True, hash_inners = True, remove_trunk = 0, ignore_search_region = False, other_nodes = None, label_nodes = True, directory = None, GPU = True, fast_dil = False):
        """
        ...
        self.calculate_edges(edges, diledge = diledge, inners = inners, hash_inner_edges = hash_inners, search = search, remove_edgetrunk = remove_trunk, GPU = GPU, fast_dil = fast_dil)
        del edges
        self.save_edges(directory)
    ...
    def draw_network(self, directory = None, down_factor = None):
        """
