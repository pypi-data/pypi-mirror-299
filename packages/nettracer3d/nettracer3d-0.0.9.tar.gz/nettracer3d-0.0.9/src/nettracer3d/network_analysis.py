...
def compute_centroid(binary_stack, label):
    ...
    if indices.shape[0] == 0:
        print(f"Missing centroid for label {label}")
        return None
    else:
...
def _find_centroids(nodes, node_list=None, down_factor=None):
    ...
    # Prepare list of node IDs to process
    if node_list is None:
        node_max = np.max(nodes)
        node_list = range(1, node_max + 1)

    # Parallel computation of centroids
    with ThreadPoolExecutor(max_workers = mp.cpu_count()) as executor:
        futures = {executor.submit(compute_centroid_for_label, nodeid, nodes): nodeid for nodeid in node_list}
        for future in as_completed(futures):
            nodeid = futures[future]
            centroid = future.result()
            if centroid[1] is not None:
                centroid_dict[centroid[0]] = centroid[1]

    return centroid_dict

def compute_centroid_for_label(nodeid, nodes):
    return nodeid, compute_centroid(nodes, nodeid)
