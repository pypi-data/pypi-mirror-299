...
def load_from_structure(input_file, id, smiles, output_file):
    ...
    data_csi = pkgutil.get_data("mlinvitrotox", "data/csi_fingerid.tsv")
    print(type(data_csi))
    df_csi = pd.read_csv(io.BytesIO(data_csi), sep="\t")
    #sirius_fps_df_path = "./src/mlinvitrotox/data/csi_fingerid.tsv"
