...
"""
            ...
            if not arguments:
                ...
                response = CallOllama.getSingleChatResponse(userInput, [{'role': 'assistant', 'content': '```python\n'}], stop=["```"]).replace(r"\\n", "\n")
    ...
    @staticmethod
    @check_ollama_errors
    def getSingleChatResponse(userInput: str, messages: list=[], temperature: Optional[float]=None, num_ctx: Optional[int]=None, num_batch: Optional[int]=None, num_predict: Optional[int]=None, model: Optional[str]=None, stop: Optional[list]=None):
...
Remember, response with the required python code ONLY, WITHOUT extra notes or explanations."""
            ...
            code = CallOllama.getSingleChatResponse(code_instruction, ongoingMessages[:-1] + [{'role': 'assistant', 'content': '```python\n'}], stop=["```"]).replace(r"\\n", "\n")
