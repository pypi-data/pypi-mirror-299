...
```"""
    ...
    @staticmethod
    @check_llm_errors
    def getSingleChatResponse(userInput, messages=[], temperature: Optional[float]=None, max_tokens: Optional[int]=None, stop: Optional[str]=None):
        """
    ...
    def regularCall(messages: dict, temperature: Optional[float]=None, max_tokens: Optional[int]=None, chat_model: Optional[str]=None):
        ...
        return getGroqClient().chat.completions.create(
            model=chat_model if chat_model is not None else config.groqApi_tool_model,
            messages=chatMessages,
            n=1,
            temperature=temperature if temperature is not None else config.llmTemperature,
            max_tokens=max_tokens if max_tokens is not None else config.groqApi_tool_model_max_tokens,
...
Remember, response with the required python code ONLY, WITHOUT extra notes or explanations."""
            ...
            code = CallGroq.getSingleChatResponse(code_instruction, ongoingMessages[:-1] + [{'role': 'assistant', 'content': '```python\n'}], temperature, max_tokens, stop="```").replace(r"\\n", "\n")
