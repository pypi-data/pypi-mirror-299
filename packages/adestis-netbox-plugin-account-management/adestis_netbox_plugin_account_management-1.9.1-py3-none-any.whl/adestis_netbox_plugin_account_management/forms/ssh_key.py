...
class SshKeyForm(NetBoxModelForm):
    ...
    fieldsets = (
        (None, ('raw_ssh_key', 'contact',
                'ssh_key_current_status', 'ssh_key_desired_status','tags')),
        ('Validity', ('valid_from', 'valid_to')),
    )

    class Meta:
        model = SshKey
        fields = ['raw_ssh_key', 'key_comment', 'key_type', 'encoded_key', 'contact',  'valid_from', 'valid_to',
                  'ssh_key_current_status','ssh_key_desired_status', 'comments', 'tags']
        widgets = {
...
class SshKeyBulkEditForm(NetBoxModelBulkEditForm):
    ...
    ssh_key_current_status = forms.ChoiceField(
        required=False,
        choices=SshKeyStatusChoices,
        label=_('Status')
    )

    ssh_key_desired_status = forms.ChoiceField(
        required=False,
        choices=SshKeyStatusChoices,
        label=_('Status')
    )
...
class SshKeyFilterForm(ContactModelFilterForm, NetBoxModelFilterSetForm):
    ...
    fieldsets = (
        (None, ('q', 'index', 'tag')),
        ('SSH Key', ('contact', 'raw_ssh_key', 'key_comment', 'key_type', 'ssh_key_current_status','ssh_key_desired_status')),
    )

    key_comment = forms.CharField(
        required=False,
        label=_('Comment')
    )
    ...
    ssh_key_current_status = forms.MultipleChoiceField(
        choices=SshKeyStatusChoices,
        required=False,
        label=_('Status')
    )

    ssh_key_desired_status = forms.MultipleChoiceField(
        choices=SshKeyStatusChoices,
        required=False,
        label=_('Status')
    )    
