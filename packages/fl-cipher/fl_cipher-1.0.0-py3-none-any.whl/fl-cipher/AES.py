...
# 秘钥 必须为16个字符
SECRET_KEY = "fenglingfengling"
# 初始向量/IV向量(CBC CFB OFB加密机制才需要设置)  必须为16个字符
IV = "1234567812345678"
...
class AESEncryption:
    def __init__(self, key, iv):
        ...
        self.iv = iv.encode()
        self.__allow_mode = ["ECB", "CBC", "CFB", "OFB", "CTR"]
    ...
    def _handle_encrypt_data(data):
        """
        处理待加密数据: 将传入数据转换成128位倍数的二进制数据
        """
        if isinstance(data, str):
            data = data.encode()

        pading_size = AES.block_size - len(data) % AES.block_size
    ...
    def _handle_decrypt_data(data):
        """
        处理待解密数据: 将传入128位的二进制数据转换成原始字符串数据
        """
        padding_size = data[-1]
...
if __name__ == '__main__':
    ...
    aes = AESEncryption(SECRET_KEY, IV)

    encrypted_data = aes.encrypt(ready_data, "CTR")
    print(f"【密文】: {encrypted_data.decode()}")
    decrypted_data = aes.decrypt(encrypted_data, "CTR")
