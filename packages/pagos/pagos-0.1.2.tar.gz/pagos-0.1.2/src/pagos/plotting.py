    ...
    for i, v in enumerate(paramnames):  # for each column/parameter
        for j, q in enumerate(to_plot[v]):  # for each entry
            ...
            # parse and plot data
                ...
                if isinstance(q, (Variable, AffineScalarFunc, u.Quantity)):
                    x = q.nominal_value
                    xerr = q.std_dev
                else:
                    x = q
        ...
        display_unit = str(to_plot[v][0].units) # TODO this could be bad if somehow the array has different units for different entries. To look at later...
