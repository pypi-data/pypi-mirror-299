...
from pagos.core import u as _u, Q as _Q
...
def gas_exchange_model(func, instance:object, args, kwargs):
    ...
    if 'fit_mode' in kwargs:
        if kwargs['fit_mode'] == True:
            fit_mode = True
            #print('success!')
    # number handling (deals with non-Quantity type arguments)
    """for a in args:
        if type(a) not in [_u.Quantity, Variable] and isinstance(a, numbers.Number):
            a = ufloat(a, 0)
    for k in kwargs:
        a = kwargs[k]
        if type(a) not in [_u.Quantity, Variable] and isinstance(a, numbers.Number):
...
def fitmodel(modelfunc:Callable, data:pd.DataFrame, to_fit:Iterable[str], init_guess:Iterable[float], tracers_used:Iterable[str], arg_tracer_labels:dict=None, constraints:dict=None, **kwargs) -> pd.DataFrame:   # TODO init_guess is currently only a 1D list, perhaps should be allowed to take a second dimension the same length as data?
    ...
    # input to objective function: all parameters (fitted and set), tracers to calculate, observed error and their errors
    def objfunc(parameters, tracers, observed_data, observed_errors, parameter_units):
        ...
        modelled_data = modelfunc(tracers, **paramsdict, fit_mode=True)
    ...
    for r in nrows:
        ...
        if arg_tracer_labels == None:
            arg_tracer_labels == tracers_used
        obs_tracerdata_in_row = np.array([data[arg_tracer_labels[t]][r].nominal_value for t in tracers_used])
        obs_tracerdata_errs_in_row = np.array([data[arg_tracer_labels[t]][r].std_dev for t in tracers_used])
        M = minimize(objfunc, all_params, args=(tracers_used, obs_tracerdata_in_row, obs_tracerdata_errs_in_row, param_units), method='leastsq', nan_policy='omit')
