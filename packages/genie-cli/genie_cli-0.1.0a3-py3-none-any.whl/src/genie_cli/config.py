...
class ServiceChatModel(BaseModel):
...
def get_builtin_openai_models() -> list[ServiceChatModel]:
    return [
        ServiceChatModel(
...
def get_builtin_anthropic_models() -> list[ServiceChatModel]:
    return [
        ServiceChatModel(
...
def get_builtin_google_models() -> list[ServiceChatModel]:
    return [
        ServiceChatModel(
...
def get_builtin_models() -> list[ServiceChatModel]:
    return (
...
class LaunchConfig(BaseModel):
    ...
    system_prompt: str = Field(
        default=os.getenv(
            "ELIA_SYSTEM_PROMPT", "You are a helpful assistant named Genie."
        )
    )

    current_state: str = Field(default="scafoldding")
    message_code_theme: str = Field(default="monokai")
    """The default Pygments syntax highlighting theme to be used in chatboxes."""
    models: list[ServiceChatModel] = Field(default_factory=list)
    builtin_models: list[ServiceChatModel] = Field(
        default_factory=get_builtin_models, init=False
    )
    # service_path: str = Field(default="/Users/tron/repos/valory/genie/price-oracle")
    # skill_path: str = Field(
    #     default="/Users/tron/repos/valory/genie/price-oracle/packages/valory/skills/oracle_deployment_abci"
    # )

    @property
    def all_models(self) -> list[ServiceChatModel]:
        return self.models + self.builtin_models

    @property
    def default_model_object(self) -> ServiceChatModel:
