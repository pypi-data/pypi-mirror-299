...
def upload_annotations(path: Optional[Path] = None):
    ...
    if not path:
        path = (
            inquirer.filepath(message=messages.ANNOTATION_FOLDER_MESSAGE)
            .execute()
            .strip()
        )
...
def download_annotations(
    ...
    if not path:
        path = (
            inquirer.filepath(
                message=messages.EXPORT_ANNOTATION_FOLDER_MESSAGE, default=os.getcwd()
            )
            .execute()
            .strip()
        )
...
def assets_group(group: Optional[str] = None):
    ...
    if not group:
        ...
        group = inquirer.select(
            messages.CHOOSE_GROUP_MESSAGE, choices=groups, border=True
        ).execute()
