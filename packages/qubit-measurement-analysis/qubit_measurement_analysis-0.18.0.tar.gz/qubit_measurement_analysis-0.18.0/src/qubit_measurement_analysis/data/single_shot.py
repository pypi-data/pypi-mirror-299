...
class SingleShot:
    ...
    def __copy__(self):
        return type(self)(
            self.value, self.state_regs, self._is_demodulated, self.device
        )
    ...
    def update_state_regs(self, updated_elements: dict):
        self._state_regs.update(updated_elements)
    ...
    def whittaker_eilers_smoother(self, lamb, d):
        ...
        new_instance = SingleShot(
            smoothed_value, self.state_regs, self._is_demodulated, self.device
        )
        return new_instance
    ...
    def save(
        ...
        """
        directory = os.path.join(parent_dir, self.q_registers, subfolder, self.state)
    ...
    def load(
        ...
        loaded_instance = cls(value, state_regs, is_demodulated)
