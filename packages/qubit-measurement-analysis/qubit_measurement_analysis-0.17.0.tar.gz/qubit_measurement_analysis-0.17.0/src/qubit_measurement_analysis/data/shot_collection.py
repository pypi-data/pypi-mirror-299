...
class ShotCollection:
    ...
    def __add__(self, other):
        ...
        new_shots = [
            ...
            for value, state_regs in zip(new_values, self.all_states)
        ]
    ...
    def mean(self, axis: int = -1) -> Union["SingleShot", "ShotCollection"]:
        ...
        if axis == 0:
            ...
            return SingleShot(
                self.all_values.mean(axis), state_regs, self.is_demodulated, self.device
            )
        else:
    ...
    def update_all_state_regs(self, updated_elements: dict):
        for shot in self.singleshots:
            shot._state_regs.update(updated_elements)
        self._update_states()
    ...
    def compute(self, free_all_blocks=False) -> "ShotCollection":
        ...
        new_shots = [
            SingleShot(value, state_regs, is_demodulated, self.device)
            ...
            for subfolder in ["train", "val", "test"]:
                for state in self.unique_states:
                    directory = os.path.join(
                        parent_dir, self.q_registers, subfolder, state
                    )
