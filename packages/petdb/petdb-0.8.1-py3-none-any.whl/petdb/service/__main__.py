...
DATABASES_CONF_PATH = os.path.join(STORAGE_PATH, "databases.json")
...
if not os.path.exists(DATABASES_CONF_PATH):
	with open(DATABASES_CONF_PATH, "w") as f:
		f.write("{}")
...
def init_service(port: int = DEFAULT_PORT):
	if os.path.exists(STORAGE_PATH):
		if not check_current_configuration(STORAGE_PATH):
			return
...
def run_service(port: int):
	...
	for database in os.listdir(STORAGE_PATH):
		if "pw" in os.listdir(os.path.join(STORAGE_PATH, database)):
