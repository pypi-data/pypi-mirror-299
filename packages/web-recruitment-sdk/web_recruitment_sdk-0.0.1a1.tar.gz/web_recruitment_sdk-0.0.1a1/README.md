...
The Web Recruitment SDK Python library provides convenient access to the Web Recruitment SDK REST API from any Python 3.7+
...
```sh
# install from PyPI
pip install --pre web_recruitment_sdk
```
...
account_read = client.admin.accounts.create(
    name="name",
    tenant="tenant",
    tenant_lookup_identifier="tenantLookupIdentifier",
)
print(account_read.id)
```

While you can provide an `api_key` keyword argument,
we recommend using [python-dotenv](https://pypi.org/project/python-dotenv/)
to add `WEB_RECRUITMENT_SDK_API_KEY="My API Key"` to your `.env` file
...
async def main() -> None:
    account_read = await client.admin.accounts.create(
        name="name",
        tenant="tenant",
        tenant_lookup_identifier="tenantLookupIdentifier",
    )
...
client.with_options(max_retries=5).admin.accounts.create(
    name="name",
    tenant="tenant",
    tenant_lookup_identifier="tenantLookupIdentifier",
)
...
client.with_options(timeout=5.0).admin.accounts.create(
    name="name",
    tenant="tenant",
    tenant_lookup_identifier="tenantLookupIdentifier",
)
...
You can enable logging by setting the environment variable `WEB_RECRUITMENT_SDK_LOG` to `debug`.

```shell
$ export WEB_RECRUITMENT_SDK_LOG=debug
```
...
account = response.parse()  # get the object that `admin.accounts.create()` would have returned
print(account.id)
```
...
```python
with client.admin.accounts.with_streaming_response.create(
    name="name",
    tenant="tenant",
    tenant_lookup_identifier="tenantLookupIdentifier",
) as response:
