from grid_engine import blueprint, grid, dungeon, gui
import argparse
import os
import sys
import IPython
...
parser.add_argument('-d', '--dungeon', action='store_true', default=False, help='Whether to generate a dungeon.')
parser.add_argument('-mr', '--max-rooms', dest='max_rooms', type=int, default=30, help='Maximum number of rooms in the dungeon')
parser.add_argument('-mrs', '--min-room-size', dest='min_room_size', type=int, default=6, help='Minimum room size')
parser.add_argument('-mxrs', '--max-room-size', dest='max_room_size', type=int, default=10, help='Maximum room size')
...
parser.add_argument('-A', '--animate', dest='animate', action='store_true', default=False, help='Save the grid as an animated gif')
...
if args.dungeon:
    ...
    d = dungeon.Dungeon(width=args.columns, height=args.rows, max_rooms=args.max_rooms, min_room_size=args.min_room_size, max_room_size=args.max_room_size)
    print('Success! Dungeon generated.')
    bp = d.grid.blueprint
    g = grid.Grid(gblueprint=bp, with_terrain=False)
    d.curses_dungeon()
    exit()
else:            
