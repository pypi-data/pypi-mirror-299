...
# DataIntegration
from ..components import SkipErrors
...
class Task(AbstractTask):
    ...
    async def prepare(self):
        if self._task:
            ...
            try:
                ...
                raise TaskDefinition(f"Bad Task Definition: {err!s}") from err
            ...
            raise TaskDefinition(f"DI: Invalid Task: {self._program}.{self._taskname}")
    ...
    async def start(self):
        ...
        self.logger.info(f"Task Started {self._taskname}")
        # Open Task:
        try:
            self._task = await self.taskstore.open_task(self._taskname, self._program)
            if not self._task:
                raise TaskNotFound(f"Task Missing or empty: {self._taskname}")
        ...
        try:
            ...
            self._on_exception(status="parsing error", exc=exc)
        ...
        for step in self._pile:
            ...
            try:
                ...
                if asyncio.iscoroutinefunction(run):
                    ...
                    raise TaskFailed(f"DI: Component {step_name} is not callable")
                # close operations
    ...
    def plot(self, filename: str = None) -> None:
        self._pile.plot_task(filename=filename)
