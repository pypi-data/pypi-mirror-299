...
# DI
from ..conf import default_dsn, SYSTEM_LOCALE, TASK_STORAGES, FILE_STORAGES
from ..utils.stats import TaskMonitor
from ..models import TaskState
from ..exceptions import TaskError, TaskParseError
from ..events import EventManager, LogEvent, LogError
...
class AbstractTask(ABC):
        ...
        **kwargs,
        ...
        if loop:
            ...
            try:
                ...
                raise RuntimeError("Task: No event loop running.")
        # Task ID:
        self.task_id = (
            task_id
            if task_id
    ...
    async def get_taskrow(self, table: str, conn: BaseDriver) -> Record:
        ...
        self.logger.debug(f":: Task Query: {task}")
        try:
            ...
            self.logger.exception(str(err), stack_info=False)
    ...
    def check_syntax(self, task):
        ...
        schema = {
            ...
            "properties": {
                ...
                "steps": {"type": "array", "items": {"anyOf": [{"type": "object"}]}},
            },
            "required": ["name", "steps"],
        }
        try:
