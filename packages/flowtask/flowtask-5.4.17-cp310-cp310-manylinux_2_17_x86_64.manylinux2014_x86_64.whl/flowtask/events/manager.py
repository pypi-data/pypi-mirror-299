...
class EventManager:
    ...
    class Event:
        ...
        def __call__(self, *args, **kwargs):
            ...
            #             target=self._executeTask, args=(handler, args, kwargs)
            #         )
            #         thread.start()
            #     except Exception as e:
            ...
            # Dispatch regular events within the same loop
            # for handler in self._handlers:
            #     if asyncio.iscoroutinefunction(handler):
            #         asyncio.create_task(handler(*args, event_loop=loop, **kwargs))
            #     else:
        ...
        def _executeTask(self, handler: Callable, *args, **kwargs):
            ...
            try:
                ...
                self.logger.error(f"RunTask Handler Error: {e}")
            finally:
                if _new is True:
                    loop.stop()
                    ...
                    if asyncio.iscoroutinefunction(fn) or isinstance(fn, AbstractEvent):
                        try:
                            try:
                                ...
                                self.logger.error(f"Event Error: {ex}")
                            finally:
                            ...
                            raise RuntimeError(f"Event Error: {err!s}") from err
                    else:
                        try:
                            fn(*args, **kwargs)
                        except Exception as err:
