...
class ProjectsRunner:
    ...
    async def run_project(self, basename, role, original_prompt_language, max_retries=20):
        ...
        logger.info("\n #### The `ProjectsRunner` is initiating work")
        pipeline = result["pipeline"]

        if pipeline == "1":
            return await self.run_xcode_project(basename, role)
        elif pipeline == "2":
            return await self.compile.start_CLI_compile_process("Run and open this project in the fastest way. Read the configuration file and tree structure to ensure the project runs properly. Modify anything necessary to achieve this goal, assuming all dependencies are installed.", basename, original_prompt_language)
        else:
