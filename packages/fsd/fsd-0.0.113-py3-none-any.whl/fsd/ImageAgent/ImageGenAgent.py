...
class ImageGenAgent:
    ...
    def save_and_resize_image(self, image_data: str, file_path: str, image_format: str, target_size: Tuple[int, int]):
        ...
        try:
            ...
            # Save the image
            image.save(file_path, format=image_format)
            logger.info(f"\n #### `ImageGenAgent`: saved & resized to {target_size} - {os.path.basename(file_path)}")
        except Exception as e:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url) as resp:
                    if resp.status == 200:
                        ...
                        # Save the image
                        image.save(file_path, format=image_format)
                        logger.info(f"\n #### The `ImageGenAgent` successfully saved it to {os.path.basename(file_path)}")
                    else:
    ...
    async def process_image_generation(self, steps: List[Dict]):
        ...
        for step in steps:
            try:
                ...
                logger.info(f"\n #### The `ImageGenAgent` successfully generated the image for step: {os.path.basename(step['file_path'])}")
            except Exception as e:
