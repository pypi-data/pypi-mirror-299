...
class ControllerAgent:
    ...
    async def get_lightIdea_plans(self, user_prompt):
        """Generate idea plans based on user prompt and available files."""
        return await self.short_idea.get_idea_plans(user_prompt)
    ...
    async def run_requests(self, request_list, role):
        ...
        return await self.runner.run_project(request_list, role)
    ...
    async def get_coding_requests(self, instructions, context, file_lists, context_files, role, crawl_logs):
        ...
        logger.info("\n #### The task planner has completed preparing the task.")
        is_first = True
            ...
            if self.is_coding_file(file_name):
                ...
                if main_path:
                    ...
                    try:
                        ...
                        logger.debug(f"\n #### The coding agent has finished working on `{file_name}`")
                        is_first = False
                    except Exception as e:
    ...
    async def code_format_pipeline(self, finalPrompt, role):
        ...
        if working_files:
            ...
            logger.info("\n #### The compile agent is checking for compile errors")
    ...
    async def build_and_fix_compile_error(self, file_list, role):
        ...
        await self.run_requests(file_list, role)


    async def fix_compile_error_pipeline(self,file_list, role):
        """Pipeline for fixing compile errors."""
        await self.build_and_fix_compile_error(file_list, role)
    ...
    async def regular_code_task_pipeline(self, finalPrompt, role):
        ...
        result_check = await self.planCheckAgent.get_started(finalPrompt)
        ...
        if crawl_logs:
            if isLight:
                idea_plan = await self.get_lightIdea_plans(finalPrompt + f"crawled data: {crawl_logs}")
            else:
                idea_plan = await self.get_idea_plans(finalPrompt + f"crawled data: {crawl_logs}")
        else:
            if isLight:
                idea_plan = await self.get_lightIdea_plans(finalPrompt)
            else:
        ...
        logger.info("\n #### The compile agent is checking for compile errors")
        await self.build_and_fix_compile_error(final_working_files, role)
