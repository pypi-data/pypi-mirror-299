    ...
    async def run_xcode_project(self, basename, role, max_retries=50):
        ...
        while retries < max_retries:
            ...
            try:
                ...
                logger.info("Oops! Something went wrong, the BuilderAgent will work on the fix right now.")
                if e.returncode == 70:
                    logger.info("Build failed with exit status 70 or 65. The BuilderAgent will finalize now and try again later.")
                    return list(totalfile)
                ...
                try:
                    ...
                    logger.info("FileManagerAgent is now working on file processing")
                    file_result = await self.get_file_planning(fix_plans)
                    await self.process_creation(file_result)
                    logger.info("FileManagerAgent completed processing files")

                    logger.info(f"SelfHealingAgent attempts to fix for {retries + 1} try")
                    ...
                    if http_error.response.status_code == 429:
                        wait_time = 2 ** retries
                        logger.info(f"Rate limit exceeded, ProjectsRunner retrying in {wait_time} seconds...")
