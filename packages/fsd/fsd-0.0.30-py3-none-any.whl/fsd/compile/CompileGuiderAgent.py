...
class CompileGuiderAgent:
    ...
    def initial_setup(self, user_prompt):
        ...
        logger.info("\n ### The `CompileGuiderAgent` has initialized the setup with provided instructions and context")


    def clear_conversation_history(self):
        """Clear the conversation history."""
        self.conversation_history = []
        logger.info("\n ### The `CompileGuiderAgent` has cleared the conversation history")
    ...
    def read_file_content(self, file_path):
        ...
        if not os.path.exists(file_path):
            logger.info(f"\n ### The `CompileGuiderAgent` encountered a non-existent file: {file_path}")
            return ""
        ...
        for encoding in encodings:
            try:
                with open(file_path, "r", encoding=encoding) as file:
                    logger.info(f"\n ### The `CompileGuiderAgent` successfully read the file: {file_path}")
                    return file.read()
            except UnicodeDecodeError:
                continue
            except Exception as e:
                logger.info(f"\n ### The `CompileGuiderAgent` failed to read file {file_path} with {encoding} encoding: {e}")

        # If all text encodings fail, try reading as binary
        try:
            with open(file_path, "rb") as file:
                logger.info(f"\n ### The `CompileGuiderAgent` successfully read the file in binary mode: {file_path}")
                return file.read().decode('utf-8', errors='replace')
        except Exception as e:
            logger.info(f"\n ### The `CompileGuiderAgent` failed to read file {file_path} in binary mode: {e}")
    ...
    async def get_guider_plan(self, user_prompt):
        ...
        try:
            ...
            logger.info("\n ### The `CompileGuiderAgent` has successfully generated a guider plan")
            return res
        except json.JSONDecodeError:
            good_json_string = repair_json(response.choices[0].message.content)
            plan_json = json.loads(good_json_string)
            logger.info("\n ### The `CompileGuiderAgent` has repaired and processed the JSON response")
            return plan_json
        except Exception as e:
            logger.info(f"\n ### The `CompileGuiderAgent` encountered an error while generating the guider plan: {e}")
            return {
                "reason": str(e)
            }
    ...
    async def get_guider_plans(self, user_prompt):
        ...
        logger.info("\n ### The `CompileGuiderAgent` is initiating the process to retrieve guider plans")
        plan = await self.get_guider_plan(user_prompt)
        logger.info("\n ### The `CompileGuiderAgent` has completed retrieving the guider plans")
        return plan
