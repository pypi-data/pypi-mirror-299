...
class ProjectManager:
    ...
    async def create_and_add_file_to_xcodeproj(self, full_path: str) -> Optional[str]:
        ...
        if os.path.exists(full_path):
            logger.info(f"File '{file_name}' already exists in {os.path.dirname(full_path)}. ProjectManager is skipping creation.")
            return None
        ...
        if not xcodeproj_path:
            logger.info(f"ProjectManager found no .xcodeproj file in {base_project_dir}")
            return None
    ...
    async def create_file_or_folder(self, full_path: str) -> Optional[str]:
        ...
        if os.path.exists(full_path):
            logger.info(f"File '{os.path.basename(full_path)}' already exists in {os.path.dirname(full_path)}. ProjectManager is skipping creation.")
            return None
    ...
    async def move_file_within_xcodeproj(self, old_full_path: str, new_full_path: str) -> Optional[str]:
        ...
        if not os.path.exists(old_full_path):
            logger.info(f"File '{os.path.basename(old_full_path)}' does not exist. ProjectManager is skipping move.")
            return None

        if os.path.exists(new_full_path):
            logger.info(f"File '{os.path.basename(new_full_path)}' already exists in {os.path.dirname(new_full_path)}. ProjectManager is skipping move.")
            return None
        ...
        if not xcodeproj_path:
            logger.info(f"ProjectManager found no .xcodeproj file in {os.path.dirname(new_full_path)}")
            return None
    ...
    def _create_file_with_content(self, full_path: str, app_name: str) -> None:
        ...
        logger.info(f"ProjectManager created file '{os.path.basename(full_path)}' successfully in {os.path.dirname(full_path)}.")
    ...
    def _add_file_to_xcode_project(self, xcodeproj_path: str, full_path: str, relative_path: str) -> None:
        ...
        project.save()
        logger.info(f"ProjectManager added file '{os.path.basename(full_path)}' to the Xcode project successfully.")
    ...
    def _move_file(self, existing_file_path: str, new_full_path: str) -> None:
        ...
        logger.info(f"ProjectManager moved file '{os.path.basename(new_full_path)}' successfully to {os.path.dirname(new_full_path)}.")
    ...
    def _update_xcode_project(self, xcodeproj_path: str, existing_file_path: str, new_full_path: str, new_relative_path: str) -> None:
        ...
        for file_ref in existing_file_refs:
            ...
            logger.info(f"ProjectManager removed old file reference: {file_ref}")
        ...
        project.save()
        logger.info(f"ProjectManager added file '{os.path.basename(new_full_path)}' to the Xcode project successfully in the new location: {new_full_path}")

    @staticmethod
    def _log_instruction(instruction: Dict) -> None:
        try:
            logger.info(f"ProjectManager is executing Step {instruction['Title']}")
        except KeyError:
            logger.info(f"ProjectManager encountered something wrong with instruction: {instruction}")
    ...
    async def _execute_instruction(self, function_name: str, pipeline: str, parameters: Dict) -> None:
        if "create_and_add_file" in function_name:
            if pipeline == "1":
                ...
                logger.info(f"ProjectManager encountered unknown creating pipeline: {pipeline}")
            ...
            if pipeline == "1":
                ...
                logger.info(f"ProjectManager encountered unknown moving pipeline: {pipeline}")
        else:
            logger.info(f"ProjectManager encountered unknown function: {function_name}")
