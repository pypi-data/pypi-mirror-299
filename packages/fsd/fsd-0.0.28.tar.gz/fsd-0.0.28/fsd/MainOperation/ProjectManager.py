...
class ProjectManager:
    ...
    async def create_and_add_file_to_xcodeproj(self, full_path: str) -> Optional[str]:
        ...
        if os.path.exists(full_path):
            logger.info("\n ### The `file manager agent` has detected an existing file. Skipping creation of '{file_name}' in {os.path.dirname(full_path)}.")
            return None
        ...
        if not xcodeproj_path:
            logger.info("\n ### The `file manager agent` couldn't locate a .xcodeproj file in {base_project_dir}.")
            return None
    ...
    async def create_file_or_folder(self, full_path: str) -> Optional[str]:
        ...
        if os.path.exists(full_path):
            logger.info("\n ### The `file manager agent` has detected an existing file. Skipping creation of '{os.path.basename(full_path)}' in {os.path.dirname(full_path)}.")
            return None
    ...
    async def move_file_within_xcodeproj(self, old_full_path: str, new_full_path: str) -> Optional[str]:
        ...
        if not os.path.exists(old_full_path):
            logger.info("\n ### The `file manager agent` couldn't find the file to move. Skipping relocation of '{os.path.basename(old_full_path)}'.")
            return None

        if os.path.exists(new_full_path):
            logger.info("\n ### The `file manager agent` has detected a file at the destination. Skipping relocation to '{os.path.basename(new_full_path)}' in {os.path.dirname(new_full_path)}.")
            return None
        ...
        if not xcodeproj_path:
            logger.info("\n ### The `file manager agent` couldn't locate a .xcodeproj file in {os.path.dirname(new_full_path)}.")
            return None
    ...
    async def execute_files_creation(self, instructions: List[Dict]) -> None:
        for instruction in instructions:
            logger.info("\n ### The `file manager agent` is initiating a new task.")
    ...
    def _create_file_with_content(self, full_path: str, app_name: str) -> None:
        ...
        logger.info("\n ### The `file manager agent` has successfully created '{os.path.basename(full_path)}' in {os.path.dirname(full_path)}.")
    ...
    def _add_file_to_xcode_project(self, xcodeproj_path: str, full_path: str, relative_path: str) -> None:
        ...
        project.save()
        logger.info("\n ### The `file manager agent` has successfully added '{os.path.basename(full_path)}' to the Xcode project.")
    ...
    def _create_empty_file(self, full_path: str) -> None:
        ...
        logger.info("\n ### The `file manager agent` has successfully created an empty file '{os.path.basename(full_path)}' in {os.path.dirname(full_path)}.")

    def _move_file(self, existing_file_path: str, new_full_path: str) -> None:
        os.makedirs(os.path.dirname(new_full_path), exist_ok=True)
        os.rename(existing_file_path, new_full_path)
        logger.info("\n ### The `file manager agent` has successfully relocated '{os.path.basename(new_full_path)}' to {os.path.dirname(new_full_path)}.")
    ...
    def _update_xcode_project(self, xcodeproj_path: str, existing_file_path: str, new_full_path: str, new_relative_path: str) -> None:
        ...
        for file_ref in existing_file_refs:
            ...
            logger.info("\n ### The `file manager agent` has removed an old file reference: {file_ref}")
        ...
        project.save()
        logger.info("\n ### The `file manager agent` has successfully added '{os.path.basename(new_full_path)}' to the Xcode project at the new location: {new_full_path}")

    @staticmethod
    def _log_instruction(instruction: Dict) -> None:
        try:
            logger.info("\n ### The `file manager agent` is executing Step {instruction['Title']}")
        except KeyError:
            logger.info("\n ### The `file manager agent` encountered an issue with the following instruction: {instruction}")
    ...
    async def _execute_instruction(self, function_name: str, pipeline: str, parameters: Dict) -> None:
        if "create_and_add_file" in function_name:
            if pipeline == "1":
                ...
                logger.info("\n ### The `file manager agent` encountered an unknown creation pipeline: {pipeline}")
            ...
            if pipeline == "1":
                ...
                logger.info("\n ### The `file manager agent` encountered an unknown moving pipeline: {pipeline}")
        else:
            logger.info("\n ### The `file manager agent` encountered an unknown function: {function_name}")
