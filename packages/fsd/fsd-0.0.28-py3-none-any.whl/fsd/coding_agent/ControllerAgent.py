...
class ControllerAgent:
    ...
    async def get_coding_requests(self, instructions, context, file_lists, context_files, role, crawl_logs):
        ...
        logger.info("\n ### The task planner is preparing the task and getting its digital gears turning.")
        plan = await self.get_taskPlanner(instructions, file_lists)
        logger.info("\n ### The task planner has successfully completed preparing the task!")
        is_first = True
        for step in plan.get('steps', []):
            ...
            if self.is_coding_file(file_name):
                ...
                if main_path:
                    ...
                    logger.info(f"\n ### The coding agent is processing file: `{file_name}`")
                    logger.info(f"\n ### Task: `{prompt}`")
                    ...
                    try:
                        ...
                        logger.info(f"\n ### The coding agent has successfully finished working on `{file_name}`")
                        is_first = False
                    except Exception as e:
    ...
    async def code_format_pipeline(self, finalPrompt, role):
        ...
        logger.info("\n ### The file manager agent is initiating file processing")
        file_result = await self.get_formatting_files(finalPrompt)
        logger.info(file_result)
        await self.process_creation(file_result)
        logger.info("\n ### The file manager agent has successfully completed processing files")
        logger.info("\n ### The formatter/refactor agent is preparing to start its work")
        working_files = file_result.get('working_files', [])
        logger.info(f"\n ### The formatter/refactor agent is formatting: `{working_files}`")
        if working_files:
            await self.format.get_formats(working_files, finalPrompt, role)
            self.format.clear_conversation_history()
            logger.info("\n ### The compile agent is preparing to build and check for any compile errors")
            await self.build_and_fix_compile_error(finalPrompt, working_files, role)
            logger.info("\n ### The formatter/refactor agent has successfully completed the formatting/refactoring phase")
    ...
    async def fix_compile_error_pipeline(self, initial_instruction, file_list, role):
        ...
        logger.info("\n ### Initiating fix_compile_error_pipeline")
    ...
    async def add_files_folders_pipeline(self, finalPrompt, role):
        ...
        logger.info("\n ### The file manager agent is beginning work on file processing")
    ...
    async def move_files_folders_pipeline(self, finalPrompt, role):
        ...
        logger.info("\n ### The file manager agent is commencing work on file processing")
    ...
    async def replace_code_pipeline(self, finalPrompt, role):
        ...
        logger.debug("\n ### Initiating replace_code_pipeline")
    ...
    async def regular_code_task_pipeline(self, finalPrompt, role):
        ...
        if crawl_plan:
            for step in crawl_plan.get('crawl_tasks', []):
                ...
                if crawl_url:
                    ...
                    logger.info(f"\n ### The crawler agent has successfully finished reading: `{crawl_url}`")
                    crawl_logs.append({
                        'url': crawl_url,
                        'result': result
                    })
        ...
        logger.info("\n ### The planner agent is creating an initial development plan for clarification.")
        ...
        logger.info("\n ### The file manager agent is initiating work on file processing")
        ...
        logger.info("\n ### The dependency agent has successfully finished with dependency checks")
        logger.info("\n ### The coding agent is preparing to start the coding phase")
        ...
        logger.info("\n ### The coding agent has successfully completed the coding phase")
    ...
    async def get_started(self, user_prompt):
        ...
        logger.info("\n ### Greetings! I am Zinley, the director agent. I will now begin processing your request.")
