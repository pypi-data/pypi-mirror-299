    ...
    async def get_fixing_requests(self, instructions):
        ...
        for instruction in instructions:
            ...
            if len(list_related_file_name) == 0:
                ...
                logger.info(f"\n ### The `SelfHealingAgent` is initiating work on: `{instruction['Solution_detail_title']}`")
                ...
                logger.info(f"\n ### The `SelfHealingAgent` has completed tasks for: `{instruction['Solution_detail_title']}`")
            else:
                main_path = file_name
                all_path = list_related_file_name
                file_content = self.read_file_content(main_path)
                all_file_content = self.read_all_file_content(all_path)
                logger.info(f"\n ### The `SelfHealingAgent` is beginning work on: `{instruction['Solution_detail_title']}`")
                result = await self.get_fixing_request(all_comprehensive_solutions_for_each_bugs, file_content, all_file_content, tech_stack)
                await self.replace_all_code_in_file(main_path, result)
                logger.info(f"\n ### The `SelfHealingAgent` has successfully completed tasks for: `{instruction['Solution_detail_title']}`")
