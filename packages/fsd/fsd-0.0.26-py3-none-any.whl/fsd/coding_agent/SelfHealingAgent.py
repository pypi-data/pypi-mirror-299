...
class SelfHealingAgent:
    ...
    def read_file_content(self, file_path):
        ...
        if not os.path.exists(file_path):
            logger.info(f"SelfHealingAgent: File does not exist: {file_path}")
            return ""
        ...
        for encoding in encodings:
            try:
                ...
                logger.info(f"SelfHealingAgent: Failed to read file {file_path} with {encoding} encoding: {e}")
        ...
        try:
            ...
            logger.info(f"SelfHealingAgent: Failed to read file {file_path} in binary mode: {e}")
    ...
    async def get_fixing_request(self, instruction, file_content, all_file_content, tech_stack):
        ...
        try:
            ...
            logger.info(f"SelfHealingAgent: Failed: {e}")
            return {
                "reason": str(e)
            }
    ...
    async def get_fixing_requests(self, instructions):
        ...
        for instruction in instructions:
            ...
            if len(list_related_file_name) == 0:
                ...
                logger.info(f"SelfHealingAgent: Working on: {instruction['Solution_detail_title']}")
                ...
                logger.info(f"SelfHealingAgent: Done tasks for: {instruction['Solution_detail_title']}")
            else:
                main_path = file_name
    ...
    async def replace_all_code_in_file(self, file_path, result):
        ...
        if file_path:
            ...
            logger.warning(f"SelfHealingAgent: File not found: {file_path}")
