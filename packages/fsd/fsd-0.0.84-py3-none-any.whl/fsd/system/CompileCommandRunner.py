...
class CompileCommandRunner:
    ...
    def run_command(self, command, method='bash'):
        ...
        try:
            ...
            # Check if the command is related to starting a local server or service
            if any(keyword in command.lower() for keyword in [
                'localhost', '127.0.0.1',
                'run', 'serve', 'start', 'dev', 'develop',
                'build', 'compile', 'make',
                'npm run', 'yarn run', 'pnpm run',
                'python manage.py runserver', 'flask run', 'node server.js',
                'go run', 'dotnet run', 'java -jar',
                'mvn spring-boot:run', 'gradle bootRun',
                'rails server', 'php -S',
                'ng serve', 'react-scripts start', 'vue-cli-service serve',
                'next dev', 'nuxt dev', 'gatsby develop',
                'webpack serve', 'parcel serve', 'vite',
                'docker-compose up', 'docker run',
                'cargo run', 'mix run',
                './run', './start', './build', './compile'
            ]) and not any(keyword in command.lower() for keyword in [
                'install', 'update', 'upgrade', 'add', 'remove', 'uninstall', 'delete', ' chmod'
            ]):
                return self.run_localhost_command(command, current_path)
    ...
    def run_localhost_command(self, command, current_path):
        ...
        """
        logger.info(f"\n #### `LocalhostRunner` is running the command: {command}")
        process = subprocess.Popen(
            command,
            shell=True,
            stdout=subprocess.PIPE,
        ...
        while True:
            ...
            if return_code is not None:
                ...
                for line in stderr.splitlines():
                    logger.error(f"Command error: {line}")
        ...
        if port_already_in_use_line:
            logger.info("\n #### `LocalhostRunner`: Port is already in use. Do you want us to fix this or skip?")
            logger.info("Press Tab to process the fix, or Enter to skip: ")
            user_input = input()
            if user_input == '\t':
            ...
            logger.error(f"\n #### `LocalhostRunner`: Command failed with return code {return_code}")
            return -1, output + error_output
        else:
            logger.info("\n #### `LocalhostRunner`: Command completed successfully. Everything is good.")
            logger.info("\n #### `LocalhostRunner` will complete after 10 seconds. No action needed.")
    ...
    async def execute_steps(self, steps_json, compile_files, code_files):
        ...
        for step in steps:
            ...
            user_prompt, _, _ = parse_payload(user_permission)
