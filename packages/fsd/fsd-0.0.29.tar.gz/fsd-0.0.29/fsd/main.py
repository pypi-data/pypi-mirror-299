...
async def start(project_path):
    try:
        ...
        while True:
            ...
            if pipeline == "1":
                logger.info("\n ### The `Explainer Agent` is initiating the explanation process")
                await explainer_controller.get_started(user_prompt)
            elif pipeline == "2":
                logger.info("\n ### The `Coding Agent` is beginning the code generation task")
                await coding_controller.get_started(user_prompt)
                repo.add_all_files(user_prompt)
                logger.info("\n ### The `Coding Agent` has successfully completed the coding request")
            elif pipeline == "3":
