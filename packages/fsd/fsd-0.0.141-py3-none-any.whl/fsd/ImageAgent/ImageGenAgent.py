...
class ImageGenAgent:
    ...
    def save_image_data(self, image_data: str, file_path: str, image_format: str):
        ...
        try:
            ...
            logger.info(f"\n #### `ImageGenAgent` saved image to {os.path.relpath(file_path)}")
    ...
    def save_and_resize_image(self, image_data: str, file_path: str, image_format: str, target_size: Tuple[int, int]):
        ...
        try:
            ...
            os.makedirs(os.path.dirname(file_path), exist_ok=True)
            image.save(file_path, format=image_format)
            logger.info(f"\n #### `ImageGenAgent` saved and resized image to {target_size} at {os.path.relpath(file_path)}")
        except Exception as e:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url) as resp:
                    if resp.status == 200:
                        ...
                        os.makedirs(os.path.dirname(file_path), exist_ok=True)
                        image.save(file_path, format=image_format)
                        logger.info(f"\n #### `ImageGenAgent` fetched and saved image to {os.path.relpath(file_path)}")
                    else:
    ...
    async def generate_image(self, prompt: str, dalle_dimension: str, actual_dimension: str, image_format: str, file_path: str):
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.info(f"\n #### `ImageGenAgent` generated and saved image to {os.path.relpath(file_path)}")
                filename = os.path.basename(file_path)
                logger.info(f"Displaying: {file_path}")
    ...
    async def process_image_generation(self, steps: List[Dict]):
        ...
        for step in steps:
            try:
                ...
                logger.info(f"\n #### `ImageGenAgent` generated image: {os.path.relpath(step['file_path'])}")
            except Exception as e:
