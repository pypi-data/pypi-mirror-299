...
class ShortIdeaDevelopment:
    ...
    def initial_setup(self, role, crawl_logs):
        ...
        all_file_contents = self.repo.print_summarize_with_tree()
            ...
            # Start of Selection
        system_prompt = (
            f"You are a senior {role}. Analyze the project files and develop a concise plan to achieve the user's request. Focus on the following:\n\n"

            "1. What to do: Provide a clear, step-by-step outline of the actions needed to implement the user's request.\n\n"

            "2. Why: For each step, briefly explain the reasoning behind it. This helps understand the purpose and importance of each action.\n\n"

            "3. Working flow (if applicable): If the implementation involves a process or sequence of operations, describe the workflow. This could include data flow, user interactions, or system processes.\n\n"

            "4. New files or images: If new files need to be added, specify the full path where they should be saved. For example: '/path/to/new/file.py'. For images:\n"
            "   - Use SVG format for most images, specifying the full path. For example: '/path/to/new/image.svg'\n"
            "   - Use PNG or JPEG only for high-quality, large images, specifying the full path. For example: '/path/to/new/large_image.png'\n\n"

            "5. Quick tree: If necessary, provide a brief tree structure of the project or affected parts to illustrate the file organization.\n\n"

            "Format: Keep your response concise and focused on these aspects. Use clear headings (no larger than h4) and bullet points for clarity. Avoid unnecessary details or elaboration.\n\n"
        )
    ...
    async def get_idea_plan(self, user_prompt):
        ...
        prompt = (
            ...
            f"Return a nicely formatted response. Spacing between each section clearly, ensure the text is clear and easy to read, not crowded together. Use clear headings (no larger than h4)."
