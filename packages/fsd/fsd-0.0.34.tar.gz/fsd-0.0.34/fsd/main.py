...
async def start(project_path):
    try:
        ...
        while True:
            ...
            if pipeline == "1":
                logger.info("\n ### The `Explainer Agents` is initiating the explanation process")
                await explainer_controller.get_started(user_prompt)
            elif pipeline == "2":
                logger.info("\n ### The `Action Agents` is beginning the code generation task")
                await coding_controller.get_started(user_prompt)
                repo.add_all_files(user_prompt)
                logger.info("\n ### The `Action Agents` has successfully completed the coding request")
            elif pipeline == "3":
                logger.info("\n ### The `System` is preparing to terminate the process")
                break
...
def parse_payload(user_prompt_json, project_path):
    try:
        ...
        if file_path and os.path.exists(file_path):
            logger.debug(f"\n ### The `File Manager` has detected an existing file at {file_path}")
            ...
            logger.debug(f"\n ### The `File Manager` has successfully relocated the file to {project_path}")
        else:
            logger.debug(f"\n ### The `File Manager` could not locate the specified file at {file_path}")
            file_path = None
    except json.JSONDecodeError:
        # If input is not valid JSON, treat it as plain text
        user_prompt = user_prompt_json
        logger.debug(f"\n ### The `Input Processor` has received a plain text prompt: {user_prompt}")

    logger.info(f"\n ### The `Input Processor` has received a plain text prompt: {user_prompt}")
    logger.info(f"\n ### The `Input Processor` has received a plain text prompt: {file_path}")
    logger.info(f"\n ### The `Input Processor` has received a plain text prompt: {tracked_file}")
