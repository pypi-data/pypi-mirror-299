...
def log(input_file, output_filename, environment, input_version, output_version):
    if input_version != 1 and output_version != 1:
        raise ValueError("Version combination of " + str(input_version) + " and " + str(output_version) + " is not supported.")
    ...
    special_data = ["system", "version", "variant", "success", "runtime", "nodes", "taskspernode", "threadspertask", "jobid", "queue"]
