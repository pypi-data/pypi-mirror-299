        ...
        if file_path.suffix.lower() == ".parquet":
            ...
            for encoding in encodings:
                try:
                    ...
                    continue
            raise ValueError(
                f"Unable to read {file_path} with any of the attempted encodings"
            )
        else:
    ...
    try:
        ...
        if temp_output_path and temp_output_path.exists():
            try:
                ...
                logger.warning(
                    f"Failed to delete temporary file {temp_output_path}: {e!s}"
                )
...
def process_registers(
    ...
    for file_path in files:
        ...
        progress.update(
            task,
            advance=1,
            description=f"Processed: {files_processed}, Left: {files_left}",
        )
