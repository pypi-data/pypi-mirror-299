...
def get_tabular_data(
    ...
    for metaid, keys in table_field.items():
        ...
        for code, file_path in data_path_dict.items():
            ...
            if not os.path.exists(file_path): # 如果file_path不存在
                ...
                if metaid not in extra_requests:
                    extra_requests[metaid] = []
                extra_requests[metaid].append(code)
                continue
            ...
            for data in bson_datas:
                bson_list.append(xtbson.decode(data))

            df = pd.DataFrame(bson_list)
            if df.empty:
                continue
    ...
    for metaid, codes in extra_requests.items():
        ...
        bson_datas = [xtbson.decode(d) for d in bson_datas]
        df = pd.DataFrame(bson_datas)
        if df.empty:
            continue

        if CONSTKEY_CODE not in df.columns:
            continue

        keys = table_field[metaid]
        for code in codes:
            cdf = df.loc[df[CONSTKEY_CODE] == code]
            if count > 0:
                cdf = cdf.tail(count)
            # 移除结果中多余的字段
            drop_columns = [c for c in df.columns if c not in keys]
            if drop_columns:
                cdf = cdf.drop(drop_columns, axis=1)
            # 补充请求的字段
            default_null_columns = [c for c in keys if c not in cdf.columns]
            for c in default_null_columns:
                cdf.loc[:, c] = keys[c]

            cdf = cdf.copy()
            cdf.rename(columns=key2field[metaid], inplace=True)
            dfs.append(cdf)
