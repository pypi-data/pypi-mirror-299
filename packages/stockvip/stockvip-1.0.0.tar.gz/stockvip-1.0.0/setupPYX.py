...
extensions = [
    Extension(
        ...
        ["stockvip/config/config.pyx"]  # Đường dẫn đến file config.c
    ),
    Extension(
        "stockvip.config.ensure_logged_in",  # Tên module cho file ensure_logged_in.c
        ["stockvip/config/ensure_logged_in.pyx"]  # Đường dẫn đến file ensure_logged_in.c
    ),
    Extension(
        "stockvip.process_client.login",  # Tên module cho file login.c
        ["stockvip/process_client/login.pyx"]  # Đường dẫn đến file login.c
    ),
    Extension(
        "stockvip.process_stock.ohlcv",  # Tên module cho file trong thư mục stock/price
        ["stockvip/process_stock/ohlcv.pyx"]  # Đường dẫn đến file price.c
    )
]
