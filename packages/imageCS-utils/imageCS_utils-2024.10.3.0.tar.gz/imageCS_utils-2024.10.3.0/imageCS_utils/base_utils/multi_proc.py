...
class ChildProc(Process):
    ...
    def list_data(self, idx):
        """get data from list: self.__shared_list[idx]"""
        with self.__shared_list_lock:
            data = self.__shared_list[idx]
        return data

    def list_pop(self, idx):
        """pop data from list"""
        with self.__shared_list_lock:
    ...
    def dict_update(self, key, data):
        """update shared dict"""
        with self.__shared_dict_lock:
            self.__shared_dict[key] = data
    
    def dict_data(self, key):
        """get shared dict via key"""
        with self.__shared_dict_lock:
...
def __example_func(in_data:int, this_proc:ChildProc):
