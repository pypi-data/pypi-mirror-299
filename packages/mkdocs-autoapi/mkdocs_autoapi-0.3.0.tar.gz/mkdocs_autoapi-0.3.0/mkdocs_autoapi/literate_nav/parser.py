...
# local imports
from mkdocs_autoapi.literate_nav import exceptions
...
class NavParser:
    ...
    def markdown_to_nav(self, roots: Tuple[str, ...] = (".",)) -> Nav:
        ...
        if dir_nav := self.get_nav_for_dir(root):
            ...
            nav = _extract_nav_from_content(
                self._markdown_config, markdown_content
            )

            if nav is not None:
                self_path = posixpath.normpath(
                    posixpath.join(root, nav_file_name)
                )
                if not (
                    self.implicit_index
                    and self_path == self.globber.find_index(root)
                ):
                    self.seen_items.add(self_path)

                first_item: Optional[Wildcard] = None
                if self.implicit_index:
                    if found_index := self.globber.find_index(root):
                        first_item = Wildcard(
                            root, "/" + found_index, fallback=False
                        )
        ...
        return self._resolve_wildcards(
            [Wildcard(root, "*", fallback=False)], roots
        )
    ...
    def _list_element_to_nav(
        ...
        for item in section:
            ...
            if (
                type(out_item) in (str, list, DirectoryWildcard)
                and out_title is not None
            ):
    ...
    def _resolve_wildcards(
        self, nav: NavWithWildcards, roots: RootStack = (".",)
    ) -> Nav:
        ...
        for entry in nav:
            if isinstance(entry, dict) and len(entry) == 1:
                ...
                if isinstance(val, list):
                    ...
                    new_val = (
                        self._resolve_wildcards([val], roots) or val.fallback
                    )
                else:
                    new_val = val
                if new_val:
            ...
            for item in self.globber.glob(entry.value.rstrip("/")):
                ...
                if self.globber.isdir(item):
                    title = mkdocs.utils.dirname_to_title(
                        posixpath.basename(item)
                    )
...
def _extract_nav_from_content(
    markdown_config: dict, markdown_content: str
...
class _Preprocessor(markdown.preprocessors.Preprocessor):
    ...
    def _register(self) -> None:
        self.md.preprocessors.register(self, "mkdocs_autoapi", priority=25)
...
class _Treeprocessor(markdown.treeprocessors.Treeprocessor):
    ...
    def run(self, root: etree.Element) -> None:
        preprocessor: _Preprocessor = self.md.preprocessors["mkdocs_autoapi"]  # type: ignore[assignment]
        ...
        if nav_placeholder is not None:
            ...
            items = itertools.dropwhile(
                lambda el: el.text != nav_placeholder, root
            )
        else:
    ...
    def _register(self) -> None:
        self.md.treeprocessors.register(self, "mkdocs_autoapi", priority=19)
...
def _iter_children_without_tail(
