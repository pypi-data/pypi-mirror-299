...
def escape_special_characters(value):
    """
    Экранирует специальные символы в строке для использования в SQL-запросах в ClickHouse.

    Аргументы:
    value -- строка или любое значение, которое нужно экранировать.
    ...
    # Экранирование с помощью html.escape() для символов &, <, >, " (амперсанд и угловые скобки)
    escaped_value = html.escape(value_str)

    # Дополнительно экранируем одинарные кавычки для SQL-запросов
...
def format_value_clickhouse(value):
    if pd.isna(value):
        ...
        return escape_special_characters("'{}'".format(value.replace("'", "''")))
