...
CF_ACTIVITY_URL = os.getenv("CF_ACTIVITY_URL_NEW") or "https://service.optilogic.app/websocket/message" # Because of Andromeda
    ...
    async def _request_with_retries(
        ...
        while attempt < RETRY_COUNT:
            try:
                ...
                attempt += 1
                self.logger.warning(
                    f"Retry {attempt} for {method} request to {url} due to {str(e)}, correlation_id: {self.correlation_id}"
                )
...
def activity_signal(
    ...
    app_key: str,
    signal_topic: Literal['TADPOLE CREATION', 'TABLE INSERT', 'REFRESH COUNT', 'REFETCH SCENARIO ERRORS', 'REFETCH SCENARIOS', 'REFETCH MAPS'],
    model_name: str = None,
    user_name: str = None,
