from typing import Self, Iterator, Callable, Any
...
class DataPath:
    ...
    def but(self, **kwargs) -> Self:
        ...
        for key, val in kwargs.items():
            result.__setattr__(f"_{key}", val)
        return result
...
class Data:
    ...
    def get(self, name: str = "") -> list[Path]:
        if name == "":
            return [data_path.path() for data_path in self._paths]
        try:
            return [
                data_path.path()
