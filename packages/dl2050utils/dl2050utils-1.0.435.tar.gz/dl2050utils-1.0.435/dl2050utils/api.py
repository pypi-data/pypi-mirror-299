...
# Example Worker:
#
#   w = API_Worker('kardio', WORKER_KEY, 'http://ju:9000')
...
from dl2050utils.restutils import enforce_required_args

WORKER_KEY = 'inxmlqJmtLjHQ4LgQzc40vzQPgsKnoaflgAZYsuOBFAfBNomJojiV8nXrFVaOYVYp1bLRCVUhuoZXqfc6JvJ84Hhf1TxbYOqkBTf'
...
class API_Server:
    def __init__(self, service):
        self.db = Sqlite(f'/data/{service}/api/api-db')
        self.LOG,self.port,self.dbname = BaseLog(service=service),9000,f'/data/{service}/api/api-db'
    ...
    def check_worker_key(self, key):
        ...
        if key!=WORKER_KEY: raise_error(self.LOG, err_msg='Unauthorized Worker key', status_code=403)
    ...
    async def req(self, request):
        ...
        self.LOG(2, 0, label='API', label2='/api/req', msg=f"req_id={req_id}")
        return rest_ok(result={'req_id':req_id})
    
    # ####################################################################################################
    # Worker Entrypoints (with WORKER_KEY)
    # ####################################################################################################
    ...
    def run(self, port=None):
        routes = [
            Route('/api/check', endpoint=self.check, methods=['GET']),
            Route('/api/req', endpoint=self.req, methods=['POST']),
...
class API_Worker:
    def __init__(self, service, key, url):
        self.key,self.url = key,url
    ...
    def grab(self):
        res = self.request('worker/grab', {'key':self.key})
        return oget(res,['result'])

    def start(self, task_id, eta):
        return self.request('worker/start', {'key':self.key, 'id':task_id, 'eta':eta})

    def done(self, task_id, result, used_credits=0.):
        return self.request('worker/done', {'key':self.key, 'id':task_id, 'result':result, 'credits':used_credits})

    def error(self, task_id):
        return self.request('worker/error', {'key':self.key, 'id':task_id})
