...
from dl2050utils.restutils import raise_error, get_param, sync_request
...
class API_Server:
    def __init__(self, service, wkey, LOG=None):
        self.db,self.wkey = Sqlite(f'/data/{service}/apiserver/api-db'),wkey
        self.LOG = LOG or BaseLog(service=service)
        self.port = 9000
    ...
    def new_key(self, email, rate_limit=100):
        key = get_new_key()
        self.db.insert('api_keys',{'key':key, 'email':email, 'active':True, 'rate_limit':rate_limit, 'reset_calls_at':get_reset_time()})
        return key
    ...
    def run(self, port=None):
        ...
        uvicorn.run(app, port=port or self.port, host='0.0.0.0', log_level='critical')
...
class API_Worker:
    def __init__(self, service, wkey, url, LOG=None):
