...
export class ToolNode extends BaseNode {
  constructor(editor, name, tool) {
    ...
    for (; index < tool.params.length; index++) {
      const input = inputFromParam(tool.params[index], index);
      this.addInput(`param${index}`, input);
    }
  ...
  static toolFromData(data) {
    ...
    for (const port of data.model.tool.ports) {
      // Ignore built-in inputs ports and output ports.
      if (port.port_direction === 'out' || Object.keys(commonInputs).includes(port.type)) {
        continue;
      }
