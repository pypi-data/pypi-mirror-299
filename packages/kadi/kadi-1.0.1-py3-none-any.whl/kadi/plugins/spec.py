...
def kadi_get_preview_templates(file):
    ...
    render the preview data. As currently all previews are rendered using Vue.js 2
...
def kadi_get_publication_form(provider, resource):
    ...
    Needs to be used together with :func:`kadi_publish_resource`. Note that the hook
...
def kadi_get_scripts():
    ...
    An example of using this hook could be the registration of a custom, global Vue.js 2
    component, which can be used in combination with a template such as the one shown in
    :func:`kadi_get_preview_templates`:
...
def kadi_get_translations_bundles(locale):
    ...
    :func:`kadi_get_scripts`, including the use in custom Vue.js 2 components.
