...
@qparam(
    ...
    " current record. Depending on direction, linked records will be included as"
    " ``record_to`` or ``record_from`` respectively.",
)
@qparam(
    "filter",
    parse=normalize,
...
def get_upload(record_id, upload_id):
    ...
    finished successfully or an ``error`` message otherwise.
    """
    record = Record.query.get_active_or_404(record_id)
