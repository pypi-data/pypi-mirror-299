...
class Connection_pattern(Coordinates_list):
    def __init__(self):
        Coordinates_list.__init__(self)

    def get_pairs(self) -> JsonList:
        pairs = JsonList(list_type=Connection_pattern)
        for cnn in self:
            present = False
            for pair in pairs:
                for c in pair:
