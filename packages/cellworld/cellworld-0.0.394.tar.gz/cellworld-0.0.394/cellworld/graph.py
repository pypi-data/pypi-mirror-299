...
class Graph:
    ...
    def get_centrality(self, depth: int = 1) -> JsonList:
        ...
        centrality = JsonList(list_type=float, iterable=[1.0 for _ in self.cells])
        for i in range(depth):
            iteration_centrality = JsonList(list_type=float, iterable=[0.0 for _ in self.cells])
            for cell_id in range(len(self.cells)):
                for connection in self._connections[cell_id]:
                    iteration_centrality[cell_id] += centrality[connection]
            total = sum(iteration_centrality)
            for cell_id in range(len(self.cells)):
                centrality[cell_id] = iteration_centrality[cell_id] / total
        return centrality

    def get_lppo(self, depth: int = 1, count: int = 10):
        cell_centrality = self.get_centrality(depth=depth)
        centrality_index = sorted(range(len(cell_centrality)), key=lambda i: cell_centrality[i], reverse=True)
        return centrality_index[:count]