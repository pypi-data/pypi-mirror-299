...
MODEL_PATH = pkg_resources.resource_filename('model', 'yeast-GEM.xml')

# MODEL_PATH = 'src/yEscher/model/yeast-GEM.xml'
...
def read_yeast_model(make_bigg_compliant=False):
    ...
    if not is_bigg_compliant and make_bigg_compliant:
        ...
        met_bigg_dict = load_bigg_dict(
            f"{data_path}/BiGGmetDictionary_newIDs.csv")
        rxn_bigg_dict = load_bigg_dict(
            f"{data_path}/BiGGrxnDictionary_newIDs.csv")
        ...
        for met in model.metabolites:
            ...
            met.name = met.name.replace(
                f" [{model.compartments[met.compartment]}]", "")
            ...
            if "bigg.metabolite" in met.annotation:
                ...
                met.id = met.id.replace(
                    f"[{met.compartment}]", f"_{met.compartment}")
