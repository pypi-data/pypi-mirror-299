...
class BaseEngineMeta(ABCMeta):
    ...
    def __init__(cls, name, bases, attrs):
        ...
        super(BaseEngineMeta, cls).__init__(name, bases, attrs)

        if name != 'BaseEngine':
            BaseEngineMeta.engines[attrs['name']] = cls


class BaseEngine(metaclass=BaseEngineMeta):
    """登录引擎基类"""
