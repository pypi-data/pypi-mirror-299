...
def fit_curve(func: Callable,x_vals: List[float],y_vals: List[float],startx:float=None,endx:float=None,starty:float=None,endy:float=None,guess:List[float]=None,maxfev:int=10000)->tuple[Callable,List[float],List[float]]:
    ...
    for i in range(0,len(popt)):
        poptr.append(popt[i]/scalex)
        pcovr.append(pcov[i]/scalex) 
    return (lambda x: (func((np.array(x)-startx)*scalex,*popt)+starty) if (isinstance(x,(list,np.ndarray))) else (func((x-startx)*scalex, *popt))+starty),(poptr),(pcovr)
...
def fit_and_confidence(func_1:Callable, x_vals:list,y_vals:list,confidence: float = .95,startx:float=None,endx:float=None,starty:float=None,endy:float=None,guess:List[float]=None,maxfev:int=10000) -> tuple[Callable,list,list,list]:
