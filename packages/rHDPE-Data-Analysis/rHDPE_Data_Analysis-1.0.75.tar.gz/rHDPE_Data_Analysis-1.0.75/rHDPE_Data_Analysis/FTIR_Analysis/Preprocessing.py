...
def read_shiny_file( filepath, filename ):
    ...
    file_data.append( [resin, specimen, "Test upload", ""] )

    return file_data, data[1]

def write_csv( output_directory, file_data, data, update = False ):
    ...
    if update:
        ...
        np.savetxt( output_directory + "FTIR/Condensed_Data/FTIR_data_update.csv", array, delimiter = ",", fmt = "%.5f" )

    else:

        np.savetxt( output_directory + "FTIR/Condensed_Data/FTIR_data.csv", array, delimiter = ",", fmt = "%.5f" )

    array = np.array( file_data )

    if update:

        np.savetxt( output_directory + "FTIR/Condensed_Data/file_data_update.csv", array, delimiter = ",", fmt = "%s" )

    else:

        np.savetxt( output_directory + "FTIR/Condensed_Data/file_data.csv", array, delimiter = ",", fmt = "%s" )

def read_csv( directory, output_directory, merge_groups, update = False ):
    '''Read the preprocessed .csv file.'''

    if update:

        resin_data = gu.get_list_of_resins_data( directory, True ) # Obtain the spreadsheet of data for the resins.
    ...
    if update:
        ...
        df = pd.read_csv( output_directory + "FTIR/Condensed_Data/file_data_update.csv", sep = ",", header = None )
    ...
    if update:
        ...
        df = pd.read_csv( output_directory + "FTIR/Condensed_Data/FTIR_data_update.csv", sep = ",", header = None )
...
def compute_mean( output_directory, file_data, data ):
    ...
    np.savetxt( output_directory + "FTIR/Condensed_Data/Means.csv", array, delimiter = ",", fmt = "%.5f" )
