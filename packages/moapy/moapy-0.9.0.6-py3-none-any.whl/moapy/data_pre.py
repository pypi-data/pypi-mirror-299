...
from moapy.enum_pre import (
    enum_to_list, enDgnCode, enEccPu, enReportType,
...
class UnitLoads(MBaseModel):
    ...
    Unit Loads class
    """
    construction: Load = dataclass_field(default_factory=Load, title="construction", description="Construction Load")
    live: Load = dataclass_field(default_factory=Load, title="live", description="Live Load")
    finish: Load = dataclass_field(default_factory=Load, title="finish", description="Finishing Load")
...
class MemberForce(MBaseModel):
    ...
    """
    Nz: Force = dataclass_field(default_factory=Force, title="Nz", description="Axial force")
...
class Lcoms(MBaseModel):
    ...
    """
    lcoms: list[Lcom] = dataclass_field(default=[Lcom(name="uls1", f=MemberForce(Nz=Force(value=100.0, unit=enUnitForce.kN),
