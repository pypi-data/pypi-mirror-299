...
def git_commit_cmp(c1, c2):
    ...
    if sh.git("merge-base", "--is-ancestor", c1, c2, _ok_code=[0, 1]).exit_code == 0:
        return -1
    elif sh.git("merge-base", "--is-ancestor", c2, c1, _ok_code=[0, 1]).exit_code == 0:
        return 1
    else:
        return ts1 - ts2
...
class ValidationInfo:
    ...
    def __init__(self, **kwargs):
        ...
        self.orig_fnames = [
            posixpath.normpath(of) for of in kwargs.get("orig_fnames", [])
        ]
            ...
            with chdir(self.cfg.upstream_root):
                ...
                for fpath in self.orig_fnames:
                    ...
                    for commit in sh.git(
                        ...
                        if (
                            self.hash is not None and git_commit_eq(commit, self.hash)
                        ) or not self.cfg.is_commit_excluded(commit):
...
def set_info(fname, info, default_location):
    ...
    with open(fname, "r") as fin, tempfile.NamedTemporaryFile(
...
def action_show(cfg, args):
    ...
    if getattr(args, "filename", None) is not None:
        _action_show(
            cfg, get_fname(cfg.validation_root, args.filename), counts, show_stat
        )
    else:
    ...
    print()
    print(
        "%(good)d OK, %(outdated)d out of date, %(unknown)d unknown, %(error)d error, %(missing)d missing, %(ignored)d ignored"
        % counts
    )
    ...
    if status == "OLD" and show_stat and info:
        with chdir(cfg.upstream_root):
            ...
            for fname in info.orig_fnames:
                os.system(
                    "git diff --shortstat -w --follow %s %s"
                    % (revs, shlex_quote(fname))
                )
...
def update_src(cfg, info):
    ...
    info.orig_fnames = choose_suggestion(
...
def main():
    ...
    sp = subparsers.add_parser("diff", help=inspect.getdoc(action_diff))
    ...
    sp = subparsers.add_parser("show", help=inspect.getdoc(action_show))
    ...
    sp = subparsers.add_parser("set-valid", help=inspect.getdoc(action_validate))
    sp.add_argument("filename")
    sp.add_argument("orig_fnames", nargs="*")
    sp.add_argument("--initials", default=None)

    sp = subparsers.add_parser("set-notrack", help=inspect.getdoc(action_notrack))
    sp.add_argument("filename")

    sp = subparsers.add_parser("show-log", help=inspect.getdoc(action_show_log))
    sp.add_argument("filename")

    sp = subparsers.add_parser("update-src", help=inspect.getdoc(action_update_src))
    sp.add_argument("filename")

    sp = subparsers.add_parser(
        "upstream-checkout", help=inspect.getdoc(action_upstream_checkout)
    )

    sp = subparsers.add_parser(
        "upstream-pull", help=inspect.getdoc(action_upstream_pull)
    )

    sp = subparsers.add_parser(
        "upstream-track", help=inspect.getdoc(action_upstream_track)
    )

    sp = subparsers.add_parser(
        "help", help="Show help (--help does not work as a git subcommand)"
    )
