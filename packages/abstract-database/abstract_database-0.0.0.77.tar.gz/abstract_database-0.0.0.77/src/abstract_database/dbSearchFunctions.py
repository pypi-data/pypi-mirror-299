...
def get_insert_table(tableName):
    insertTable = [table for table in configClass().table_configs if tableName == table.get('tableName').lower()]
    if insertTable:
...
def search_multiple_fields(query,conn,**kwargs):
    ...
    try:
        ...
        cur.close()
        conn.close()
def perform_database_operations(operation_type, tableName, searchValue, conn):
    """ Perform dynamic database operations based on type and table info """
    insertyInfo = get_insert(tableName.lower())
    if operation_type == 'fetch':
        return fetch_data(insertyInfo['searchQuery'], searchValue, conn)
    elif operation_type == 'insert':
        insert_data(insertyInfo['insertQuery'], searchValue, conn)

def fetchFromDb(tableName,tableName,conn):
    cached_response =perform_database_operations('fetch', tableName, searchValue, conn)
    if cached_response:
        return cached_response
...
def insert_intoDb(tableName,searchValue,insertValue,conn):
    if isinstance(insertValue,dict):
        insertValue = json.dumps(insertValue)
    perform_database_operations('insert', tableName, (searchValue,insertValue),conn )
