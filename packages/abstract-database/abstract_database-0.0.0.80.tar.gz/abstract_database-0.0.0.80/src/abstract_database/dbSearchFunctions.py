...
class TableManager(metaclass=SingletonMeta):
    ...
    def fetchFromDb(self,tableName,searchValue,conn):
        fetchFromDb(self.get_insert(tableName), searchValue, conn)
        if cached_response:
...
def search_multiple_fields(query,conn,**kwargs):
    ...
    try:
        ...
        cur.close()
        conn.close()
def perform_database_operations(operation_type, get_insert(tableName), searchValue,insertValue, conn):
