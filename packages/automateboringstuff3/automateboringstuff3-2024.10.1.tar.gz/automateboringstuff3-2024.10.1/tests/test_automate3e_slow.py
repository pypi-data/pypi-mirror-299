...
def test_chapter23_pyautogui_slow():
    """

    >>> import pyautogui
    >>> pyautogui.PAUSE = 0.0  # TEST SPEED UP
    >>> for i in range(1): # Move mouse in a square.  # TEST CHANGED TO 1 FROM 10
    ...
    >>> import pyautogui
    >>> pyautogui.PAUSE = 0.0  # TEST SPEED UP
    >>> for i in range(1):  # TEST CHANGED TO 1 FROM 10
...
def test_chapter24_texttospeech_slow():
    """
    >>> import pyttsx3
    >>> engine = pyttsx3.init()
    ...
    200
    >>> DUMMY = engine.getProperty('voices')

    >>> for voice in engine.getProperty('voices'):  # List all the available voices.
    ...     DUMMY = voice.name, voice.gender, voice.age, voice.languages
    ...
