...
PLUGIN_CHOICES: set[str] = set()
PACKAGE_CHOICES: set[str] = set()
...
class DepsPlugin(Plugin):
    ...
    def reset_cached_properties(self):
        self.__dict__.pop("installed_packages", None)
        self.__dict__.pop("pip_inspect", None)
    ...
    def pip_install(self, *packages: str, allow_error: bool = False):
        ...
        try:
            ...
            self.console_log("Installed packages using pip.")
        except ProcessExecutionError as e:
            if allow_error:
                return
    ...
    def find_plugins_in_package(self, name: str) -> tuple[dict[str, Any], set[type["Plugin"]]]:
        """Find plugins in package."""

        plugins = set()

        if pkg := self.inspect_installed_package(name):
            self.console_log(f"Found package {pkg['metadata']['name']}")

            pkg_modules = set(
                [
                    (self.crossover.SITE_PACKAGES_PATH.joinpath(path), path.parts[0])
                    for path in self.find_package_in_metadata(
                        Path(pkg["metadata_location"])
                    )
    ...
    def add_requirement(self, package: AnyUrl | Path, version: str | None = None):
        ...
        pkg, plugins = self.find_plugins_in_package(package)

        if not plugins:
            self.set_status("Uninstalling package...")
            self.crossover.pip["uninstall", pkg["metadata"]["name"], "-y"].run(
                stderr=sys.stderr
            )
    ...
    @click.argument("package", type=Choice(PACKAGE_CHOICES))
    @clickable
    @save_config
    ...
    def remove_requirement(self, package: str):
        ...
        _, plugins = self.find_plugins_in_package(package)
        for plugin in plugins:
            self.config.plugins.pop(plugin.name)
    ...
    @click.argument("name", type=Choice(PLUGIN_CHOICES))
    @clickable
    @save_config
    ...
    def disable_plugin(self, name: str):
        ...
        if name not in self.config.plugins:
            self.console_log(f"Plugin `{name}` not found in list of plugins.", err=True)
            sys.exit(1)

        self.config.plugins[name]["enabled"] = False

        self.console_log(f"Plugin {name} disabled.")
    ...
    def setup_cli(self):
        ...
        PLUGIN_CHOICES.update(self.config.plugins.keys())
        PACKAGE_CHOICES.update(self.requirements.keys())
