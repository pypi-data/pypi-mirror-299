...
class MacPlugin(Plugin, CrossOverControlPlugin):
    ...
    BIN_PATH = APP_PATH.parent.joinpath("SharedSupport/CrossOver/bin")
    ...
    @cached_property
    def deps(self) -> "DepsPlugin":
        return Plugin.get_plugin("deps")
    ...
    def patch_gptk(self):
        ...
        if not self.GPTK_PATH.exists():
            ...
            self.console_log(
                "Please download Game Porting Toolkit from Apple's website."
            )
    ...
    def install(self):
        ...
        self.set_status("Installing plugins...")
        self.deps.reset_cached_properties()
        self.deps.ensure_plugins()
        self.console_log("Plugins have been installed.")
        ...
        entrypoint = inspect.cleandoc(
            f"""
                ...
                from cx_tool.cli import cli
                from cx_tool.config import config

                config.init_plugins()

                if __name__ == "__main__":
                    sys.exit(cli(["run"], standalone_mode=False))
            """
        )
