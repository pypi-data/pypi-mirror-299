    ...
    def process_data(self, vcf_lines, section="merged_evidence_data", match_type="exact_match"):
            ...
            for qid in variant_keys:
                    ...
                    try:
                        ...
                        for i, treatment in enumerate(vcf_lines[qid]["onkopus_aggregator"]["merged_match_types_data"]):
                            for d, drug in enumerate(treatment["drugs"]):
                                drug_class = ""
                                if "drug_name" in drug:
                                    #drug_name = drug["drug_name_norm"]
                                ...
                                try:
                                    if isinstance(vcf_lines[qid]["onkopus_aggregator"]["merged_match_types_data"][i]["drugs"][d], dict):
                                        if "drug_class" not in \
                                                vcf_lines[qid]["onkopus_aggregator"]["merged_match_types_data"][i]["drugs"][d]:
                                            vcf_lines[qid]["onkopus_aggregator"]["merged_match_types_data"][i]["drugs"][d]["drug_class"] = []
                                        # print("add drug class ",drug_class)
                                        ...
                                        print("No parseable result: ",
                                              print(vcf_lines[qid]["onkopus_aggregator"]["merged_match_types_data"][i]["drugs"][d]))
                                except:
                                    print("error assigning drug class")
                                    print(traceback.format_exc())
