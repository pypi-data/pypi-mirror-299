...
class FixtureMethods(unittest.TestCase, FixtureProperties):
    ...
    def _create_record(
        self, dataset_id: str, values: Dict[str, Any] = {"mol": 42}
    ) -> str:
        ...
        info = self.authorized_client.add_record(dataset_id, dto)
        return info.id

    def _delete_records(self, dataset_id: str) -> None:
...
class TestRecord(FixtureMethods):
    ...
    def test_add_parent_child_record(self) -> None:
        ...
        parent_info = self.authorized_client.add_record(self.PUBLIC_ID, parent_dto)

        self.assertEqual(parent_info.status_code, 201)
        self.assertEqual(parent_info.id, parent_id)

        content["values"] = [{"key": "parentID", "value": parent_id}]
        ...
        self.assertIsInstance(child, Record)
        if type(child) is not Record:  # make mypy happy
            raise Exception()
