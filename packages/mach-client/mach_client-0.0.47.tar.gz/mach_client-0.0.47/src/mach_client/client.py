...
class MachClient:
    ...
    def __init__(self):
        ...
        self.symbol_by_contract: dict[tuple[ChainId, ChecksumAddress], str] = {}  # type: ignore
    ...
    async def init(self) -> None:
        ...
        async with self.session as session:
            async with session.get(MachClient.get_config) as response:
                self.raw_deployments: dict[str, dict[str, Any]] = await self._validate_response(response)["deployments"]  # type: ignore
    ...
    async def submit_order(self, chain: ChainId, place_taker_tx: HexBytes) -> Order:
        ...
        async with self.session as session:
            async with session.post(MachClient.orders, json=json) as response:
    ...
    async def get_orders(self, wallet: ChecksumAddress) -> list[Order]:
        ...
        async with self.session as session:
            async with session.get(MachClient.orders, params=params) as response:
                return await self._validate_response(response)
    ...
    async def estimate_gas(self, chain: ChainId) -> GasEstimate:
        ...
        async with self.session as session:
            async with session.get(MachClient.gas, params=params) as response:
    ...
    async def request_quote(
        ...
        async with self.session as session:
            async with session.post(MachClient.quotes, json=json) as response:
                json = await self._validate_response(response)
    ...
    async def get_all_points(self, limit: int = 10) -> list[WalletPoints]:
        ...
        async with self.session as session:
            async with session.get(MachClient.points, params=params) as response:
                json = await self._validate_response(response)
    ...
    async def get_points(self, wallet: ChecksumAddress) -> int:
        async with self.session as session:
            async with session.get(MachClient.points + f"/{wallet}") as response:
                json = await self._validate_response(response)
    ...
    async def get_token_balances(
        ...
        async with self.session as session:
            async with session.get(
                MachClient.token_balances, params=params
            ) as response:
...
# Singleton
client = asyncio.get_event_loop().run_until_complete(MachClient.create())
