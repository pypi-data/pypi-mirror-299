...
class MachClient:
    ...
    # Warning: won't be called if program is terminated abnormally
    # Leads to unclosed client session error from aiohttp
    ...
    async def init(self) -> None:
        ...
        async with self.session.get(MachClient.get_config) as response:
    ...
    async def submit_order(self, chain: ChainId, place_taker_tx: HexBytes) -> Order:
        ...
        async with self.session.post(MachClient.orders, json=json) as response:
    ...
    async def get_orders(self, wallet: ChecksumAddress) -> list[Order]:
        ...
        async with self.session.get(MachClient.orders, params=params) as response:
            return await self._validate_response(response)

    async def estimate_gas(self, chain: ChainId) -> GasEstimate:
        params = {"chain": str(chain)}

        async with self.session.get(MachClient.gas, params=params) as response:
    ...
    async def request_quote(
        ...
        async with self.session.post(MachClient.quotes, json=json) as response:
            json = await self._validate_response(response)
    ...
    async def get_all_points(self, limit: int = 10) -> list[WalletPoints]:
        ...
        async with self.session.get(MachClient.points, params=params) as response:
            json = await self._validate_response(response)
    ...
    async def get_points(self, wallet: ChecksumAddress) -> int:
        async with self.session.get(MachClient.points + f"/{wallet}") as response:
            json = await self._validate_response(response)
...
# Singleton
client = asyncio.get_running_loop().run_until_complete(MachClient.create())
