...
#
def transform(shape, reference = None, to = None, angles = (0.0,0.0)):
...
def rotate_by_alpha(shape, alpha=0.0, to=None, reference = None):
    # 计算第一条边（第一两个点）  
    ...
    vertices = transform_vertices(shape, transMatrix, origin)  

    """ 绕z轴，逆时针旋转 alpha  """
    U = tranformMatrix(alpha, 0.0)
    R0  = np.asarray(vertices[0])
    R1  = np.asarray(R0)
    dR1 = R1 - R0
    R   = vertices + dR1
    dR  = R - R1
    new_vertices = np.dot(dR, U.T) + R1

    """ 返回原坐标系 """
    transformed_vertices = inverse_transform_vertices(new_vertices, transMatrix, origin)
...
def rotate_by_beta(shape, beta=0.0, to=None, reference = None):
    # 计算第一条边（第一两个点）  
    v1,v2,v3 = shape[0],shape[1],shape[2]  
    y_vector = v2 - v1  
...
def transform_vertices(polygon, transformation_matrix, origin):  
...
def inverse_transform_vertices(polygon, transformation_matrix, origin):  
    """ 将多边形顶点从全局坐标系转换到局部坐标系 """  
    transformed_vertices = [] 
    ...
    for vertex in polygon:  
        ...
        transformed_vertices.append(dR + origin)      
    return transformed_vertices 

def moveto(shape,to = None, reference = None):
    pass
...
def move(shape = rectangle(1.0,1.0), reference = None, to = None, by = (0.0,0.0)):
    (alpha, beta) = by
