...
from VisualShape3D.shapes    import transform,rotate_by_alpha,rotate_by_beta
...
class Shape(Polygon):
    ...
    def transform(self, reference = None, to = None, angles = None, alpha=None, beta=None):
        if reference is None : reference = self.R0
        if to is None : to = self.R0
        shape = self.duplicate(reference, to)

        if angles is not None :
            alpha, beta = angles
            shape = self.rotate_by_alpha(shape,alpha)
            shape = self.rotate_by_beta(shape,beta)
    ...
    def duplicate(self, reference = None, to = None, angles = (0.0,0.0)):
        angles = list(angles)
        vertices = transform(self.R, reference, to, angles = angles)
        return self.make_shape(vertices=vertices, R0=self.R0)

    def rotate_by_alpha(self, shape, angle):
        vertices = rotate_by_alpha(shape.vertices, angle)
        return self.make_shape(vertices=vertices, R0=vertices[0])

    def rotate_by_beta(self,shape, angle):
        vertices = rotate_by_beta(shape.vertices, angle)
        return self.make_shape(vertices=vertices, R0=vertices[0])
