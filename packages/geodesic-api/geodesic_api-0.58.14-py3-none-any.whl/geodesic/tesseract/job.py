...
def get_jobs(
    ...
    resp = tesseract_client.get("jobs", **params)
    raise_on_error(resp)
...
list_jobs = deprecated("v1.0.0", "list_jobs")(get_jobs)  # noqa
    ...
    def add_create_assets_step(
        ...
        * You do not need to specify the ``dataset_project`` unless the ``dataset``'\s project is
        ...
        Args:
            ...
            warp_options (deprecated): If this is set, Tesseract will assume that this step will 
                create a tensor output from either the specified ``dataset`` or the ``stac_items``
                provided.
