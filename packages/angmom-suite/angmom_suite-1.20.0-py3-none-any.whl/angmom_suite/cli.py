...
def model_func(args, unknown_args):
    ...
    def trafo(op, rot=False):
        if args.quax is not None and rot:  # compute basis change from basis rotation
            rotation = args.model_space.rotate(R.from_matrix(args.quax))
            return unitary_transform(op, rotation.conj().T @ cg_vec)
        else:
    ...
    ops = {"hamiltonian": trafo(hamiltonian), "spin": spin, "angm": angm}
