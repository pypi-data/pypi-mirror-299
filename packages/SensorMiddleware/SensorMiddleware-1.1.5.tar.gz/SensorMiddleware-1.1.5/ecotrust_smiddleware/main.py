...
from ecotrust_smiddleware.core import SensorControler, CannotReadScanData, BadManifestSyntax
...
def crash_dump(errmsg: str):
    ...
    upload_report_to_s3_bucket({'crash': crash_d}, is_crash_dump=True)
...
def runscan() -> NoReturn:
    ...
    is_report_ready = scontrol.wait_scan()
...
def scontroler_main():
    if not DEBUG:
        try:
            ...
            crash_dump(str(middleware_error)) # noqa
            full_exit() # noqa
    else:
        # Show full traceback
        app()
