...
def get_columns_after_transform(column_transformer, original_columns):
    """
    Returns the appropriate column names after applying the ColumnTransformer.
    
    Parameters:
    - column_transformer: The ColumnTransformer object (e.g., quant_cat_processing)
    - original_columns: The list of original column names (before any transformations)
    
    Returns:
    - List of column names (updated if OneHotEncoding is applied)
    """
    # Try to get the feature names from the ColumnTransformer after fit
    try:
        return column_transformer.get_feature_names_out()
    except AttributeError:
        # If OneHotEncoding or other transformations were not applied, return the original columns
        return original_columns

class ToPandas(BaseEstimator, TransformerMixin):
    def __init__(self, column_transformer, original_columns):
        """
        Transformer to convert NumPy array back to pandas DataFrame.
        
        Parameters:
        - column_transformer: The fitted ColumnTransformer from which to extract feature names dynamically.
        - original_columns: The original column names (used if no OneHotEncoding is applied).
        """
    ...
    def fit(self, X, y=None):
        """
        Fit the ColumnTransformer and retrieve the correct feature names.
        """
        # Fit the ColumnTransformer
        self.column_transformer.fit(X)
        
        # Use the helper function to get the correct columns
        self.columns = get_columns_after_transform(self.column_transformer, self.original_columns)
        return self

    def transform(self, X):
        """
        Convert the NumPy array to a pandas DataFrame with the correct columns.
        
        Parameters:
        - X: The NumPy array to convert to a DataFrame (already transformed by the ColumnTransformer).
        
        Returns:
        - A pandas DataFrame with the correct column names.
        """
        # Directly convert the NumPy array (which has already been transformed)
        return pd.DataFrame(X, columns=self.columns)
