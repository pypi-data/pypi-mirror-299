...
def get_argument_parser():
    ...
    required_args.add_argument('-a', '--apikey', help='The IBM user API key.')
    ...
    required_args.add_argument('--password', help='ICP password. Required if "icp" environment is chosen, not required if --iam-token is specified')
    # Optional parameters
    optional_args = parser._action_groups.pop()
    parser.add_argument('--env', default='ypprod', help='Environment. Default "ypprod"', choices=['ypprod', 'frprod', 'ypqa', 'ypcr', 'ys1dev', 'mcspdev', 'mcspqa', 'icp'])
    ...
    parser.add_argument("--iam-integration", action="store_true", help="To be specified when running in a Cloud Pak for Data environment where IAM integration is enabled.") # Refer #21708 for more details
    parser.add_argument("--bedrock-url", help="The bedrock URL of the Cloud Pak for Data environment for token generation.") # Refer #22439 for more details
    ...
    parser.add_argument("--upload-payloads-to-db", action="store_true", help=argparse.SUPPRESS, required=False, default=False)
