...
def _to_physical_proc(identifier: int, block: dict) -> dict:
    proc: dict = {"identifier": identifier}
...
def parse(content: str) -> list[PhysicalProcessor]:
    ...
    for block in blocks:
        ...
        if not _has_entry_for_key(cores, "identifier", core_id):
            cores.append({"identifier": core_id, "threads": []})

        processor_id = get_key_or_default_int(block, "processor", 0)
        threads = procs[physical_id]["cores"][core_id]["threads"]
        if not _has_entry_for_key(threads, "identifier", processor_id):
            threads.append({"identifier": processor_id})
