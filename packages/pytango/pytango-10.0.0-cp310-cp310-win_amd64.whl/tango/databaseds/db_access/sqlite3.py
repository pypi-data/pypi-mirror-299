...
class SqlDatabase:
    ...
    def __init__(
        self,
        name,
        db_name=os.environ.get("PYTANGO_DATABASE_NAME", "tango_database.db"),
        history_depth=10,
    ...
    def delete_device_attribute(self, dev_name, attr_name):
        dev_name = replace_wildcard(dev_name)
        self.cursor.execute(
            r"DELETE FROM property_attribute_device WHERE device LIKE ? ESCAPE '\' AND "
            "attribute LIKE ?",
            (dev_name, attr_name),
        )
    ...
    def get_class_attribute_property2(self, class_name, attributes):
        ...
        for attribute in attributes:
            ...
            props = []
            rows = cursor.fetchall()
            for prop, rows in groupby(rows, itemgetter(0)):
    ...
    def get_device_property_hist(self, device_name, prop_name):
        cursor = self.cursor
        stmt = r"SELECT DISTINCT id FROM property_device_hist WHERE device=? AND name LIKE ?  ESCAPE '\' ORDER by date ASC"
    ...
    def put_attribute_alias(self, attribute_name, attribute_alias):
        ...
        now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        cursor.execute(
            "INSERT INTO attribute_alias (alias, device, attribute, updated, accessed) VALUES(?, ?, ?, ?, ?)",
            (attribute_alias, tmp_device, tmp_attribute, now, now),
        )
    ...
    def put_class_attribute_property(self, class_name, nb_attributes, attr_prop_list):
        ...
        for i in range(0, nb_attributes):
            ...
            for j in range(k + 2, k + nb_properties * 2 + 2, 2):
                ...
                # then insert the new value for this tuple
                cursor.execute(
                    "INSERT INTO property_attribute_class SET class=? ,attribute=?,name=?,count='1',vsalue=?,updated=NULL,accessed=NULL",
