...
# Constants
TANGO_VERSION = "10.0.0"
TANGO_VERSION_INT = int(TANGO_VERSION.replace(".", ""))

LATEST_DEVICE_IMPLEMENTATION = 6
...
class ExtensionMock(MagicMock):
    ...
    def __getattr__(self, name):
        ...
        if name == f"Device_{LATEST_DEVICE_IMPLEMENTATION+1}Impl":
...
def set_device_implementations(module):
    ...
    for ind in range(3, LATEST_DEVICE_IMPLEMENTATION + 1):
        setattr(
            module,
            f"Device_{ind}Impl",
            type(f"Device_{ind}Impl", (getattr(module, f"Device_{ind-1}Impl"),), attrs),
        )
