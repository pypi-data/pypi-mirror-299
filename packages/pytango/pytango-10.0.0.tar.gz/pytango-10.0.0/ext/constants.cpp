...
template <typename T>
static void export_conditionally(bopy::scope &consts_scope, const char *name, T value)
{
    consts_scope.attr(name) = value;
}

template <>
void export_conditionally(bopy::scope & /*consts_scope*/, const char * /*name*/, PyTangoFallbackConstant)
{
...
namespace Tango
{
static void export_optional_constants(bopy::scope &consts_scope)
{
...
{
    bopy::object consts_module(bopy::handle<>(bopy::borrowed(PyImport_AddModule("tango.constants"))));
    bopy::scope().attr("constants") = consts_module;
    bopy::scope consts_scope = consts_module;
    ...
    consts_scope.attr("TELEMETRY_SUPPORTED") = true;
#else
    consts_scope.attr("TELEMETRY_SUPPORTED") = false;
#endif
