...
class LM:
    ...
    def __init__(self, model_name: str, formatting_model_name: str, temperature: float, max_retries: Literal["None","Few","Many"] = "Few", structured_output_mode: Literal["stringified_json","forced_json"] = "stringified_json"):
        ...
        print("Initializing with this lm_config: ", self.lm_config)
    ...
    async def respond_async(self, system_message: str, user_message: str, images_as_bytes: List[Any] = [], response_model: Optional[BaseModel] = None, use_ephemeral_cache_only: bool = False):
        messages = build_messages(system_message, user_message, images_as_bytes, self.model_name)
        print("LM config here: ", self.lm_config)
        if response_model:
...
if __name__ == "__main__":
    ...
    class Thought(BaseModel):
        ...
        argument_values: List[str] = Field(description="Stringified JSON for the values of the arguments")
    ...
    class TestModel(BaseModel):
        ...
        class Config:
            schema_extra = {
                "required": ["thought", "emotion", "concern", "action"]
            }
    lm = LM(model_name="gpt-4o-mini", formatting_model_name="gpt-4o-mini", temperature=1, max_retries="Few", structured_output_mode="forced_json")
