...
def write_json(igp: IntermediateGeneratedProblem, path: Path) -> Path:
    ...
    for job_id, processing_times in igp.job_to_processing_times.items():
        # create tasks
        for machine_id, processing_time in zip(igp.machines.values(), processing_times):
            task_id = igp.tasks[task_cnt]
            tasks[task_id] = Task(
                task_id=task_id, resource_ids=[machine_id], job_id=job_id, processing_time=processing_time
            )
