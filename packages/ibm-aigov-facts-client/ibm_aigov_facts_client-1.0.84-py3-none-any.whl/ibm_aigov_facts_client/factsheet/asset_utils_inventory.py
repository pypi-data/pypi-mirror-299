...
class AIGovInventoryUtilities():
        ...
        **Delete the inventory**.
        ...
        try:
            ...
            if response.status_code == 204:
                ...
                error_message = response.json().get('message', 'Unknown error occurred')
                raise ClientError(f"Failed to delete inventory with ID '{self._inventory_id}'. Status code: {response.status_code}, Error: {error_message}")
    ...
    def add_collaborator(self, role: str, user_id: str = None, access_group_id: str = None):
        """
        **Add a collaborator to the inventory**.
        ...
        try:
            ...
            if access_group_id:
                ...
                creator_name = self._fetch_user_name(user_id=user_id)
            ...
            if user_id:
                collaborator_url = f"{base_url}/{user_id}"
            if access_group_id:
                collaborator_url = f"{base_url}/{access_group_id}"
            ...
            if response.status_code == 200:
                ...
                error_message = f"Failed to add collaborator. Status code: {response.status_code}, Response: {response.text}"
