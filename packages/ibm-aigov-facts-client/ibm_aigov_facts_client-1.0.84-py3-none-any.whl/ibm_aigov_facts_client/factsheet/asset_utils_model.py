...
# utils===============================================================
    ...
    def set_attachment_fact(self,
        ...
        """
                Set attachment fact for a given asset. Supported for CPD version >=4.6.5.

                :param str file_to_upload: File Path. Path of the file which needs to be uploaded as an attachment.
                :param str fact_id: Fact ID against which the attachment needs to be uploaded. If no fact ID is passed, 
                                    it will be found under 'Other Attachments' in the factsheet.
                :param str description: (Optional) Description of the file to be attached.
                :param str phase_name: (Optional) Phase name in which the attachment should appear. Available options 
                                    are in :func:`~ibm_aigov_facts_client.utils.enums.Phases`.
                :param str html_rendering_hint: (Optional) HTML rendering hint. Available options are in 
                                                :func:`~ibm_aigov_facts_client.utils.enums.RenderingHints`.

                A way to use this function is:

                >>> model.set_attachment_fact(file_to_upload="./artifacts/image.png", 
        ...
        if get_factid_attachment:
            ...
            try:
                    ...
                    attachment_url = self._set_model_attachments_url(
                        asset_id=model_asset_id,container_id=model_container_id, container_type=model_container_type, name=file_name, description=description, mimetype=mimetype, fact_id=fact_id, phase_type=phase_name,html_rendering_hint=html_rendering_hint)
                    ...
                    if create_attachment_response.status_code == 200:
                        ...
                        if attachment_id:
                            ...
                            print(f" * attachment size : {response_data.get('size','')}")
    ...
    def set_cell_attachment_fact(self,
        ...
        if get_factid_attachment:
            ...
            try:
                ...
                attachment_url = self._set_model_attachments_url(
                    asset_id=model_asset_id, container_id=model_container_id, container_type=model_container_type,
                    name=file_name, description=description, mimetype=mimetype, fact_id=fact_id, phase_type=phase_name)
