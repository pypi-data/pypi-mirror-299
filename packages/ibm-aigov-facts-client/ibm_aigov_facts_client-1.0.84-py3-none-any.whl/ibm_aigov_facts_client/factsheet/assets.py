...
from ibm_aigov_facts_client.utils.enums import AssetContainerSpaceMap, AssetContainerSpaceMapExternal, ContainerType, FactsType, ModelEntryContainerType, AllowedDefinitionType, FormatType, AttachmentFactDefinitionType, Status, Risk,Task,InputMode,Role
...
class Assets:
    ...
    def create_inventory(self, name: str, description: str, cloud_object_storage_name:str=None)->AIGovInventoryUtilities:
        """
    ...
    def list_inventories(self) -> list[AIGovInventoryUtilities]:
        """
        **List all inventories**.

        This method retrieves and returns a list of all inventory associated with the account. 
        ...
        try:
            url = self._retrieve_inventory_url()
            response = requests.get(url, headers=self._get_headers())
            ...
            if response.status_code == 200:
                ...
                if catalogs:
                    ...
                    for catalog in catalogs:
                        ...
                        inventory_list.append(AIGovInventoryUtilities(
                            self,
                            inventory_id=inventory_details['Inventory ID'],
                            inventory_name=inventory_details['Name'],
                            inventory_description=inventory_details['Description'],
                            inventory_creator_name=creator_name,
                    ...
                    _logger.info("Inventory details fetched successfully")
                    _logger.info("No Inventories were found.")
                    return []
...
#=====================================  PROMPT ASSET  ======================================================================================
    ...
    def create_prompt(self,input_mode:str,name:str,task_id:str,model_id:str,prompt_details:'PromptTemplate',description:str=None,container_type:str=None,container_id:str=None)->AIGovAssetUtilities:
        """
    ...
    def get_workspace_association_info(self: object, workspace_info:List[dict[str,str]]) -> None:
        ...
        if self._is_cp4d and self._cp4d_version < "5.0.1":
            raise ClientError(
                "Version mismatch: Associated Workspaces functionality is only supported in CP4D version 5.0.1 or higher. Current version of CP4D is " + self._cp4d_version)
