...
class AuthorizationService:
    ...
    async def refresh_token(self) -> None:
        ...
        logger.debug("refresh_token: {}", self.token_repo.get_refresh_token())
        logger.debug("node_id: {}", self.node_id)
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(request_url) as response:
                try:
                    ...
                    if e.status == 404:
                        logger.error("Token not found, creating new refresh token ...")
                        return await self.create_new_refresh_token()
                except Exception as e:
                    logger.error(f"Failed to refresh token: {e}")
