...
@click.pass_obj
def setup(env):
    """subcommands for managing workspaces for plascode"""
...
def _service(env, command):
    """List existing workspaces for plascode"""
    ...
    service_unit = f"""
[Unit]
Description=plascode Supervisor Service
...
def service(env, command):
    """Install a reset service plascode"""
...
def autocomplete(env):
    ...
    subprocess.run(
        [
            "_PLASCODE_COMPLETE=bash_source plascode > ~/.plascode-complete.bash"
        ],
        shell=True,
    )
    ...
    print("source ~/.plascode-complete.bash")
