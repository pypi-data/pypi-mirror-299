...
class JsonConfigRepository(Generic[T]):
    ...
    def __init__(self, file_path: str, target_key: Optional[str] = None) -> None:
        self.base_model_cls: BaseModel = self.__class__._get_model_cls()  # type: ignore
    ...
    def _get_model_cls(cls) -> Type[T]:
        return get_args(tp=cls.__mro__[0].__orig_bases__[0])[0]
    ...
    def _load_config(self) -> T:
        with open(self.file_path, "r") as file:
            ...
            if self.target_key is None:
                return self.base_model_cls.model_validate_json(file.read())  # type: ignore
            ...
            return self.base_model_cls.model_validate(target_py_object[self.target_key])  # type: ignore
