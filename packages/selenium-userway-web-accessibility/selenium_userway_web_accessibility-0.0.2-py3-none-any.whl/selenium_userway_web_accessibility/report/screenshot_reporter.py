...
def generate_screenshot_filename(rule_id: str, idx: int) -> str:
    ...
    return f"{rule_id}-{idx}-{timestamp_base36}.png"


def save_page_screenshot(web_driver: WebDriver, report_path: str) -> dict[str, str]:
    timestamp_base36 = base36_encode(int(time.time()))
    screenshot_name = f"screenshots/full-page-screenshot-{timestamp_base36}.png"
    ...
    return {"fullPageScreenshot": f"../{screenshot_name}"}
...
def save_screenshots(
    ...
    if config.get("full_page_screenshot", False):
            ...
            for selector in selectors:
                ...
                if element.rect["width"] > 0 and element.rect["height"] > 0:
                    ...
                    screenshot_path = os.path.join(
                        config["report_path"], screenshot_name
                    )
                    try:
