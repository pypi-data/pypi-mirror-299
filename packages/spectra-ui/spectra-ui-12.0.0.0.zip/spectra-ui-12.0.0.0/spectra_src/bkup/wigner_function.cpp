		...
		vector<vector<double>> *W, int rank, int mpiprocesses)
{
	...
	m_camp->GetSrcPoints(m_srcpoint);
	if(!m_idealsrc){
...
double WignerFunction::Function4Digitizer(double uv, vector<double> *W)
{
	...
	for(int nc = 0; nc < 2*m_ncomps*m_nUVpoints; nc++){
		(*W)[nc] *= tex;
		if(nc%2 == 0){ // summation of real part
			wref += fabs((*W)[nc]);
		}
	}
...
void WignerFunction::f_Integrand_u_econv(double u, vector<double> *W)
{
	...
	if(m_camp->EnergySpreadSigma() == 0){
		...
		for(int j = 0; j < 2; j++){
			m_camp->SetTargetEnergy(index+j);
	...
	: SpectraSolver(spsolver)
{
	...
	m_wigner = new WignerFunction(m_camp, m_acclevel, Nw, m_isoddpole, m_calcstatus, m_wiglayer);
	...
	vector<vector<double>> &xyvar, vector<double> &W, int rank, int mpiprocesses)
{
	...
	if(m_single){
		...
		xyvar[SrcVarX] = vector<double> {m_conf[Xfix_]};
		xyvar[SrcVarY] = vector<double> {m_conf[Yfix_]};
		xyvar[SrcVarXp] = vector<double> {m_conf[Xpfix_]};
		xyvar[SrcVarYp] = vector<double> {m_conf[Ypfix_]};
		indices.push_back(vector<int> {SrcVarX, SrcVarY});
	}
	else{
		wigtype = GetVariables(xyvar, indices);
	}
...
int WignerFunctionCtrl::GetVariables(
