...
// AllocAlongTheta = "..\\debug\\alloc_along_theta.dat";
	...
	if(m_isidealund){
		...
		// targetlayer, rank, mpiprocesses: ÉÇÉìÉeÉJÉãÉçëŒâû
		int targetlayer = 0;
		if(IsMonteCarlo()){
			targetlayer = 1;
		}
		...
		if(trajectory.GetZsection(secidx) > 0){
			f_CreateDataSet(thetamax, m_acclevel, &fluxdens, &secidx, m_calcstatus, targetlayer);
		}
		else{
			f_CreateDataSet(thetamax, m_acclevel, &fluxdens, nullptr, m_calcstatus, targetlayer);
		}
		Prepare(m_fixep);
	}
}
...
double ComplexAmplitude::Function4Digitizer(double theta, vector<double> *exy)
{
	if(theta == 0){
		*exy = m_ExyAxis;
		...
		m_fluxdens->GetFluxItemsAt(qobs, exy, true);
		*exy *= m_coefExy;
	}
...
void ComplexAmplitude::GetSrcPoints(double srcpoints[])
{
	for(int j = 0; j < 2; j++){
		srcpoints[j] = m_srcpoint[j];
	}
}

void ComplexAmplitude::SetSrcPoints(double xy[])
{
	for(int j = 0; j < 2; j++){
		m_srcpoint[j] = xy[j]/m_dXYdUV;
	}
}
...
bool ComplexAmplitude::GetExyAmplitude(double uv[], double Exy[])
{
	if(m_noidealsrc){
		if(m_avgorbits.size() > 0){
			...
			double Etmp[2][4];
			double epred = m_ecenter/m_eparticle/m_eparticle;
...
void ComplexAmplitude::UpdateParticle(FluxDensity *fluxdens)
{
	vector<double> vtmp(4*m_ndata);
	m_fluxdens = fluxdens;
	for(int n = 0; n < m_ndivphi; n++){
		...
		for(int m = 0; m < m_nqpoints[n]; m++){
			Function4Digitizer(m_theta[n][m], &vtmp);
			for(int nj = 0; nj < 4*m_ndata; nj++){
				m_ExyR[n][nj][m] = vtmp[nj];
			}
		}
#ifdef _DEBUG
...
//-------->>>>>
	...
	m_ecenter = m_eparray[m_necenter];
	...
	m_fluxdens->GetFluxItemsAt(axis, &m_ExyAxis, true);
	m_ExyAxis *= m_coefExy;
	...
	status->SetSubstepNumber(targetlayer, m_ndivphi*3);
	...
	status->ResetCurrentStep(targetlayer);
	...
	int nsection = (int)(*secidx)[0].size();
	...
	for(int ns = 0; ns < nsection; ns++){
		...
		m_fluxdens->GetFluxItemsAt(axis, &m_GxyAxis[ns], true, zrange);
		...
		for (int n = 0; n < m_ndivphi; n++){
			...
			for(int m = 0; m < m_nqpoints[n]; m++){
				...
				m_fluxdens->GetFluxItemsAt(qobs, &ws, true, zrange);
				for (int k = 0; k < 4*nfd; k++){
	...
	vector<OrbitComponents> &orbits, double Exy[])
{
	...
	for(int nsec = 0; nsec < orbits.size(); nsec++){
		...
		for (int j = 0; j < 2; j++){
			phs = -knumber*uv[j]*m_dqduv*orbits[nsec]._xy[j];
			csn[0] = cos(phs);
			csn[1] = sin(phs);
...
void ComplexAmplitude::f_AssignEFieldUnd(double epsilon, bool issn)
{
	...
	delta[0] = delta[1] = 1.0/16.0/(double)m_nh/(double)m_acclevel;
...
void ComplexAmplitude::f_AssignEFieldWiggler(double epsilon, double deltau, bool isbma)
{
	...
	double hDelta[2], u, v, vs, gtxy[2], delta[2], adelta[2];
