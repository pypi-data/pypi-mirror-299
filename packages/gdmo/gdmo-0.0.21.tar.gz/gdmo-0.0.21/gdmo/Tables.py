...
class Landing:
    ...
    def get_all_sharepoint_files(self, MatchingRegexPattern = '^.*\.(xlsx)$'):
        ...
        print(f'SharePoint folder contains {len(files)} files, out of which {len(MyFiles)} file(s) are selected.')
    ...
    def put_bronze(self):
        ...
        #Always make sure the bronze table has delta__load_date, delta__deleted, and delta__sourcefile columns.
       
        stage = stage.withColumn('delta__load_date', lit(datetime.now()))\
                     .withColumn('delta__deleted', lit(0))\
                     .withColumn('delta__sourcefile', lit(self._file_name))

        # Reorder the columns to have delta columns at the start of the table.                      
        delta_columns     = ['delta__load_date', 'delta__deleted', 'delta__sourcefile']
        remaining_columns = [col for col in stage.columns if col not in delta_columns]

        # Select the columns in the desired order
        stage = stage.select(*delta_columns + remaining_columns)
        ...
        if self._loadtype =="overwrite":
            ...
            try:
                stage.write.mode("overwrite").saveAsTable(self._bronze_table_path)
            except Exception as e:
            ...
            try:
                stage.write.mode("append").saveAsTable(self._bronze_table_path)
            except Exception as e:
...
class Delta:
    ...
    def __init__(self, db_name, table_name, spark, catalog = 'default', container = 'default'):
        ...
        location = os.environ.get("ADLS").format(container=container, path=self._db_name)
