...
def display_snapshot_with_topo_chg(sites, conf, title='Spins', superlatt=None, quiver_kws=quiver_kws,
    ...
    kws = dict(superlatt=superlatt, quiver_kws=quiver_kws)

    fig,ax = plt.subplots(1,2,sharex=True,figsize=(9,4))
    qv,scat,tl = ax_plot_spin_2d(ax[0],sites,conf,tri=tri,Q_distri=Q_distri,color_mapping='Q',title='Q = {:6.3f}'.format(Q),scatter_size=10,**kws)
    cb_ax1 = fig.add_axes(colorbar1_position)
    cb_ax2 = fig.add_axes(colorbar2_position)
    cb1 = fig.colorbar(scat,shrink=0.3,cax=cb_ax1)
    cb1.ax.set_title('dQ')

    qv,scat,tl = ax_plot_spin_2d(ax[1],sites,conf,color_mapping='Sz_full',scatter_size=20,title=title,**kws)
    cb2 = fig.colorbar(scat,shrink=0.3,cax=cb_ax2)
    cb2.ax.set_title('$n_z$')
    cb2.set_ticks([-1,0,1])
    cb_ax2.set_ylim(-1,1)
    #fig.tight_layout()
    fig.savefig('Snapshot_with_topo_chg',dpi=300)
    if show: plt.show()
    return fig
