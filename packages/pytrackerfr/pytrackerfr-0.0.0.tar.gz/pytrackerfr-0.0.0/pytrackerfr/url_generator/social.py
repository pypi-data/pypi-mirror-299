...
class SocialURLGenerator(URLGeneratorBase):
    def valider_paramètres(self, params):
        """Valide les paramètres pour le levier Social."""
    ...
    def générer_urls(self, params):
        """Génère les URLs pour le levier Social."""
        params = {k.strip(): (v.strip() if v is not None else '') for k, v in params.items()}

        # URL de paramètres (tracking par URL)
        url_paramètres = (
            f"{params['url_destination']}?social-publisher={params['nom_support']}&"
            f"social-name={params['nom_support']}-{params['nom_campagne']}&social-location={params['nom_emplacement']}-{params['format_bannière']}&"
            f"social-creative={params['nom_bannière']}-{params['format_bannière']}&social-creativetype={params['format_bannière']}&"
            f"eseg-name={params.get('nom_segment', '')}&eseg-item={params.get('valeur_segment', '')}&"
            f"social-mediaplan={params.get('nom_plan_medias', '')}&ea-android-adid={params.get('adid', '')}&ea-ios-idfa={params.get('idfa', '')}"
        )
        
        # URL du pixel d'impression
        url_impression = (
            f'<img src="https://{params["domaine_tracking"]}/dynview/{params["site"]}/1x1.b?'
            f"social-publisher={params['nom_support']}&social-name={params['nom_support']}-{params['nom_campagne']}&"
            f"social-location={params['nom_emplacement']}-{params['format_bannière']}&social-creative={params['nom_bannière']}-{params['format_bannière']}&"
            f"social-creativetype={params['format_bannière']}&eseg-name={params.get('nom_segment', '')}&eseg-item={params.get('valeur_segment', '')}&"
            f"social-mediaplan={params.get('nom_plan_medias', '')}&ea-android-adid={params.get('adid', '')}&ea-ios-idfa={params.get('idfa', '')}&"
