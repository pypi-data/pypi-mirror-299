# url_generator/omparateur.py

from pytrackerfr.url_generator.core import URLGeneratorBase

class ComparateurPreciosURLGenerator(URLGeneratorBase):
    ...
    def générer_urls(self, params):
        ...
        url_paramètres = (
            f"{params['url_destination']}?comp-publisher={params['nom_support']}&"
            f"comp-name={params['nom_support']}-{params['nom_campagne']}&comp-creative={params['nom_bannière']}&"
            f"eseg-name={params.get('nom_segment', '')}&eseg-item={params.get('valeur_segment', '')}&"
            f"comp-mediaplan={params.get('nom_plan_medias', '')}&ea-android-adid={params.get('adid', '')}&ea-ios-idfa={params.get('idfa', '')}"
        )
        
        # URL du pixel d'impression
        url_impression = (
            f'<img src="https://{params["domaine_tracking"]}/dynview/{params["site"]}/1x1.b?'
            f"comp-publisher={params['nom_support']}&comp-name={params['nom_support']}-{params['nom_campagne']}&"
            f"comp-creative={params['nom_bannière']}&"
            f"eseg-name={params.get('nom_segment', '')}&eseg-item={params.get('valeur_segment', '')}&"
            f"comp-mediaplan={params.get('nom_plan_medias', '')}&ea-android-adid={params.get('adid', '')}&ea-ios-idfa={params.get('idfa', '')}&"
