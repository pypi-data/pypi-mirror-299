...
def generate_completions(share_path, completions):
    ...
    for path, _, files in os.walk(completions):
        if len(files)==0:
            continue
        list_entry = (os.path.join(share_path, remove_prefix(path, completions)), [os.path.join(path, f) for f in files ])
        data_files.append(list_entry)
...
setuptools.setup(
    ...
    data_files = [("share/ramalama", ["shortnames/shortnames.conf"])] + generate_completions("share", "build/completions") +  generate_man_pages("man/man1", "docs")
)
