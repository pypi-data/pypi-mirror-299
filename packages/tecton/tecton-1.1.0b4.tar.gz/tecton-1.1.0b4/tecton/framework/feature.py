...
class Calculation(Feature):
    """
    The `Calculation` class describes a Calculation feature that is applied to Realtime Feature View via the `features` param.

    ```python
    from tecton import Calculation, RealtimeFeatureView

    current_transaction_fv = RealtimeFeatureView(
        ...
        features = [
            Calculation(
                ...
                # Calculation operates directly on the `sources`
    ...
    def _to_proto(self):
        # TODO (FE-2392): Replace the placeholder logic below with something that actually parses the expression.
        return feature_view_pb2.Calculation(
            name=self.name,
            expr=self.expr,
            column_dtype=Int64.tecton_type.proto,
            root=calculation_node_pb2.CalculationNode(
                operation=calculation_node_pb2.Operation.ADDITION,
                operands=[
                    calculation_node_pb2.Operand(literal_value=calculation_node_pb2.LiteralValueType(Int64_value=2)),
                    calculation_node_pb2.Operand(literal_value=calculation_node_pb2.LiteralValueType(Int64_value=2)),
                ],
                column_dtype=Int64.tecton_type.proto,
            ),
