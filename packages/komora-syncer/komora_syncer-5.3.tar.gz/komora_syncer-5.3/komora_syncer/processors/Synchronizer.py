...
class Synchronizer:
    ...
    def __prepare_assigned_regions(self):
        ...
        """
        if (
            not self.regions_all
            or not self.municipalities_all
            or not self.districts_all
            or not self.sites
        ):
            self.__prepare_all_regions()

        municipalities_on_sites = [
            [site.address.municipalityName, site.address.districtName]
            for site in self.sites
            if site.address
        ]
        self.municipalities_assigned = [
            muni
            for muni in self.municipalities_all
            if [muni.name, muni.districtName] in municipalities_on_sites
        ]
        ...
        self.districts_assigned = [
            dis
            for dis in self.districts_all
            if dis.name in districts_on_assigned_municipalities
        ]
        ...
        sites_without_address_with_region = [
            [site.regionId, site.regionName]
            for site in self.sites
            if not site.address and site.regionId
        ]
        self.regions_assigned.extend(
            [
                reg
                for reg in self.regions_all
                if [reg.id, reg.name] in sites_without_address_with_region
            ]
        )
    ...
    def sync_organizations(self):
        ...
        for organization in organizations:
            try:
                ...
                logger.exception(
                    f"Failed to synchronize organization {organization.name}: {e}"
                )
                raise e
    ...
    def sync_organization_suppliers(self):
        ...
        for organization in organizations:
            try:
                ...
                logger.exception(
                    f"Failed to synchronize organizations (suppliers) {organization.name}: {e}"
                )
                raise e
    ...
    def sync_devices(self):
        ...
        devices_not_in_komora = NetboxConnection.get_connection().dcim.devices.filter(
        ...
        try:
            if NetboxConnection.get_connection().dcim.devices.update(
                devices_to_unset_links
            ):
    ...
    def sync_inventory_items(self):
        ...
        invs_not_in_komora = (
            NetboxConnection.get_connection().dcim.inventory_items.filter(
                cf_komora_id__n=komora_ids
            )
        )
        ...
        # Update netbox DB
        try:
            if NetboxConnection.get_connection().dcim.inventory_items.update(
                invs_to_update
            ):
