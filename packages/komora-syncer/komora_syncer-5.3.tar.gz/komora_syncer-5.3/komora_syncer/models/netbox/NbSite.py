...
class NbSite(NetboxBase):
    def __init__(self, komora_obj):
        ...
        try:
            ...
            logger.exception(f"Unable to find region {komora_obj.address} for site {self.name}")
        ...
        try:
            ...
            logger.exception(f"Unable to find tenant {komora_obj.organization} for site {self.name}")
