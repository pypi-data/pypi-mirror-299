...
class NbDevice(NetboxBase):
    def __init__(self, komora_obj):
        ...
        self.komora_url = (
            f"{get_config()['komora']['KOMORA_URL']}/app/device/{self.komora_id}"
        )
    ...
    def find(self):
        ...
        try:
            ...
            logger.exception(
                f"Unable to get Device by komora_id: {self.komora_id}. Error: {e}"
            )
        ...
        try:
            ...
            netbox_device = self.nb.dcim.devices.get(
    ...
    def get_nb_devices_data():
        ...
        filter_device_roles = list(
            nb.dcim.device_roles.filter(name=["Passive component", "unknown", "Server"])
        )
        filter_device_role_ids = [device.id for device in filter_device_roles]

        devices = list(
            nb.dcim.devices.filter(
                status="active", name__empty=False, role_id__n=filter_device_role_ids
            )
        )
        device_ids = [device.id for device in devices]
        interfaces = list(nb.dcim.interfaces.filter(device_id__in=device_ids))
                ...
                "device_list": [
                    {
                        ...
                        "location": NbDevice.get_info_from_cache(
                            location_cache, device.location.id
                        )
                        if device.location
                        else None,
                        "tenant": NbDevice.get_info_from_cache(
                            tenant_cache, device.tenant.id
                        )
                        if device.tenant
                        else None,
                        "site": NbDevice.get_info_from_cache(
                            site_cache, device.site.id, include_tenant=True
                        )
                        if device.site
                        else None,
