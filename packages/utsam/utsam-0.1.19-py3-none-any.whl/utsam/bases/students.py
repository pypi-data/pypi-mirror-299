...
def get_students_ids_by_employee(employee, data):
    ...
    if len(temp_df) > 0:
        return [_SELECT_] +  list(set(temp_df[PgStudentsTable.student_id].sort_values().unique()))
    return [_SELECT_]
...
class AssignmentMarkingStudent:
    ...
    def set_score_and_grade(self):
        ...
        if len(self.student_marks_df) > 0:
            self.reset_score_and_grade()
            self.student_marks_df[PgAssignmentSubcriteriaStudentScoresTable.subcriteria_student_mark] = self.student_marks_df[PgAssignmentSubcriteriaStudentScoresTable.subcriteria_student_score] * self.student_marks_df[PgAssignmentSubcriteriasTable.subcriteria_weight]
            self.score = (self.student_marks_df[PgAssignmentSubcriteriaStudentScoresTable.subcriteria_student_mark]).sum()
            self.grade = get_grade(self.grades_df, self.score)
    ...
    def display_student_marks(self):
        ...
        if self.grade:
            ...
            for row_id in range(len(self.student_marks_df)):
                ...
                print(f"  -> {self.student_marks_df.at[row_id, PgAssignmentCriteriaStudentScoresTable.criteria_student_score]} ({get_grade(self.grades_df, self.student_marks_df.at[row_id, PgAssignmentCriteriaStudentScoresTable.criteria_student_score])})\n")
                print(f"  -> {self.student_marks_df.at[row_id, PgAssignmentCriteriaStudentScoresTable.criteria_student_score_comments]}")
        else:
