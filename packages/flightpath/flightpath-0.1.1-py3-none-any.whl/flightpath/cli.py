...
def critical_path(dag_id: str, dag_run_id: str, duckdb_location: str) -> None:
    ...
    CREATE TABLE critical_path (
        ...
        pct_of_critical_path DECIMAL(6,6)
    )
    """)
    ...
    SELECT
      *,
      total_seconds / sum(total_seconds) over () as pct_of_critical_path
    FROM final
    """
    
    # Insert results into the critical_path table
    conn.execute(f"""
    INSERT INTO critical_path (dag_id, dag_run_id, task_id, path_index, unblocked_date, start_date, end_date, ready_seconds, running_seconds, total_seconds, pct_of_critical_path)
    {query}
    """, [dag_id, dag_run_id] * 5)
    
    # Fetch results for printing
    results = conn.execute("""
    SELECT path_index,task_id, unblocked_date, start_date, end_date, ready_seconds, running_seconds, total_seconds, pct_of_critical_path
    FROM critical_path
    ...
    headers = ["Path Index", "Task ID", "Unblocked Date", "Start Date", "End Date", "Ready (Seconds)", "Running (Seconds)", "Total (Seconds)", "Pct of Critical Path"]
