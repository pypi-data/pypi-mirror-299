...
class tabulatedLFS(OFTabulation,laminarFlameSpeedModel):
    ...
    def Su(self,p,T,phi,EGR=None, **argv):
        ...
        if not len(phi) == self.nFuels:
            self.fatalErrorInClass(f"Entry 'phi' of wrong length. Must be consistent with nFuels ({nFuels}).")
    ...
    def deltaL(self,p,T,phi,EGR=None, **argv):
        ...
        for pp in phi:
            laminarFlameTeThickness.Su(self,p,T,pp,EGR)
