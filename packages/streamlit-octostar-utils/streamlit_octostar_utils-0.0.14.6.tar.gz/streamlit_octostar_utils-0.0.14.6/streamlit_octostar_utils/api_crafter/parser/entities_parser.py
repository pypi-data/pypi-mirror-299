...
class EntitiesParserRuleset(object):
    ...
    def load(self, full_ruleset, parser):
        ...
        with open(self.source_file, "r") as file:
            ...
            tree = eval(self.ruleset_var_name, globals())
            ...
            del globals()[self.ruleset_var_name]
...
class EntitiesParser(object):
    ...
    def validate_and_format_entities(self, entities, relationships):
        ...
        for entity in entities:
            ...
            if any(entity_properties not in ontology_properties):
        ...
        ontology_relationships = {r['relationdship_name']: r for r in self.ontology['relationships']}
        for rel in relationships:
            if rel.name not in ontology_relationships.keys():
                processsed_relationships.append((EntitiesInvalidReason.INVALID_REL_TYPE))
                continue
            ...
            if rel.concept_from not in entities_by_id.keys() or rel.concept_to not in entities_by_id.keys():
                processsed_relationships.append((EntitiesInvalidReason.UNKNOWN_UUID))
                continue
            ...
            if not is_child_concept(rel_concept_types[0], ontology_relationship['source_concept'], self.ontology) or \
               not is_child_concept(rel_concept_types[1], ontology_relationship['target_concept'], self.ontology):
                processsed_relationships.append((EntitiesInvalidReason.WRONG_SOURCE_OR_TARGET_TYPE))
                continue
