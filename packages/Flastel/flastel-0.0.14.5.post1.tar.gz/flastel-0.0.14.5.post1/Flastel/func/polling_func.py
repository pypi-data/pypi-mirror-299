...
def connect_db():
    return sqlite3.connect('module_data.db')

def create_table():
    with connect_db() as conn:
        cursor = conn.cursor()
        cursor.execute('''CREATE TABLE IF NOT EXISTS bot_updates 
                        (bot_token TEXT PRIMARY KEY, last_update_id INTEGER)''')
        conn.commit()
...
class TelegramPollingBot:
    def __init__(self, bot_token, refusal_disconnect=False, in_old=True, ram_control=False, logic_on=False, pro_logaut=False):
        self.bot_token = bot_token if isinstance(bot_token, list) else [bot_token]
        self.api_url = [f"https://api.telegram.org/bot{token}/" for token in self.bot_token]
        ...
        self.universal_handler = {}
        self.last_update_id = {token: self.def_load_last_update_id(token) for token in self.bot_token}
    ...
    def save_last_update_id(self, token=None, offset=None):
        with connect_db() as conn:
            cursor = conn.cursor()
            cursor.execute('''INSERT OR REPLACE INTO bot_updates (bot_token, last_update_id) 
                              VALUES (?, ?)''', (token, offset))
            conn.commit()

    def def_load_last_update_id(self, token):
        with connect_db() as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT last_update_id FROM bot_updates WHERE bot_token = ?', (token,))
    ...
    async def get_updates(self, offset=None):
        for url in self.api_url:
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.get(url + "getUpdates", params=params) as response:
                        if response.status == 200:
                            data = await response.json()
    ...
    async def run_polling(self):
        ...
        for token in self.bot_token:
            bot_info = await self.get_me()
            print(f"Running {bot_info.first_name} in @{bot_info.username}")
            if self.in_old:
                offset = self.def_load_last_update_id(token)
            else:
                offset = 0
        while True:
            for token in self.bot_token:
                    ...
                    for update in updates:
                        ...
                        if message_data:
                            ...
                            if successful_payment:
                                ...
                                self.save_last_update_id(offset, token)
                                if handler:
    ...
    async def process_message(self, message, offset=0):
        ...
        if text and text.startswith("/"):
            ...
            elif message.text and "text" in self.message_handlers:
                logger.info("Обробляється текст")
                await self.message_handlers["text"](message)
            elif text and "message_auto_delete_timer_changed" in self.message_handlers:
                await self.message_handlers["auto_delete_timer_changed"](message)
            elif message.reply_to_message and "reply_to_message" in self.universal_handler:
                await self.universal_handler["reply_to_message"](message)
            elif message.forward_origin and "forward_origin" in self.universal_handler:
                await self.universal_handler["forward_origin"](message)
            elif message.new_chat_members and "new_chat_members" in self.universal_handler:
                await self.universal_handler["new_chat_members"](message)
            elif message.left_chat_member and "left_chat_member" in self.universal_handler:
                await self.universal_handler["left_chat_member"](message)
            else:
                if message.text and "text" in self.message_handlers:
                    await self.message_handlers["text"](message)
    ...
    async def ok_pay(self, query_data):
        ...
        for Url in self.api_url:
            url = f"{Url}answerPreCheckoutQuery"
            async with aiohttp.ClientSession() as session:
    ...
    async def send_message(self, chat_id, text, reply_to_message_id=None, parse_mode=None, callback=None, reply_keyboard=None):
    ...
    def universal_handler(self, event_type):
        def wrapper(func):
            self.universal_handlers[event_type] = func
            return func
        return wrapper
