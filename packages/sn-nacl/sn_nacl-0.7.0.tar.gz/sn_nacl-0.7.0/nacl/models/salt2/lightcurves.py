...
class LightcurveEvalUnit:
    ...
    def __init__(self, model, **kwargs):
        ...
        self.color_law_out_of_the_integral = kwargs.get('color_law_out_of_the_integral', False) or self.model.color_law_out_of_the_integral
        ...
        try:
            # pre computing extinction
            ebv = self.training_dataset.sn_data.nt['mwebv'][self.training_dataset.lc_data.sn_index]
            ext_model = self.model.dust_extinction_model
            
            wl_mic = self.basis_wavelengths * 1e-4
            wl_inv = 1/wl_mic
            ext = ext_model.extinguish(wl_inv, Ebv=ebv.reshape(-1,1))
            
            #nn, _ = self.original_meas_filter_projections.shape
            #ext = np.broadcast_to(ext, (nn, len(ext)))
            
            self.extinction = ext #* ebv[:, np.newaxis]
            self.meas_filter_projections = np.multiply(self.original_meas_filter_projections, self.extinction)
        except:
            logging.info('No dust extinction model added')
            self.meas_filter_projections = self.original_meas_filter_projections
        # correction terms -- we may not need them anymore
        # if not self.color_law_out_of_the_integral:
        #     gram2 = self.model.G2
        #     self.filter_projections_2 = (gram2 @ F.T).T
        #     self.meas_filter_projections_2 = \
        #         self.filter_projections_2[self.training_dataset.lc_data.lc_index]
