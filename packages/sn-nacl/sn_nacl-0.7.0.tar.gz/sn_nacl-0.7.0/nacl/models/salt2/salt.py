...
class SALT2Like(object):
    ...
    def __init__(self, training_dataset,
        ...
        wl_range : tuple of float, optional
            Nominal wavelength range of the model, by default (3000., 9000.).
        basis_knots : list of int, optional
            Number of wavelength and photometric knots, by default [127, 20].
        basis_filter_knots : int, optional
            Number of knots for the filter basis, by default, 900.
        ...
        self.filter_wl_range = filter_wl_range
        ...
        # how we handle the color law
        self.color_law_out_of_the_integral = color_law_out_of_the_integral
        ...
        valid = SALT2Like.flag_out_of_range_datapoints(training_dataset,
                                                       phase_range=phase_range,
                                                       wl_range=wl_range,
                                                       basis_knots=basis_knots,
                                                       basis_filter_knots=basis_filter_knots,
                                                       wl_grid=wl_grid, phase_grid=phase_grid,
                                                       compress=False)
        n_out = list(map(lambda x: 0 if x is None else (~x).sum(), valid))
        if n_out[0] > 0:
            logging.warning(f'{n_out[0]} lc data points outside model definition range')
        if n_out[1] > 0:
            logging.warning(f'{n_out[1]} spec data points outside model definition range')
        if n_out[2] > 0:
            logging.warning(f'{n_out[2]} spectrophotometric data points outside model definition range')

        # Filter database
        # TODO: we should be able to optimize this a little
        # (i.e. adapt the filter bases, to lower the size of the grams)
        self.filter_basis = bspline.BSpline(np.linspace(self.filter_wl_range[0],
                                                        self.filter_wl_range[1],
                                                        basis_filter_knots), order=4)
        self.filter_db = SNFilterSet(basis=self.filter_basis)
        # should be able to specify this as parameters
        self.filter_db.insert('swope2::b', z=0.)
        self.filter_db.insert('swope2::v', z=0.)
        ...
        self.basis = SALT2Like._init_bases(wl_grid, phase_grid,
    ...
    def flag_out_of_range_datapoints(cls, training_dataset,
        ...
        wl_range : tuple of float, optional
            The valid range of wavelengths (default is (2000., 9000.)).
        basis_knots : list of int, optional
            The number of knots for the B-spline bases in wavelength and phase (default is [127, 20]).
        basis_filter_knots : int, optional
        ...
        model_basis = cls._init_bases(wl_grid, phase_grid,
                                      wl_range, phase_range, basis_knots)
        ...
        if tds.lc_data is not None:
            ...
            # logging.info(f' --> {(~((restframe_phase >= ph_min) & (restframe_phase <= ph_max))).sum()}')
            n_out = (~lc_valid).sum()
            if n_out > 0:
                logging.warning(f'{n_out} lc data points outside the model definition range')
        else:
            lc_valid = None
        ...
        if tds.spec_data is not None:
            ...
            if n_out > 0:
                logging.warning(f'{n_out} spec data points outside the model definition range')
        else:
            spec_valid = None
        ...
        if tds.spectrophotometric_data is not None:
            ...
            if n_out > 0:
                logging.warning(f'{n_out} spectrophot data points outside the model definition range')
        else:
    ...
    @classmethod
    def _init_bases(cls, wl_grid=None, phase_grid=None,
                    wl_range=(2000., 9000.),
        ...
        wl_range : tuple, optional
            Wavelength range for the default grid, by default (2000., 9000.).
        phase_range : tuple, optional
            Phase range for the default grid, by default (-20., 50.).
        basis_knots : tuple, optional
            Number of knots in wavelength and phase, by default (127, 20).
    ...
    def __call__(self, pars, jac=False): # , plotting=False, ilc_plot=None, spec_plot=False):
        ...
        if cached_res is not None:
            logging.debug('SALT2Like.__call__: returning cached result')
            self.cached = True
            return cached_res
        else:
            self.cached = False
