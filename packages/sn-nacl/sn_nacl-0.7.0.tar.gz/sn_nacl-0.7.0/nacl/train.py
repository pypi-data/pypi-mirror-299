...
class TrainSALT2Like:
    ...
    def __init__(self, tds, variance_model=None, calib_variance=None, color_scatter=False):
        """
        Parameters
        ----------
        ...
        >>> trainer = TrainSALT2Like(tds, variance_model='simple_snake')  #doctest: +ELLIPSIS
        ...
        if calib_variance is not None:
            self.prior = [salt2.CalibErrorModel(self.model, calib_variance=calib_variance)]
        else:
            self.prior = None
    ...
    def train_salt2_model(self, save=False, path=None):
        ...
        if save and path is not None:
            ...
            try:
                ...
                logging.info('No SN weights to save')
            ...
            try:
                ...
                logging.error(f"Hessian is not positive definite. I save H for degguging.")
    ...
    def train_salt2_model_test(self, save=False, path=None):
        """
        Model training without any error models
        """
        first_fixed_pars = ['M0', 'M1', 'CL']
        full_pars = ['X0', 'X1', 'c', 'tmax', 'M0', 'M1', 'CL', 'SpectrumRecalibration']
        ...
        f3 = self.fit(fix=['gamma_sn', 'gamma_snake'],
                      fit_variance_model=True,
                      p_init=f2.ll.pars,
                      max_iter=100)
        #f2.v_var = np.zeros(len(f2.v))
        ...
        if save and path is not None:
            ...
            columns_rename = {'x0':'x0_init', 'x1':'x1_init', 'c':'c_init', 'tmax':'tmax_init'}
            sn_data = sn_data.rename(columns = columns_rename)
            sn_data = sn_data.assign(x0 = f3.pars['X0'].full[self.tds.sn_data.sn_index])
            sn_data = sn_data.assign(x1 = f3.pars['X1'].full[self.tds.sn_data.sn_index])
            sn_data = sn_data.assign(c = f3.pars['c'].full[self.tds.sn_data.sn_index])
            sn_data = sn_data.assign(t0 = f3.pars['tmax'].full[self.tds.sn_data.sn_index])
            sn_data.to_parquet(path + '_params' + '.parquet')
            try:
                cov_mat = f2.minz.get_cov_matrix()[0]
                cov_reduced = cov_mat[:4 * len(self.tds.sn_data), :4 * len(self.tds.sn_data)]
                ...
                logging.error(f"Hessian is not positive definite. I save H for degguging.")
                llk, grad, H = f3.ll(f3.ll.pars.free, deriv=True)
                H_reduced = H[:4 * len(self.tds.sn_data), :4 * len(self.tds.sn_data)]
                ii = [self.tds.sn_data.sn_index + i * len(self.tds.sn_data) for i in range(4)]
                iii = np.hstack(ii)
                H_reduced = H_reduced[:, iii][iii]
...
#        axes[2].plot(ii[idx], v_recal[idx], ls='', marker='.', color='r')
    ...
    def plot_control(self):
        ...
        if self.color_scatter:
            ...
            control_plot.plot_cl(self.tds, self.log, self.log[-1].ll.model.color_law)
        control_plot.plot_stacked_residuals_snake(self.tds, self.log[-1].pars, self.log[-1].v, self.log[-1].v_var)
        control_plot.plot_band_residuals_z(self.tds, self.log[-1].v, self.log[-1].v_var)
        control_plot.plot_band_residuals_phase(self.tds, self.log[-1].pars, self.log[-1].v, self.log[-1].v_var)
        control_plot.plot_band_pulls_phase(self.tds, self.log[-1].pars, self.log[-1].v, self.log[-1].v_var)
