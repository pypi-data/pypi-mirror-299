...
class AttitudeLawType(str, Enum):
    """
    - ```EARTH_ORIENTED_MANEUVER```** Aligns the thruster axis to a given maneuver direction,    expressed in a local orbital frame. A rotation around the thruster axis is computed in order    to have a user-defined axis oriented towards the Earth as much as possible.    __Note: applies to BoxObject with a propulsion system only.__ - ```LOCAL_FRAME_ATTITUDE``` Attitude law defined by rotation angles around the axes of a local    orbital frame. By default the space object axes are aligned with those of the orbital frame. - ```NADIR_POINTING``` Attitude law with a space object-fixed frame axis pointing towards nadir.   Other directions are arbitrary. - ```SUN_ENHANCED_MANEUVER``` Aligns the thruster axis to a given maneuver direction, expressed    in a local orbital frame. A rotation around the thruster axis is computed in order to optimize   the lighting of the solar array.   __Note: applies to BoxObject with a propulsion system only.__ - ```SUN_POINTING``` Attitude pointing the solar array normal towards the Sun, with the last    degree of freedom of the rotation set arbitrarily.   __Note: applies to BoxObject with solar array geometry only.__ 
    """
