...
class ProcessManager():
    def __init__(self, dir_, config):
        ...
        if self.config["exclude_strings"]:
            self.student_n_regex = f'(?<={self.config["filename_format"]})_?\\d*_?s?(?P<digits>{self.config["student_n_format"]})((?!{self.config["exclude_strings"]}).)*$'
        else:
            self.student_n_regex = f'(?<={self.config["filename_format"]})_?\\d*_?s?(?P<digits>{self.config["student_n_format"]}).*$'
    ...
    def start(self):
        ...
        for file in self.dir.rglob("*.py"):
            ...
            if not re.search(str(self.dir), str(file.as_posix()), flags=re.IGNORECASE):
                continue
