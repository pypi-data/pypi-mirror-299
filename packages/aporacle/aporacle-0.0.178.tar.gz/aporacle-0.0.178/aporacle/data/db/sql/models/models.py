...
def generate_code(mapper, connection, target):
    ...
    if max_code is None:
        new_code = 100000  # Start from 100000 if no existing feeds
    else:
        new_code = max_code + 10000  # Increment by 10000 for each new feed
...
def generate_symbol_code(mapper, connection, target):
    ...
    if max_symbol_code is None:
        new_symbol_code = feed_code + 10  # Start from feed_code + 10 for the first symbol
    else:
        new_symbol_code = max_symbol_code + 10  # Increment by 10 for each new symbol
