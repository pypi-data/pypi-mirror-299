...
from mobilesamv2.build_sam import sam_model_registry_mobile
from mobilesamv2.predictor import SamPredictorMobile
...
class GSamnet(nn.Module):
    ...
    def __Build_MobileSAM(self,
        ...
        """
        try:
            sam_checkpoint = "src\mobilesamv2\weights\mobile_sam.pt"
            sam = sam_model_registry_mobile[SAM](sam_checkpoint)
            sam.to(device=self.device)
            sam.eval()
...
def load_models(model,submodel: Optional[str] = None):

    if model == "dino":
        modelo = GSamnet()._GSamnet__Build_GroundingDINO(return_model=True)
        modelo.name = "dino"
        return modelo
    elif model == "mobile" and submodel is not None:
        modelo = GSamnet._GSamnet__Build_MobileSAM(submodel,return_model=True)
        modelo.name = "SAM1"
    else:
...
if __name__ == "__main__":
    ...
    """
    model = load_models("mobile","vit_b")
