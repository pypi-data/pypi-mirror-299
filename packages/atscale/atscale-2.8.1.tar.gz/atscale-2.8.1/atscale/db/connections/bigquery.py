    ...
    def write_df_to_db(
        ...
        for col in dataframe.columns:
            ...
            if pd.notnull(first_valid_index):
                ...
                null_columns.append(col)  # don't want to warn multiple times

        if null_columns:
            logging.warning(
                f"Writing columns without valid values may results in incorrect column types due to"
                f"the pandas 'object' datatype. No cell with a nonnull value exists for the "
                f"following columns: {null_columns}"
            )
        ...
        try:
            ...
            if table_exists is False:
                sql = f"DROP TABLE IF EXISTS {self._create_table_path(table_name)}"
                self.execute_statements([sql])
            if isinstance(err, ArrowTypeError):
