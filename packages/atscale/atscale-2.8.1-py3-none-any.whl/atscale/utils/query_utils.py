...
def _generate_atscale_query(
    ...
    for param in list_params + [ordering_features] + [group_by]:
        model_utils._check_features(
    ...
    if group_by:
        group_string = f' GROUP BY `{"`, `".join(group_by)}`'
    else:
        categorical_columns = [
            f"`{name}`"
        ...
        group_string = f' GROUP BY {", ".join(categorical_columns)}' if categorical_columns else ""
    ...
    query = (
        ...
        f"{filter_string}{group_string}{order_string}{limit_string}{comment_string}"
    )
    return query
...
def _generate_atscale_query_postgres(
    ...
    join_str = '", "'
    if group_by:
        group_string = f' GROUP BY "{join_str.join(group_by)}"'
    else:
        categorical_columns = [
            f'"{name}"'
            for name in feature_list
