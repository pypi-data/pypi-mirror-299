...
sed -i "s/@@VMNAME@@/${VMNAME}/;s/@@KERNEL@@/${KERNEL_FILENAME}/;s/@@DISK@@/${DISK_FILENAME}/;s/@@APPSIMAGE@@/${APPSIMAGE_FILENAME}/" "${TMPDIR}/vm_create.sh"
chmod +x "${TMPDIR}/vm_create.sh"

tar -cjpf "${OUTPUTNAME}" "${KERNEL_PATH}" "${DISK_PATH}" "${APPSIMAGE}" "${TMPDIR}/vm_create.sh" --transform='s#.*/##'
