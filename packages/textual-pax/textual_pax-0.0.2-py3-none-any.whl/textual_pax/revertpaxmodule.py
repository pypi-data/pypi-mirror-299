...
class PaxTerms():
    ...
    def __init__(self, serialNo:str) -> None:
        ...
        self.termtool = {
            ...
            "3A4": {"modelName": "SP30s", "serial_range":"3A4", "name":"TR_SP30", "type":"A_Series", "terminal_no":"180", "resellerName": "Repair", "hasAccessory":False},
            "3A6": {"modelName": "SP30s", "serial_range":"3A6", "name":"TR_SP30", "type":"A_Series", "terminal_no":"180", "resellerName": "Repair", "hasAccessory":False},
            "3A7": {"modelName": "SP30s", "serial_range":"3A7", "name":"TR_SP30", "type":"A_Series", "terminal_no":"180", "resellerName": "Repair", "hasAccessory":False},
            "3L0": {"modelName": "SP30s", "serial_range":"3L0", "name":"TR_SP30", "type":"A_Series", "terminal_no":"180", "resellerName": "Repair", "hasAccessory":False},
            ...
            "114": {"modelName": "Q20L", "serial_range":"114","name":None, "type":"Qv1_Series", "hasAccessory":False, "resellerName":"Repair", "fmName":None},
            "240": {"modelName": "Q10A", "serial_range":"240","name":None, "type":"Qv2_Series","fmName": "Q10A_PayDroid_10_Cedar_V17.1.13_20240322","resellerName":"Repair", "hasAccessory":False},
            "189": {"modelName": "D135", "serial_range":"189", "name":"TR_D135", "type":"D-Series", "terminal_no":"421", "resellerName": "Repair", "hasAccessory":False},
        }
        
        self.pushPATool = {
                "A_SeriespackageName": "com.pax.us.pay.std.broadpos.p2pe",
                "A_SeriespushTemplateName": "PA 6.8 A-Series BroadPos P2PE V1.05.05",
                "A_SeriestemplateName":  "p2pe_20231218.zip",
                "A_Seriesversion": "V1.05.05_20231218",
                ...
                "Qv1_SeriespushTemplateName": "RPA 6.8 Q20 BroadPOS P2PE Q20L V1.01.05_20230413",
...
async def accessoryDetails(df:pd.DataFrame):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        """for resp in responses:
            #print(resp)
            if "terminalAccessory" in resp['data'].keys():
                basic = resp['data']['terminalAccessory']['basic']
                accessoryList.append(str(*[dic.get("content") for dic in basic if dic.get('name')=="SN"]))
            else: accessoryList.append(None)"""
...
class apiPaxFunctions(): 
    ...
    async def startPaxGroup(self, serialNoList):
        self.idList = []
    ...
    async def createTerminals(self,serialNoList):
        ...
        async with aiohttp.ClientSession() as session:
            for serialNo in serialNoList: 
                ...
                tasks.append(asyncio.ensure_future(buildRequest(serialNo=serialNo,session=session, operation="createTerminal",merchantName="North American Bancard",name=terminal.name,modelName=terminal.modelName,resellerName=terminal.resellerName,status = "A")))
            result = await asyncio.gather(*tasks) 
            for resp in result:
...
class PushConfigs():
    ...
    async def pushPAConfig(self, serialNoList, idList = None, df = None):
        ...
        #bpospush = await task.pushTerminalAPK(serialNoList,True)
        #print("BroadPosP2PEPush:",bpospush)
        installerPush = await task.pushTerminalAPK(serialNoList,False, "com.nabancard.painstaller")
...
async def main():
    ...
    serialNoList = ['0822776026', '0821674870', '0821256789']
    for serialNo in serialNoList:
        term = PaxTerms(serialNo)
        print(term.modelName,term.hasAccessory,term.accessoryModelName)
    thing = apiPaxFunctions()    
    thing2 = await findTerminal(serialNoList)
    print(thing2)
    
    #q20List = await accessoryDetails(thing.idList)
