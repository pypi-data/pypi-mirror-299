...
class PandasPart(IFeedPart):
    ...
    def __getattr__(self, name: str) -> pd.Series:
        """
        Allows accessing DataFrame columns as attributes.

        Args:
            name (str): The name of the column to access.

        Returns:
            pd.Series: The pandas Series corresponding to the column.

        Raises:
            AttributeError: If the specified column does not exist in the DataFrame.
        """
        try:
            return self._df[name]
        except KeyError:
            raise AttributeError(f"'Wrapper' object has no attribute '{name}'")
    
    def __setattr__(self, name: str, value: list) -> None:
        """
        Allows setting DataFrame columns as attributes.
        
        Args:
            name (str): The name of the column to set.
            value (list): The list of values to assign to the column.
        """
        self.df[name] = value
