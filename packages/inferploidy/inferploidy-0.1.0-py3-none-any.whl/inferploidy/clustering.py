...
# from scoda.icnv import initially_detect_major_clusters
...
def clustering_alg(X_pca, clust_algo = 'lv', N_clusters = 25, resolution = 1, 
    ...
    elif clust_algo[:2] == 'cc':
        if adj_dist is None:
            adj_dist = kneighbors_graph(X_pca, int(N_neighbors), mode='distance', include_self=True, 
                                   n_jobs = n_cores)
            
        cluster_label, adj_dist, louvain, membership_mat = \
            Consensus_clustering(X_pca, adj_dist, n_neighbors = N_neighbors,
                                 resolution = resolution,
                                 n_runs = n_resampling_runs,
                                 resampling_rate = resampling_rate,
                                 min_cluster_size = min_cluster_size,
                                 consensus_thresold = consensus_thresold,
                                 max_iter = 1000, n_cores = n_cores,
                                 verbose = False )    
...
def merge_clusters_with_seed_old( cluster_adj_mat, clust_size, seed = None,
                              n_neighbors = 10,
    ...
    for c in cluster_lst:
        if c not in selected_clusters:
            ...
            merged_clusters, addeded_clusters, connectivities = \
                merge_clusters_with_seed( cluster_adj_mat, clust_size, seed = seed_clusters,
                                          n_neighbors = n_neighbors,
...
def cluster_tight(df_mat_in, th = 0.5, min_cluster_size = 20, verbose = False):

    df_mat = df_mat_in.copy(deep = True)
    idxs = df_mat.index.values
    idx_processed = []
    cluster = pd.Series([-1]*df_mat.shape[0], index = df_mat.index )
    # cluster.loc[:] = None
    visit = pd.Series([1]*df_mat.shape[0], index = df_mat.index )
    cnt = 0
    for i in range(df_mat.shape[0]):
        mx = ((df_mat - np.diag(np.diag(df_mat))).max(axis = 0)*visit).max()
        sm = ((df_mat >= th).sum(axis = 0)*visit).max()
        if (visit.sum() == 0) | (mx <= th) | (sm < min_cluster_size):
            break
        else:
            j = ((df_mat >= th).mean(axis = 0)*visit).idxmax()
            # print(j)
            b = df_mat.loc[j,:] >= th
            # print(np.sum(b))
            idx_sel = idxs[b]
            visit.loc[idx_sel] = 0
            df_mat_sel = df_mat.loc[list(idx_sel),list(idx_sel)].copy(deep = True)
            # print(df_mat_sel.sum().sum())
            # for m in list(df_mat_sel.index.values):
            for k in range(df_mat_sel.shape[0]):
                # print(df_mat_sel.shape)
                m = df_mat_sel.mean(axis = 0).idxmin()
                # print(df_mat_sel.shape, df_mat_sel.loc[m,:].min())
                if df_mat_sel.loc[m,:].min() < th:
                    df_mat_sel.drop(index = [m], columns = [m], inplace = True)
                if df_mat_sel.min().min() >= th:
                    # idx_sel = df_mat_sel.index.values.tolist()
                    break
        
            if verbose: print('%i -> %i' % (len(idx_sel), df_mat_sel.shape[0]))
            idx_sel2 = list(df_mat_sel.index.values) 
            idx_processed = idx_processed + idx_sel2   
            cluster.loc[idx_sel2] = cnt
            cnt += 1
            df_mat.loc[list(idx_sel2),:] = 0
            df_mat.loc[:,list(idx_sel2)] = 0
            for k in idx_sel2:
                df_mat.loc[k,k] = 1
            # if (len(idx_processed) >= df_mat.shape[0]*0.9) | (len(idx_sel2) < df_mat.shape[0]*0.1):
            if visit.sum() == 0:
                break

    return cluster # .astype(str)


def Consensus_clustering( X_pca, adj = None, n_neighbors = 10,
                         resolution = 1,
                         n_runs = 50, 
                         resampling_rate = 0.9,
                         min_cluster_size = 20,
                         consensus_thresold = 0.5,
                         max_iter = 1000, n_cores = 4,
                         verbose = True ):

    if adj is None:
        adj = kneighbors_graph(X_pca, n_neighbors, mode='distance', include_self=True)
    louvain = Louvain(resolution = resolution, shuffle_nodes = True)
    
    membership_mat = pd.DataFrame( np.zeros((X_pca.shape[0], X_pca.shape[0])) )
    selection_mat = pd.DataFrame( np.zeros((X_pca.shape[0], X_pca.shape[0])) )
    for i in range(n_runs):
        idx_sel = random.sample( list(np.arange(X_pca.shape[0])), int(X_pca.shape[0]*resampling_rate))
        adj_tmp = adj[idx_sel,:][:,idx_sel]
        cluster_label_sel = louvain.fit_predict(adj_tmp)     
        cluster_label = pd.Series(None, index = membership_mat.index, dtype = float )
        cluster_label.loc[idx_sel] = list(cluster_label_sel)
        clst = list(set(list(cluster_label_sel)))
        for c in clst:
            b = (cluster_label == c) & (~cluster_label.isnull())
            membership_mat.loc[b,b] += 1
        selection_mat.loc[idx_sel,idx_sel] += 1
    
    membership_mat = membership_mat/selection_mat    
    cluster = cluster_tight(membership_mat, th = consensus_thresold, 
    ...
    if verbose: 
        print('N_clusters: %i, Not assigned/N: %i, %i ' % (len(cluster.unique())-1, len(b)-b.sum(), len(b)))
    ...
    cluster[~b] = y_pred_test
    
    if verbose: 
        y_pred_train = classifier.predict(X_train)
        accuracy_train = 100*sum(y_train==y_pred_train)/len(y_train)
        print('Acc: %5.1f ' % accuracy_train)

    return adj, cluster, membership_mat
