...
from xync_schema.models import Pair, Pm, Ex, TradeType
...
async def cycle():
    cn = await init_db(dsn, models)
    bn_sapi = Sapi(BKEY, BSEC)
            ...
            for coin in ex.coins:
                for tt in TradeType:
                    ...
                    if total := res['total']:
                        ...
                        pair_dict = {'fee': fee, 'total': res['total']}
...
async def tick(pairs: [Pair]):
    ...
    for pair in pairs:
        ...
        if not pair.sell:  # add in-group pts only for buy
            ...
            pts += [(pt.name, None, pt.group) for pt in await Pm.filter(group__in=pt_groups).prefetch_related('ptcs') if not True in {ptc.blocked for ptc in pt.ptcs}]
        if (res := await bn_sapi.search_ads(pair.coin_id, pair.cur_id, pair.sell, list(set(pt[0] for pt in pts)))).get('data'):
