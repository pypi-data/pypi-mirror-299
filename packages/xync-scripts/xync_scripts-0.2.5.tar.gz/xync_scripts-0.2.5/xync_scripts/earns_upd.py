...
from cex_clients.binance.earn_api import Ebac
from cex_clients.bybit.web import Bybit, ProductType
from cex_clients.htx.web import Earn as HtxEarn, Private as HtxPrivate
from cex_clients.kucoin.web import Earn as Kucoin
from cex_clients.okx.web import Public as Okx
from http_client import repeat_on_fault
...
async def kucoin() -> tuple[int, int]:
    ...
    for ke in kes:
        for p in ke['products']:
            pid = p['product_id']
            pids.append(pid)
            unq = {'pid': pid, 'coin': await Coin.getOrCreateByName(p['currency']), 'ex_id': 5}
            apr = float(p['apr'])*0.01 if p['apr'] != '-' else 0
            if p['category'] not in ('PROTECTIVE_EARN', 'DUAL') and apr>0 and (p['status'] == 'ONGOING') or (p['status'] != 'ONGOING' and await Dep.exists(**unq, is_active=True)):
                typ = {'ACTIVITY': DepType.earn, 'ETH2': DepType.beth, 'STAKING': DepType.stake}.get(p['category'], DepType.earn)
                dtl = await ku.get_product_detail(p['product_id'])
                mn = dtl.get('user_lower_limit', 0)
                mx = p['return_rates']['STEP'][1]['amount']-mn if p['return_type'] == 'STEP' else dtl.get('user_upper_limit', 0)
                dfd = {'apr': apr, 'is_active': p['status']=='ONGOING',
                       'early_redeem': False if p['category']=='ACTIVITY' or int(p['duration'])>0 else True,
                       'min_limit':mn, 'max_limit': mx,
                       'type': typ, 'duration': p['duration'], 'fee': dtl.get('pol_fee_percent', 0)}
    ...
    for ko in kos:
        ...
        pids.append(pid)
        unq = {'pid': pid, 'coin': await Coin.getOrCreateByName(ko['currency']), 'ex_id': 5, 'type': DepType.stake}
        if ko['state']=='ENABLE' or (ko['state']!='ENABLE' and await Dep.exists(**unq, is_active=True)):
            dfd = {'apr': ko['yearInterestRate'], 'is_active': ko['state']=='ENABLE',
                   'min_limit': ko['lendMinAmount'], 'lendMaxAmount': ko['lendMaxAmount']}  # , 'duration': 0
            d, c = await Dep.update_or_create(dfd, **unq)
            if c:
                i += 1
    await ku.close()
...
async def huobi() -> tuple[int, int]:
    ...
    pids.append('HBETH')
    d, c = await Dep.update_or_create({'apr': hbr}, pid='HBETH', type=DepType.beth, apr_is_fixed=False, min_limit=0.0001, coin=await Coin.getOrCreateByName('BETH'), ex_id=2)
    if c:
        i+=1
    ...
    for hbstk in hbstks:
        ...
        pid = hbstk["id"]
        pids.append(pid)
        unq = {'pid': pid, 'min_limit': 0, 'coin': await Coin.getOrCreateByName(c), 'ex_id': 2, 'type': DepType.stake}
        if hbstk['status']==1 or (hbstk['status']!=1 and await Dep.exists(**unq, is_active=True)):
            d, c = await Dep.update_or_create({'apr': hbstk['annualizedRate'], 'is_active': hbstk['status']==1, 'duration': hbstk['lockupPeriod']}, **unq)
            if c:
                i += 1
    # New
    ...
    for hbn in hbns:
        ...
        pid = hbn['projectId']
        pids.append(pid)
        unq = {'pid': pid, 'coin': await Coin.getOrCreateByName(c), 'ex_id': 2, 'type': DepType.earn}
        if hbn['projectStatus']==1 or (hbn['projectStatus']!=1 and await Dep.exists(**unq, is_active=True)):
            d, c = await Dep.update_or_create({'apr': hbn['viewYearRate'], 'is_active': hbn['projectStatus']==1, 'min_limit': hbn['startAmount'], 'max_limit': hbn['totalAmount']-hbn['finishAmount'], 'duration': hbn['term']}, **unq)
            if c:
                i += 1
    # Fixed
    ...
    for hbl in hbls:
        ...
        if hbl['projectStatus']==1 or (hbl['projectStatus']!=1 and await Dep.exists(**unq, is_active=True)):
            d, c = await Dep.update_or_create({'apr': hbl['viewYearRate'], 'apr_is_fixed': True, 'is_active': hbl['projectStatus']==1, 'max_limit': hbl['totalAmount']-hbl['finishAmount'], 'min_limit': hbl['startAmount'], 'duration': hbl['term']}, **unq)
            if c:
                i += 1
    # Flexible
    ...
    for hbf in hbfs:
        ...
        if hbf['projectStatus']==1 or (hbf['projectStatus']!=1 and await Dep.exists(**unq, is_active=True)):
            d, c = await Dep.update_or_create({'apr': hbf['viewYearRate'], 'is_active': hbf['projectStatus']==1, 'max_limit': hbf['totalAmount']-hbf['finishAmount'], 'min_limit': hbf['startAmount'], 'duration': hbf['term']}, **unq)
            if c:
                i += 1
    # Large
    ...
    for hbm in hbms:
        ...
        if hbm['projectStatus']==1 or (hbm['projectStatus']!=1 and await Dep.exists(**unq, is_active=True)):
            d, c = await Dep.update_or_create({'apr': hbm['viewYearRate'], 'apr_is_fixed': True, 'is_active': hbm['projectStatus']==1, 'max_limit': hbm['totalAmount']-hbm['finishAmount'], 'min_limit': hbm['startAmount'], 'duration': hbm['term']}, **unq)
            if c:
                i+=1
...
async def Binance() -> tuple[int, int]:
    ...
    fpss = await ebac.flex_position()
    lpss = await ebac.lock_position()
    ...
    for fp in fps:
        ...
        if not fp['isSoldOut'] or (fp['isSoldOut'] and await Dep.exists(**unq, is_active=True)):
            upd = {
                'coin': await Coin.getOrCreateByName(fp['asset']),
    ...
    for lp in lps:
        d = lp['detail']
        if ((er := d.get('extraRewardAsset')) and d.get('rewardAsset')) or ((era := d.get('extraRewardAPR')) and d.get('apr')):
            raise Exception
        pid = lp['projectId']
        pids.append(pid)
        unq = {'pid': pid, 'ex_id': 1, 'type': DepType.earn}
        if not d['isSoldOut'] or (d['isSoldOut'] and await Dep.exists(**unq, is_active=True)):
            upd = {
                'coin': await Coin.getOrCreateByName(d['asset']),
                'reward_coin': er and await Coin.getOrCreateByName(er),
                'apr': d.get('apr', era),
                'apr_is_fixed': True,
    ...
    pids.append('BETH')
    d, c = await Dep.update_or_create({'apr': br}, pid='BETH', type=DepType.beth, apr_is_fixed=False, min_limit=0.0001, coin=await Coin.getOrCreateByName('BETH'), ex_id=1)
    if c:
        i+=1
...
async def bybit() -> tuple[int, int]:
    ...
    #     return 0, 0
    bls = (await client.get_home_earn_products(ProductType.fixed_term_saving_products))['fixed_term_saving_products'] # fix
    bes = (await client.get_product_detail())['pos_staking_product_detail'] # eth2
    bfs = (await client.get_home_earn_products(ProductType.flexible_saving_products))['flexible_saving_products'] # flex
    await client.close()
    # eth2
    pid = bes['product_id']
    pids.append(pid)
    unq = {'pid': pid, 'coin': await Coin.getOrCreateByName(bes['name']), 'ex_id': 4, 'type': DepType.beth}
    apy = float(bes['yesterday_apy_e8']) * 0.00000001
    dfd = {'apr': apy, 'is_active': bes['status'] == 1, 'min_limit': bes['min_investment'], 'max_limit': bes['min_investment']}
    d, c = await Dep.update_or_create(dfd, **unq)
    if c:
        i+=1
    # flex
    for bf in bfs:
        pid = bf['product_id']
        pids.append(pid)
        unq = {'pid': pid, 'coin': await Coin.getOrCreateByName(bf['name']), 'ex_id': 4,
               'type': {'Savings': DepType.earn, 'Flexible Staking': DepType.stake}[bf['category']]}
        apy = float(bf['apy'][:-1]) * 0.01
        dfd = {'apr': apy, 'is_active': bf['display_status'] == 1, 'min_limit': 0,
    ...
    for bl in bls:
        pid = bl['product_id']
        pids.append(pid)
        unq = {'pid': pid, 'coin': await Coin.getOrCreateByName(coins[bl['coin']]), 'ex_id': 4, 'type': DepType.earn}
        apy = float(bl['apy'][:-1]) * 0.01
        dfd = {'apr': apy, 'is_active': bl['display_status'] == 1, 'min_limit': 0, 'duration': bl['staking_term'],
               'apr_is_fixed': True, 'max_limit': float(bl['product_max_share']) - float(bl['total_deposit_share'])}
...
async def okx() -> tuple[int, int]:
    ...
    for f in res['data']['flexible']:
        coin: Coin = await Coin.getOrCreateByName(f['investCurrency']['currencyName'])
        rate = float(f['rate']['rateNum']['value'][0])*0.01
        if f['lockUpPeriod'] or f['canPurchase'] or f['campaignUid'] or f['products'][0]['bonusCurrency'] or f['rate']['rateType']!='SINGLE_RATE' or len(f['rate']['rateNum']['value'])!=1:
            pass
    ...
    for f in res['data']['fixed']:
        ...
        for p in f['products']:
            rate = float(f['rate']['rateNum']['value'][0])*0.01
            dur = p["term"]["value"]
            if p['lockUpPeriod'] or f['canPurchase'] or p['campaignUid'] or p['bonusCurrency'] or p['rate']['rateType'] != 'SINGLE_RATE' or len(p['rate']['rateNum']['value'])!=1:
                pass
    ...
    for of in offers:
        ...
        pids.append(pid)
        unq = {'pid': pid, 'ex_id': 3, 'coin': await Coin.getOrCreateByName(of['ccy'])}
        if len(of['investData'])==1 and (of['state']=='purchasable' or (of['state']!='purchasable' and await Dep.exists(**unq, is_active=True))):
            if of['investData'][0]['ccy'] != of['ccy']:
                raise Exception
            if of['protocol'] not in ('Regular interest', 'Compound', 'AAVE', 'APE', 'LOOKS', 'Tokenlon', 'GMX', 'BETH'):
                raise Exception('New protocol:', of['protocol'])
            dfd = {'apr': of['apy'], 'is_active': of['state']=='purchasable', 'min_limit': of['investData'][0]['minAmt'],
                   'max_limit': of['investData'][0]['maxAmt'], 'duration': of['term'], 'early_redeem': of['earlyRedeem'],
                   'apr_is_fixed': of['protocol']=='Regular interest', 'type': DepType.stake}
            if len(of['earningData']) > 1:
                ...
                if len(rcs) > 1:
                    raise Exception
                if rcs[0] != of['ccy']:
                    dfd.update({'reward_coin': await Coin.getOrCreateByName(of['ccy'])})
            d, c = await Dep.update_or_create(dfd, **unq)
            if c:
                i+=1
        else:
